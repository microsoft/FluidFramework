...
This guide applies to Fluid Framework 2.x. Client 3.0 removes `CompatibilityMode` and support for collaborating with 1.x clients. Applications upgrading to Client 3.0 must pass an `OldestSupportedClientVersion` as `oldestSupportedClient`. Legacy mode `"1"` has no Client 3.0 equivalent; every active deployment that must collaborate needs to use Fluid Framework 2.0.0 or later before the upgrade, and `oldestSupportedClient` must be `"2.0.0"` or a later supported version.
