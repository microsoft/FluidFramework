...
Your code can test for the publication status with the <ApiLink packageName="fluid-framework" apiName="IFluidContainer.attachState">container.AttachState</ApiLink> property which has an <ApiLink packageName="fluid-framework" apiName="AttachState">AttachState</ApiLink> value.
...
-   A <ApiLink packageName="fluid-framework" apiName="IFluidContainer.connectionState">container.connectionState</ApiLink> property of type `ConnectionState`. There are four possible values for the property:
