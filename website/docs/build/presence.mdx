...
The public Presence APIs provide session-focused utilities for lightweight data sharing and messaging in ephemeral experiences.
...
The key scenarios for which the Presence APIs are suitable include:
...
`local` is a <ApiLink package="presence" api="StateMap" /> that mimics `Map` though it only supports `string | number` as property keys.

```typescript
function updateCounter(
	counterTracker: LatestMap<number, string>,
	counterName: string,
	value: number,
): void {
...
function startTrackingOthersCounters(counterTracker: LatestMapRaw<number, string>): () => void {
	const stopUpdated = counterTracker.events.on("remoteItemUpdated", (update) => {
		console.log(
			`Attendee ${update.attendee.attendeeId} updated counter ${update.key} to ${update.value}.`,
		);
	});
...
The Presence API provides a mechanism to validate that state data received from other clients during a session matches the declared types.
...
presence.states.getWorkspace("app:v1states", {
	myState: StateFactory.latest({ local: { x: "text" } }),
});
```
...
```typescript
presence.states.getWorkspace("app:v1states", {
	myState2: StateFactory.latest({ local: { x: true } }),
});
```
