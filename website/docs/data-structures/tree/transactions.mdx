...
To create a transaction use <ApiLink package="fluid-framework" api="Tree.runTransaction" />.
...
<ApiLink
	...
	api={{
		previous: "UntypedTreeViewAlpha.runTransactionAsync",
		new: "UntypedTreeView.runTransactionAsync",
	}}
>
	runTransactionAsync
</ApiLink> works like <ApiLink
	package="fluid-framework"
	api={{ previous: "UntypedTreeViewAlpha.runTransaction", new: "UntypedTreeView.runTransaction" }}
>
	runTransaction
</ApiLink> but accepts an `async` callback.
...
>
	runTransactionAsync
</ApiLink> can be called from within the callback of another transaction, with some limitations.
...
view.runTransaction(
	...
	},
	{
		...
		preconditions: [{ type: "nodeInDocument", node: someNode }],
	},
);
```
...
view.runTransaction(
	...
	{
		preconditions: [{ type: "nodeInDocument", node: importantNode }],
	},
);
```
