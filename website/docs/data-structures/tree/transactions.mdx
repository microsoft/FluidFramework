...
To create a transaction use <ApiLink packageName="fluid-framework" apiName="Tree.runTransaction">Tree.runTransaction</ApiLink>.
...
Unlike <ApiLink packageName="fluid-framework" apiName="Tree.runTransaction">Tree.runTransaction</ApiLink>, the Alpha <ApiLink packageName="fluid-framework" apiName="TreeBranchAlpha.runTransaction">runTransaction</ApiLink> does **not** automatically roll back the transaction if the callback throws an error.
If your callback may throw, you should handle errors explicitly and return `{ rollback: true }` when required.

:::

The alpha APIs introduce <ApiLink packageName="fluid-framework" apiName="TreeBranchAlpha.runTransaction">UntypedTreeViewAlpha.runTransaction</ApiLink> as a replacement for `Tree.runTransaction`.
...
For a node in a tree, use <ApiLink packageName="fluid-framework" apiName="TreeAlpha.context">TreeAlpha.context</ApiLink> to get its <ApiLink packageName="fluid-framework" apiName="TreeContextAlpha" />, then call <ApiLink packageName="fluid-framework" apiName="TreeContextAlpha.runTransaction">runTransaction</ApiLink> on that context.
...
const typedView = tree.viewWith(config);
typedView.runTransaction(/* ... */);
```
...
<ApiLink packageName="fluid-framework" apiName="TreeBranchAlpha.runTransactionAsync">runTransactionAsync</ApiLink> works like <ApiLink packageName="fluid-framework" apiName="TreeBranchAlpha.runTransaction">runTransaction</ApiLink> but accepts an `async` callback.
...
<ApiLink packageName="fluid-framework" apiName="TreeBranchAlpha.runTransaction">runTransaction</ApiLink> and <ApiLink packageName="fluid-framework" apiName="TreeBranchAlpha.runTransactionAsync">runTransactionAsync</ApiLink> can be called from within the callback of another transaction, with some limitations.
...
Whether a given constraint in a transaction is violated is ultimately settled after the transaction is sequenced, that is, once it has been reconciled with all concurrent changes that were sequenced before it.
The client on which the transaction is created optimistically applies the transaction locally before it is sequenced,
and progressively updates its view to reflect the impact of concurrent changes sequenced before the transaction as that client learns of these changes.
This means that, on the client where the transaction is created, the transaction may be applied then reverted and possibly re-applied multiple times as the client learns of concurrent changes and re-evaluates the transaction's constraints.
On other clients, the transaction's constraints are fully settled by the time the transaction is applied, so these clients will only ever experience the transaction in its final state.
...
Requires that the whole document (including its schema) is in the exact same state when the transaction is applied as it was when the transaction was created.

:::note

In some cases, SharedTree will consider this constraint not to be violated if the concurrent edits that were made happen to cancel each other out.
For example, if a node was inserted and then removed (possibly by undoing its insertion),
then the `noChange` constraint in a transaction that is concurrent to these changes may not be considered violated once it has rebased over both changes.
This is currently only the case for `noChange` constraints in reverts, but we may generalize this behavior to all usages of the `noChange` constraint in the future.
