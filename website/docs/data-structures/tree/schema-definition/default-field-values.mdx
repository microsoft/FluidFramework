...
The <ApiLink package="fluid-framework" api="SchemaStaticsAlpha.withDefault">`withDefault`</ApiLink> API is available on <ApiLink package="fluid-framework" api="SchemaFactoryAlpha" />.
It allows you to specify default values for fields, making them optional in constructors even when the field is marked as <ApiLink package="fluid-framework" api="SchemaStatics.required">`required`</ApiLink> in the schema.
This provides a better developer experience by reducing boilerplate when creating objects.

<ApiLink package="fluid-framework" api="SchemaStaticsAlpha.withDefault">`withDefault`</ApiLink> wraps a field schema and defines a default value to use when the field is not provided during construction.
The default value must be of an allowed type of the field. You can provide defaults in two ways (see <ApiLink package="fluid-framework" api="NodeProvider" />):
...
[Optional fields](./index.mdx#setting-properties-as-optional) (<ApiLink package="fluid-framework" api="SchemaStatics.optional">`sf.optional`</ApiLink>) already default to `undefined`, but `withDefault` allows you to specify a different
default value:
...
<ApiLink package="fluid-framework" api="SchemaStaticsAlpha.withDefaultRecursive">`withDefaultRecursive`</ApiLink> is available for use inside [recursive schemas](./index.mdx#recursive-schema). Use `objectRecursiveAlpha` (rather than
...
- <ApiLink package="fluid-framework" api="SchemaStaticsAlpha.withDefault">`withDefault` API reference</ApiLink> — Type signatures and parameter details
- <ApiLink package="fluid-framework" api="SchemaStaticsAlpha.withDefaultRecursive">`withDefaultRecursive` API reference</ApiLink> — Recursive variant
- <ApiLink package="fluid-framework" api="NodeProvider">`NodeProvider` API reference</ApiLink> — The type used for default values
- <ApiLink package="fluid-framework" api="SchemaFactoryAlpha">`SchemaFactoryAlpha` API</ApiLink> — Full API reference for `SchemaFactoryAlpha`
