...
Once a document's stored schema has been upgraded, it cannot be downgraded. However, rolling back a feature flag is safe — this is handled automatically. When a document has already been upgraded, `initialize` and `upgradeSchema` automatically detect the enabled upgrades in stored schema and continue including them, even if the feature flag is off. You do not need to manually check or pass upgrade tokens for already-upgraded documents. With the feature flag rolled back, new documents and any documents that haven't been upgraded yet will not have the upgrade enabled.
...
- Documents that have already been upgraded are automatically handled — `initialize` and `upgradeSchema` detect enabled upgrades in stored schema and preserve them without any manual intervention. You can optionally use `isStagedUpgradeEnabled` for observability (e.g., logging which documents have been upgraded).
