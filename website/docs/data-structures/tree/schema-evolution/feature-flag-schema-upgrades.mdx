...
- **Use `snapshotSchemaCompatibility` tests.**
    Write <ApiLink packageName="fluid-framework" apiName="snapshotSchemaCompatibility">`snapshotSchemaCompatibility`</ApiLink> tests to verify compatibility across all schema states in your rollout. Only use schema configurations in production that are covered by these tests.
