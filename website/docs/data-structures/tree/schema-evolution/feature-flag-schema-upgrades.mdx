...
However, rolling back a feature flag is safe — this is handled automatically.
When a document has already been upgraded, <ApiLink packageName="fluid-framework" apiName="TreeViewAlpha" apiType="Interface" headingId="initialize-methodsignature">initialize</ApiLink> and <ApiLink packageName="fluid-framework" apiName="TreeViewAlpha" apiType="Interface" headingId="upgradeschema-methodsignature">upgradeSchema</ApiLink> automatically detect the enabled upgrades in stored schema and continue including them, even if the feature flag is off.
You do not need to manually check or pass upgrade tokens for already-upgraded documents.
...
- Documents that have already been upgraded are automatically handled — <ApiLink packageName="fluid-framework" apiName="TreeViewAlpha" apiType="Interface" headingId="initialize-methodsignature">initialize</ApiLink> and <ApiLink packageName="fluid-framework" apiName="TreeViewAlpha" apiType="Interface" headingId="upgradeschema-methodsignature">upgradeSchema</ApiLink> detect enabled upgrades in stored schema and preserve them without any manual intervention.
  You can optionally use <ApiLink packageName="fluid-framework" apiName="TreeViewAlpha" apiType="Interface" headingId="isstagedupgradeenabled-methodsignature">isStagedUpgradeEnabled</ApiLink> for observability (e.g., logging which documents have been upgraded).
