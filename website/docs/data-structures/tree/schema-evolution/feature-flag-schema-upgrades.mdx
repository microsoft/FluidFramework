...
When you define a [staged schema upgrade](./index.mdx#staged-schema-upgrades) (using <ApiLink package="fluid-framework" api="SchemaFactoryBeta.staged">`staged()`</ApiLink> for allowed types or <ApiLink package="fluid-framework" api="SchemaFactoryAlpha.stagedOptional">`stagedOptional()`</ApiLink> for field optionality), the staged parts are excluded from stored schema by default.
...
- <ApiLink packageName="fluid-framework" apiName="TreeViewAlpha" apiType="Interface" headingId="initialize-methodsignature">`initialize()`</ApiLink> includes the enabled staged changes in the new document's stored schema.
- <ApiLink packageName="fluid-framework" apiName="TreeView" headingId="upgradeschema-methodsignature">`upgradeSchema()`</ApiLink> includes them when upgrading an existing document.
...
You can use <ApiLink packageName="fluid-framework" apiName="TreeViewAlpha" apiType="Interface" headingId="isstagedupgradeenabled-methodsignature">isStagedUpgradeEnabled</ApiLink> to check whether a document has already been upgraded and conditionally include that upgrade token for those documents.
...
- **Use `snapshotSchemaCompatibility` tests.**
    Write <ApiLink package="fluid-framework" api="snapshotSchemaCompatibility">`snapshotSchemaCompatibility`</ApiLink> tests to verify compatibility across all schema states in your rollout. Only use schema configurations in production that are covered by these tests.
