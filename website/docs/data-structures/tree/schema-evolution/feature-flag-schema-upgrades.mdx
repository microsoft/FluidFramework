...
When a document has already been upgraded, [`initialize`](/docs/api/tree/treeviewalpha-interface#initialize-methodsignature) and [`upgradeSchema`](/docs/api/tree/treeviewalpha-interface#upgradeschema-methodsignature) automatically detect the enabled upgrades in stored schema and continue including them, even if the feature flag is off.
...
- Documents that have already been upgraded are automatically handled — [`initialize`](/docs/api/tree/treeviewalpha-interface#initialize-methodsignature) and [`upgradeSchema`](/docs/api/tree/treeviewalpha-interface#upgradeschema-methodsignature) detect enabled upgrades in stored schema and preserve them without any manual intervention. You can optionally use [`isStagedUpgradeEnabled`](/docs/api/tree/treeviewalpha-interface#isstagedupgradeenabled-methodsignature) for observability (e.g., logging which documents have been upgraded).
