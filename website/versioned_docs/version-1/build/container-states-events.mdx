...
Your code can test for the publication status with the <ApiLink package="fluid-static" api="IFluidContainer.attachState"/> property which has an <ApiLink package="container-definitions" api="AttachState" /> value.
