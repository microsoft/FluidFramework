...
{/* markdown-magic:begin {"transform":"include","path":"../../../../packages/dds/counter/README.md","start":40,"end":176} */}
...
You may be asking yourself, why not just store the shared integer value directly in another DDS like a [SharedMap](https://fluidframework.com/docs/data-structures/map)?
...
In this example, the application is storing its vote counts in a [SharedMap](https://fluidframework.com/docs/data-structures/map), where the map keys are `strings` representing the IDs of the options, and the values are `numbers` representing the associated vote counts.
...
For an example of how to create one, please see our workflow examples for [SharedMap creation](https://fluidframework.com/docs/data-structures/map/#creation).
