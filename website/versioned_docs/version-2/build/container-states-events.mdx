...
Your code can test for the publication status with the <ApiLink package="fluid-framework" api="IFluidContainer.attachState" /> property which has an <ApiLink package="fluid-framework" api="AttachState" /> value.
