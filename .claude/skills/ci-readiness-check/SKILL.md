...
Run `pnpm build:fast` from the repository root when changed files affect the
repository build graph beyond one isolated package. Relevant changes include:
...
- files matched by a registered package's declarative task inputs, including
	Rust sources and manifests used to generate WASM clients; and
- cross-package TypeScript contract or generated-artifact changes.

Do not substitute a package-scoped compile for this command. Skip it only when
the changes are demonstrably outside registered package build inputs, such as
documentation-only changes, and report the reason in the final status.
