...
Here `standard` means one fresh reviewer reads the complete diff and relevant surrounding implementations; `deep` additionally reads every changed reviewable file in full.
...
If the diff is too large for a complete review, split it into declared reviews that collectively cover the checkpoint, or report incomplete coverage.
...
Launch one fresh read-only reviewer with separate context, explicitly supplying this skill, the shared criteria, and the complete review input.
...
The reviewer applies the shared criteria across the complete checkpoint change and directly affected boundaries.
...
- Reviewed identity: checkpoint, fixed base, HEAD, snapshot identity, and depth.
...
- Disposition: `No actionable findings`, `Changes requested`, or `Incomplete`.
...
Request a fresh reviewer against the same checkpoint-start base and complete updated scope.
