...
Use one row per ownership area and category wave.
Do not create a crate-by-category matrix unless that matrix is itself useful to the run.
Link the accepted or rejected patch that demonstrates the work.

| Ownership area and wave | Owner | Patch or commit | Review status | Explicit gap or exclusion |
| --- | --- | --- | --- | --- |
| <!-- TODO(required): replace with each promised patch scope --> | | | | |

## Reviewed Candidates

Use one row per reviewable patch or significant rejected structural hypothesis, not per comment, rename, helper, or deletion.
...
<!-- Link fixed-base reviews and command evidence. Summarize material findings, corrections, and reversions. Do not copy command status or patch descriptions into multiple sections. -->
