...
describe('StringInterner', () => {
	...
	it('can associate a string with an intern ID', () => {
		...
		expect(interner.getInternId(inputStrings[0])).to.equal(0);
		expect(interner.getInternId(inputStrings[1])).to.equal(1);
		expect(interner.getInternId(inputStrings[0])).to.equal(0);
	});
	...
	it('can handle empty strings', () => {
		...
		expect(interner.getInternId('')).to.equal(0);
	});
	...
	it('can be initialized with a list of input strings', () => {
		const interner = new StringInterner(inputStrings);

		expect(interner.getInternId(inputStrings[3])).to.equal(3);
		expect(interner.getInternId(inputStrings[2])).to.equal(2);
		expect(interner.getInternId(inputStrings[1])).to.equal(1);
		expect(interner.getInternId(inputStrings[0])).to.equal(0);
	});

	it('can be initialized with a list of input strings that include duplicate IDs', () => {
		const interner = new StringInterner([...inputStrings, ...inputStrings]);
