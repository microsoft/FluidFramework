...
export class BadArray<T> {
  ...
  public static push<T>(
    ...
    console.log(
      `pushed: ${toStore.length} items to ${
        store.id
      }; new sequence length: ${store.getLength()}`
    );
  ...
  public static pop<T>(store: SharedObjectSequence<T>): T {
    ...
    store.remove(store.getLength() - 1, store.getLength());
    return item;
  }
