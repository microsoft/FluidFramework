...
import { ICreateBlobParams } from "../routes/blobs";
import { ICreateCommitParams } from "../routes/commits";
import { ICreateRefParams } from "../routes/refs";
import { ICreateTreeParams } from "../routes/trees";
    ...
    describe("Routes", () => {
        ...
        describe("Repos", () => {
            it("Can create a new repo", () => {
                return createRepo(supertest, testRepoName);
            });
        ...
        describe("Tags", () => {
            it("Can create and retrive an annotated tag", async () => {
                ...
                const tag = await supertest
                    ...
                    .send(tagParams)
                    .expect(201);
                assert(tag.body.sha, "a8588b3913aa692c3642697d6f136cec470dd82c");

                return supertest
                    .get(`/repos/${testRepoName}/git/tags/${tag.body.sha}`)
                    .expect(200)
                    .expect((result) => {
                        assert(result.body.sha, tag.body.sha);
                    });
            });
        });
    });
