...
export class ContentPublisher {
    private mongoManager: BatchManager<IMongoTarget, IOpContent>;
    private ioManager: BatchManager<IoTarget, IOpContent>;
    private offset: number = 0;

    private workManager = new WorkManager();

    constructor(private io: core.IPublisher, private collection: core.ICollection<any>) {
    ...
    public publish(message: IOpContent): void {
        ...
        const target: IoTarget = {
            ...
            topic: `${message.tenantId}/${message.documentId}`,
        };
        this.ioManager.add(target, message, this.offset);
     ...
     */
    private async processMongoBatch(batch: Batch<IMongoTarget, IOpContent>): Promise<void> {
        // Serialize the current batch to Mongo
        await batch.map(async (id, work) => {
            // tslint:disable-next-line:max-line-length
            winston.verbose(`Inserting to mongodb ${id.documentId}/${id.tenantId}:${work.length}`);
     ...
     * BatchManager callback invoked once a new batch is ready to be processed
     */
    private async processIoBatch(batch: Batch<IoTarget, IOpContent>): Promise<void> {
        // Serialize the current batch to Mongo
        await batch.map(async (id, work) => {
