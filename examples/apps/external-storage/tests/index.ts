...
import { DownloadableViewContainerRuntimeFactory } from "../src/container";
import { CollaborativeRootView } from "../src/collaborativeRootView";
...
async function createContainerAndRenderInElement(element: HTMLElement) {
	const sessionStorageModelLoader = new SessionStorageModelLoader<LoadableDataObject>(
		new StaticCodeLoader(new DownloadableViewContainerRuntimeFactory()),
	);

	let id: string;
	let model: LoadableDataObject;
	...
	// Render it
	ReactDOM.render(
		React.createElement(CollaborativeRootView, {
			model: model,
		}),
		element,
	);
