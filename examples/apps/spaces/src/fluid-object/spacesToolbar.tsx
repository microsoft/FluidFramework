...
const SpacesToolbarAddItemPicker: React.FC<ISpacesToolbarAddItemPickerProps> = (
    ...
    const itemButtonList = Array.from(props.itemMap.entries(), ([type, itemEntry]) => (
        <Button
...
export const SpacesToolbar: React.FC<ISpacesToolbarProps> = (props: React.PropsWithChildren<ISpacesToolbarProps>) => {
    ...
    if (props.editable) {
        toolbarItems.push(<SpacesToolbarAddItemPicker key="items" itemMap={props.itemMap} addItem={props.addItem} />);
    ...
    return <div className="spaces-toolbar">{toolbarItems}</div>;
};
