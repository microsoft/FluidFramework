...
describe("mock-external-data-service", () => {
    ...
    it("fetch-tasks: Ensure server yields the data we expect", async () => {
        ...
        await request(externalDataService!).get("/fetch-tasks").expect(200, {taskList: expectedData});
    });

    it("set-tasks: Ensure external data is updated with provided data", async () => {
        const newData = "42:Determine meaning of life:37";
        await request(externalDataService!).post("/set-tasks").send({taskList: newData}).expect(200);
    ...
    it("set-tasks: Ensure server rejects update with malformed data", async () => {
        const oldData = await externalDataSource!.fetchData();
        await request(externalDataService!).post("/set-tasks").send({tasks: "42:Determine meaning of life:37"}).expect(400);
    ...
    it("register-for-webhook: Registering valid URI succeeds", async () => {
        await request(externalDataService!).post("/register-for-webhook").send({ url: "https://www.fluidframework.com" }).expect(200);
    });

    it("register-for-webhook: Registering invalid URI fails", async () => {
        await request(externalDataService!).post("/register-for-webhook").send({ url: "I am not a URI" }).expect(400);
    });
...
describe("mock-external-data-service: webhook", () => {
    ...
    it("register-for-webhook", async () => {
        ...
        try {
            ...
            const webhookRegistrationResponse = await fetch(`http://localhost:${externalDataServicePort}/register-for-webhook`, {
                method: 'POST',
                headers: {
