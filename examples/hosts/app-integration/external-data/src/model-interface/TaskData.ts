...
export function assertValidTaskData(input: unknown): TaskData {
    ...
    for (const [key, value] of Object.entries(jsonInput)) {
        ...
        if (!Object.prototype.hasOwnProperty.call(jsonValue, "name")) {
            throw new Error(`Input task entry under ID "${key}" does not contain required "name" property. Received: "${jsonValue}".`);
        }
        ...
        if (!Object.prototype.hasOwnProperty.call(jsonValue, "priority")) {
            throw new Error(`Input task entry under ID "${key}" does not contain required "priority" property. Received: "${jsonValue}".`);
        }
        if (typeof jsonValue.priority !== "number") {
            throw new TypeError(`Invalid TaskData "priority" value received: "${jsonValue.priority}".`);
        }
    }
