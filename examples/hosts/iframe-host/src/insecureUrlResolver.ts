...
export class InsecureUrlResolver implements IUrlResolver {
    ...
    public async resolve(request: IRequest): Promise<IResolvedUrl> {
        ...
        if (parsedUrl.host === window.location.host) {
            ...
            const maybeResolvedUrl = await this.cache.get(request.url);
            if (maybeResolvedUrl !== undefined) {
                return maybeResolvedUrl;
            }

            const resolvedP = fetch(
            ...
            const resolvedDataP = resolvedP
                .then(async (resolved) => await resolved.json() as IResolvedUrl);
            this.cache.set(request.url, resolvedDataP);

            return resolvedDataP;
        }
    }
