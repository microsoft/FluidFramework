 ...
 *
 * This compiles a minimal container-runtime entry with webpack in-process (no pre-defined webpack
 * config file, no prior build step) and inspects the resulting chunk graph. It is intentionally
 * hermetic: the only external requirement is that the container-runtime package is built.
 *
...
describe("summarizer delay-load", () => {
	it("splits the summarizer into a lazy chunk excluded from the container-runtime entry", async function () {
		...
		const tempDir = mkdtempSync(join(tmpdir(), "ff-bundle-split-"));
		try {
			// A minimal entry that uses only loadContainerRuntime, mirroring how a real app consumes the
			// runtime. It must not import the package index or statically reference the summarizer, either of
			// which would pull the summarizer into the initial chunk via the static `./summary/index.js` import.
			const entryPath = join(tempDir, "entry.mjs");
			writeFileSync(
				entryPath,
				`import { loadContainerRuntime } from "@fluidframework/container-runtime/internal";\n` +
					`export function apisToBundle() {\n` +
					`\tloadContainerRuntime(undefined);\n` +
					`}\n`,
			);
				...
				webpackBundler(
					{
						...
						externals: [
							(
								...
								// Relative/absolute requests are internal modules; keep them in the graph.
								if (
									request === undefined ||
									request.startsWith(".") ||
									isAbsolute(request)
								...
								// container-runtime's own entrypoints (e.g. ".../internal") stay internal — they are
								// what we are analyzing. The trailing-slash check avoids matching sibling packages
								// such as "@fluidframework/container-runtime-definitions".
								if (
								...
								// Everything else (other packages, node builtins) is stubbed. The bundle is never
								// executed, so the externals type is irrelevant.
						...
						// loadContainerRuntime import) are retained rather than tree-shaken away.
						output: { path: join(tempDir, "out"), filename: "[name].js", library: "bundle" },
						node: false,
