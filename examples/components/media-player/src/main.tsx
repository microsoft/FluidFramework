    ...
    protected async componentInitializingFirstTime() {
        ...
        if (newVideoId) {
            await YoutubeClient.getVideoById(newVideoId).then((response) => {
                ...
                const firstPlaylistItem: IPlaylistItem = {
...
class MediaPlayerView extends React.Component<IMediaPlayerViewProps, IMediaPlayerViewState> {
    ...
    private reactPlayerRef: ReactPlayer | undefined = undefined; // ReactPlayer component
    ...
    private readonly isSeeking = false;
    ...
    render(){
        const { root } = this.props;
        const { playedProportion, newUrl, playlist, playerState, leaderSeconds } = this.state;
                    ...
                    <td>
                        ...
                        <tr>
                            <div style={{marginTop: "2vh", textAlign: "center", background: "transparent"}}>
                                <Text style={{color: "green"}}>{this.toTimeStamp(this.state.playedSeconds)}</Text>
                                <Text>{" / "}</Text>
    ...
    private readonly onProgress = (played: number, playedSeconds: number) => {
        const {root} = this.props;
    ...
    private readonly seekPlayer = (seekSeconds: number) => {
    ...
    private readonly toTimeStamp = (seconds: number) => {
    ...
    private readonly onAddClicked = (newUrl: string) => {
    ...
    private readonly onNextClicked = () => {
        const { root } = this.props;
        const nextIndex = root.get<number>(PlaylistIndexKey) + 1;
