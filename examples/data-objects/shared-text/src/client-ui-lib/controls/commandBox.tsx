...
export const CommandBox: React.FC<ICommandBoxProps> = (props: ICommandBoxProps) => {
			...
			const filterCommands = () => {
				...
				setMatchingCommands(
					commands.filter((command) => {
						return command.friendlyName
							.toLowerCase()
							.startsWith(inputEl.value.toLowerCase());
					}),
				);
			};
	...
	const keypressHandler = (e: React.KeyboardEvent<HTMLInputElement>) => {
		if (e.key === "Enter") {
			...
			const exactMatchingCommand =
				filterValue !== undefined
					? commands.find(
							(command) =>
								filterValue.toLowerCase() === command.friendlyName.toLowerCase(),
					  )
					: undefined;
	...
	const commandElements = matchingCommands.map((command, index) => {
		...
		return index === arrowedCommand ? (
			...
			<div key={command.friendlyName} onClick={clickCommand}>
				{command.friendlyName}
			</div>
		);
	});
