...
function endRenderSegments(marker: MergeTree.Marker) {
	return (
		refHasTileLabel(marker, "pg") ||
		(refHasRangeLabel(marker, "cell") && marker.refType & MergeTree.ReferenceType.NestEnd)
	);
}
...
function showCell(pos: number, flowView: FlowView) {
	...
	if (startingPosStack.cell && !startingPosStack.cell.empty()) {
		...
		console.log(
			`cell ${cellMarker.getId()} seq ${cellMarker.seq} clid ${
				cellMarker.clientId
			} at [${start},${end})`,
		);
			...
			if (currentPos < totalLength) {
				...
				if (currentPos < totalLength) {
					...
					if (segoff.segment && MergeTree.Marker.is(segoff.segment)) {
						if (
							refHasRangeLabel(segoff.segment, "cell") &&
...
export class FlowView extends ui.Component {
	...
	private checkRow(
		...
		while (rowDiv && rowDiv !== oldRowDiv && rowDiv.rowView) {
			...
			for (const cell of rowDiv.rowView.cells) {
				if (cell.div) {
					const innerDiv = this.lineDivSelect(
						fn,
						(cell as ICellView).viewport.div,
						true,
						rev,
					);
					if (innerDiv) {
		...
		function checkInTable() {
			...
			while (rowDiv && rowDiv.rowView) {
				if (rowDiv.rowView) {
					...
					if (cell) {
						targetLineDiv = up
							? cell.viewport.lastLineDiv()
							: cell.viewport.firstLineDiv();
	...
	private copyFormat() {
		const segoff = getContainingSegment(this.sharedString, this.cursor.pos);
		if (segoff.segment && MergeTree.TextSegment.is(segoff.segment)) {
			this.formatRegister = MergeTree.extend(
				MergeTree.createMap(),
				segoff.segment.properties,
			);
		}
	}
	...
	public loadFinished(clockStart = 0) {
		...
		if (clockStart > 0) {
			console.log(
				`time to edit/impression: ${this.timeToEdit} time to load: ${
					Date.now() - clockStart
				}ms len: ${this.sharedString.getLength()} - ${performance.now()}`,
			);
		}
				...
				if (remotePresenceInfo.origMark >= 0) {
					...
					if (markSegoff.segment) {
						localPresenceInfo.markLocalRef =
							this.sharedString.createLocalReferencePosition(
								markSegoff.segment,
								markSegoff.offset!,
								MergeTree.ReferenceType.SlideOnRemove,
								undefined,
