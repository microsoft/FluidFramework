...
export interface ParagraphTokenActions<TContext> {
	textToken(
		text: string,
		type: ParagraphItemType,
		leadSegment: MergeTree.TextSegment,
		context?: TContext,
	): void;
...
export class ParagraphLexer<TContext> {
	...
	constructor(
		public tokenActions: ParagraphTokenActions<TContext>,
		public actionContext?: TContext,
	) {}
	...
	public lex(textSegment: MergeTree.TextSegment) {
		if (
			this.leadSegment &&
			!MergeTree.matchProperties(this.leadSegment.properties, textSegment.properties)
		) {
			this.emit();
	...
	private emitGlue() {
		if (this.spaceCount > 0) {
			this.tokenActions.textToken(
				MergeTree.internedSpaces(this.spaceCount),
				ParagraphItemType.Glue,
				this.leadSegment!,
				this.actionContext,
			);
			this.spaceCount = 0;
...
function getPrecedingTile(
	...
	tilePos: number,
	label: string,
	filter: (candidate: MergeTree.Marker) => boolean,
...
const numberSuffix = (itemIndex: number, suffix: string): ISymbol => ({
	text: itemIndex.toString() + suffix,
});
	...
	if (isListTile(tile)) {
		if (tile.listCache === undefined) {
			if (tile.properties!.series) {
				...
				const precedingTilePos = getPrecedingTile(
					sharedString,
					tile,
					tilePos,
					"list",
					(t) => isListTile(t) && t.properties!.listKind === listKind,
					precedingTileCache,
				);
				if (precedingTilePos && tilePos - precedingTilePos.pos < maxListDistance) {
					getListCacheInfo(
						sharedString,
						<MergeTree.Marker>precedingTilePos.tile,
						precedingTilePos.pos,
						precedingTileCache,
					);
...
export function textTokenToItems(
	...
	if (type === ParagraphItemType.Block) {
		...
		itemsContext.itemInfo.items.push(
			makeGlue(textWidth, text, leadSegment, wordSpacing / 2, wordSpacing / 3),
		);
	}
}
