...
Bubblemark-inspired DDS benchmark using ShareJS JSON1 OT (Operational Transformation).
...
- Uses `@fluid-experimental/sharejs-json1` and `ot-json1` for OT-based state management
- Depends on `@fluid-example/bubblebench-common` for shared utilities
- Browser-based benchmark bundled with Webpack
- Compare performance against CRDT-based implementations (baseline, tree, shared-tree)
