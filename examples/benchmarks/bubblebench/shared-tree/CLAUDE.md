...
Bubblemark-inspired DDS benchmark using the production SharedTree DDS.
...
- Uses `@fluidframework/tree` (production SharedTree) for state management
- Depends on `@fluid-example/bubblebench-common` for shared utilities
- Browser-based benchmark bundled with Webpack
- This is the recommended tree implementation to compare against baseline
