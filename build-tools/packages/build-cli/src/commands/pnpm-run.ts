...
export default class PnpmRunCommand extends Command {
	static readonly description =
		...
		`empty, preserving the historical recursive behavior. Intended to be called from ` +
		`package.json scripts so CI jobs can scope pnpm execution to a subset of packages without ` +
		`rewriting each script. Every argument after the command name is forwarded to pnpm verbatim.`;
