...
import { PreviousVersionStyle, getAndUpdatePackageDetails } from "@fluidframework/build-tools";

import { BaseCommand } from "../../base";
import { releaseGroupFlag } from "../../flags";

export default class PrepareTypeTestsCommand extends BaseCommand<
	typeof PrepareTypeTestsCommand.flags
> {
	static description = `Prepares/updates type test configuration based on the branch and config.

    Prepare determines the baseline previous version to use, then sets that version in package.json. If the previous version changes after running preparation, then npm install must be run before the generate step will run correctly.
	...
	public async run(): Promise<void> {
		...
		if (!flags.verbose) {
			CliUx.ux.action.start("Preparing type tests...", "preparing", {
				stdout: true,
			});
		}
		...
		const runningPrepares: Promise<boolean>[] = [];

		// this loop incrementally builds up the runningPrepares promise list
		...
		for (const [i, packageDir] of packageDirs.entries()) {
			runningPrepares.push(
				(async () => {
					if (i >= concurrency) {
						await runningPrepares[i - concurrency];
					}
		...
		const results = (await Promise.all(runningPrepares)).every((v) => v);
