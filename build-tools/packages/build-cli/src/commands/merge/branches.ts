...
export default class MergeBranch extends BaseCommand<typeof MergeBranch.flags> {
    ...
    public async run(): Promise<void> {
        ...
        if (i === 0) {
            ...
            this.info(
                `Fetch pull request info for single commit id ${unmergedCommitList[0]}: ${prInfo[0]}`,
            );
            ...
            this.info(
                `Fetch pull request info for bulk commit id ${unmergedCommitList[commit]}: ${prInfo[0]}`,
            );
            prNumber = await api.createPullRequest(
                flags.auth,
                branchName,
