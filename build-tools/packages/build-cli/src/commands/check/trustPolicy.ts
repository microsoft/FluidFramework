 ...
 *
 * `sigstore` is loaded via `createRequire` rather than a static `import` so
 * TypeScript does not pull its transitive `.d.ts` graph into our program.
 * That graph (`@sigstore/core`, `@sigstore/sign`, `@sigstore/tuf`,
 * `make-fetch-happen`, `@sigstore/rekor-types`) has type errors against this
 * repo's `@types/node` baseline that we cannot fix from here.
 */
 ...
 *
 * All fields are populated only after the bundle has been cryptographically
 * verified via `sigstore.verify` (which checks the Fulcio-issued certificate
 * chain, the Rekor inclusion proof, and the DSSE signature). A missing field
 * means the bundle did not carry that piece of metadata, not that it failed
 * verification — verification failures land on `verificationError` instead
 * and leave the rest of the fields undefined.
 */
	...
	for (const v of violations) {
		try {
			...
			if (v.evidence === "provenance") {
				v.provenanceDetails = await enrichProvenance(v.name, v.version, offender, verbose);
			}
			if (v.priorTrusted?.evidence === "provenance") {
				const ptManifest = versions[v.priorTrusted.version];
				if (ptManifest !== undefined) {
					v.priorTrusted.provenanceDetails = await enrichProvenance(
						v.name,
 ...
 */
async function enrichProvenance(
	name: string,
	version: string,
	manifest: pacote.Manifest & TrustFields,
	verbose: (msg: string) => void,
	...
	try {
		...
		return {
			...
			builderId: builder,
			runUrl,
			subjectDigestVerified: compareSubjectDigest(manifest, statement),
		};
	} catch (err) {
 ...
 */
function renderProvenanceDetails(
	indent: string,
