...
import { type Handler, readFile, readFilePartial, writeFile } from "./common.js";
...
function makeHandler(config: IFileConfig): (file: string) => Promise<string | undefined> {
	...
	return async (file: string): Promise<string | undefined> => {
		// Read only the first 512B since copyright headers are required to appear at the beginning of the file.
		const content = readFilePartial(file);
