...
const test_tags = [
	...
	"client_v1.2.4",
	"client_v1.2.3",
	"client_v2.0.0-beta.1.0.0",
	"client_v2.0.0-beta.1.0.1",
	"client_v2.0.0-beta.2.0.0",
...
describe("generate:buildVersion", () => {
	...
	test.env({
		...
		.it("beta version, prerelease", (ctx) => {
			expect(ctx.stdout).to.contain("version=2.0.0-dev-beta.3.0.0.212045");
			expect(ctx.stdout).to.contain("isLatest=false");
		});
	...
	test.env({
		...
		.it("beta version, release", (ctx) => {
			expect(ctx.stdout).to.contain("version=2.0.0-beta.3.0.0");
			expect(ctx.stdout).to.contain("isLatest=false");
		});
});
