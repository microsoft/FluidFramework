 ...
 * @param onStatus - Optional callback invoked with a status message when shallow-clone deepening is considered
 * or performed.
...
export async function getMergeBaseRemote(
	...
	try {
		...
		if (remote === undefined) {
			onStatus?.(
				`Merge-base with ${compareRef} not found; no remote was provided to deepen the clone.`,
			);
			throw error;
		}

		const isShallow = await git.raw("rev-parse", "--is-shallow-repository").then(
			(result) => result.trim(),
			() => {
				throw error;
			},
 ...
 *
 * @remarks It assumes paths with POSIX separators (forward slashes), like the
 * ones returned by `git ls-files` and `git ls-tree` (even in Windows).
 * Be careful if using this to test paths not generated by git calls.
 */
 ...
 */
export async function getPackageDirs(git: SimpleGit, ref?: string): Promise<Set<string>> {
