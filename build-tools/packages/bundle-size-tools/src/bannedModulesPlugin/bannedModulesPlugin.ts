...
export class BannedModulesPlugin {
    constructor(private readonly options: BannedModulesPluginOptions) {
        if (!options || !Array.isArray(options.bannedModules)) {
            throw new Error(
                `${pluginName} must have a config object with a bannedModules array as a property`,
            );
        }
    }
    ...
    apply(compiler: Webpack.Compiler) {
        ...
        compiler.hooks.done.tap(pluginName, (stats) => {
            stats.toJson().modules?.forEach((mod) => {
                ...
                const modulePath = mod.name?.substring(
                    mod.name.indexOf("node_modules") + "node_modules".length + 1,
                );
