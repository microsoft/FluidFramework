...
async function generateMonoRepoPackageLockJson(monoRepo: MonoRepo, repoPackageJson: any, log?: Logger) {
    ...
    log?.info(`${monoRepo.kind}: ${format(totalDevCount)}/${format(totalCount)} locked devDependencies`);
    log?.info(`${monoRepo.kind}: ${format(topLevelDevCount)}/${format(topLevelTotalCount)} top level locked devDependencies`);
...
export async function generateMonoRepoInstallPackageJson(monoRepo: MonoRepo, log?: Logger) {
    ...
    log?.info(`${monoRepo.kind}: ${format(devDepCount)}/${format(depCount + devDepCount)} devDependencies`);
