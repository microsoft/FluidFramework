...
describe("Interface", () => {
    ...
    function checkIncrement(oldSource: string, newSource: string): BreakingIncrement {
        const oldSourceFile = project.createSourceFile(`${pkgDir}/src/interfaceOld.ts`, oldSource, {
            overwrite: true,
        });
        const oldTypeData = getTypeDataForSource(oldSourceFile);
        const newSourceFile = project.createSourceFile(`${pkgDir}/src/interfaceNew.ts`, newSource, {
            overwrite: true,
        });
