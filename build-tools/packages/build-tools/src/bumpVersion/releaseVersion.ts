...
async function releasePackages(
    ...
    if (packageToRelease.length !== 0) {
        ...
        console.log(
            `Please manually queue a release build for the following packages in ADO for branch ${context.originalBranchName}`,
        );
        for (const pkg of packageToRelease) {
            console.log(`  ${pkg.name}`);
        }
        console.log(
            `After the build is done successfully run --release again to bump version and update dependency`,
        );
        return;
    }
...
async function releaseMonoRepo(
    context: Context,
    monoRepo: MonoRepo,
    updateLock: boolean,
    virtualPatch: boolean,
...
async function postRelease(
    ...
    tagNames: string,
    packageNames: string,
    bumpDep: Map<string, string | undefined>,
    updateLock: boolean,
    virtualPatch: boolean,
) {
    ...
    const message = await bumpDependencies(
        context,
        bumpDep,
        updateLock,
        false,
