...
`oldestSupportedClient` defines the oldest Fluid client version that must be able to access documents written by the runtime. Customers are encouraged to set `oldestSupportedClient` to the highest version their users are saturated on. Client 3.0 accepts stable supported 2.x and 3.x values, so all active deployments that must collaborate need to be running Fluid Framework 2.0.0 or later. If the customer does not set `oldestSupportedClient`, Client 3.0 uses `"2.0.0"` and applies the same defaults and validation as an explicit `"2.0.0"`. `oldestSupportedClient` controls the "default configurations" and "unsafe configuration prevention" mechanisms explained below.
