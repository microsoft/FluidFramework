...
`oldestSupportedClient` defines the oldest Fluid client version that must be able to access documents written by the runtime. Callers must specify exactly one of `oldestSupportedClient` or the deprecated `minVersionForCollab`; omitting both is an error. Customers are encouraged to choose the highest version their users are saturated on. Client 3.0 accepts supported 2.x and 3.x values, so all active deployments that must collaborate need to be running Fluid Framework 2.0.0 or later. `oldestSupportedClient` controls the "default configurations" and "unsafe configuration prevention" mechanisms explained below.
