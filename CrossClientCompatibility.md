...
`minDocumentRuntimeVersion` is the container runtime load parameter for configuring cross-client compatibility. It is defined in [containerRuntime.ts](./packages/runtime/container-runtime/src/containerRuntime.ts) and specifies the minimum Fluid version required to open or process documents created or loaded by the runtime. It serves two purposes:
...
`minVersionForCollab` is the deprecated name for this parameter. It remains available as a compatibility alias until 3.0.0.
...
`minDocumentRuntimeVersion` is a semver string representing the minimum Fluid version required to open or process a document. It automatically configures default values for runtime options to ensure compatibility with the specified version. For example, setting `minDocumentRuntimeVersion` to `"2.0.0"` enables features like grouped batching that are safe for all 2.x clients. `minVersionForCollab` remains accepted as a compatibility alias.
