...
The `requestFluidObject_UNSAFE` is removed from the PureDataObject. If you still need to fallback on using URIs, you can utilize `handleFromLegacyUri`. We are making this change as we want to encourage retreiving shared objects only via handles.

### Modified PureDataObject.getFluidObjectFromDirectory
Going forward, `getFluidObjectFromDirectory` will not return FluidObject if you have have used to store uri string for a given key. If you still need to fallback on using URIs, you can utilize `handleFromLegacyUri`. Also, getFluidObjectFromDirectory now expects callback that is only returning `IFluidHandle` or `undefined`. Returnig id (string) is not supported as, again, we want to encourage retreiving shared objects only via handles.
