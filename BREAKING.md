...
- [`deltaManager.ts` removed from @fluidframework/container-loader export](#deltamanager-removed-from-fluid-framework-export)
...
The `DeltaManager` class, the `IDeltaManagerInternalEvents` interface, and the `ReconnectedMode` enum have been removed from `@fluidframework/container-loader` package exports. `IConnectionArgs` has been moved to `connectionArgs.ts`. Instead of `DeltaManager`, `IDeltaManager` should be used where appropriate.
