...
1. The routes produced by handles are different!
   ...
   - Old routes (and old handles) will continue to work (i.e. backward compatibility in fully supported), but they are discouraged. In general, no assumption should be made on structure or shape of internal URIs (that are for handle serialization / de-serialization only). Container authors should map external URIs to internal handles + request to resolved object (IFluidObject).
2. APIs like `_createDataStoreWithProps`, `createDataStore`, `createRootDataStore` has been returning `IFluidRouter` for a while. Callers were able to get away with casting returned objects to `IFluidDataStoreChannel`, this will no longer work, as object returned is not datastore runtime object. IFluidDataStoreChannel is internal object and there is no way to get to it.
   - If you need to get to data store runtime, implement a request route on your data object that returns data store.
   - You can no longer get to channels (DDSs) using this IFluidRouter object - see next point.
3. There is no way to request a DDS by doing FluidDataStoreRuntime.request() - this path is only for custom routes (i.e. requesting objects that live off data store, like DataObject).
   - If you need to expose a DDS from data store, your data object needs to expose it through some custom route.
