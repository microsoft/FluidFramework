...
export class TabsDataModel extends EventEmitter implements ITabsDataModel {
    ...
    constructor(
        ...
        private readonly createAndAttachComponent: <T extends IFluidObject & IFluidLoadable>
            (pkg: string, props?: any) => Promise<T>,
        private readonly getComponentFromDirectory: <T extends IFluidObject & IFluidLoadable>(
            id: string,
    ...
    public async createTab(type: string): Promise<string> {
        const newKey = uuid();
        const component = await this.createAndAttachComponent<IFluidObject & IFluidLoadable>(type);
