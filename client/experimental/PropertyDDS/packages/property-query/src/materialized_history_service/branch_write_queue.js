...
class BranchWriteQueue {
          ...
          do {
            ...
            fetchedResults.forEach((fr) => {
              ...
              lastWritePromise.catch((ex2) => {
                logger.warn(`Failed writing a fetched commit for branch ${branchGuid}, commit ${fr.guid}`, ex2);
              });
              parentGuid = fr.guid;
            });
            minGuid = parentGuid;
  ...
  async _fetchSome(branchGuid, commitGuid, parentCommitGuid) {
    ...
    if (commits.commits.length === 0) {
      throw new OperationError(`Commit ${commitGuid} not found!`, '_fetchSome',
  ...
  async _validateTopologyWorker({ task, taskDp }) {
    const _tryToFetchMissingAndQueue = async (lastKnownCommit) => {
      ...
      if (itemSearchingParent !== lastKnownCommit) {
        ...
        do {
          // Let's fetch a page of results
          let fetchedResults = await this._fetchSome(task.branchGuid, itemSearchingParent, minCommit);
          // We know we're done fetching the commits when we fetched the parent of the commit
          // Triggering the fetches
          allCommitsFound = fetchedResults.find((fr) => fr.guid === itemSearchingParent) !== undefined;
          ...
          fetchedResults.forEach((fr) => {
            ...
            let correspondingInTopologyQueue = topologyWorkerQueue.find(({ task: queueItem }) =>
            ...
            if (correspondingInTopologyQueue) {
              ...
              lastPromise = this._addPendingCommitPromise(task.branchGuid, fetchedResults.guid, this._queueCommit(fr));
            }
          });
      ...
      if (ex.statusCode === HTTPStatus.NOT_FOUND) {
        ...
        if (lastCommitGuid !== task.parentGuid && lastCommitGuid !== task.guid) {
          ...
          if (task.rebase) {
            ...
            await _tryToFetchMissingAndQueue(lastCommitGuid)
              .catch((ex2) => {
  ...
  queueCommitGracefully(commitReq) {
    if (this._lockedBranches.has(commitReq.branchGuid)) {
      return Promise.reject(
        new OperationError(`Branch ${commitReq.branchGuid} locked for deletion`, 'lockQueuesForDeletion',
          HTTPStatus.BAD_REQUEST, OperationError.FLAGS.QUIET)
      );
    }

    // Commit already pending, return the existing promise
    if (this._pendingCommitPromises[commitReq.branchGuid] &&
    ...
    let work = async () => {
      if (!this._validateTopologyQueue[commitReq.branchGuid]) {
        ...
        this._validateTopologyQueue[commitReq.branchGuid] = asyncQueue(this._validateTopologyWorker.bind(this));
  ...
  async queueBranchGracefully(branchReq) {
    ...
    let work = async () => {
      if (branchReq.parentBranchGuid) {
        await this.waitUntilCommitApplied(branchReq.parentBranchGuid, branchReq.rootCommitGuid);
      }
      if (this._lockedBranches.has(branchReq.guid)) {
        throw new OperationError(`Branch ${branchReq.guid} locked for deletion`, 'lockQueuesForDeletion',
          HTTPStatus.BAD_REQUEST, OperationError.FLAGS.QUIET);
  ...
  async _queueCommit(commitReq) {
    ...
    logger.debug(`Queuing commit ${commitGuid} for branch ${branchGuid}, parent ${commitReq.parentGuid}`);
    if (!this._writeQueue[branchGuid]) {
      this._writeQueue[branchGuid] = asyncQueue(async (task) => {
        ...
        try {
          ...
          let childrenError = new Error(`Failed applying a parent commit ${task.guid} for branch ${task.branchGuid}` +
    ...
    return new Promise((resolve, reject) => {
      this._writeQueue[branchGuid].push(commitReq, (error, result) => {
        ...
        logger.debug(
          `Write callback: error = ${JSON.stringify(error)} ${error && error.stack}, result = ${JSON.stringify(result)}`
        );
  ...
  _flushBranchQueue(branchGuid, ex) {
    ...
    if (this._writeQueue[branchGuid]) {
      this._writeQueue[branchGuid].drain().then(() => {
        if (!this._validateTopologyQueue[branchGuid] || this._validateTopologyQueue[branchGuid].idle()) {
          delete this._errorsPerBranch[branchGuid];
        }
      });
    }
  ...
  async lockQueuesForDeletion(branchGuids) {
    branchGuids.forEach((bg) => {
      ...
      this._flushBranchQueue(bg, new OperationError(`Branch ${bg} locked for deletion`, 'lockQueuesForDeletion',
        HTTPStatus.BAD_REQUEST, OperationError.FLAGS.QUIET)
      );
    });

    return Promise.all(branchGuids.map((bg) =>
      new Promise(async (res, rej) => {
        let branchResolved = !this._pendingBranchPromises[bg];
        let wqResolved = !this._writeQueue[bg] || this._writeQueue[bg].idle();
        let vtqResolved = !this._validateTopologyQueue[bg] || this._validateTopologyQueue[bg].idle();
