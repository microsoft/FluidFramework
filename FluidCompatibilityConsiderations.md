...
Cross-client compatibility lets applications specify the oldest client version that must be able to access documents written by newer clients. This also ensures supported clients can fully collaborate with each other.
...
This is done using an [OldestSupportedClientVersion](https://fluidframework.com/docs/api/runtime-definitions/oldestsupportedclientversion-typealias).
...
Client 3.0 supports stable 2.x versions and 3.x minor checkpoints whose patch is zero, so every active deployment that must collaborate needs to use Fluid Framework 2.0.0 or later before upgrading.
...
2. The support window is bounded: data-at-rest requires support for reading data from any previous version, while Cross-client compatibility only requires support for writing in formats old enough to be read by the oldest supported `OldestSupportedClientVersion`.
