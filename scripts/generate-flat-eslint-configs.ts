...
const typescriptMode = process.argv.includes("--typescript");
      ...
      try {
        ...
        if (extendsMatch) {
          ...
          if (extendsContent.includes("eslint-config-fluid/strict")) {
            ...
            extendsContent.includes("eslint-config-fluid/minimal-deprecated")
          ) {
        ...
        try {
          ...
          const legacyPathEscaped = legacyPath
            .replace(/\\/g, "\\\\")
            .replace(/'/g, "\\'");
        ...
        if (legacyConfig !== undefined) {
          ...
          console.error(
            `Skipping package at ${full} due to failed legacy config load.`,
          );
        }
      } catch {
...
function buildFlatConfigContent(
  ...
  const flatSource = path
    ...
    .replace(/\\/g, "/");
  const importPath = flatSource.startsWith(".")
    ? flatSource
    : `./${flatSource}`;
...
`;
  ...
  const hasLocalRules =
    legacyConfig?.rules && Object.keys(legacyConfig.rules).length > 0;
  const hasOverrides =
    legacyConfig?.overrides && legacyConfig.overrides.length > 0;
  const hasEslintIgnore =
    eslintIgnorePatterns && eslintIgnorePatterns.length > 0;
      ...
      for (const [ruleName, ruleConfig] of Object.entries(legacyConfig.rules)) {
        ...
        const isDisabled =
          ...
          (Array.isArray(ruleConfig) &&
            (ruleConfig[0] === "off" || ruleConfig[0] === 0));
...
async function writeFlatConfigs(
  ...
  for (const t of targets) {
    ...
    console.log(
      `  Generated: ${path.relative(repoRoot, outPath)} (${t.flatVariant})`,
    );
    ...
    if (finalize) {
      ...
      try {
        ...
        console.log(
          `    Deleted: ${path.relative(repoRoot, t.legacyConfigPath)}`,
        );
      } catch (e) {
      ...
      if (t.eslintIgnorePatterns && t.eslintIgnorePatterns.length > 0) {
        ...
        try {
          ...
          console.log(
            `    Deleted: ${path.relative(repoRoot, eslintIgnorePath)}`,
          );
        } catch (e) {
...
async function main() {
  if (finalizeMode) {
    console.log(
      "Running in FINALIZE mode - will delete legacy configs after generation.\n",
    );
  }
  if (typescriptMode) {
    // See: https://eslint.org/docs/latest/use/configure/configuration-files#typescript-configuration-files
    console.log(
      "Running in TYPESCRIPT mode - generating .mts files (requires jiti).\n",
    );
  }
