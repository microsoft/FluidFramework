...
if [ -f "lerna.json" ]; then
  if [ "$VERSION_RELEASE" = "release" ]; then
      ...
      echo command="flub bump $RELEASE_GROUP --exact $SETVERSION_VERSION --exactDepType=\"$INTERDEPENDENCY_RANGE\" --no-commit--verbose"
      flub bump $RELEASE_GROUP --exact $SETVERSION_VERSION --exactDepType="$INTERDEPENDENCY_RANGE" --no-commit --verbose
  else
    ...
    echo command="flub bump $RELEASE_GROUP --exact $SETVERSION_VERSION --exactDepType=\"\" --no-commit --verbose"
    flub bump $RELEASE_GROUP --exact $SETVERSION_VERSION --exactDepType="" --no-commit --verbose
  fi
else
