...
// Local imports.
import { Cell, CellState, addDependent, removeDependent } from './cell';
import { printSheetGridCell, printPrecedents, printSheetGridRange } from './helpers';
...
export class Workbook {
     ...
     */
    constructor(rows: number, cols: number, init?: string[][]) {
        for (let row = 0; row < rows; row++) {
            const rowArray: Cell[] = [];
            this.cells.push(rowArray);
            for (let col = 0; col < cols; col++) {
                const newValue = init[row]
                    ? init[row][col] || ""
                    : "";
                rowArray.push({ oper: blankOper, state: CellState.Final });
                this.setCellText(row, col, newValue);
            }
        }
    }

    /**
     * Returns the number of populated rows in the workbook.
     * (Unlike numCols(), this is an O(1) calculation).
     */
    public get numRows() {
    ...
    /**
     * Returns the maximum length of any populated column in the workbook.
     * (Note this is currently an O(n) calculation).
     */
    public get numCols() {
        return this.cells.reduce((previousMax, currentRow) => {
            return Math.max(previousMax, currentRow.length)
        }, 0);
    }
     ...
     * return undefined.
     */
    public maybeGetCellAt(row: number, col: number): Cell | undefined {
    ...
    /**
     * Sets the cell at the give row/col to the given newValue.  Strings begining
     * with "=" are parsed as formulas.  Strings containing numbers or booleans are
     * parsed as primitive values.
     */
    public setCellText(row: number, col: number, newValue: UnboxedOper) {
        const cell = this.maybeGetCellAt(row, col);
    ...
    private getCellFormula(cellLoc: SheetGridCell): Result<CellFormula | undefined, Unavailable> {
        ...
        // Handle the case where there is data in the grid, but no compiled formula.
    ...
    private setCellValue(cell: SheetGridCell, oper: WriteOper): void {
        ...
        if(this.maybeGetCellAt(row, col)) {
            ...
            console.error('setCellValue tried to work with a cell that didnt yet exist in the data array!');
    ...
    private setCellFailure(range: SheetGridRange, reason: FailureReason): void {
        this.forEachCellLocInRange(range, (cell: SheetGridCell) => {
            ...
            if (this.maybeGetCellAt(row, col)) {
                ...
                cellContent.reason = reason.kind; //TODO: this isn't the pattern in the excelCell.test.ts example. Neeed to verify.
            } else {
                console.error('setCellFailure tried to work with a cell that didnt yet exist in the data array!');
                //TODO: handle this case.
            }
        });
    }
    ...
    private setDynamicPrecs(range: SheetGridRange, precedents: Precedents): void {
        this.forEachCellLocInRange(range, dependentCell => {
            ...
            //TODO: abstract this logic into a helper method.
            if (this.maybeGetCellAt(row, col)) {
                ...
                //TODO: handle this case.
                console.error('setDynamicPrecs tried to set a precent on a cell that didnt yet exist in the data array!');
            }
            ...
            precedents.cells.forEach(precedentRange => {
                this.forEachCellLocInRange(precedentRange, precedentCell => {
                    ...
                    if (c) {
                        ...
                        //TODO: handle this case.
                        console.error('setDynamicPrecs tried to set a dependent a cell that didnt yet exist in the data array!');
                    }
                });
            });
        });
