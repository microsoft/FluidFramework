...
The summaries generated by SharedTree include the current view and edit history. New clients that load the summary can be used with the current view alone. This allows the history to be virtualized to decrease load time of clients for large edit histories.

In SharedTree, this consists of the following processes:

Summary is requested

-   Start creation of new summary
-   Populate summary with the current view and the handles of previously blobbed groups
-   Add new sequenced edits received since the last summary
-   Initiate the upload of a new blob for these new sequenced edits
-   Return summary tree

Client loads summary

-   Deserialize the summary
-   Populate the client with the current view, handles, and any edits not yet associated with a handle

An edit is requested

-   Check if edit is loaded
-   If edit is loaded, return the edit
-   If edit is not loaded, load its group, add the edit ID to the MRU cache, and evict the LRU cached edit
