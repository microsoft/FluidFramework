...
SharedTree accomplishes this by taking advantage of Fluid's uploadBlob API. Chunks of edits are uploaded and the handle associated with the chunks are serialized and sent with the summary. New clients that load these summaries are then able to deserialize the handle and retrieve the associated chunks when needed.
