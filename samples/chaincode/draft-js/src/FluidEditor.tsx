...
export class FluidEditor extends React.Component<IProps, IState> {
  ...
  componentWillMount() {
    this.props.sharedString.on("sequenceDelta", (event: SequenceDeltaEvent) => {
      try {
        // console.debug(this.props.sharedString.getTextRangeWithMarkers(undefined, undefined));
  ...
  private readonly onChange = (editorState: EditorState, changedStyle?: string) => {
    ...
    if (newContent !== oldContent) {
      ...
      if (changeType === "insert-characters" || changeType === "insert-fragment") {
         ...
         *  2. We need to convert the undo/redo events into some kind of diff that can be applied to the SharedString
         * For now, force the selection back to how it was and ignore the event
         */
        this.setState({ editorState: EditorState.acceptSelection(this.state.editorState, this.state.editorState.getSelection()) });
        return;
      }
    }
