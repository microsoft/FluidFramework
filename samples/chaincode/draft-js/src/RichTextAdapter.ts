...
export const textRangeToBlockTextRange = (absPos: TextRange, content: ContentState): BlockTextRange => {
  return textRangeToBlockTextRangeFromBlocks(absPos, content.getBlocksAsArray());
};
...
export const sharedStringToBlockArary = (sharedString: SharedString): ContentBlock[] => {
  ...
  sharedString.client.walkSegments(0, sharedString.client.getLength(), (segment: ISegment) => {
    if (segment.type === "Marker") {
      ...
      // tslint:disable-next-line: prefer-array-literal no-null-keyword
...
export const updateTextRange = (
  opRanges: ReadonlyArray<Readonly<ISequenceDeltaRange>>,
  textRange: TextRange,
): TextRange => {
