...
Run the same flow with Sea-managed selection by setting `SEA_SNAPSHOT_POLICY=sea` on the `node` command; passing tests report `snapshotParticipation` as `3` and `2`, respectively.
