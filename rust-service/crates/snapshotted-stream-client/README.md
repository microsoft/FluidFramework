...
```bash
cargo test -p snapshotted-stream-client --all-targets
RUSTDOCFLAGS='-D warnings -D missing_docs' cargo doc -p snapshotted-stream-client --no-deps
```
