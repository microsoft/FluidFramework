...
fn encode_message(message: &Message, limits: Limits) -> Result<(u8, Bytes), ProtocolError> {
    ...
    let kind = match message {
        Message::Request(Request::Create { document }) => {
            put_bytes(&mut body, document, limits.max_document_bytes)?;
            CREATE
        }
        Message::Request(Request::OpenSession {
            document,
            writer,
            session,
            reference,
        ...
        Message::Request(Request::Submit(submission)) => {
        ...
        Message::Request(Request::Read { document, after }) => {
            put_bytes(&mut body, document, limits.max_document_bytes)?;
            put_optional_bytes(&mut body, after.as_ref(), limits.max_position_bytes)?;
            READ
        }
        Message::Request(Request::LatestSnapshot { document }) => {
            put_bytes(&mut body, document, limits.max_document_bytes)?;
            LATEST_SNAPSHOT
        }
        Message::Request(Request::PublishSnapshot {
            document,
            includes_through,
            expected_parent,
            payload,
        ...
        Message::Request(Request::Shutdown) => SHUTDOWN,
