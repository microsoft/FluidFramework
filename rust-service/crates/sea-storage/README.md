...
`BuiltInServiceStorage` implements the central `ServiceStorage` contract from `sea-core`.
...
These policies are built-in choices, not requirements imposed by the core contracts or `sea-service`.
...
```bash
cargo test -p sea-storage
cargo clippy -p sea-storage --all-targets --all-features -- -D warnings
RUSTDOCFLAGS='-D warnings -D missing_docs' cargo doc -p sea-storage --no-deps
```