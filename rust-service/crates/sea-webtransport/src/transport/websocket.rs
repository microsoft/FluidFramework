...
/// Connects a transport for the shared session client, without replaying Sea operations.
///
...
pub async fn connect_browser_transport(
...
) -> Result<SelectedTransport, JsValue> {
    ...
    if try_webtransport {
        match deadline(
            BrowserTransport::connect(&webtransport_url, &certificate_hash.to_vec()),
            timeout_milliseconds,
        )
        .await
        {
            Ok(transport) => return Ok(SelectedTransport::WebTransport(transport)),
    ...
    if mode != SeaBrowserTransportMode::WebSocket {
        match SeaWebSocketTransport::connect(websocket_url.clone(), timeout_milliseconds).await {
            Ok(transport) => return Ok(SelectedTransport::WebSocket(transport)),
            Err(error) if mode != SeaBrowserTransportMode::PreferAvailable => return Err(error),
            Err(_) => {}
        }
    }
    Ok(SelectedTransport::WebSocket(
        SeaWebSocketTransport::connect_ordinary(websocket_url, timeout_milliseconds).await?,
    ))
}

/// Fixed transport selection for one neutral SEA session.
pub enum SelectedTransport {
    /// Native QUIC-backed browser transport.
    WebTransport(BrowserTransport),
    /// Streaming or explicitly permitted ordinary WebSocket group.
    WebSocket(SeaWebSocketTransport),
}

/// Cloneable stream directions used by the shared session client.
#[derive(Clone)]
pub enum SelectedStream {
    /// Native browser stream with shared read/write ownership.
    WebTransport(BrowserBidirectionalStream),
    /// Socket stream with shared directional state and cancellation.
    WebSocket(Rc<SeaWebSocketBidirectionalStream>),
}

#[async_trait(?Send)]
impl ClientTransport for SelectedTransport {
    ...
    async fn open_bidirectional(&self) -> Result<Self::Stream, Self::Error> {
        match self {
            Self::WebTransport(transport) => transport
                .open_bidirectional()
                .await
                .map(SelectedStream::WebTransport),
            Self::WebSocket(transport) => transport
                .open_bidirectional()
                .await
                .map(|stream| SelectedStream::WebSocket(Rc::new(stream))),
        }
    }

    fn disconnect(&self) -> Result<(), Self::Error> {
        match self {
            Self::WebTransport(transport) => transport.disconnect(),
...
#[async_trait(?Send)]
impl BidirectionalStream for SelectedStream {
    type Error = JsValue;

    async fn send(&mut self, bytes: &[u8]) -> Result<(), Self::Error> {
        match self {
            Self::WebTransport(stream) => stream.send(bytes).await,
            Self::WebSocket(stream) => stream.send(Uint8Array::from(bytes)).await,
        }
    }

    async fn finish(&mut self) -> Result<(), Self::Error> {
        match self {
            Self::WebTransport(stream) => stream.finish().await,
            Self::WebSocket(stream) => stream.finish().await,
        }
    }

    async fn receive(&mut self) -> Result<Option<Vec<u8>>, Self::Error> {
        match self {
            Self::WebTransport(stream) => stream.receive().await,
    ...
    async fn cancel(&mut self) -> Result<(), Self::Error> {
        match self {
            Self::WebTransport(stream) => stream.cancel().await,
...
/// Feature-gated WebSocket primitive for the shared session client.
