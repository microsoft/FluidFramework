...
This crate provides shared WebAssembly (WASM) session bindings for browser and Node.js consumers.
...
## Session Adapter and Bindings

The [session adapter](src/session.rs) is available on native and WASM targets.
The [JavaScript bindings](src/bindings.rs) and [signal adapters](src/signals.rs) compile only for WASM.
The [core session contracts](../sea-core/src/session.rs) define archive, author, and snapshot behavior; this crate adapts those contracts rather than defining another session model.
...
Errors retain their core service classification, and snapshot publication delegates to the original session with its original handles.
...
The [core signal contract](../sea-core/src/signals.rs) defines delivery and membership semantics independently of the archive.

## Features and Entry Points

Default features are empty; Cargo features select optional dependencies and binding factories.

| Feature | Capability |
| --- | --- |
| `memory` | `SeaMemoryService` creates independent in-memory document namespaces. |
| `webtransport` | `openWebTransport` opens a browser WebTransport session and never falls back. |
| `websocket-stream` | `openRemote` selects a fixed initial WebTransport or WebSocket transport using the same session adapter. |
| `compression` | Makes the compression decorator available; session options explicitly enable it. |

`websocket-stream` forwards the socket capability to `sea-webtransport`; neither memory-only nor minimal WebTransport builds include it.
