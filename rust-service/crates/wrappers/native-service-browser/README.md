...
This WASM-only crate adapts a memory-backed native Fluid service to the JavaScript transport contract consumed by `InjectedClient` in [`fluid-webtransport-browser`](../webtransport-browser/README.md).
...
RUSTFLAGS='--cfg=web_sys_unstable_apis' cargo build --locked \
  -p fluid-native-service-browser --target wasm32-unknown-unknown
```
