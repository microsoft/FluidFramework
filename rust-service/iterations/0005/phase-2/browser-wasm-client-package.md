...
Final commit: Wave 2 projected-read/recovery implementation `80a3ad754983f9d09409273c680192e854b48a13`; final workstream commit waits for the blob/summary prerequisite
...
Started and finished: started `2026-09-12T19:44:42+00:00`; Wave 1 checkpoint `2026-09-12T20:11:12+00:00`; projected-read/recovery checkpoint `2026-09-12T20:24:23+00:00`; final completion pending blob/summary

## Outcome

Wave 2 projected-read and ambiguity-recovery consumption is complete with high confidence after the accepted prerequisite commits `1c6d8977f8a..8e9d36126db`. The environment-neutral `InjectedClient` and certificate-pinned `BrowserClient` now expose the same typed projected page and explicit submission-resolution operations through the shared bounded FSP4 core. Actual generated WASM passed deterministic Node validation, including exact accepted wire kinds and no-hidden-retry evidence, and the same release WASM passed the unchanged native-service Chromium flow. The workstream remains in progress waiting only for the blob/summary handoff; no blob or summary messages were invented or predefined.

## Hypothesis Results

Supported through the projected-read/recovery checkpoint. The package encodes accepted request kinds `8` and `9`, decodes response kinds `68` and `69`, preserves opaque positions/cursors/references, exposes all three resolution outcomes, and performs no submit replay. A rejected resolution transport was invoked exactly once and changed the client to `disconnected`. Fresh Chromium passed the unchanged create/open/submit/read/snapshot/disconnect/reconnect flow against the native service after the public browser adapter gained the same methods. The full charter hypothesis remains inconclusive only until this workstream consumes the blob/summary handoff.
...
The projected-read/recovery handoff composed without changing FSP4 version `1`, duplicating wire models, or adding transport policy. Both adapters call the same shared codec methods and lifecycle path. Blob and summary operations still depend on `content-addressed-blobs-summaries` and remain intentionally absent. The generated Node and browser loaders differ as required by `wasm-bindgen`, but their exported client operations and WASM core are the same.

## Human Interventions

The coordinator supplied the accepted projected-read/recovery prerequisite by cherry-picking `1c6d8977f8a..8e9d36126db` after Wave 1. No semantic intervention was required.
...
- Wait only for the accepted content-addressed blob/summary handoff, then add its public operations without predefining wire kinds, redefining protocol models, or copying retry/recovery policy.
- Add actual-WASM Node coverage for content-addressed blobs and summaries, then rerun the unchanged Chromium trace if that handoff changes the public browser adapter path.
