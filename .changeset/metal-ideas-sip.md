...
- `findLast(callbackFunction)` and `findLastIndex(callbackFunction)` search the array from the last item to the first, returning the last matching item (or `undefined`) and its index (or `-1`) respectively, like their `Array.prototype` equivalents.
