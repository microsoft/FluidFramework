...
Storage drivers that implement `getSnapshot` can observe the alpha target option through `ISnapshotFetchOptionsAlpha.loadToSequenceNumber`.
...
Drivers may also implement alpha `IDocumentDeltaStorageServiceAlpha.resolvePointInTimeMarker` to resolve an app or service marker `{ referenceSequenceNumber, clientId, clientSequenceNumber }` to the global `sequenceNumber` that historical loading accepts.
