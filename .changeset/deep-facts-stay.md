...
- The Application layer and Datastore layer from this version will not be compatible with the Runtime layer before 2.61.0.
...
- `IFluidDataStoreContext.minVersionForCollab` in `@fluidframework/runtime-definitions`.
  - `IFluidDataStoreContext` is referenced in Datastore layer.
- `IFluidDataStoreContextDetached.minVersionForCollab` in `@fluidframework/runtime-definitions`.
  - `IFluidDataStoreContextDetached` is referenced in the Application layer.
