...
- The metadata shares the lifetime of the commit it is attached to. Once that commit is trimmed from the trunk, the metadata goes with it, and reading it through a `TreeBranchCommitMetadata` obtained earlier may throw, so such objects should not be retained indefinitely. Under the `retainHistory` option on `SharedTreeOptions` it is the lifetime of the document.
