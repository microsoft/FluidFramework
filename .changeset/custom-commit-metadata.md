...
- Nested transactions all contribute to the single commit they produce, so their metadata is merged into one object. Where two of them use the same property, the outermost transaction wins. Metadata from a nested transaction that is rolled back does not contribute.
