...
Applications should use SharedTree or another existing DDS type (SharedDirectory, SharedMap, etc.) rather than implementing custom DDSes. The `SharedObjectKind` type will remain public as the safe, sealed type for referencing DDS kinds.
