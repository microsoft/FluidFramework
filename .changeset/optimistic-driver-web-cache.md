...
When `set` is called, the write proceeds and atomically replaces whatever row exists at the key, including cross-partition or stale rows the updater saw as `undefined`.
This matches the unconditional overwrite behavior of `put`.
`update` returns `true` if `set` was called and the write committed, and `false` if the updater returned without calling `set`, threw, or an IDB error occurred.
