...
Container runtimes now reject deployed-client values below `"2.0.0"`. The internal
`"2.0.0-defaults"` sentinel remains available for test and replay infrastructure and continues
to select the historical runtime defaults.

Before upgrading an application to Client 3.0, upgrade every active deployment that must
collaborate to Fluid Framework 2.0.0 or later. Then configure the canonical property or
service-client argument:
