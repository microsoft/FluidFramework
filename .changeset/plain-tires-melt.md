...
Adds a `splice` method on `TreeArrayNode` that supports removing and inserting items in a single operation. Returns the removed items as an array. Supports negative `start` indicies (wraps from end). Optional `deleteCount` (omitting removes everything from `start` onward). Requires safe integers values for `start` and `deleteCount`.
