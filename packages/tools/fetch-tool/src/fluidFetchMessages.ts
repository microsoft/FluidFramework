...
async function* loadAllSequencedMessages(
        ...
        for (let i = 0; i < files.length; i++) {
            ...
            try {
                ...
                assert(messages[0].sequenceNumber === lastSeq + 1,
                    0x177 /* Unexpected value for sequence number of first message in file */);
                yield messages;
    ...
    while (true) {
        ...
        if (messages.length === 0) {
            assert(!partialResult, 0x178 /* No messages to load, but nonzero partial result */);
            break;
        }
        yield messages;
    ...
    while (true) {
        ...
        if (!result.done) {
            ...
            assert(sequencedMessages[0].sequenceNumber === curr,
                0x179 /* Unexpected sequence number on first of messages to save */);
            assert(sequencedMessages[sequencedMessages.length - 1].sequenceNumber
                === curr + sequencedMessages.length - 1,
                0x17a /* Unexpected sequence number on last of messages to save */);
        }
        ...
        while (sequencedMessages.length >= chunk || (result.done && sequencedMessages.length !== 0)) {
            ...
            assert(sequencedMessages.length === 0 || sequencedMessages[0].sequenceNumber === curr,
                0x17b /* Stopped writing at unexpected sequence number */);
            index++;
        }
