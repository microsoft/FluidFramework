...
describe("TelemetryView Accessibility Check", () => {
	...
	it("Can tab/arrow navigate through the TelemetryView", async () => {
		...
		assert.equal(document.activeElement, maxEventsDropdown);
		await user.tab();
		...
		await user.tab();
		assert.equal(document.activeElement, filterCategory);
		await user.tab();
		const eventNameFilter = screen.getByRole("combobox", { name: /Event Name Filter/ });
		assert.equal(document.activeElement, eventNameFilter);
	});
});
