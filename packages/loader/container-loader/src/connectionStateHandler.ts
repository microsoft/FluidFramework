...
class ConnectionStateHandler implements IConnectionStateHandler {
    ...
    private startJoinOpTimer() {
        ...
        assert(this.connection !== undefined, 0x4b2 /* have connection */);
    ...
    private shouldWaitForJoinSignal() {
        assert(this.connection !== undefined, 0x4b3 /* all callers call here with active connection */);
    ...
    public initProtocol(protocol: IProtocolHandler) {
        ...
        this.membership?.on("removeMember", (clientId) => {
            assert(protocol.quorum.getMember(clientId) === undefined, 0x4b5 /* Audience is subset of quorum */);
