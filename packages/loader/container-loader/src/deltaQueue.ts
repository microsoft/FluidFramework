    ...
    private ensureProcessing() {
        if (this.anythingToProcess() && this.processingPromise === undefined) {
            ...
            this.processingPromise = Promise.resolve().then(() => {
                assert(this.processingPromise !== undefined, 0x34b /* reentrancy? */);
                const result = this.processDeltas();
                assert(this.processingPromise !== undefined, 0x34c /* reentrancy? */);
            ...
            assert(this.processingPromise !== undefined, 0x34d /* processDeltas() should run async */);
        }
    }
