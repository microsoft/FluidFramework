    ...
    private ensureProcessing() {
        if (this.anythingToProcess() && this.processingPromise === undefined) {
            ...
            this.processingPromise = Promise.resolve().then(() => {
                assert(this.processingPromise !== undefined, 0x368 /* reentrancy? */);
                const result = this.processDeltas();
                assert(this.processingPromise !== undefined, 0x369 /* reentrancy? */);
            ...
            assert(this.processingPromise !== undefined, 0x36a /* processDeltas() should run async */);
        }
    }
