    ...
    private ensureProcessing() {
        if (this.anythingToProcess() && this.processingPromise === undefined) {
            ...
            this.processingPromise = Promise.resolve().then(() => {
                assert(this.processingPromise !== undefined, 0x349 /* reentrancy? */);
                const result = this.processDeltas();
                assert(this.processingPromise !== undefined, 0x34a /* reentrancy? */);
            ...
            assert(this.processingPromise !== undefined, 0x34b /* processDeltas() should run async */);
        }
    }
