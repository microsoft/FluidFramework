...
`@fluidframework/odsp-driver/legacy` as a lightweight async wrapper. The point-in-time implementation
is loaded on demand, so consumers that do not inject it exclude the implementation from both ESM and
CommonJS main bundles. The consumer injects the wrapper through `createOdspDocumentServiceFactory`
options.
