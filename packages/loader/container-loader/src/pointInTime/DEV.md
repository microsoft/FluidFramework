...
- `OdspDocumentServiceFactory` implements `IPointInTimeDocumentServiceFactory` directly. The deprecated
  `getOdspPointInTimeDocumentServiceFactory` compatibility helper returns the same concrete factory.
...
from `OdspDocumentServiceFactory` and passes that factory directly to `loadContainerToSequenceNumber`.
Keeping detection in Fluid also gives non-ODSP drivers one generic
...
| `@fluidframework/odsp-driver` | The point-in-time capability on `OdspDocumentServiceFactory`, the deprecated `getOdspPointInTimeDocumentServiceFactory` compatibility helper, `pointInTimeDriver/`, and `odspVersionManager/` | These components depend on ODSP file-version APIs, resolved URLs, caches, storage policies, and epoch tracking. Moving them would either leak ODSP internals into the feature package or duplicate driver construction logic. |
...
`OdspDocumentServiceFactory` implements `createPointInTimeDocumentService` as a prototype method and
dynamically imports a single dedicated implementation module. `OdspDocumentServiceFactoryCore` and
the local ODSP factory remain point-in-time-free. The deprecated
`getOdspPointInTimeDocumentServiceFactory` compatibility helper returns the concrete factory.
...
| `packages/drivers/odsp-driver/src/odspDocumentServiceFactory.ts` | Exports `IPointInTimeDocumentServiceFactory` and provides the concrete factory's lazy point-in-time prototype method. |
| `packages/drivers/odsp-driver/src/pointInTimeDriver/createPointInTimeDocumentService.ts` | Selects the ODSP base version and creates the historical service with shared epoch tracking behind one lazy import. |
