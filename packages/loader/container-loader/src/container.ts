...
interface PendingOpStateEvents extends IEvent {
	...
	implements IContainer, ContainerAlpha
{
	...
	private readonly pendingOpStateEvents = new TypedEventEmitter<PendingOpStateEvents>();
	...
	private readonly updateDirtyContainerState = (dirty: boolean): void => {
		...
		// Older runtimes only report op-only dirty state. Commit both states before notifying the host.
		if (dirty) {
		...
		// Releasing reconnect waiters can synchronously raise "connected" and submit another op.
		// Keep the public state dirty until callbacks finish so reentrant work cannot emit a stale "saved".
