...
export class ConnectionManager implements IConnectionManager {
    ...
    */
    ...
    private nonRuntimeOpsCount: number = 0;
    ...
    public beforeProcessingIncomingOp(message: ISequencedDocumentMessage) {
        ...
        if (this.lastSubmittedClientId !== undefined && this.lastSubmittedClientId === message.clientId) {
            ...
            if (message.type === "noop") {
                this.nonRuntimeOpsCount++;
            }
            if (this.lastClientSequenceNumber !== undefined) {
                const clientGap = clientSequenceNumber - this.clientSequenceNumberObserved - 1;
                if (clientGap > 0) {
                    if (this.nonRuntimeOpsCount > 0) {
                        this.nonRuntimeOpsCount -= clientGap;
                    } else {
                        throw new Error("gap in client sequence #");
                    }
                }
            }
            this.lastClientSequenceNumber = clientSequenceNumber;
