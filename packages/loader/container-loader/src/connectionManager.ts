...
export class ConnectionManager implements IConnectionManager {
	...
	public shouldJoinWrite(): boolean {
		...
		// no-ops). Op dirty state also covers stashed ops that have not reached the container layer yet.
		// Aggregate dirty state may request write mode for local work that has not produced an op.
	 ...
	 *
	 * @remarks Unlike {@link ConnectionManager.shouldJoinWrite}, this excludes aggregate dirty-state
	 * contributions that have not produced an operation. Reconnect ordering only needs to wait for the
