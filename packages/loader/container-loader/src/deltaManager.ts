	...
	implements IDeltaManagerFull, IEventProvider<IDeltaManagerInternalEvents>
{
	...
	public close(severity: Severity, error?: ICriticalContainerError): void {
		...
		this.connectionManager.dispose(severity, error, true /* switchToReadonly */);
		this.clearQueues();
	 ...
	 */
	public dispose(severity: Severity, error?: Error | ICriticalContainerError): void {
		if (this._disposed) {
			return;
		}
		...
		this.connectionManager.dispose(severity, error, false /* switchToReadonly */);
		this.clearQueues();
