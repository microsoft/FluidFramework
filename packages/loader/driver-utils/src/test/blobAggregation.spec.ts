...
class InMemoryStorage {
	...
	async getSnapshotTree(version) {
		...
		return buildSnapshotTree(
			convertSummaryTreeToITree(this.summaryWritten).entries,
			this.blobs,
		);
	}
						...
						".channels": {
							...
							tree: {
								...
								blob3: {
									type: SummaryType.Blob,
									content:
										"not very small string - exceeding 40 bytes limit for sure",
								},
								channel1: {
									type: SummaryType.Tree,
									tree: {
...
async function prep(allowPacking: boolean, blobSizeLimit: number | undefined) {
	...
	assert(
		Object.keys(service.flattenedTree).length === 7,
	...
	assert(
		bufferToString(await service.readBlob(".channels/dataStore1/.channels/blob2"), "utf8") ===
			"small string 2",
		"blob2 failed",
	);
	assert(
		bufferToString(await service.readBlob(".channels/dataStore1/.channels/blob3"), "utf8") ===
			"not very small string - exceeding 40 bytes limit for sure",
		"blob3 failed",
	);
	assert(
		bufferToString(
			await service.readBlob(".channels/dataStore1/.channels/channel1/blob4"),
			"utf8",
		) === "small string again",
		"blob4 failed",
	);
