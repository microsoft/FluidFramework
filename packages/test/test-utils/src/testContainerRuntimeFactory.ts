...
export const createTestContainerRuntimeFactory = (
	...
	return class extends RuntimeFactoryHelper {
		...
		public async instantiateFirstTime(
			...
			const dataStore = await rootContext.attachRuntime(
		...
		async preInitialize(
			...
			if (containerRuntimeCtor.loadRuntime === undefined) {
				...
				return (
					containerRuntimeCtor as any as backCompat_ContainerRuntime
				).load(
					context,
					[
...
export const TestContainerRuntimeFactory =
	createTestContainerRuntimeFactory(ContainerRuntime);
