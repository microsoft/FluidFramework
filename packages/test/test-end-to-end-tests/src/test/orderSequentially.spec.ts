...
describeCompat(
	...
	(getTestObjectProvider, apis) => {
		...
		beforeEach("setup", async () => {
			...
			sharedString2 =
				await dataObject.getSharedObject<ISharedString>(string2Id);
		...
		it("Should rollback simple edits on multiple DDS types", () => {
			...
			assert.deepEqual(changedEventData[1].event, {
				key: "key1",
			...
			assert.deepEqual(changedEventData[4].event, {
				key: "key1",
				previousValue: 0,
			});
			...
			assert.deepEqual(changedEventData[7].event, {
				key: "key1",
				previousValue: undefined,
			});
		...
		it("Should rollback complex edits on multiple DDS types", () => {
			...
			assert.deepEqual(changedEventData[16].event, {
				key: "key1",
				previousValue: 3,
			});
		});
		...
		it("Should handle rollback on multiple instances of the same DDS type", () => {
			...
			assert.deepEqual(changedEventData[3].event, {
				key: "key",
				previousValue: 1,
			});
			...
			assert.deepEqual(changedEventData[8].event, {
				key: "key",
				previousValue: undefined,
			});
		});
		...
		it("Should handle nested calls to orderSequentially", () => {
			...
			assert.deepEqual(changedEventData[2].event, {
				key: "key",
				previousValue: 1,
			});
			...
			assert.deepEqual(changedEventData[5].event, {
				key: "key",
				previousValue: 0,
			});

			// rollback - outer orderSequentially call
			assert.deepEqual(changedEventData[6].event, {
				key: "key",
				previousValue: undefined,
			});
