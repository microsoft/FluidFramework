 ...
 */
async function dereferenceToTestFluidObject(
	handle: IFluidHandle,
...
async function dereferenceToSharedObject<TSharedObject>(
	handle: IFluidHandle,
	sharedObjectKind: ISharedObjectKind<TSharedObject> &
		SharedObjectKind<TSharedObject>,
...
describeCompat(
	...
	(getTestObjectProvider, apis) => {
		...
		const handleStorageFactories: HandleStorageFactory[] = [
			{
				...
				createDDS(runtime) {
					const map = runtime.createChannel(
						undefined,
			...
			},
			{
				...
				createDDS(runtime) {
					const cell = runtime.createChannel(
						undefined,
			...
			},
			{
				...
				createDDS(runtime) {
					const directory = runtime.createChannel(
						undefined,
			...
			},
			{
				...
				createDDS(runtime) {
					const string = runtime.createChannel(
						undefined,
			...
			},
			{
				...
				downCast(channel): HandleStorage {
					const view: TreeView<typeof Bar> = treeSetup(
						channel as IFluidLoadable as ITree,
					);
			...
			},
			{
				...
				createDDS(runtime) {
					const register = runtime.createChannel(
						undefined,
						ConsensusQueue.getFactory().type,
					);
					return this.downCast(register);
				},
				downCast(channel): HandleStorage {
					...
					return {
						...
						async readHandle(): Promise<unknown> {
							const value = await timeoutAwait(
									...
									queue
										...
										.then((wasNonempty) => {
											if (!wasNonempty) {
												...
												reject(
													new Error("No values found in consensus queue."),
												);
											}
										})
								...
								{
									errorMsg:
										"Timeout waiting for acquiring value from consensus queue.",
								},
							);
							return value;
						},
		...
		async function getReferencedDDS(
			handle: IFluidHandle,
		): Promise<HandleStorage> {
			...
			describe(handleStorageFactory.id, () => {
				...
				it(`can store a handle to detached dataObject while detached then attach`, async () => {
...

					{
						...
						const dataObjectB =
							(await dataStoreB.entryPoint.get()) as ITestFluidObject;
					...
					// Validate that the created objects were attached and have correct data in the new container.
					{
						...
						assert(
							isFluidHandle(dataObjectHandle),
							`not a handle: ${dataObjectHandle}`,
						);

						const dataObjectB =
							await dereferenceToTestFluidObject(dataObjectHandle);

						assert.equal(dataObjectB.root.get("foo"), "bar");
						assert.equal(
							dataObjectB.root.get("dataStoreId"),
							dataObjectB.context.id,
						);
					}
				});
		...
		for (const {
			...
			it(`stores ${detachedFactory1.id} handle in ${detachedFactory2.id} and attaches by storing in ${attachedFactory.id}`, async () => {
				 ...
				 */
				const createdDds1 = detachedFactory1.createDDS(
				 ...
				 */
				const createdDds2 = detachedFactory2.createDDS(
					attachedDataStore.runtime,
				);
				 ...
				 */
				const attachedDds = await getExistingHandleStorage(
					attachedDataStore,
					attachedFactory,
				);
				...
				const attached2 = await getExistingHandleStorage(
					default2,
					attachedFactory,
				);
				/**
