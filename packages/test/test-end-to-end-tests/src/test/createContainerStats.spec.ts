	...
	(getTestObjectProvider, apis) => {
		...
		async function waitForSummary(): Promise<string> {
			...
			const ackedSummary: IAckedSummary =
				await summaryCollection.waitSummaryAck(
				...
				[
					{
						...
						clientType: summarizer
							? "noninteractive/summarizer"
							: "interactive",
					},
				],
				message,
		...
		beforeEach("setup", async function () {
			...
			mainDataStore =
				await getContainerEntryPointBackCompat<TestDataObject>(mainContainer);
			...
			const loadStatEvents = mockLogger.events.filter(
				(event) =>
					event.eventName ===
					"fluid:telemetry:ContainerRuntime:ContainerLoadStats",
			);
			assert(
				loadStatEvents.length === 1,
				"There should only be one event for the created container",
			);
			createContainerTimestamp = loadStatEvents[0]
				.createContainerTimestamp as number;
			createContainerRuntimeVersion = loadStatEvents[0]
				.createContainerRuntimeVersion as number;
		...
		it("should load summarizer with correct create stats", async () => {
		...
		it("should load container with correct create stats", async () => {
			// Wait for summarizer to load and submit a summary. Validate that it loads from summaryNumber 1 and has
			// 1 data store which is referenced.
			...
			validateLoadStats(
				...
				1,
				1,
				"Second container should load with correct create stats",
			);
		});

		it("should load container with correct data store load stats", async () => {
			...
			const dataStore2 = await dataObjectFactory.createInstance(
				mainDataStore.containerRuntime,
			);
			...
			validateLoadStats(
				...
				2,
				2,
				"Second container should load with correct data store stats",
			);
		});

		it("should load second summarizer with correct stats", async () => {
