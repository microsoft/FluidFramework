...
describeCompat(
	...
	(getTestObjectProvider, apis) => {
		...
		const sharedTreeShimFactory = new SharedTreeShimFactory(
		...
		it("Detached handles", async () => {
			...
			const childObject1 = await childObjectFactory.createInstance(
			...
			await provider.ensureSynchronized();
			assert(
				obj1._root.get("foo") === "bar",
				"expected obj1 to be live and sync",
			);
			assert(
				obj2._root.get("foo") === "bar",
				"expected obj2 to be live and sync",
			);
		});
	},
);
