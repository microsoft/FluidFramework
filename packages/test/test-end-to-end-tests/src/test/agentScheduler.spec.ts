...
describeCompat(
	...
	(getTestObjectProvider, apis) => {
		const getContainerRuntimeFactory = (forLoad: boolean) => {
			...
			const agentSchedulerFactory =
				forLoad && apis.dataRuntimeForLoading !== undefined
					? apis.dataRuntimeForLoading?.packages.agentScheduler
							.AgentSchedulerFactory
					: apis.dataRuntime.packages.agentScheduler.AgentSchedulerFactory;

			const TestContainerRuntimeFactory =
				createTestContainerRuntimeFactory(runtime);
			return {
		...
		const getAgentScheduler = async (
			container: IContainer,
		): Promise<IAgentScheduler> => {
			const scheduler =
				await getContainerEntryPointBackCompat<IAgentScheduler>(container);
		...
		describe("State transitions", () => {
			...
			beforeEach("createContainersAndSchedulers", async () => {
				container1 = await createContainer();
				scheduler1 =
					await getContainerEntryPointBackCompat<IAgentScheduler>(container1);

				container2 = await loadContainer();
				scheduler2 =
					await getContainerEntryPointBackCompat<IAgentScheduler>(container2);
			});
			...
			it("Tasks are released after forcing read mode", async () => {
				...
				assert.strict(
					taskSubscription.haveTask(),
					"Failed to get task in write mode",
				);
				...
				assert.strict(
					!taskSubscription.haveTask(),
					"Still have task after forcing readonly",
				);
			});
			...
			it("Tasks are released after closing the container", async () => {
				...
				assert.strict(
					taskSubscription1.haveTask(),
					"Container 1 should have task",
				);
				assert.strict(
					!taskSubscription2.haveTask(),
					"Container 2 should not have task",
				);
				...
				assert.strict(
					!taskSubscription1.haveTask(),
					"Container 1 should not have task",
				);
				assert.strict(
					taskSubscription2.haveTask(),
					"Container 2 should have task",
				);
			});
		});
	},
