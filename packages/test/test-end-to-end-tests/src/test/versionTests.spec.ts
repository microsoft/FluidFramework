...
describeWithVersions({
    specificVersions: oldVersions,
})("Tests with different Fluid versions installed", (getTestObjectProvider) => {
    ...
    oldVersions.forEach((version: string) => {
        it(`Test runtime version: ${version}`, async () => {
            ...
            assert.strictEqual(
                (oldDataObject.context.containerRuntime as any).createContainerMetadata.createContainerRuntimeVersion,
                version);
        });
    });
});
