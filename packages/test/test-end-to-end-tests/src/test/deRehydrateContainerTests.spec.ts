...
describeFullCompat(
    ...
    (getTestObjectProvider) => {
        ...
        function assertBlobContents<T>(
            subtree: ISnapshotTreeWithBlobContents,
            key: string,
        ): T {
        ...
        const assertProtocolAttributes = (s: ISnapshotTreeWithBlobContents) =>
            assertBlobContents<IDocumentAttributes>(
                assertProtocolTree(s),
                "attributes",
            );
        ...
        const getSnapshotTreeFromSerializedSnapshot = (
            ...
            return getSnapshotTreeFromSerializedContainer(
                JSON.parse(container.serialize()),
            );
        };
        ...
        const tests = () => {
            it("Dehydrated container snapshot", async () => {
                ...
                const protocolAttributes =
                    assertProtocolAttributes(snapshotTree);
                assert.strictEqual(
                ...
                const protocolAttributesBlobId =
                    snapshotTree.trees[".protocol"].blobs.attributes;
                assert(
                    snapshotTree.trees[".protocol"].blobsContents[
            ...
            it("Dehydrated container snapshot 2 times with changes in between", async () => {
                ...
                // Check for protocol attributes
                const protocolAttributes1 =
                    assertProtocolAttributes(snapshotTree1);
                const protocolAttributes2 =
                    assertProtocolAttributes(snapshotTree2);
                assert.strictEqual(
            ...
            it(
                "Dehydrated container snapshot with dataStore handle stored in map of " +
                    "other bound dataStore",
                async () => {
                    const { container, defaultDataStore } =
                        await createDetachedContainerAndGetRootDataStore();
                    ...
                    const dataStore2 =
                        peerDataStore.peerDataStore as TestFluidObject;

                    // Create a channel
                    const rootOfDataStore1 =
                        await defaultDataStore.getSharedObject<SharedMap>(
                            sharedMapId,
                        );
            ...
            it("Rehydrate container from snapshot and check contents before attach", async () => {
                ...
                const container2 =
                    await loader.rehydrateDetachedContainerFromSnapshot(
                        snapshotTree,
                    );
                ...
                const sharedString =
                    await defaultDataStore.getSharedObject<SharedString>(
                        sharedStringId,
                    );
                const sharedCell =
                    await defaultDataStore.getSharedObject<SharedCell>(
                        sharedCellId,
                    );
                const sharedCounter =
                    await defaultDataStore.getSharedObject<SharedCounter>(
                        sharedCounterId,
                    );
                const crc = await defaultDataStore.getSharedObject<
                ...
                const ink = await defaultDataStore.getSharedObject<Ink>(
                    sharedInkId,
                );
                const sharedMatrix =
                    await defaultDataStore.getSharedObject<SharedMatrix>(
                        sharedMatrixId,
                    );
                const sparseMatrix =
                    await defaultDataStore.getSharedObject<SparseMatrix>(
                        sparseMatrixId,
                    );
                assert.strictEqual(
                    sharedMap.id,
            ...
            it("Rehydrate container multiple times round trip serialize/deserialize", async () => {
                ...
                assert.strictEqual(
                    response.status,
                    200,
                    `Component should exist!! ${response.value}`,
                );
                const defaultDataStore = response.value as TestFluidObject;
            ...
            itExpects(
                ...
                async () => {
                    ...
                    await defaultDataStore.context.storage
                    ...
                    const defaultDataStore2 =
                        response2.value as TestFluidObject;
                    assert(
                    ...
                    await defaultDataStore2.context.storage
                        .getSnapshotTree(undefined)
                        .catch((err) => {
                            success2 = false;
                        });
            ...
            it("Change contents of dds, then rehydrate and then check summary", async () => {
                ...
                const defaultDataStoreBefore =
                    responseBefore.value as TestFluidObject;
                const sharedStringBefore =
                    await defaultDataStoreBefore.getSharedObject<SharedString>(
                        sharedStringId,
                    );
                const intervalsBefore =
                    sharedStringBefore.getIntervalCollection("intervals");
                sharedStringBefore.insertText(0, "Hello");
                let interval0 = intervalsBefore.add(
                    0,
                    0,
                    IntervalType.SlideOnRemove,
                );
                let interval1 = intervalsBefore.add(
                    0,
                    1,
                    IntervalType.SlideOnRemove,
                );
                ...
                const defaultComponentAfter =
                    responseAfter.value as TestFluidObject;
                const sharedStringAfter =
                    await defaultComponentAfter.getSharedObject<SharedString>(
                        sharedStringId,
                    );
                const intervalsAfter =
                    sharedStringAfter.getIntervalCollection("intervals");
                assert.strictEqual(
                ...
                for (const interval of intervalsAfter) {
                    assert.fail(
                        `Unexpected interval after rehydration:
                        ${interval.start.getOffset()}-${interval.end.getOffset()}`,
                    );
                }
            });
            ...
            it("Rehydrate container from summary, change contents of dds and then check summary", async () => {
                ...
                const sharedString1 =
                    await defaultComponent1.getSharedObject<SharedString>(
                        sharedStringId,
                    );
                ...
                const sharedMapBefore =
                    await defaultDataStoreBefore.getSharedObject<SharedMap>(
                        sharedMapId,
                    );
                str += "BB";
                ...
                const sharedMapAfter =
                    await defaultComponentAfter.getSharedObject<SharedMap>(
                        sharedMapId,
                    );
                assert.strictEqual(
                    JSON.stringify(sharedStringAfter.summarize()),
            ...
            it(
                "Rehydrate container, don't load a data store and then load after container attachment. " +
                    "Make changes to dds from rehydrated container and check reflection of changes in other container",
                async function() {
                    if (
                        this.test?.parent?.title?.includes("N-2") &&
                        provider.driver.type === "tinylicious"
                    ) {
                        // 0.58.x doesn't have the fix to routerlicious-driver in this PR:
                        // https://github.com/microsoft/FluidFramework/pull/8913
                    ...
                    const sharedMap1 =
                        await dataStore2.getSharedObject<SharedMap>(
                            sharedMapId,
                        );
                    sharedMap1.set("0", "A");
                    const snapshotTree = container.serialize();
                    ...
                    const dataStore2FromRC =
                        responseBefore.value as TestFluidObject;
                    const sharedMapFromRC =
                        await dataStore2FromRC.getSharedObject<SharedMap>(
                            sharedMapId,
                        );
                    ...
                    const sharedMap3 =
                        await dataStore3.getSharedObject<SharedMap>(
                            sharedMapId,
                        );
                    ...
                    assert.strictEqual(
                        sharedMap3.get("1"),
                        "B",
                        "Contents should be as required",
                    );
                    assert.strictEqual(
            ...
            it(
                "Rehydrate container, create but don't load a data store. Attach rehydrated container and load " +
                    "container 2 from another loader. Then load the created dataStore from container 2, " +
                    "make changes to dds in it check reflection of changes in rehydrated container",
                async function() {
                    if (
                        this.test?.parent?.title?.includes("N-2") &&
                        provider.driver.type === "tinylicious"
            ...
            it(
                "Container rehydration with not bounded dataStore handle stored in root of " +
                    "other bounded dataStore",
                async () => {
                    const { container, defaultDataStore } =
                        await createDetachedContainerAndGetRootDataStore();
            ...
            it("Container rehydration with not bounded dds handle stored in root of bounded dataStore", async () => {
                ...
                assert.strictEqual(
                    ddd2FromRC.id,
                    dds2.id,
                    "Both dds id should match",
                );
            });

            it(
                "Container rehydration with not bounded dds handle stored in root of bound dataStore. " +
                    "The not bounded dds also stores handle not bounded data store",
                async () => {
                    const { container, defaultDataStore } =
                        await createDetachedContainerAndGetRootDataStore();
                    ...
                    assert(
                        ddd2FromRC,
                        "ddd2 should have been serialized properly",
                    );
                    assert.strictEqual(
                        ddd2FromRC.id,
                        ddsId,
                    ...
                    const responseForDataStore =
                        await rehydratedContainer.request({
                            url: `/${dataStore2.context.id}`,
                        });
                    const dataStore2FromRC =
                        responseForDataStore.value as TestFluidObject;
                    assert(
                        dataStore2FromRC,
                        "DataStore2 should have been serialized properly",
                    );
            ...
            it("can rehydrate from arbitrary summary that is not generated from serialized container", async () => {
                const summaryTree = buildSummaryTree(
                    baseAttributes,
                    baseQuorum,
                    baseSummarizer,
                );
