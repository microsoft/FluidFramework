...
describeCompat(
	...
	(getTestObjectProvider, apis) => {
		...
		beforeEach("createContainers", async () => {
			...
			sharedDirectory1 =
				await dataObject1.getSharedObject<SharedDirectory>(directoryId);
			...
			sharedDirectory2 =
				await dataObject2.getSharedObject<SharedDirectory>(directoryId);
			...
			sharedDirectory3 =
				await dataObject3.getSharedObject<SharedDirectory>(directoryId);
		...
		function expectAllSize(size: number, path?: string) {
			const dir1 = path
				? sharedDirectory1.getWorkingDirectory(path)
				: sharedDirectory1;
			const dir2 = path
				? sharedDirectory2.getWorkingDirectory(path)
				: sharedDirectory2;
			const dir3 = path
				? sharedDirectory3.getWorkingDirectory(path)
				: sharedDirectory3;
			...
			assert.equal(
				keys1.length,
				size,
				"Incorrect number of Keys in container 1",
			);
			const keys2 = Array.from(dir2.keys());
			assert.equal(
				keys2.length,
				size,
				"Incorrect number of Keys in container 2",
			);
			const keys3 = Array.from(dir3.keys());
			assert.equal(
				keys3.length,
				size,
				"Incorrect number of Keys in container 3",
			);
		...
		describe("Root operations", () => {
			...
			describe("Eventual consistency after simultaneous operations", () => {
				...
				it("set/clear", async () => {
					...
					assert.equal(
						sharedDirectory3.size,
						0,
						"Incorrect map size after clear",
					);
		...
		describe("SubDirectory operations", () => {
			it("should set a key in a SubDirectory in three containers correctly", async () => {
				sharedDirectory1
					.createSubDirectory("testSubDir1")
			...
			it("should have the correct size in three containers", async () => {
				...
				sharedDirectory3
					.createSubDirectory("otherSubDir2")
				...
				sharedDirectory1.on("valueChanged", (changed, local) => {
					if (!local) {
						assert.equal(
							changed.key,
							"testKey1",
							"Incorrect value for key in container 1",
						);
						assert.equal(
							changed.path,
							"/testSubDir1",
				...
				sharedDirectory2.on("valueChanged", (changed, local) => {
					if (!local) {
						assert.equal(
							changed.key,
							"testKey1",
							"Incorrect value for key in container 2",
						);
						assert.equal(
							changed.path,
							"/testSubDir1",
				...
				sharedDirectory3.on("valueChanged", (changed, local) => {
					if (!local) {
						assert.equal(
							changed.key,
							"testKey1",
							"Incorrect value for key in container 3",
						);
						assert.equal(
							changed.path,
							"/testSubDir1",
			...
			it("Only creates a subdirectory once when simultaneously created", async () => {
				...
				assert.strictEqual(
					root1SubDir.get("testKey2"),
					"testValue2",
					"Value 2 not present",
				);
				assert.strictEqual(
					root2SubDir.get("testKey"),
					"testValue",
					"Value 1 not present",
				);
			});
			...
			describe("Load new directory with data from local state and process ops", () => {
				...
				it("can process set in local state", async () => {
					...
					const newDirectory2Handle =
						sharedDirectory2.get<IFluidHandle<SharedDirectory>>(
							"newSharedDirectory",
						);
...
describeCompat(
	...
	(getTestObjectProvider, apis) => {
		...
		beforeEach("setup", async () => {
			...
			sharedDir =
				await dataObject.getSharedObject<SharedDirectory>(directoryId);
		...
		it("Should rollback set to prior value", () => {
			...
			assert.equal(
				sharedDir.get("key"),
				"old",
				`Unexpected value ${sharedDir.get("key")}`,
			);
			assert.equal(clearEventCount, 0);
		...
		it("Should rollback deleted subdirectory with content", () => {
			...
			assert.equal(
				subdir.countSubDirectory ? subdir.countSubDirectory() : 0,
				1,
			);
		...
		it("Should rollback deleted subdirectories with the original order", () => {
			...
			let dirNames = Array.from(sharedDir.subdirectories()).map(
				([dirName, _]) => dirName,
			);
			...
			// rollback
			dirNames = Array.from(sharedDir.subdirectories()).map(
				([dirName, _]) => dirName,
			);
			assert.deepStrictEqual(dirNames, ["dir2", "dir3", "dir1"]);
		});
		...
		it("Should rollback deleted subdirectory when multiple subdirectories exist", () => {
			...
			assert.deepStrictEqual(subDirCreatedEventData, [
				"dir2",
				"dir3",
				"dir1",
				"dir3",
...
describeCompat(
	...
	(getTestObjectProvider, apis) => {
		...
		function expectSubdirsOrder(
			...
			const subdirs = Array.from(dir.subdirectories()).map(
				([subdirName, _]) => {
					return subdirName;
				},
			);
