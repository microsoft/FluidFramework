...
const DataObject1Type = "TestSharedDataObject1";
const DataObject2Type = "TestSharedDataObject2";
const DataObjectWithHeadersType = "TestSharedDataObject1WithHeaders";

class BaseTestDataObject extends DataObject {
    public get _root() {
        return this.root;
    }
    ...
    public type = "BaseTestDataStore";
}

function getVersionedDataObjectFactory1(dataStoreApi: DataRuntimeApiType) {
    class TestSharedDataObject1 extends BaseTestDataObject {
        public readonly type = DataObject1Type;
    ...
    return new dataStoreApi.DataObjectFactory(
...
function getVersionedDataObjectFactory2(dataStoreApi: DataRuntimeApiType) {
    class TestSharedDataObject2 extends BaseTestDataObject {
        public readonly type = DataObject2Type;
...
function getVersionedDataObjectFactoryWithHeaders(dataStoreApi: DataRuntimeApiType) {
    /**
     ...
     */
    class TestDataObjectWithRequestHeaders extends BaseTestDataObject {
        public readonly type = DataObjectWithHeadersType;
...
function getVersionedContainerRuntimeFactory(
    runtimeApi: ContainerRuntimeApiType,
    dataObjectFactory1: DataObjectFactory<DataObject>,
    dataObjectFactory2: DataObjectFactory<DataObject>,
    dataObjectFactoryWithHeaders: DataObjectFactory<DataObject>,
) {
    const innerRequestHandler = async (request: IRequest, runtime: IContainerRuntimeBase) =>
        runtime.IFluidHandleContext.resolveHandle(request);

    return new runtimeApi.ContainerRuntimeFactoryWithDefaultDataStore(
        dataObjectFactory1,
        [
            [dataObjectFactory1.type, Promise.resolve(dataObjectFactory1)],
            [dataObjectFactory2.type, Promise.resolve(dataObjectFactory2)],
            [dataObjectFactoryWithHeaders.type, Promise.resolve(dataObjectFactoryWithHeaders)],
        ],
        undefined,
        [innerRequestHandler],
    );
...
// Full compat can be enabled after LTS is version >= 0.59.1000. This test uses container.connect() which was added
// by this PR - https://github.com/microsoft/FluidFramework/pull/9439.
describeNoCompat("Loader.request", (getTestObjectProvider) => {
    let provider: ITestObjectProvider;
    let loader: IHostLoader;
    let container: IContainer;

    let dataStore1: BaseTestDataObject;
    let dataStore2: BaseTestDataObject;
    let dataObjectFactory1: DataObjectFactory<BaseTestDataObject>;
    let dataObjectFactory2: DataObjectFactory<BaseTestDataObject>;
    let dataObjectFactoryWithHeaders: DataObjectFactory<BaseTestDataObject>;

    beforeEach(async () => {
        provider = getTestObjectProvider();
        assert(provider.containerRuntimeApi !== undefined, "Container runtime api not available");
        assert(provider.dataRuntimeApi !== undefined, "Data runtime api not available");

        dataObjectFactory1 = getVersionedDataObjectFactory1(provider.dataRuntimeApi);
        dataObjectFactory2 = getVersionedDataObjectFactory2(provider.dataRuntimeApi);
        dataObjectFactoryWithHeaders = getVersionedDataObjectFactoryWithHeaders(provider.dataRuntimeApi);

        const runtimeFactory = getVersionedContainerRuntimeFactory(
            provider.containerRuntimeApi,
            dataObjectFactory1,
            dataObjectFactory2,
            dataObjectFactoryWithHeaders,
        );
        ...
        dataStore2 = await dataObjectFactory2.createInstance(dataStore1._context.containerRuntime);
    ...
    it("can create the data objects with correct types", async () => {
        ...
        const testDataStore1 = await requestFluidObject<BaseTestDataObject>(loader, testUrl1);
        const testUrl2 = await container.getAbsoluteUrl(dataStore2.handle.absolutePath);
        assert(testUrl2, "dataStore2 url is undefined");
        const testDataStore2 = await requestFluidObject<BaseTestDataObject>(loader, testUrl2);

        assert(testDataStore1.type === DataObject1Type, "requestFromLoader returns the wrong type for default");
        assert(testDataStore2.type === DataObject2Type, "requestFromLoader returns the wrong type for object2");
    });
    ...
    it("can create data object using url with second id, having correct type and id", async () => {
        ...
        const testDataStore = await requestFluidObject<BaseTestDataObject>(loader, dataStore2Url);

        assert(testDataStore.type === DataObject2Type, "request returns the wrong type with long url");
    ...
    it("can create data object using url with second id, having distinct value from default", async () => {
        ...
        const testDataStore = await requestFluidObject<BaseTestDataObject>(loader, url);
    ...
    it("loaded container is paused using loader pause flags", async () => {
        ...
        const newDataStore = await dataObjectFactory2.createInstance(dataStore1._context.containerRuntime);
        ...
        let success = true;
        try {
            await requestFluidObject<BaseTestDataObject>(container2, {
                url: newDataStore.id,
        ...
        const newDataStore2 = await requestFluidObject<BaseTestDataObject>(container2, { url: newDataStore.id });
        assert(newDataStore2.type === DataObject2Type, "requestFromLoader returns the wrong type for object2");
    });
    ...
    it("caches the loaded container across multiple requests as expected", async () => {
        ...
        const sameDataStore1 = await requestFluidObject<BaseTestDataObject>(container1, {
            url: newDataStore.id,
            headers: { wait: false },   // data store load default wait to true currently
        });
        const sameDataStore2 = await requestFluidObject<BaseTestDataObject>(container2, {
            url: newDataStore.id,
            headers: { wait: false },   // data store load default wait to true currently
        });
    ...
    it("can handle requests with headers", async () => {
        const dataStoreWithRequestHeaders =
            await dataObjectFactoryWithHeaders.createInstance(dataStore1._context.containerRuntime);
