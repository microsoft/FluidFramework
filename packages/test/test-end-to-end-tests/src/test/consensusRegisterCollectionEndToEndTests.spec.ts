...
describeCompat(
	...
	(getTestObjectProvider, apis) => {
		...
		it("Basic functionality", async () => {
			const collection1 = ConsensusRegisterCollection.create(
				dataStore1.runtime,
			);
			...
			const collection2Handle =
				sharedMap2.get<IFluidHandle<IConsensusRegisterCollection>>(
					"collection",
				);
			const collection3Handle =
				sharedMap3.get<IFluidHandle<IConsensusRegisterCollection>>(
					"collection",
				);
		...
		it("Should store all concurrent writings on a key in sequenced order", async () => {
			...
			assert.strictEqual(
				versions.length,
				3,
				"Concurrent updates were not preserved",
			);
		...
		it("Happened after updates should overwrite previous versions", async () => {
			...
			assert.strictEqual(
				versions2.length,
				1,
				"Happened after value did not overwrite",
			);
			assert.strictEqual(
				versions2[0],
				"value4",
		...
		it("Resolves write promise with false when disposed", async () => {
			...
			assert.strictEqual(
				write1Result,
				false,
				"Write should resolve with false when disposed",
			);
		});
		...
		it("Can store handles", async () => {
			...
			const sharedMap1HandleB = collection1.read(
				"handleB",
			) as IFluidHandle<ISharedMap>;
			const sharedMap1Prime = await sharedMap1HandleB.get();
			const sharedMap2HandleA = collection2.read(
				"handleA",
			) as IFluidHandle<ISharedMap>;
	...
	(getTestObjectProvider, apis) => {
		describe("grouped batching", () => {
			...
			it("grouped batching doesn't hit 0x071", async () => {
				...
				const collection = ConsensusRegisterCollection.create(
					dataObject.runtime,
				);
		...
		describe("rollback", () => {
			...
			it("Resolves write promise with false when rollback happens", async () => {
				...
				assert.notStrictEqual(
					write1P,
					undefined,
					"Write promise should be defined",
				);
				const write1Result = await write1P;
				assert.strictEqual(
					write1Result,
