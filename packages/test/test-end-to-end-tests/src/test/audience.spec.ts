...
describeCompat(
	...
	(getTestObjectProvider, apis) => {
		...
		it("should add clients in audience as expected", async () => {
			...
			assert(
				client1Container.clientId !== undefined,
				"client1 does not have clientId",
			);
			...
			assert(
				client2Container.clientId !== undefined,
				"client2 does not have clientId",
			);
			await waitForClientAdd(
				client2Container,
		...
		it("should add clients in audience as expected in write mode", async () => {
			...
			const client1DataStore =
				await getContainerEntryPointBackCompat<TestDataObject>(
					client1Container,
				);
