...
describeCompat("GC version update", "NoCompat", (getTestObjectProvider, apis) => {
	 ...
	 * that the GC data is regenerated by all the nodes. Also, validates that GC is enabled or disabled as per the
	 * "gcDisabled" param.
	 */
	async function summarizeAndValidateGCStateReset(summarizer: ISummarizer, gcEnabled: boolean) {
		...
		assert.strictEqual(
			containerRuntime.garbageCollector.shouldRunGC,
			gcEnabled,
	...
	it("should reset GC state when GC version is newer that the one in base snapshot", async () => {
		...
		// Validate that the GC state is reset in the base GC details and that GC is enabled.
		await summarizeAndValidateGCStateReset(summarizer2, true /* gcEnabled */);
	});

	it("should disable GC and reset GC state when GC version is older than the one in base snapshot", async () => {
		// Stores the ids of data stores whose summary tree should be handles.
		const dataStoresAsHandles: string[] = [];
		...
		// Validate that the GC state is reset in the base GC details and that GC is enabled.
		await summarizeAndValidateGCStateReset(summarizer2, false /* gcEnabled */);
	});
});
