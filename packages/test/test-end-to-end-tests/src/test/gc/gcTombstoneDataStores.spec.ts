	...
	(getTestObjectProvider, apis) => {
		...
		const summarizationWithUnreferencedDataStoreAfterTime = async (
			...
			const { container: summarizingContainer2, summarizer: summarizer2 } =
				await loadSummarizer(container, summaryVersion);
		...
		const getTombstonedDataObjectFromSummary = async (
			...
			// Get dataObject
			const containerRuntime = defaultDataObject._context
				.containerRuntime as any;
			const dataStoreContext =
				containerRuntime.channelCollection.contexts.get(id);
			const dataStoreRuntime: IFluidDataStoreChannel =
				await dataStoreContext.realize();
		...
		const setupContainerCloseErrorValidation = (
			...
			container.on("closed", (error) => {
				...
				assert(
					error.message ===
						`Context is tombstoned! Call site [${expectedCall}]`,
				);
			});
		};
		...
		async function createDataStore(sourceDataStore: ITestDataObject) {
			const ds =
				await sourceDataStore._context.containerRuntime.createDataStore(
					TestDataObjectType,
				);
		...
		describe("Loading tombstoned data stores", () => {
			...
			itExpects(
				...
				async () => {
					...
					const ddsHandle =
						dataObject._root.get<IFluidHandleInternal<ISharedMap>>("dds1");
					assert(
						ddsHandle !== undefined,
						"Expected to find a handle to the DDS",
					);
					...
					const summarizerRuntime = (summarizer as any)
			...
			itExpects(
				...
				async () => {
					...
					testContainerConfig.runtimeOptions!.gcOptions!.gcGeneration =
						undefined;
				},
			);
			...
			itExpects(
				...
				async () => {
					...
					assert.equal(
						tombstoneError?.underlyingResponseHeaders?.[
							TombstoneResponseHeaderKey
						],
						true,
			...
			itExpects(
				...
				async () => {
					...
					const containerRuntime = entryPoint._context
						.containerRuntime as ContainerRuntime;
					const tombstoneResponse = await containerRuntime.resolveHandle({
						url: unreferencedId,
					});
					assert.equal(
						tombstoneResponse.status,
						404,
						"Expected 404 tombstone response",
					);
					assert.equal(
					...
					const tombstonedObject =
						tombstoneAllowedResponse.value as ITestDataObject;
					const defaultDataObject =
						(await tombstoneContainer.getEntryPoint()) as ITestDataObject;
					defaultDataObject._root.set("store", tombstonedObject.handle);

					// The datastore should be un-tombstoned now
					const { summaryVersion: revivalVersion } =
							...
							[
								{
									eventName:
										"fluid:telemetry:ContainerRuntime:GarbageCollector:GC_Tombstone_DataStore_Revived",
									clientType:
										i === 0 ? "interactive" : "noninteractive/summarizer",
								},
							],
			...
			itExpects(
				...
				async () => {
					...
					const { summaryVersion: summaryVersion0 } =
						await summarize(summarizer0);
					...
					garbageCollector_toBeCorrupted.baseGCDetailsP =
						garbageCollector_toBeCorrupted.baseGCDetailsP.then(
							(baseGCDetails) => {
					...
					const { summaryVersion: summaryVersion_repaired } = await summarize(
						summarizer,
						{
			...
			it("trailing ops cause tombstone ready data store to be realized by summarizer", async () => {
				...
				mockLogger.assertMatch([
					{
						eventName:
							"fluid:telemetry:Summarizer:Running:TombstoneReadyObject_Realized",
						id: {
							value: `/${unreferencedId}`,
							tag: TelemetryDataTag.CodeArtifact,
						},
		...
		itExpects(
			...
			async () => {
				...
				await assert.doesNotReject(async () => {
					dataObject = await resolveHandleHelper(
						containerRuntime,
						unreferencedId,
					);
		...
		describe("Tombstone information in summary", () => {
			...
			function validateTombstoneState(
				...
				assert(
					actualTombstones !== undefined,
					"GC tree should have tombstones in summary",
				);
				for (const url of tombstones) {
					assert(
						actualTombstones.includes(url),
						`${url} should be in tombstone blob`,
					);
				}
				for (const url of notTombstones) {
					assert(
						!actualTombstones.includes(url),
						`${url} should not be in tombstone blob`,
					);
				}
			}
			...
			it("does not re-summarize GC state on only tombstone state changed", async () => {
				...
				const tombstoneState = getGCTombstoneStateFromSummary(
					summary2.summaryTree,
				);
				assert(
