...
describeCompat("GC data store tombstone tests", "NoCompat", (getTestObjectProvider, apis) => {
	...
	const summarizationWithUnreferencedDataStoreAfterTime = async (
		...
		//* Revisit this - no longer a problem given Op-Based References?
		...
		itExpects(
			...
			async () => {
				...
				assert.equal(
					...
					404,
					"Expected tombstone error - Auto-Recovery shouldn't affect in-progress Interactive sessions",
				);
				...
				//* TODO?: Verify that the object is not Tombstoned in summarizingContainer

				// Container2 loaded after auto-recovery: This request succeeds because the datastore is no longer tombstoned
				...
				assert.equal(
					...
					200,
					"Auto-Recovery should have made the object no longer tombstoned",
				);
				assert.notEqual(
					successResponse2.headers?.[TombstoneResponseHeaderKey],
					true,
					"DID NOT Expect tombstone header to be set on the success response",
				);

				//* TODO: Also confirm request for child or other referenced node succeeds
				...
				assert.equal(
					...
					404,
					"Object should become tombstoned again after the timeout",
				);
				assert.equal(
	...
	describe("No unexpected tombstone revival in unreachable subtrees", () => {
		...
		itExpects(
			...
			async () => {
				...
				const containerRuntime2 = entryPoint2._context.containerRuntime as ContainerRuntime;
				assert.equal(
					(containerRuntime2 as any).dataStores.contexts.get(dataStore2._context.id)
						.tombstoned,
					true,
					"Expected dataStore2 to be Tombstoned",
				);
				assert.equal(
					(containerRuntime2 as any).dataStores.contexts.get(dataStore3._context.id)
						.tombstoned,
					true,
					"Expected dataStore3 to be Tombstoned",
				);
