 ...
 */
describeCompat(
	"GC attachment blob sweep tests",
	"NoCompat",
	(getTestObjectProvider) => {
		...
		async function validateBlobRetrievalFails(
			...
			assert.strictEqual(
				response?.status,
				404,
				`${messagePrefix}: Expecting a 404 response`,
			);
			assert.equal(
				response.value,
			...
			assert(
				...
				`${messagePrefix}: Container should not have closed`,
			);
		}
			...
			for (const gcOps of ["one+", "multiple"]) {
				itExpects(
					...
					[
						...
						{
							eventName:
								"fluid:telemetry:BlobManager:GC_Deleted_Blob_Requested",
							clientType: "interactive",
						},
					],
					async () => {
