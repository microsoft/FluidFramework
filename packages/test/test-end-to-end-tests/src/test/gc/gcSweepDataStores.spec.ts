...
describeNoCompat("GC data store sweep tests", (getTestObjectProvider) => {
    ...
    const summarizationWithUnreferencedDataStoreAfterTime =
        async (approximateUnreferenceTimestampMs: number) => {
            ...
            const summarizerDataObject = await requestFluidObject<ITestDataObject>(summarizingContainer2, testDataObject.handle.absolutePath);
    ...
    describe("Using swept data stores not allowed", () => {
        ...
        async () => {
            ...
            assert.throws(() => summarizerDataObject._root.set("send", "op"),
                (error) => {
                    const correctErrorType = error.errorType === "dataCorruptionError";
                    const correctErrorMessage = error.message?.startsWith(`Context is deleted`) === true;
        ...
        async () => {
            ...
            assert.throws(() => summarizerDataObject._runtime.submitSignal("send", "signal"),
                (error) => {
                    const correctErrorType = error.errorType === "dataCorruptionError";
                    const correctErrorMessage = error.message?.startsWith(`Context is deleted`) === true;
                    return correctErrorType && correctErrorMessage;
    ...
    describe("Loading swept data stores not allowed", () => {
        ...
        itExpects.skip("Requesting swept datastores fails in client loaded after sweep timeout and summarizing container",
        [
        ...
        async () => {
            ...
            const errorResponse = await containerRuntime_resolveHandle(container, { url: unreferencedId });
            assert.equal(errorResponse.status, 404, "Should not be able to retrieve a swept datastore loading from a non-summarizer client");
            assert.equal(errorResponse.value, `DataStore was deleted: ${unreferencedId}`, "Expected the Sweep error message");

            // This request fails since the datastore is swept
            const summarizerResponse = await containerRuntime_resolveHandle(summarizingContainer, { url: unreferencedId });
            assert.equal(summarizerResponse.status, 404, "Should not be able to retrieve a swept datastore from a summarizer client");
            assert.equal(errorResponse.value, `DataStore was deleted: ${unreferencedId}`, "Expected the Sweep error message");
        });
    });
});
