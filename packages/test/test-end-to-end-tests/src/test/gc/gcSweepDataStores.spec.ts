...
const tombstoneTimeoutMs = computeTombstoneTimeout(
	defaultSessionExpiryDurationMs,
)!;
 ...
 */
describeCompat(
	"GC data store sweep tests",
	"NoCompat",
	function (getTestObjectProvider) {
		/**
		...
		describe("Using swept data stores not allowed", () => {
			itExpects(
				...
				[
					{
						eventName:
							"fluid:telemetry:ContainerRuntime:GC_DeletingLoadedDataStore",
						clientType: "noninteractive/summarizer", // summarizeWithSweepReadyDS has a summarizer spanning before/after the delete
					},
					{
						eventName:
							"fluid:telemetry:FluidDataStoreContext:GC_Deleted_DataStore_Changed",
		...
		describe("Using deleted data stores", () => {
			itExpects(
				...
				async () => {
					...
					mockLogger.assertMatch([
						...
						{
							eventName:
								"fluid:telemetry:ContainerRuntime:GC_DeletedDataStore_PathInfo",
							...tagCodeArtifacts({
								id: `/${unreferencedId}`,
			...
			itExpects(
				...
				[
					...
					{
						eventName:
							"fluid:telemetry:ContainerRuntime:GC_Deleted_DataStore_Changed",
						clientType: "noninteractive/summarizer",
				...
				async () => {
					...
					const sendingContainer = await loadContainer(
						unreferencedSummaryVersion,
					);
					const entryPoint =
			...
			it("trailing ops cause sweep ready data store to be realized by summarizer", async () => {
				...
				const { summaryVersion } =
					await ensureSynchronizedAndSummarize(summarizer1);
				summarizer1.close();
				...
				logger.assertMatch([
					{
						eventName:
							"fluid:telemetry:Summarizer:Running:SweepReadyObject_Realized",
						id: {
		...
		describe("Sweep with summarize failures and retries", () => {
			...
			async function overrideSummarizeAndGetCompletionPromise(
				...
				if (blockInboundGCOp) {
					await toIDeltaManagerFull(
						containerRuntime.deltaManager,
					).inbound.pause();
				}
				...
				const summarizeOverride = async (options: any) => {
					...
					if (
						...
						latestAttemptProps.maxAttempts - latestAttemptProps.currentAttempt >
							1
					) {
			...
			for (const gcOps of ["one+", "multiple"]) {
				itExpects(
					...
					async () => {
						...
						// eslint-disable-next-line @typescript-eslint/no-non-null-assertion
						config_gcSweepDisabled.runtimeOptions!.gcOptions!.enableGCSweep =
							undefined;
						...
						await assert.rejects(
							...
							(error: any) => {
								...
								).startsWith("DataStore was deleted:");
								const correctHeaders =
									error.underlyingResponseHeaders[DeletedResponseHeaderKey] ===
									true;
								return (
									correctErrorType && correctErrorMessage && correctHeaders
								);
							},
