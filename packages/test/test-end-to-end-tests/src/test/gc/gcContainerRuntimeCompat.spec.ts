...
describeCompat(
	...
	(getTestObjectProvider, compatApis) => {
		...
		const version2Apis: LayerApis = {
			containerRuntime:
				compatApis.containerRuntimeForLoading ?? compatApis.containerRuntime,
		...
		beforeEach("setupContainer", async function () {
			...
			if (provider.driver.type === "odsp") {
				...
				if (
					...
					loaderVersionForLoading?.startsWith("1.")
				) {
					this.skip();
				}
			}
			...
			dataStoreA =
				await getContainerEntryPointBackCompat<ITestFluidObject>(mainContainer);
		 ...
		 */
		it("load version validates unreferenced timestamp from summary by create version", async () => {
			...
			const dataStoreB =
				await dataStoreA.context.containerRuntime.createDataStore(
					dataObjectType,
				);
