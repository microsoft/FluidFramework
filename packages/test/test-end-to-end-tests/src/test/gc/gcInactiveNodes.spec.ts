 ...
 */
describeCompat(
	"GC inactive nodes tests",
	"NoCompat",
	(getTestObjectProvider, apis) => {
		const { SharedMap, SharedDirectory } = apis.dds;
		const revivedEvent =
			"fluid:telemetry:ContainerRuntime:InactiveObject_Revived";
		const changedEvent =
			"fluid:telemetry:ContainerRuntime:InactiveObject_Changed";
		const loadedEvent =
			"fluid:telemetry:ContainerRuntime:InactiveObject_Loaded";
		...
		async function createSummarizerClient(config: ITestContainerConfig) {
			...
			const summarizerContainer = await provider.loadTestContainer(
				config,
				requestHeader,
			);
		...
		describe("Inactive timeout", () => {
			...
			async function createNewDataObject(
				runtime: IContainerRuntimeBase = containerRuntime,
			) {
					...
					mockLogger.assertMatch(
						[
							// For DDS
							{
								...
								type: "SubDataStore",
								id: {
									value: toFluidHandleInternal(dataObject._root.handle)
										.absolutePath,
									tag: TelemetryDataTag.CodeArtifact,
								},
								pkg: {
			...
			itExpects(
				...
				async () => {
					...
					const entryPoint =
						await summarizerRuntime.getAliasedDataStoreEntryPoint("default");
					if (entryPoint === undefined) {
						throw new Error("Summarizer must have default data store");
					}
					const summarizerDefaultDataStore =
						(await entryPoint.get()) as ITestDataObject;
					...
					const summarizerBlobHandle =
						summarizerDefaultDataStore._root.get<
							IFluidHandleInternal<ArrayBufferLike>
						>("blob");
					assert(
			...
			itExpects(
				...
				async () => {
					...
					const dataObject = await createNewDataObject();
					const dataStoreUrl = toFluidHandleInternal(
						dataObject.handle,
					).absolutePath;
					const ddsUrl = toFluidHandleInternal(
						dataObject._root.handle,
					).absolutePath;
					...
					const response1 = await summarizerRuntime.resolveHandle({
						url: ddsUrl,
					});
					assert.equal(
						response1.status,
					...
					const response2 = await summarizerRuntime.resolveHandle({
						url: untrackedUrl,
					});
					assert.equal(
						response2.status,
			...
			itExpects(
				...
				[
					{
						eventName:
							"fluid:telemetry:Summarizer:Running:InactiveObject_Revived",
					},
				],
				async () => {
			...
			it("Reviving an InactiveObject clears Inactive state immediately in interactive client (but not for its subtree)", async () => {
				...
				const handleA_3 =
					defaultDataObject3._root.get<IFluidHandle<ITestDataObject>>("A");
				...
				const handleB_3 =
					dataObjectA_3?._root.get<IFluidHandle<ITestDataObject>>("B");
				await handleB_3!.get();
			...
			const newDataStoreFn = async (
				...
				const dataObject2 = await createNewDataObject(
					dataObject._context.containerRuntime,
				);
				return [dataObject2._root, dataObject2.handle];
			};
			[
				[newDDSFn, "DDS"] as const,
				[newDataStoreFn, "DataStore"] as const,
			].forEach(([newDirectoryFn, type]) => {
				it(`Reviving an InactiveObject via [${type}] Attach Op clears Inactive state immediately in interactive client (but not for its subtree)`, async () => {
					...
					// Since A was directly revived, its unreferenced state was cleared immediately so we shouldn't see InactiveObject logs for it
					const handleA_3 =
						newDirectory_3.get<IFluidHandle<ITestDataObject>>("A");
					const dataObjectA_3 = await handleA_3!.get();
					mockLogger3.assertMatchNone(
						[
							{
