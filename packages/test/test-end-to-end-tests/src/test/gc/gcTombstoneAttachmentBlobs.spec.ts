 ...
 */
describeCompat(
	"GC attachment blob tombstone tests",
	"NoCompat",
	(getTestObjectProvider) => {
		...
		describe("Attachment blobs in attached container", () => {
			...
			itExpects(
				...
				async () => {
					...
					assert(
						container2.closed !== true,
						"Container should not have closed",
					);
			...
			itExpects(
				...
				async () => {
					...
					const containerRuntime3 = entryPoint3._context
						.containerRuntime as ContainerRuntime;
			...
			it("should allow access to blobs that are de-duped in different containers", async () => {
				...
				const container2BlobHandle =
					await container2MainDataStore._runtime.uploadBlob(
						stringToBuffer(blobContents, "utf-8"),
					);
				...
				const container3MainDataStore =
					(await container3.getEntryPoint()) as ITestDataObject;

				const container3BlobHandle =
					await container3MainDataStore._runtime.uploadBlob(
						stringToBuffer(blobContents, "utf-8"),
					);
				...
				container2MainDataStore._root.set(
					"container2BlobHandle",
					container2BlobHandle,
				);
				container3MainDataStore._root.set(
					"container3BlobHandle",
					container3BlobHandle,
				);
		...
		describe("Attachment blobs in detached container", () => {
			...
			async function createDetachedContainerAndDataStore() {
				...
				const mainContainer = await loader.createDetachedContainer(
			...
			itExpects(
				...
				async () => {
					...
					const container2 = await provider
		...
		describe("Attachment blobs in disconnected container", () => {
			...
			itExpects(
				...
				async () => {
					const { mainContainer, mainDataStore } =
						await createContainerAndDataStore();
