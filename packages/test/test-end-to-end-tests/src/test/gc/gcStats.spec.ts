...
describeCompat(
	...
	(getTestObjectProvider) => {
		...
		function getDataStoreSummaryStats(
			...
			for (const [id, summaryObject] of Object.entries(channelsTree)) {
				if (dataStoreIds.includes(id)) {
					...
					summaryStats = mergeStats(
						summaryStats,
						calculateStats(summaryObject),
					);
				}
			}
			return summaryStats;
		...
		beforeEach("setup", async function () {
			...
			summarizerRuntime = summarizerDataObject._context
		...
		it("can correctly generate GC stats when nodes are unreferenced", async () => {
			...
			assert.deepStrictEqual(
				gcStats,
				expectedGCStats,
				"2. GC stats is not as expected",
			);

			const summarizeResult = await summarizerRuntime.summarize({
				fullTree: true,
			});
			const unrefDataStoreStats = getDataStoreSummaryStats(
				summarizeResult.summary,
				[dataStore1._context.id, dataStore2._context.id],
			);
			assert.strictEqual(
				summarizeResult.stats.unreferencedBlobSize,
