...
describeCompat(
	...
	(getTestObjectProvider) => {
		...
		function getDataStoreInSummaryTree(
		...
		it("Virtualized datastore has same gc state even when not downloaded", async () => {
			...
			assert(
				handleA !== undefined,
				"Expected a handle when creating a datastoreA",
			);
			assert(
				handleB !== undefined,
				"Expected a handle when creating a datastoreB",
			);
			...
			// Load new container
			clearCacheIfOdsp(provider, persistedCache);
			const mainContainer2 = await provider.loadTestContainer(
				testContainerConfig,
				{
			...
			const mainDataStore2 =
				(await mainContainer2.getEntryPoint()) as ITestDataObject;
			...
			assert(
				snapshotCaptured !== undefined,
				"Expected snapshot to be captured",
			);

			// Validate that we loaded the snapshot without datastoreA on the snapshot
			const tree = (snapshotCaptured as ISnapshot).snapshotTree.trees[
				".channels"
			].trees;
			const datastoreATree = tree[dataStoreId];
			assert(
				datastoreATree !== undefined,
				"DataStoreA should be in the snapshot",
			);
			...
			const dataStoreTreeA = getDataStoreInSummaryTree(
				summaryTree2,
				dataStoreId,
			);
			assert(
				dataStoreTreeA?.type === SummaryType.Handle,
				"DataStoreA should not have changed!",
			);
		});
	},
);
