	...
	(getTestObjectProvider) => {
		...
		const tests = (deleteUnreferencedContent: boolean = false) => {
			...
			async function summarizeAndGetUnreferencedNodeStates(
				...
				assert(
					gcState !== undefined,
					"GC tree is not available in the summary",
				);

				const nodeTimestamps: Map<string, "referenced" | "unreferenced"> =
					new Map();
			...
			async function loadContainer() {
				...
				const newContainer = await provider
					.makeTestLoader(gcContainerConfig)
					.resolve({ url });
			...
			async function createSummarizerRuntime() {
				...
				return summarizerDefaultDataStore._context
			...
			it("collects blobs uploaded in attached container", async () => {
				...
				assert(
					s1.get(blobHandle.absolutePath) === "referenced",
					"blob should be referenced",
				);
			...
			it("collects blobs uploaded in detached container", async () => {
				...
				const blobHandle2 =
					defaultDataStore2._root.get<IFluidHandleInternal<ArrayBufferLike>>(
						"blob",
					);
				assert.strictEqual(
			...
			it("collects blobs uploaded in detached and de-duped in attached container", async () => {
				...
				assert.strictEqual(
					s1.size,
					2,
					"There should be 2 blob entries in GC data",
				);
				assert(
			...
			it("collects blobs uploaded and de-duped in detached container", async () => {
				...
				assert.strictEqual(
					s1.size,
					3,
					"There should be 3 blob entries in GC data",
				);
				assert(
					s1.get(localHandle1.absolutePath) === "referenced",
			...
			it("collects blobs uploaded in disconnected and de-duped in connected container", async () => {
				...
				assert.strictEqual(
					s1.size,
					1,
					"There should be 1 blob entries in GC data",
				);
				assert(
					s1.get(localHandle1.absolutePath) === "referenced",
