 ...
 */
describeCompat(
	"GC Data Store Aliased Full Compat",
	"FullCompat",
	(getTestObjectProvider) => {
		...
		async function waitForSummary(container: IContainer) {
			...
			return (
				dataStore._context.containerRuntime as ContainerRuntime
			).summarize({
				runGC: true,
				trackState: false,
				fullTree: true,
		...
		it("An unreferenced datastore when aliased becomes referenced.", async function () {
			...
			assert(
				gcStatePreAlias !== undefined,
				"Should get gc pre state from summary!",
			);
			assert(
				gcStatePreAlias.gcNodes[
					toFluidHandleInternal(dataObject2.handle).absolutePath
			...
			assert(
				gcStatePostAlias !== undefined,
				"Should get gc post state from summary!",
			);
			assert(
				gcStatePostAlias.gcNodes[
					toFluidHandleInternal(dataObject2.handle).absolutePath
				].unreferencedTimestampMs === undefined,
...
describeCompat(
	"GC Data Store Aliased No Compat",
	"NoCompat",
	(getTestObjectProvider) => {
		let provider: ITestObjectProvider;
		...
		it("Aliased datastore is referenced even without storing handles", async () => {
			...
			assert(
				gcState?.gcNodes[aliasedDataStore.entryPoint.absolutePath]
					.unreferencedTimestampMs === undefined,
