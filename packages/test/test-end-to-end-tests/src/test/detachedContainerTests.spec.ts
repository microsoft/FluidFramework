...
describeFullCompat("Detached Container", (getTestObjectProvider) => {
	...
	it("Fire ops during container attach for shared string", async () => {
		...
		dataStore.context.containerRuntime.on("op", (message, runtimeMessage) => {
			...
			assert.equal(((message.contents as { contents: unknown }).contents as { type?: unknown }).type, DataStoreMessageType.ChannelOp);

			assert.strictEqual(
				(((message.contents as { contents: unknown }).contents as { content: unknown }).content as { address?: unknown }).address,
				sharedStringId,
				"Address should be shared string",
			);
			assert.strictEqual(
				JSON.stringify((((message.contents as { contents: unknown }).contents as { content: unknown }).content as { contents?: unknown }).contents),
				JSON.stringify(ops),
				"Ops should be equal",
			);
			defPromise.resolve();
	...
	it("Fire channel attach ops during container attach", async () => {
		...
		dataStore.context.containerRuntime.on("op", (message, runtimeMessage) => {
			...
			assert.strictEqual(
				(((message.contents as { contents: unknown }).contents as { content: unknown }).content as { id?: unknown }).id,
				testChannelId,
				"Channel id should match",
			);
			assert.strictEqual(
				(((message.contents as { contents: unknown }).contents as { content: unknown }).content as { type?: unknown }).type,
	...
	it("Fire ops during container attach for shared ink", async () => {
		...
		dataStore.context.containerRuntime.on("op", (message, runtimeMessage) => {
			...
			assert.strictEqual(
				((((message.contents as { contents: unknown }).contents as { content: unknown }).content as { contents: unknown }).contents as { type?: unknown }).type,
				"createStroke",
				"Op type should be same",
			);
			assert.strictEqual(
				(((((message.contents as { contents: unknown }).contents as { content: unknown }).content as { contents: unknown }).contents as { pen: unknown }).pen as { thickness?: unknown }).thickness,
				20,
	...
	it("Fire ops during container attach for sparse matrix", async () => {
		...
		dataStore.context.containerRuntime.on("op", (message, runtimeMessage) => {
			...
			if (
				(((((message.contents as { contents: unknown }).contents as { content: unknown }).content as { contents: unknown }).contents as { ops: unknown[] }).ops[0] as { type?: unknown }).type === MergeTreeDeltaType.INSERT
			) {
				assert.strictEqual(
					JSON.stringify((((((message.contents as { contents: unknown }).contents as { content: unknown }).content as { contents: unknown }).contents as { ops: unknown[] }).ops[0] as { seg?: unknown }).seg),
					JSON.stringify(seg),
					"Seg should be same",
				);
			} else {
				assert.strictEqual(
					JSON.stringify((((((message.contents as { contents: unknown }).contents as { content: unknown }).content as { contents: unknown }).contents as { ops: unknown[] }).ops[1] as { seg?: unknown }).seg),
					JSON.stringify(seg),
					"Seg should be same",
				);
			}
