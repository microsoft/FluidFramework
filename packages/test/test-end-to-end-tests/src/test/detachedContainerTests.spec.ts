...
describeCompat(
	...
	(getTestObjectProvider, apis) => {
		...
		it("DataStores in detached container", async () => {
			...
			const subDataStore = await createFluidObject(
				dataStore.context,
				"default",
			);
		...
		it("DataStores in attached container", async () => {
			...
			const testDataStore = await createFluidObject(
				dataStore.context,
				"default",
			);
		...
		it("can create DDS in detached container and attach / update it", async () => {
			...
			const mapClient2 = await dsClient2.root
				.get<IFluidHandle<ISharedMap>>("map")
				?.get();
			assert(
				mapClient2 !== undefined,
				"Map is not available in the second client",
			);
		...
		it("Load attached container and check for dataStores", async () => {
			...
			const requestUrl2 = await provider.urlResolver.getAbsoluteUrl(
		...
		it("Fire ops during container attach for shared string", async () => {
			...
			const dataStore =
				await getContainerEntryPointBackCompat<ITestFluidObject>(container);
			const testChannel1 =
				await dataStore.getSharedObject<SharedString>(sharedStringId);
		...
		it("Fire ops during container attach for shared map", async () => {
			...
			const dataStore =
				await getContainerEntryPointBackCompat<ITestFluidObject>(container);
			const testChannel1 =
				await dataStore.getSharedObject<ISharedMap>(sharedMapId);
		...
		it("Fire dataStore attach ops during container attach", async () => {
			...
			const newDataStore =
				await dataStore.context.containerRuntime.createDataStore([
					testDataStoreType,
				]);
			const comp2 =
				await getDataStoreEntryPointBackCompat<ITestFluidObject>(newDataStore);
		...
		it("Fire ops during container attach for consensus register collection", async () => {
			...
			const dataStore =
				await getContainerEntryPointBackCompat<ITestFluidObject>(container);
			const testChannel1 =
				await dataStore.getSharedObject<ConsensusRegisterCollection<string>>(
					crcId,
				);
			...
			dataStore.context.containerRuntime.on("op", (message, runtimeMessage) => {
				...
				assert.strictEqual(
					op.refSeq,
					receivedOp.refSeq,
					"Op refSeq should be same",
				);
				if (receivedOp.value) {
					assert.deepEqual(
						op.value,
						receivedOp.value,
						"Op value should be same",
					);
				}
				defPromise.resolve();
				return 0;
		...
		it("Fire ops during container attach for shared directory", async () => {
			...
			const dataStore =
				await getContainerEntryPointBackCompat<ITestFluidObject>(container);
			const testChannel1 =
				await dataStore.getSharedObject<SharedDirectory>(sharedDirectoryId);
		...
		it("Fire ops during container attach for shared cell", async () => {
			...
			const dataStore =
				await getContainerEntryPointBackCompat<ITestFluidObject>(container);
			const testChannel1 =
				await dataStore.getSharedObject<ISharedCell>(sharedCellId);
		...
		it("Fire ops during container attach for consensus ordered collection", async () => {
			...
			const dataStore =
				await getContainerEntryPointBackCompat<ITestFluidObject>(container);
			const testChannel1 =
				await dataStore.getSharedObject<ConsensusQueue>(cocId);
		...
		it("Fire ops during container attach for sparse matrix", async () => {
			...
			const dataStore =
				await getContainerEntryPointBackCompat<ITestFluidObject>(container);
			const testChannel1 =
				await dataStore.getSharedObject<SparseMatrix>(sparseMatrixId);
		...
		it.skip("Fire ops during container attach for shared matrix", async () => {
			...
			const dataStore =
				await getContainerEntryPointBackCompat<ITestFluidObject>(container);
			const testChannel1 =
				await dataStore.getSharedObject<SharedMatrix>(sharedMatrixId);
...
describeCompat(
	...
	(getTestObjectProvider, apis) => {
		...
		it("Retry attaching detached container", async () => {
			...
			const container = await mockLoader.createDetachedContainer(
				provider.defaultCodeDetails,
			);
			await container.attach(request);
			assert.strictEqual(
			...
			async () => {
				const loaderWithConfig = provider.createLoader(
					[[provider.defaultCodeDetails, provider.createFluidEntryPoint()]],
					{
						configProvider: {
							getRawConfig: (name) =>
								name === "Fluid.Container.RetryOnAttachFailure"
									? true
									: undefined,
						},
					},
				...
				assert.strictEqual(
					container.closed,
					false,
					"Container should not be closed",
				);
		...
		itExpects(
			"Attach can be called multiple times with the same parameters",
			[],
			async () => {
				const container = await loader.createDetachedContainer(
					provider.defaultCodeDetails,
				...
				assert.strictEqual(
					attaches[0],
					attaches[1],
					"promises should match for parallel calls",
				);
