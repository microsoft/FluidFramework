...
describeCompat(
	...
	(getTestObjectProvider, apis) => {
			...
			const loaderProps = createLoaderProps(
				[
					[
						...
						provider.createFluidEntryPoint(testContainerConfig),
					],
				],
		...
		it("fails gracefully when summary upload throws", async () => {
			...
			assert(
				!result.success,
				"expected summarization failure when upload fails",
			);
		...
		it("on-demand summary succeeds after normal summary completes", async () => {
			...
			const mainContainer = await provider.makeTestContainer(
			...
			const onDemandResult =
				await loadSummarizerContainerAndMakeSummary(onDemandProps);

			assert(
				onDemandResult.success,
				"on-demand summary should succeed after normal summary",
			);
			const summaryResults = onDemandResult.summaryResults;
			assert(
				summaryResults.summarySubmitted,
				"on-demand summary not submitted",
			);
			assert(
				summaryResults.summaryOpBroadcasted,
				"on-demand summary op not broadcasted",
			);
		});
		/**
		...
		it.skip("on-demand summary succeeds while normal summary is inflight", async () => {
			...
			assert(
				onDemandResult.success,
				"on-demand summary should succeed during normal summary",
			);
			const summaryResults = onDemandResult.summaryResults;
			assert(
				summaryResults.summarySubmitted,
		...
		it("clients with summaries disabled can make changes and load from on-demand summary handle", async function () {
			...
			const loaderProps = createLoaderProps(
				[
					[
						provider.defaultCodeDetails,
						provider.createFluidEntryPoint(summarizerConfig),
					],
				],
				provider.documentServiceFactory,
				provider.urlResolver,
