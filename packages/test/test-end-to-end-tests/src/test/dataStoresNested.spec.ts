...
describeCompat(
	...
	(getTestObjectProvider, apis) => {
		...
		const testObjectFactory: TestFluidObjectFactory =
		...
		async function waitForSummary(): Promise<string> {
			...
			assert(
				ackedSummary.summaryAck.contents.handle !== undefined,
				"summary acked",
			);
		...
		it("Basic test", async () => {
			...
			const context = dataStores.createDataStoreContext([
				testObjectFactory.type,
			]);
			const url = `/${context.id}`;
			const channel = await context.realize();
			channel.makeVisibleAndAttachGraph();
			const testObject1 = (await dataStores.request({ url }))
				.value as TestFluidObject;
			...
			const testObject2 = (await dataStores2.request({ url }))
				.value as TestFluidObject;
