...
describeCompat(
	...
	(getTestObjectProvider, apis) => {
		...
		beforeEach("createContainers", async () => {
			...
			const dataStore =
				await containerRuntime1.createDataStore(TestDataObjectType);
			firstContainerObject2 =
		...
		it("should generate the absolute path for FluidDataObjectRuntime correctly", () => {
			...
			const fluidHandleContext11 =
				firstContainerObject1._runtime.rootRoutingContext;
			assert.equal(
			...
			const fluidHandleContext12 =
				secondContainerObject1._runtime.rootRoutingContext;
			assert.equal(
		...
		it("can store and retrieve a DDS from handle within same data store runtime", async () => {
			...
			assert.equal(
				sharedMapHandle.absolutePath,
				absolutePath,
				"The handle's path is incorrect",
			);
			...
			const remoteSharedMapHandle =
				secondContainerObject1._root.get<IFluidHandleInternal<ISharedMap>>(
					"sharedMap",
				);
		...
		it("can store and retrieve a PureDataObject from handle in different data store runtime", async () => {
			...
			const dataObjectHandle = toFluidHandleInternal(
				firstContainerObject2.handle,
			);
			...
			firstContainerObject1._root.set(
				"dataObject2",
				firstContainerObject2.handle,
			);
			...
			const remoteDataObjectHandle =
				secondContainerObject1._root.get<IFluidHandleInternal<TestFluidObject>>(
					"dataObject2",
				);
