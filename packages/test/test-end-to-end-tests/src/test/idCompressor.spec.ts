...
import type { IFluidHandle, IRequest } from "@fluidframework/core-interfaces";
...
describeCompat(
	...
	(getTestObjectProvider, apis) => {
		function runTests(
			explicitSchemaCreation: boolean,
			explicitSchemaLoading: boolean,
		) {
			...
			it("can't enable compressor on an existing container", async () => {
				...
				const enabledMap =
					await enabledDataObject.getSharedObject<ISharedMap>("mapId");
	...
	(getTestObjectProvider, apis) => {
		...
		class TestDataObject extends DataObject {
			...
			protected async hasInitialized() {
				const mapHandle = this.root.get<IFluidHandle<ISharedMap>>(
					this.sharedMapKey,
				);
				assert(mapHandle !== undefined, "SharedMap not found");
				this.map = await mapHandle.get();

				const sharedCellHandle = this.root.get<IFluidHandle<ISharedCell>>(
					this.sharedCellKey,
				);
		...
		beforeEach("setupContainers", async () => {
			...
			const container3MainDataStore =
				(await container3.getEntryPoint()) as TestDataObject;
		...
		it("can normalize session space IDs to op space", async () => {
			...
			for (let i = 0; i < 512; i++) {
				[sharedMapContainer1, sharedMapContainer2, sharedMapContainer3].forEach(
					(map) => {
						assert.strictEqual(
							getIdCompressor(map).normalizeToOpSpace(
			...
			for (let i = 0; i < 512; i++) {
				for (let j = 0; j < compressors.length; j++) {
					assert.strictEqual(
						compressors[j].normalizeToOpSpace(
							-(i + 1) as SessionSpaceCompressedId,
						),
						i + firstIds[j],
					);
		...
		it("can normalize local op space IDs from a local session to session space", async () => {
			...
			await provider.ensureSynchronized();
			const opSpaceId =
				getIdCompressor(sharedMapContainer1).normalizeToOpSpace(sessionSpaceId);
		...
		it("finalizes IDs made in a detached state immediately upon attach", async () => {
			...
			const container =
				await loader.createDetachedContainer(defaultCodeDetails);
		...
		it("eagerly allocates final IDs after cluster is finalized", async () => {
			assert(
				getIdCompressor(sharedMapContainer1) !== undefined,
				"IdCompressor is undefined",
			);
			const localId1 =
				getIdCompressor(sharedMapContainer1).generateCompressedId();
			...
			const opSpaceId1 =
				getIdCompressor(sharedMapContainer1).normalizeToOpSpace(localId1);
			const opSpaceId2 =
				getIdCompressor(sharedMapContainer1).normalizeToOpSpace(localId2);
			const opSpaceId3 =
				getIdCompressor(sharedMapContainer1).normalizeToOpSpace(finalId3);
		...
		it("eagerly allocates IDs across DDSs using the same compressor", async () => {
			...
			assert(
				getIdCompressor(sharedCellContainer1) !== undefined,
				"IdCompressor is undefined",
			);
			...
			const opSpaceId1 =
				getIdCompressor(sharedMapContainer1).normalizeToOpSpace(localId1);
			const opSpaceId2 =
				getIdCompressor(sharedCellContainer1).normalizeToOpSpace(localId2);
			const opSpaceId3 =
				getIdCompressor(sharedMapContainer1).normalizeToOpSpace(finalId3);
			const opSpaceId4 =
				getIdCompressor(sharedCellContainer1).normalizeToOpSpace(finalId4);
		...
		it("container with multiple DDSs has same compressor state", async () => {
			...
			const sharedMapCompressedId =
				getIdCompressor(sharedMapContainer1).generateCompressedId();
			const sharedMapDecompressedId = getIdCompressor(
				sharedMapContainer1,
			).decompress(sharedMapCompressedId);
			const sharedMapCompressedId2 =
				getIdCompressor(sharedMapContainer1).generateCompressedId();
			const sharedMapDecompressedId2 = getIdCompressor(
				sharedMapContainer1,
			).decompress(sharedMapCompressedId2);
			const sharedCellCompressedId =
				getIdCompressor(sharedCellContainer1).generateCompressedId();
			const sharedCellDecompressedId = getIdCompressor(
				sharedMapContainer1,
			).decompress(sharedCellCompressedId);
			...
			assert.strictEqual(
				getIdCompressor(sharedMapContainer1).recompress(
					sharedMapDecompressedId,
				),
				getIdCompressor(sharedCellContainer1).recompress(
					sharedMapDecompressedId,
				),
			);
			...
			assert.strictEqual(
				sharedMapContainer1.get(sharedMapDecompressedId),
				"value",
			);
		});
		...
		for (const testConfig of testConfigs) {
			it(`Ids generated across batches are correctly resubmitted: ${JSON.stringify(
				...
				container1.connect();
				await waitForContainerConnection(container1);
				await assureAlignment(
					[sharedMapContainer1, sharedMapContainer2],
					idPairs,
				);
			});
		}
		...
		it("DDSs in different DataStores have the same compressor state", async () => {
			...
			compressedIds.push(
				getIdCompressor(dataStore2.map).generateCompressedId(),
			);

			const decompressedIds: StableId[] = [];
			compressedIds.forEach((id) => {
				const decompressedId =
					getIdCompressor(sharedMapContainer1).decompress(id);
				...
				[
					getIdCompressor(sharedCellContainer1),
					getIdCompressor(dataStore2.map),
				].forEach((compressor) => {
			...
			decompressedIds.forEach((id, index) => {
				...
				const compressedId =
					getIdCompressor(sharedMapContainer1).recompress(id);
...
describeCompat(
	...
	(getTestObjectProvider, apis) => {
		...
		it("Compressors sync after detached container attaches and sends an op", async () => {
			...
			const testChannel1 =
				await dataStore.getSharedObject<ISharedCell>("sharedCell");
			...
			const testChannel2 =
				await dataStore2.getSharedObject<ISharedCell>("sharedCell");
...
describeCompat(
	...
	(getTestObjectProvider, compatAPIs) => {
		...
		it("Shouldn't include unack'd local ids in summary", async () => {
			...
			const idCompressor: IIdCompressor = (defaultDataStore._root as any)
				.runtime.idCompressor;
		...
		it("Newly connected container synchronizes from summary", async function () {
			...
			const container2DataStore =
				(await container2.getEntryPoint()) as ITestDataObject;
			const container2IdCompressor: IIdCompressor = (
				container2DataStore._root as any
			).runtime.idCompressor;
			assert(
				container2IdCompressor !== undefined,
				"Second IdCompressor should be present",
			);
			assert(
				(container2IdCompressor as any).sessions.get(
		...
		async function TestCompactIds(enableRuntimeIdCompressor: IdCompressorMode) {
			...
			const ds =
				await defaultDataStore._context.containerRuntime.createDataStore(pkg);
			...
			assert(
				typeof idTest === "number" && idTest >= 0,
				"short IDs should be issued",
			);
		...
		it("always uses short data store IDs in detached container", async () => {
			...
			const dataStore1 =
				await defaultDataStore.context.containerRuntime.createDataStore(
					TestDataObjectType,
				);
