...
describeCompat(
	...
	(getTestObjectProvider, apis) => {
		...
		const getDataStoreWithGroupId = async (
			...
			const handle =
				dataObject.root.get<IFluidHandle<ITestFluidObject>>(groupId);
		...
		for (const testConfig of testConfigs) {
			it.skip(`Snapshot refresh life cycle: ${JSON.stringify(
				...
				const testContainerConfig = {
					...
					loaderProps: {
						logger: wrapObjectAndOverride<ITelemetryBaseLogger>(
							provider.logger,
							{
								send: (tb) => (event) => {
									tb.send(event);
				...
				const groupIdDataObject = await createDataStoreWithGroupId(
					dataStore,
					groupId,
				);
				await provider.ensureSynchronized();
				const groupIdDataObject1 = await getDataStoreWithGroupId(
					dataStore1,
					groupId,
				);
				...
				const groupIdDataObject2 = await getDataStoreWithGroupId(
					dataStore2,
					groupId,
				);
				const summaryExists =
				...
				const groupIdDataObject3 = await getDataStoreWithGroupId(
					dataStore3,
					groupId,
				);
				...
				const dataStore4 =
					(await container4.getEntryPoint()) as ITestFluidObject;
				const map4 = await dataStore4.getSharedObject<ISharedMap>(mapId);
				const groupIdDataObject4 = await getDataStoreWithGroupId(
					dataStore4,
					groupId,
				);
