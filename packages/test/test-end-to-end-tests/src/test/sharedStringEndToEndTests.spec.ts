...
describeCompat("SharedString", "FullCompat", (getTestObjectProvider, apis) => {
	...
	it("can sync SharedString to a newly loaded container", async () => {
		...
		const newSharedString =
			await newComponent.getSharedObject<SharedString>(stringId);
	...
	it("marker passes on attachment directly and transitively to any referenced DDS", async () => {
		...
		const detachedString1 = SharedString.create(
			dataObject1.runtime,
			"detachedString1",
		);
		...
		const detachedString2 = SharedString.create(
			dataObject1.runtime,
			"detachedString2",
		);
		...
		assert.equal(
			sharedString1.isAttached(),
			true,
			"sharedString1 should be attached",
		);
		...
		assert.equal(
			detachedString1.isAttached(),
			true,
			"detachedString1 should be attached",
		);
		assert.equal(
			detachedString2.isAttached(),
			true,
			"detachedString2 should be attached",
		);
		assert.equal(
			sharedString1.isAttached(),
			true,
	...
	(getTestObjectProvider, apis) => {
		...
		it("can load summarized grouped batch at min seqnum", async function () {
			...
			const container1 = await provider.makeTestContainer(
				groupedBatchingContainerConfig,
			);
			const dataObject1 =
				(await container1.getEntryPoint()) as ITestFluidObject;
			const sharedString1 =
				await dataObject1.getSharedObject<SharedString>(stringId);
			...
			const sharedString2 =
				await dataObject2.getSharedObject<SharedString>(stringId);
