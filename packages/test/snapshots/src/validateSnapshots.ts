...
export async function validateSnapshots(
	...
	const errors: string[] = [];
	const isCurrentFormatComparison = srcDir === destDir;
		...
		try {
			...
			const onSnapshotCb = (snapshot: IFileSnapshot): void => {
				...
				if (isCurrentFormatComparison) {
					...
					// `.recentBatchInfo` is replay-history state rather than persisted document
					// data. Older source snapshots may omit it or retain entries that a fresh
					// replay does not reproduce, so back-compat validation ignores it.
					compareIgnoringBlob(
						produced,
						`${destDir}/${snapshotFileName}`,
