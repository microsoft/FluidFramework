...
function compareIgnoringBlob(
	...
	const filteredProduced = normalizePackageVersions(withoutTopLevelBlob(produced, blobToIgnore));
...
export async function validateSnapshots(
	...
	for (const file of fs.readdirSync(srcDir, { withFileTypes: true })) {
		...
		const sourceLacksRecentBatchInfo = !snapshotHasBlob(sourceSnapshot, recentBatchInfoBlobName);
