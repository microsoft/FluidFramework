...
```jsonc
{
  ...
  "spec": "lib/test" // Defaults to "lib/test" where esm tests typically are.
};
```

It is recommended that tests files use a file name ending in ".spec.ts", but this is not currently enforced.
Packages using the defaults from the common configuration file will run all files in `/test/` regardless of name (except for `.tool.ts` files)
but some packages override the defaults in such a way that non ".spec.ts" files will be silently ignored.
