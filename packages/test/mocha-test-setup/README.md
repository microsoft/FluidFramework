...
```jsonc
{
  ...
  "spec": "lib/test" // Defaults to "dist/test" instead when run in via a package script ending with ":cjs"
};
```
...
If `npm_lifecycle_event` ends in `:cjs`, the `spec` will be modified to start with `dist/` instead of `lib/` (if it starts with `lib/`).
This is set automatically by tools like npm and pnpm to the script name when running scripts from a `package.json` file.
