...
import { DocumentDeltaEventManager, TestDeltaConnectionServer, TestDocumentServiceFactory, TestHost, TestResolver, } from "@microsoft/fluid-local-test-server";
const assertIntervalsHelper = (sharedString, intervals, expected) => {
...
describe("SharedInterval", () => {
    describe("one client", () => {
        ...
        let sharedString;
        let intervals;
        const assertIntervals = (expected) => {
    ...
    describe("multiple clients", () => {
        it("propagates", async () => {
            ...
            const sharedString1 = await host1.createType("text", SharedStringFactory.Type);
            ...
            const sharedString2 = await host2.getType("text");
    ...
    describe("Handles in value types", () => {
        ...
        it("Can store shared objects in a shared string's interval collection via properties", async () => {
            ...
            const outerString1 = await root1.get("outerString").get();
            const outerString2 = await root2.get("outerString").get();
            const outerString3 = await root3.get("outerString").get();
            ...
            const comment1From3 = await interval1From3.properties.story.get();
            assert.equal(comment1From3.getText(0, 12), "a comment...");
            const interval3From3 = serialized3[2];
            const mapFrom3 = await interval3From3.properties.story.get();
            assert.equal(mapFrom3.get("nestedKey"), "nestedValue");
            // SharedString snapshots as a blob
            const snapshotBlob = outerString2.snapshot().entries[0].value;
            ...
            parsedSnapshot.comments.value[0];
            // The "story" is the ILocalValue of the handle pointing to the SharedString
            const handleLocalValueFromSnapshot = serializedInterval1FromSnapshot.properties.story;
...
//# sourceMappingURL=sharedInterval.spec.js.map