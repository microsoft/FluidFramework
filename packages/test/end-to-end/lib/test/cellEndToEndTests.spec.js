...
describe("Cell", () => {
    ...
    beforeEach(async () => {
        ...
        root1Cell = await root1.get(cellId).get();
        root2Cell = await root2.get(cellId).get();
        root3Cell = await root3.get(cellId).get();
        // Set a starting value in the cell
        root1Cell.set(initialCellValue);
        await documentDeltaEventManager.process(user1Document, user2Document, user3Document);
    });
    function verifyCellValue(cell, expectedValue, index) {
        const userValue = cell.get();
        assert.equal(userValue, expectedValue, `Incorrect value ${userValue} instead of ${expectedValue} in document ${index}`);
    }
    ...
    function verifyCellEmpty(value1, value2, value3) {
    ...
    it("registers data if data is a shared object", async () => {
        const detachedCell1 = user1Document.createCell();
        const detachedCell2 = user1Document.createCell();
        ...
        assert(detachedCell2.isRegistered());
        await documentDeltaEventManager.process(user1Document, user2Document, user3Document);
        async function getCellComponent(cellP) {
            const cell = await cellP;
            const handle = cell.get();
            return handle.get();
        }
    ...
    afterEach(async () => {
        ...
        const closeP = [];
        if (user1Document) {
            closeP.push(user1Document.close());
        }
        if (user2Document) {
            closeP.push(user2Document.close());
        }
        if (user3Document) {
            closeP.push(user3Document.close());
        }
        await Promise.all(closeP);
        await testDeltaConnectionServer.webSocketServer.close();
    });
});
//# sourceMappingURL=cellEndToEndTests.spec.js.map