...
function generate(name, ctor, input, output) {
    describe(name, () => {
        ...
        beforeEach(async () => {
            ...
            user1Document = await api.load(id, { resolver }, {}, documentService);
            documentDeltaEventManager.registerDocuments(user1Document);
            user2Document = await api.load(id, { resolver }, {}, documentService);
            documentDeltaEventManager.registerDocuments(user2Document);
            user3Document = await api.load(id, { resolver }, {}, documentService);
        ...
        it("Should initialize after attach", async () => {
            ...
            const [collection2Handle, collection3Handle] = await Promise.all([
                root2.wait("collection"),
                root3.wait("collection"),
            ]);
        ...
        it("Wait resolves", async () => {
            ...
            assert.strictEqual(await waitOn2P, output[0], "Unexpected wait before add resolved value in document 2 added in document 1");
            const waitOn1P = collection1.waitAndRemove();
            await documentDeltaEventManager.process(user1Document, user2Document, user3Document);
            assert.strictEqual(await waitOn1P, output[1], "Unexpected wait after add resolved value in document 1 added in document 3");
            const waitOn3P = collection3.waitAndRemove();
            await documentDeltaEventManager.process(user1Document, user2Document, user3Document);
            assert.strictEqual(await waitOn3P, output[2], "Unexpected wait after add resolved value in document 13added in document 2");
        });
        it("Events", async () => {
            const collection1 = ctor.create(user1Document.runtime);
            root1.set("collection", collection1.handle);
...
//# sourceMappingURL=consensusOrderedCollectionEndToEndTests.spec.js.map