...
export class IgnoreErrorLogger extends EventAndErrorTrackingLogger {
	...
	public logEvents(seed: number): void {
		...
		fs.writeFileSync(
			`nyc/testData-${seed}/inactiveObjectEvents.json`,
			JSON.stringify(this.inactiveObjectEvents),
		);
		fs.writeFileSync(`nyc/testData-${seed}/errorEvents.json`, JSON.stringify(this.errorEvents));
		fs.writeFileSync(
			`nyc/testData-${seed}/errorEventStats.json`,
			JSON.stringify(this.errorEventStats),
		);
	}

	public validateEvents(seed: number): void {
		assert(
			this.inactiveObjectEvents.length === 0,
