...
const entityUrl = "https://westus.api.cognitive.microsoft.com/text/analytics/v2.1/entities";
...
class TextAnalyticsIntelligentService implements IIntelligentService {
    ...
    public async run(value: string): Promise<any> {
        ...
        if (condensed.length > 0) {
            ...
            const entitiesResultP = this.invokeRequest(entityUrl, data);
            const results = await Promise.all([sentimentResultP, keyPhrasesResultP, entitiesResultP]);
            ...
            const entities = results[2].documents[0] ? results[2].documents[0].entities : [];
