...
function frontMatterFromApiItem(apiItem, config) {
	...
	apiMembers.forEach((element) => {
		...
		frontMatter.members[element.kind][element.displayName] = getLinkUrlForApiItem(
			element,
			config,
		);
	});
		...
		documents.map(async (document) => {
			...
			function generateFrontMatter(apiItem) {
				...
				try {
					...
					console.error(
						`Encountered error while generating front-matter for "${document.apiItem.displayName}":`,
					);
					console.error(error);
					throw error;
				}
			...
			try {
				...
				console.error(
					`Encountered error while emitting markdown for "${document.apiItem.displayName}":`,
				);
				console.error(error);
				throw error;
			}
