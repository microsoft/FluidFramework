...
export function deleteFromRangeMap<T>(map: RangeMap<T>, start: number, length: number): boolean {
	...
	if (lengthBeforeFirst > 0 && lengthAfterLast > 0 && iFirst === iLast) {
		// The deleted range fits in the middle of an existing entry.
		// We replace the existing entry with:
		// 1) the portion which comes before the deleted range
		// 2) the portion which comes after the deleted range
		map.splice(
			iFirst,
			1,
			{ ...firstEntry, length: lengthBeforeFirst },
			{
				...lastEntry,
				start: end + 1,
				length: lengthAfterLast,
	...
	if (lengthBeforeFirst > 0) {
		...
		// The entry at `iFirst` is no longer overlapping with the deleted range
		iBefore = iFirst;
	}
	...
	if (lengthAfterLast > 0) {
		...
		// The entry at `iLast` is no longer overlapping with the deleted range
		iAfter = iLast;
	}

	const numContainedEntries = iAfter - iBefore - 1;
	map.splice(iBefore + 1, numContainedEntries);
	return true;
}
