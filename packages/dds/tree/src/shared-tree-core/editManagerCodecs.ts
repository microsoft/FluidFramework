...
> {
	...
	const encodeCommit = <T extends Commit<TChangeset>>(
		...
		...commit,
		revision: revisionTagCodec.encode(commit.revision, { originatorId: commit.sessionId, idCompressor: context.idCompressor, }),
	...
	const decodeCommit = <T extends EncodedCommit<JsonCompatibleReadOnly>>(
		...
		...commit,
		revision: revisionTagCodec.decode(commit.revision, { originatorId: commit.sessionId, idCompressor: context.idCompressor, }),
	...
	> = withSchemaValidation(
		format,
		{
			...
			decode: (json: EncodedEditManager<TChangeset>, context: EditManagerEncodingContext): SummaryData<TChangeset> => {
