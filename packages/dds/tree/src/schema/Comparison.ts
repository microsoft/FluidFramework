...
import { SchemaRepository, TreeSchema, ValueSchema, FieldSchema, FieldKind } from "./Schema";
...
export function allowsTreeSuperset(
	...
	if (!allowsFieldSuperset(repo, original.extraLocalFields, superset.extraLocalFields)) {
		return false;
	}
	...
	if (
		!compareSets(
			...
			(originalField) =>
				...
				allowsFieldSuperset(repo, repo.lookupGlobalFieldSchema(originalField), emptyField),
			// true iff the new field can be empty, since it may be empty in original
			(supersetField) =>
				allowsFieldSuperset(repo, emptyField, repo.lookupGlobalFieldSchema(supersetField)),
		)
	) {
		return false;
	}
...
export function allowsFieldSuperset(
	...
	if (!allowsKindSuperset(original.kind, superset.kind)) {
		return false;
	}
...
export function allowsRepoSuperset(
	...
	for (const [key, schema] of original.globalFieldSchema) {
		...
		if (!allowsFieldSuperset(original, schema, superset.lookupGlobalFieldSchema(key))) {
			return false;
		}
	}
	for (const [key, schema] of original.treeSchema) {
		...
		if (!allowsTreeSuperset(original, schema, superset.lookupTreeSchema(key))) {
			return false;
		}
	}
	return true;
 ...
 * This does not require a strict (aka proper) superset: equivalent schema will return true.
 */
export function allowsKindSuperset(original: FieldKind, superset: FieldKind): boolean {
	return (
		original === superset ||
		superset === FieldKind.Sequence ||
		((original === FieldKind.Forbidden || original === FieldKind.Value) &&
...
export function isNeverField(repo: SchemaRepository, field: FieldSchema): boolean {
	if (field.kind === FieldKind.Value && field.types !== undefined) {
