...
export class IdCompressor {
    ...
    public set clusterCapacity(value: number) {
        assert(value > 0, 0x480 /* Clusters must have a positive capacity */);
        assert(value <= IdCompressor.maxClusterSize, 0x481 /* Clusters must not exceed max cluster size */);
    ...
    public constructor(
        ...
        assert(reservedIdCount >= 0, 0x482 /* reservedIdCount must be non-negative */);
    ...
    public takeNextCreationRange(): IdCreationRange {
        ...
        assert(lastLocalInRange <= lastTakenNormalized, 0x484 /* Inconsistent local ID state */);
        ...
        if (lastLocalInRange !== lastTakenNormalized) {
            ...
            if (hasAtLeastLength(overrides, 1)) {
                assert(overrides[0][0] <= firstLocalInRange, 0x485 /* Inconsistent override state */);
                assert(
    ...
    public finalizeCreationRange(range: IdCreationRange): void {
        ...
        assert(finalizeCount >= 1, 0x489 /* Cannot finalize an empty range. */);
    ...
    public serialize(withSession: boolean): SerializedIdCompressor {
        ...
        assert(serializedSessions.length - this.sessions.size <= 2, 0x497 /* session not serialized */);
    ...
    public static deserialize(
        ...
        for (const serializedSession of serializedSessions) {
            ...
            if (sessionId === localSessionId) {
                assert(hasOngoingSession(serialized), 0x498 /* Cannot resume existing session. */);
