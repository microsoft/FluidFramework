...
function buildFieldComparator(
	expectedLength: number,
	nodeComparators: NodeComparator[],
): FieldComparator {
	return (cursor: ITreeCursorSynchronous): boolean => {
		...
		for (let index = 0; index < expectedLength; index++) {
			cursor.enterNode(index);
			const comparator = nodeComparators[index];
...
export function buildNodeComparator(cursor: ITreeCursorSynchronous): NodeComparator {
	...
	for (let inField = cursor.firstField(); inField; inField = cursor.nextField()) {
		...
		const nodeComparators: NodeComparator[] = [];

		for (let index = 0; index < length; index++) {
			cursor.enterNode(index);
			nodeComparators.push(buildNodeComparator(cursor));
			cursor.exitNode();
		}

		fieldComparators.push({
			key,
			compare: buildFieldComparator(length, nodeComparators),
		});
	}
	...
	return (other: ITreeCursorSynchronous): boolean => {
		if (other.value !== expectedValue) {
			return false;
		}
		for (const { key, compare } of fieldComparators) {
			other.enterField(key);
			if (compare(other) !== true) {
				other.exitField();
				return false;
			}
			other.exitField();
