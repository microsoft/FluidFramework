...
describe("RevisionTagCodec", () => {
	it("handles the root constant revision tag", () => {
		...
		const decoded = codec.decode(encoded, { originatorId: localCompressor.localSessionId, idCompressor: testIdCompressor });
	...
	it("normalizes compressed IDs between op and session space", () => {
		...
		assert.deepEqual(localId, localCodec.decode(localEncoded, { originatorId: localSession, idCompressor: testIdCompressor }));
		// A remote client should not be able to decode the local ID, as it has not received
		// the creation range for it
		assert.throws(() => remoteCodec.decode(localEncoded, { originatorId: localSession, idCompressor: testIdCompressor }));
		...
		const remoteDecoded = remoteCodec.decode(localEncoded, { originatorId: localSession, idCompressor: testIdCompressor });
		...
		assert.deepEqual(
			localEncoded,
			remoteCodec.decode(localEncoded, { originatorId: localSession, idCompressor: testIdCompressor }),
		);
		// Simulate the remote client referencing the local client's ID
		assert.deepEqual(
			localId,
			localCodec.decode(remoteEncoded, { originatorId: remoteSession, idCompressor: testIdCompressor }),
		);
	});
});
