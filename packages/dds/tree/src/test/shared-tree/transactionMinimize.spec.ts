...
describe("transaction minimize post-processor", () => {
	...
	/**
	 * Asserts that none of the node content retained by a minimized change's builds carries the
	 * transient marker (☠️).
	 *
	 * @remarks
	 * Walks every top-level built node (and its nested content) across all of the change's builds,
	 * checking each node's string value against {@link transientMarkerRegex}. Transiently-created
	 * content (created and then removed within the same transaction) is extraneous and should have been
	 * dropped by minimization, so its presence here indicates minimization did not fully remove it.
	 */
	function assertNoTransientBuildContent(minimized: ModularChangeset): void {
		if (minimized.builds === undefined) {
			return;
		}
	 ...
	 * asserts via {@link assertNoTransientBuildContent} that none of the resulting builds retain content
	 * tagged with the transient marker (☠️).
	 *
		...
		const postProcessor = createTransactionPostProcessor({
			...
			processChange: (change) =>
				mapDataChanges(change, (dataChange) => {
					...
					// Walk the minimized change's builds, asserting that no retained node
					// content carries the transient marker (☠️). Any such content is
					// extraneous data that minimization should have removed.
					assertNoTransientBuildContent(minimized);
