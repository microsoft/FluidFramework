...
describe("sharedTreeView", () => {
	...
	describe("throws an error if it is in the middle of an edit when a user attempts to", () => {
		...
		it("start a transaction", () => {
			expectErrorDuringEdit({
				duringEdit: (view) => Tree.runTransaction(view, () => {}),
				error: "Starting a transaction is forbidden during a nodeChanged or treeChanged event",
			});
		});

		it("commit a transaction", () => {
			// TODO: If/when async transactions become possible via the public API, update the usage here to use the public API
			expectErrorDuringEdit({
				setup: (view) => view.checkout.transaction.start(),
				duringEdit: (view) => view.checkout.transaction.commit(),
				error:
					"Committing a transaction is forbidden during a nodeChanged or treeChanged event",
			});
		});
		...
		it("abort a transaction", () => {
			...
			expectErrorDuringEdit({
				setup: (view) => view.checkout.transaction.start(),
				duringEdit: (view) => view.checkout.transaction.abort(),
				error: "Aborting a transaction is forbidden during a nodeChanged or treeChanged event",
			});
		});
