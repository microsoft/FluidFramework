...
describe("Editing", () => {
	describe("Sequence Field", () => {
		...
		it("can edit a concurrently removed tree", () => {
			...
			const { undoStack } = createRevertibleUndoRedoStacks(tree1.events);
			remove(tree1, 0, 1);
		...
		it("move, remove, restore", () => {
			...
			const { undoStack } = createRevertibleUndoRedoStacks(tree2.events);
		...
		it("ancestor of move destination removed", () => {
			...
			const { undoStack } = createRevertibleUndoRedoStacks(tree.events);
		...
		it("ancestor of move source removed then revived", () => {
			const tree = makeTreeFromJsonSequence([{ foo: ["a"] }, {}]);
			const tree2 = tree.branch();
			const { undoStack, unsubscribe } = createRevertibleUndoRedoStacks(tree.events);
		...
		it("remove, undo, childchange rebased over childchange", () => {
			...
			const tree2 = tree.branch();
			const { undoStack, unsubscribe } = createRevertibleUndoRedoStacks(tree2.events);
		...
		it("remove ancestor of return source", () => {
			...
			const undoTree1 = createRevertibleUndoRedoStacks(tree.events);
			const undoTree2 = createRevertibleUndoRedoStacks(tree2.events);
			...
			function runScenario(scenario: readonly ScenarioStep[], useMove: boolean): void {
				...
				innerFixture(title, () => {
					...
					const peerUndoStacks = peers.map((peer) =>
						createRevertibleUndoRedoStacks(peer.events),
					);
				...
				describe(`reverting [${action.title}] returns the content to foo`, () => {
					for (const disruption of disruptions) {
						...
						it(`even if it was ${disruption.title} concurrently to (and sequenced before) the revert`, () => {
							...
							const { undoStack, unsubscribe } = createRevertibleUndoRedoStacks(tree1.events);
	...
	describe("Optional Field", () => {
		...
		it("simplified repro for 0x7cf from anchors-undo-redo fuzz seed 0", () => {
			...
			const { undoStack, redoStack } = createRevertibleUndoRedoStacks(fork.events);
	...
	it.skip("edit removed content", () => {
		...
		const { undoStack, unsubscribe } = createRevertibleUndoRedoStacks(restoreRoot.events);
