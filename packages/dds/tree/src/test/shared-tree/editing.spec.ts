...
describe("Editing", () => {
	describe("Sequence Field", () => {
		...
		it("can rebase a change under a node whose insertion is also rebased", () => {
			...
			insert(tree2, 1, "C");
			tree3.editor.sequenceField(rootField).insert(0, chunkFromJsonField([{}]));

			const aEditor = tree3.editor.sequenceField({ parent: rootNode, field: brand("foo") });
			aEditor.insert(0, chunkFromJsonField(["a"]));
		...
		it("can edit a concurrently removed tree", () => {
			...
			listEditor.insert(3, chunkFromJsonField(["D"]));
		...
		it("can concurrently edit and move a subtree", () => {
			...
			editor.set(chunkFromJsonField(["C"]));
		...
		it("can concurrently edit and move a subtree (Move first) in a list under a node", () => {
			...
			tree2.editor.valueField(fooField).set(chunkFromJsonField(["a"]));
		...
		it("remove, undo, childchange rebased over childchange", () => {
			...
			tree2.editor
				...
				.insert(1, chunkFromJsonField(["c"]));

			tree.editor
				.sequenceField({ parent: sequenceUpPath, field: EmptyKey })
				.insert(0, chunkFromJsonField(["a"]));
		...
		it("can move inserted nodes to a different field", () => {
			...
			field.insert(0, chunkFromJsonField(["C"]));
			field.insert(0, chunkFromJsonField(["B"]));
			field.insert(0, chunkFromJsonField(["A"]));
			// Move nodes from foo into bar.
			tree.editor.move(
				{ parent: fooList, field: brand("") },
				0,
	...
	describe("Optional Field", () => {
		describe("can rebase a set over another set", () => {
			it("from a non-empty state", () => {
				...
				tree2.editor
					...
					.set(chunkFromJsonField(["2"]));

				tree3.editor
					.valueField({ parent: rootNode, field: brand("foo") })
					.set(chunkFromJsonField(["3"]));
			...
			it("from an empty state", () => {
				...
				tree2.editor
					...
					.set(chunkFromJsonField(["2"]), true);

				tree3.editor
					.optionalField({ parent: rootNode, field: brand("foo") })
					.set(chunkFromJsonField(["3"]), true);
		...
		it("can rebase a node replacement and a dependent edit to the new node", () => {
			...
			tree1.editor.optionalField(rootField).set(chunkFromJsonField(["41"]), true);

			tree2.editor.optionalField(rootField).set(chunkFromJsonField([{ foo: "42" }]), true);

			expectJsonTree([tree1], ["41"]);
			expectJsonTree([tree2], [{ foo: "42" }]);

			const editor = tree2.editor.valueField({ parent: rootNode, field: brand("foo") });
			editor.set(chunkFromJsonField(["43"]));
		...
		it("can rebase a node edit over an unrelated edit", () => {
			...
			tree1.editor
				...
				.set(chunkFromJsonField(["456"]), false);

			const editor = tree2.editor.valueField({ parent: rootNode, field: brand("foo") });
			editor.set(chunkFromJsonField(["42"]));
		...
		it("can rebase a node edit over the node being replaced and restored", () => {
			...
			tree1.editor.optionalField(rootField).set(chunkFromJsonField([{ foo: "41" }]), false);
		...
		it("can rebase over successive sets", () => {
			...
			tree1.editor.optionalField(rootField).set(chunkFromJsonField(["1"]), true);
			tree2.editor.optionalField(rootField).set(chunkFromJsonField(["2"]), true);

			tree2.rebaseOnto(tree1);
			tree1.editor.optionalField(rootField).set(chunkFromJsonField(["1 again"]), false);
		...
		it("can replace and restore a node", () => {
			...
			tree1.editor.optionalField(rootField).set(chunkFromJsonField(["43"]), false);
			...
			const revertibleAction = [
				{
					...
					delegate: (tree: ITreeCheckout) =>
						tree.editor.optionalField(rootField).set(chunkFromJsonField(["B"]), false),
					isEmptyAfter: false,
				},
				{
					title: "clear A",
			...
			const disruptions = [
				{
					...
					delegate: (tree: ITreeCheckout, isEmpty: boolean) =>
						tree.editor.optionalField(rootField).set(chunkFromJsonField(["C"]), isEmpty),
				},
				{
					title: "cleared",
					delegate: (tree: ITreeCheckout, isEmpty: boolean) =>
		...
		it("undo restores the removed node even when that node has been concurrently replaced", () => {
			...
			tree.editor.optionalField(rootField).set(chunkFromJsonField(["43"]), false);
		...
		describe("Transactions", () => {
			...
			it("Can rebase a series of edits including a transaction", () => {
				...
				tree2.transaction.start();
				tree2.editor.optionalField(rootField).set(chunkFromJsonField(["43"]), false);
				tree2.editor.optionalField(rootField).set(chunkFromJsonField(["44"]), false);
				tree2.transaction.commit();

				tree2.editor.optionalField(rootField).set(chunkFromJsonField(["45"]), false);

				tree.editor.optionalField(rootField).set(chunkFromJsonField(["46"]), false);
			...
			it("Can set and remove a node within a transaction", () => {
				...
				tree2.transaction.start();
				tree2.editor.optionalField(rootField).set(chunkFromJsonField(["42"]), true);
				tree2.editor.optionalField(rootField).set(undefined, false);
				tree2.transaction.commit();

				tree.editor.optionalField(rootField).set(chunkFromJsonField(["43"]), true);
		...
		it("simplified repro for 0x7cf from anchors-undo-redo fuzz seed 0", () => {
			...
			tree.editor.optionalField(rootField).set(chunkFromJsonField([2]), false);
	...
	describe("Constraints", () => {
		describe("Node existence constraint", () => {
			it("handles ancestor revive", () => {
				...
				treeSequence.insert(0, chunkFromJsonField(["bar"]));
				...
				tree2Sequence.insert(1, chunkFromJsonField(["b"]));
				tree2.transaction.commit();
			...
			it("handles ancestor remove", () => {
				...
				treeSequence.insert(0, chunkFromJsonField(["A"]));
				...
				tree.transaction.start();
				tree.editor.sequenceField(fooPath).insert(0, chunkFromJsonField(["C"]));
				remove(tree, 0, 1);
				tree.transaction.commit();
			...
			it("sequence field node exists constraint", () => {
				...
				tree2RootSequence.insert(1, chunkFromJsonField(["B"]));
				tree2.transaction.commit();
			...
			it("optional field node exists constraint", () => {
				...
				optional.set(chunkFromJsonField(["x"]), true);
			...
			it("existence constraint on node inserted in prior transaction", () => {
				...
				// State should be: ["a"]
				const sequence = tree.editor.sequenceField(rootField);
				sequence.insert(0, chunkFromJsonField(["a"]));
			...
			it("can add constraint to node inserted in same transaction", () => {
				...
				// State should be (to tree2): [{}, "c"]
				const tree2Sequence = tree2.editor.sequenceField(rootField);
				tree2Sequence.insert(1, chunkFromJsonField(["c"]));
			...
			it("a change can depend on the existence of a node that is built in a prior change whose constraint was violated", () => {
				...
				tree2.editor
					.sequenceField({ parent: rootNode, field: brand("bar") })
					.insert(0, chunkFromJsonField([{ baz: 42 }]));
				tree2.transaction.commit();
				...
				tree2.editor
					...
					.set(chunkFromJsonField([43]), false);
			...
			it("transaction dropped when constrained node is moved under a concurrently removed ancestor", () => {
				...
				const tree2RootSequence = tree2.editor.sequenceField(rootField);
				tree2RootSequence.insert(2, chunkFromJsonField(["b"]));
				tree2.transaction.commit();
	...
	describe("Can abort transactions", () => {
		...
		function abortTransaction(branch: ITreeCheckout): void {
			...
			foo1.remove(0, 1);
			foo1.insert(0, chunkFromJsonableField([{ type: brand("Number"), value: "RootValue2" }]));
			foo1.insert(0, chunkFromJsonableField([{ type: brand("Test") }]));
			foo1.remove(1, 1);
			foo1.insert(1, chunkFromJsonableField([{ type: brand("Number"), value: 82 }]));
