...
describe("SharedTree", () => {
	...
	it("can load a summary from a tree and receive edits that require detached tree refreshers", async () => {
		...
		const { undoStack, unsubscribe } = createClonableUndoRedoStacks(
	...
	describe("Undo and redo", () => {
		...
		it("rebased edits", () => {
			...
			} = createClonableUndoRedoStacks(tree1.checkout.events);
			...
			} = createClonableUndoRedoStacks(tree2.checkout.events);
	...
	describe("Schema changes", () => {
		...
		it.skip("can be undone at the tip", async () => {
			...
			const tree = provider.trees[0];
			const { undoStack } = createClonableUndoRedoStacks(tree.checkout.events);
