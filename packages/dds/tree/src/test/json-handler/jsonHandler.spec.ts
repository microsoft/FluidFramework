...
const insert = jh.object(() => ({
    ...
    complete: (result: JsonObject) => {
        const { content, destination } = result as unknown as { content: object; destination: Place };
...
const move = jh.object(() => ({
    ...
    complete: (result: JsonObject) => {
        ...
        console.log(`---> move from ${JSON.stringify(moveOp.source)} to ${JSON.stringify(moveOp.destination)}`);
    },
}));
...
const testHandler = (chunkSize: number) => {
    const testResponseHandler = createResponseHandler(exampleGeneratedEdit(), new AbortController());
