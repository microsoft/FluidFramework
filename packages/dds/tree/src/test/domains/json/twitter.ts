...
/**
 *
 * Kanji is not space separated but IS individual words.
 *
 * Because we don't have an english dictionary, we will count any occurance of
 * english charaters, numbers, symbols or escape characters without any spaces in between as a word.
 *
 * Thus,
 * 1. we will first space separate the text,
 * 2. we will iterate over each character in the space separated word.
 * 2a. If the char is a kanji it will be counted as a complete word.
 * 2b. If the characters are escapes or line breaks we will count it as part of a word,
 *  adding each next chars until we get to either a kanji character or a space.
 *
 *
 */
