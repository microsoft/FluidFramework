...
describe("SequenceField - Rebase", () => {
	...
	it("revive ↷ modify", () => {
		const revive = composeAnonChanges([
			Change.revive(0, 2, tag1, rebaseRepair, 0),
			Change.revive(4, 2, tag1, rebaseRepair, 2),
			Change.revive(10, 2, tag1, rebaseRepair, 4),
		]);
	...
	it("revive ↷ delete", () => {
		const revive = composeAnonChanges([
			Change.revive(0, 1, tag1, rebaseRepair, 0),
			Change.revive(3, 1, tag1, rebaseRepair, 1),
			Change.revive(8, 1, tag1, rebaseRepair, 2),
		]);
		const deletion = Change.delete(1, 3);
		...
		const expected = composeAnonChanges([
			...
			Change.revive(0, 1, tag1, rebaseRepair, 0),
			Change.revive(2, 1, tag1, rebaseRepair, 1, undefined, [{ revision: tag2, offset: 1 }]),
			Change.revive(5, 1, tag1, rebaseRepair, 2),
		]);
		assert.deepEqual(actual, expected);
	});

	it("conflicted revive ↷ related delete", () => {
		const revive = Change.revive(0, 3, tag1, rebaseRepair, 1, tag2);
		const deletion = Change.delete(1, 1);
		const actual = rebase(revive, deletion, tag2);
		const expected = composeAnonChanges([
			// Earlier revive is unaffected
			Change.revive(0, 1, tag1, rebaseRepair, 1, tag2),
			// Overlapping revive is no longer conflicted
			Change.revive(1, 1, tag2, rebaseRepair, 1),
			// Later revive is unaffected
			Change.revive(2, 1, tag1, rebaseRepair, 3, tag2),
		]);
		assert.deepEqual(actual, expected);
	});
	...
	it("conflicted revive ↷ unrelated delete", () => {
		...
		const expected = composeAnonChanges([
			...
			Change.revive(1, 1, tag1, rebaseRepair, 1, tag2, undefined, tag3),
			// Later revive gets linage
			Change.revive(1, 1, tag1, rebaseRepair, 3, tag2),
		]);
		assert.deepEqual(actual, expected);
	});

	it("blocked revive ↷ revive", () => {
		const revive1 = Change.revive(0, 3, tag1, rebaseRepair, 1, tag2, undefined, tag3);
		const revive2 = Change.revive(0, 1, tag3, rebaseRepair, 2);
		const actual = rebase(revive1, revive2, tag3);
		const expected = composeAnonChanges([
			// Earlier revive is unaffected
			Change.revive(0, 1, tag1, rebaseRepair, 1, tag2, undefined, tag3),
			// Overlapping revive remains conflicted but is no longer blocked
			Change.revive(0, 1, tag1, rebaseRepair, 2, tag2),
			// Later revive is unaffected
			Change.revive(1, 1, tag1, rebaseRepair, 3, tag2, undefined, tag3),
		]);
		assert.deepEqual(actual, expected);
	});

	it("conflicted intent-full revive ↷ related delete", () => {
		const revive = Change.intentionalRevive(0, 3, tag1, rebaseRepair, 1, tag2);
		const deletion = Change.delete(1, 1);
		const actual = rebase(revive, deletion, tag2);
		const expected = composeAnonChanges([
			// Earlier revive is unaffected
			Change.intentionalRevive(0, 1, tag1, rebaseRepair, 1, tag2),
			// Overlapping revive is no longer conflicted.
			// It now references the target node to revive using the latest delete.
			Change.intentionalRevive(1, 1, tag2, rebaseRepair, 1),
			// Later revive is unaffected
			Change.intentionalRevive(2, 1, tag1, rebaseRepair, 3, tag2),
		]);
		assert.deepEqual(actual, expected);
	});
	...
	it("conflicted intent-full revive ↷ unrelated delete", () => {
		...
		const expected = composeAnonChanges([
			...
			// Overlapping revive is no longer conflicted.
			// It now references the target node to revive using the latest delete.
			Change.intentionalRevive(1, 1, tag3, rebaseRepair, 1),
			// Later revive gets linage
			Change.intentionalRevive(2, 1, tag1, rebaseRepair, 3, tag2),
		]);
		assert.deepEqual(actual, expected);
	...
	it("revive ↷ insert", () => {
		const revive = composeAnonChanges([
			Change.revive(0, 1, tag1, rebaseRepair, 0),
			Change.revive(3, 2, tag1, rebaseRepair, 1),
			Change.revive(7, 1, tag1, rebaseRepair, 3),
		]);
		...
		const expected = composeAnonChanges([
			Change.revive(0, 1, tag1, rebaseRepair, 0),
			Change.revive(3, 2, tag1, rebaseRepair, 1),
			Change.revive(8, 1, tag1, rebaseRepair, 3),
		]);
		assert.deepEqual(actual, expected);
	});

	it("conflicted revive ↷ insert", () => {
		const revive = Change.revive(0, 3, tag1, rebaseRepair, 0, tag2);
		const insert = Change.insert(1, 1);
		const actual = rebase(revive, insert);
		const expected = composeAnonChanges([
			Change.revive(0, 1, tag1, rebaseRepair, 0, tag2),
			Change.revive(2, 2, tag1, rebaseRepair, 1, tag2),
		]);
		assert.deepEqual(actual, expected);
	});
	...
	it("modify ↷ revive", () => {
		...
		const revive = Change.revive(2, 1, tag1, rebaseRepair, 0);
		const expected = composeAnonChanges([
			// Modify at earlier index is unaffected
			Change.modify(0, TestChange.mint([0], 1)),
			// Modify at later index has its index increased
	...
	it("delete ↷ revive", () => {
		...
		const revive = Change.revive(3, 1, tag1, rebaseRepair, 0);
		const expected = composeAnonChanges([
			// Delete with earlier index is unaffected
			Change.delete(0, 1),
			// Delete at overlapping index is split
	...
	it("insert ↷ revive", () => {
		...
		const revive = Change.revive(1, 1, tag1, rebaseRepair, 0);
	...
	it("reviveAA ↷ reviveB => BAA", () => {
		const reviveAA = Change.revive(0, 2, tag1, rebaseRepair, 1, undefined, [
			{ revision: tag2, offset: 1 },
		]);
		const reviveB = Change.revive(0, 1, tag2, rebaseRepair, 0);
		const expected = Change.revive(1, 2, tag1, rebaseRepair, 1, undefined, [
			{ revision: tag2, offset: 1 },
		]);
	...
	it("reviveAA ↷ reviveB => AAB", () => {
		const reviveAA = Change.revive(0, 2, tag1, rebaseRepair, 0, undefined, [
			{ revision: tag2, offset: 0 },
		]);
		const reviveB = Change.revive(0, 1, tag2, rebaseRepair, 0);
		const expected = Change.revive(0, 2, tag1, rebaseRepair, 0, undefined, [
			{ revision: tag2, offset: 0 },
		]);
		const actual = rebase(reviveAA, reviveB);
		assert.deepEqual(actual, expected);
	...
	it("reviveBB ↷ reviveA => BBA", () => {
		const reviveBB = Change.revive(0, 2, tag2, rebaseRepair, 0);
		const reviveA = Change.revive(0, 1, tag1, rebaseRepair, 2, undefined, [
			{ revision: tag2, offset: 2 },
		]);
		const expected = Change.revive(0, 2, tag2, rebaseRepair, 0);
	...
	it.skip("reviveBB ↷ reviveA => ABB", () => {
		const reviveBB = Change.revive(0, 2, tag2, rebaseRepair, 1);
		const reviveA = Change.revive(0, 1, tag1, rebaseRepair, 0, undefined, [
			{ revision: tag2, offset: 0 },
		]);
		const expected = Change.revive(1, 2, tag2, rebaseRepair, 1);
		const actual = rebase(reviveBB, reviveA);
		assert.deepEqual(actual, expected);
	});
	...
	it.skip("reviveA ↷ reviveBB => BAB", () => {
		const reviveA = Change.revive(0, 1, tag1, rebaseRepair, 1, undefined, [
			{ revision: tag2, offset: 1 },
		]);
		const reviveBB = Change.revive(0, 2, tag2, rebaseRepair, 0);
		const expected = Change.revive(1, 1, tag1, rebaseRepair, 1, undefined, [
			{ revision: tag2, offset: 1 },
		]);
	...
	it("intentional revive ↷ same revive", () => {
		const reviveA = Change.intentionalRevive(0, 3, tag1, rebaseRepair, 1);
		const reviveB = Change.revive(0, 1, tag1, rebaseRepair, 2);
		const actual = rebase(reviveA, reviveB, tag2);
		const expected = composeAnonChanges([
			Change.intentionalRevive(0, 1, tag1, rebaseRepair, 1),
			Change.intentionalRevive(1, 1, tag1, rebaseRepair, 2, tag2),
			Change.intentionalRevive(2, 1, tag1, rebaseRepair, 3),
		]);
		assert.deepEqual(actual, expected);
	});

	it("revive ↷ same revive (base within curr)", () => {
		const reviveA = Change.revive(0, 3, tag1, rebaseRepair, 1);
		const reviveB = Change.revive(0, 1, tag1, rebaseRepair, 2);
		const actual = rebase(reviveA, reviveB, tag2);
		const expected = composeAnonChanges([
			Change.revive(0, 1, tag1, rebaseRepair, 1),
			Change.revive(1, 1, tag1, rebaseRepair, 2, tag2),
			Change.revive(2, 1, tag1, rebaseRepair, 3),
		]);
		assert.deepEqual(actual, expected);
	});

	it("revive ↷ same revive (curr within base)", () => {
		const reviveA = Change.revive(0, 1, tag1, rebaseRepair, 2);
		const reviveB = Change.revive(0, 3, tag1, rebaseRepair, 1);
		const actual = rebase(reviveA, reviveB, tag2);
		const expected = Change.revive(1, 1, tag1, rebaseRepair, 2, tag2);
		assert.deepEqual(actual, expected);
	});
	...
	it("return ↷ related revive ", () => {
		const revive = Change.revive(0, 1, tag1, rebaseRepair, 0);
