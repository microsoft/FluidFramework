	...
	describe("relevantRemovedRoots", () => {
		describe("does not include", () => {
			it("a tree that remains in-doc", () => {
				const input: Changeset = [{ count: 1 }];
				const actual = relevantRemovedRoots(input, noTreeDelegate);
				const array = [...actual];
				assert.deepEqual(array, []);
			});
			it("a tree with child changes that remains in-doc", () => {
				const input: Changeset = [Mark.modify(childChange)];
				const actual = relevantRemovedRoots(input, noTreeDelegate);
				const array = [...actual];
				assert.deepEqual(array, []);
			});
			it("a tree that remains removed", () => {
				const input: Changeset = [{ count: 1, cellId: atomId }];
				const actual = relevantRemovedRoots(input, noTreeDelegate);
				const array = [...actual];
				assert.deepEqual(array, []);
			});
			it("a tree being removed", () => {
				const input: Changeset = [Mark.remove(1, atomId)];
				const actual = relevantRemovedRoots(input, noTreeDelegate);
				const array = [...actual];
				assert.deepEqual(array, []);
			});
			it("a tree with child changes being removed", () => {
				const input: Changeset = [Mark.remove(1, atomId, { changes: childChange })];
				const actual = relevantRemovedRoots(input, noTreeDelegate);
				const array = [...actual];
				assert.deepEqual(array, []);
			});
			it("a tree being moved", () => {
				const input: Changeset = [Mark.moveOut(1, atomId), Mark.moveIn(1, atomId)];
				const actual = relevantRemovedRoots(input, noTreeDelegate);
				const array = [...actual];
				assert.deepEqual(array, []);
			});
			...
			it("a live tree being pinned", () => {
				const input: Changeset = [Mark.pin(1, brand(0))];
				const actual = relevantRemovedRoots(input, noTreeDelegate);
				const array = [...actual];
				assert.deepEqual(array, []);
			});
		...
		describe("does include", () => {
			it("a tree being inserted", () => {
				const input: Changeset = [Mark.insert(1, atomId)];
				const actual = relevantRemovedRoots(input, noTreeDelegate);
				const array = [...actual];
				assert.deepEqual(array, [deltaId]);
			});
			it("a tree being transiently inserted", () => {
				const input: Changeset = [Mark.remove(1, atomId, { cellId: atomId })];
				const actual = relevantRemovedRoots(input, noTreeDelegate);
				const array = [...actual];
				assert.deepEqual(array, [deltaId]);
			});
			it("a tree being transiently inserted and moved out", () => {
				const input: Changeset = [Mark.moveOut(1, atomId, { cellId: atomId })];
				const actual = relevantRemovedRoots(input, noTreeDelegate);
				const array = [...actual];
				assert.deepEqual(array, [deltaId]);
			});
			...
			it("a tree being restored by revive", () => {
				const input: Changeset = [Mark.revive(1, atomId)];
				const actual = relevantRemovedRoots(input, noTreeDelegate);
				const array = [...actual];
				assert.deepEqual(array, [deltaId]);
			});
			it("a tree being restored by pin", () => {
				const input: Changeset = [Mark.pin(1, brand(0), { cellId: atomId })];
				const actual = relevantRemovedRoots(input, noTreeDelegate);
				const array = [...actual];
				assert.deepEqual(array, [deltaId]);
			});
			it("a tree being transiently restored", () => {
				const input: Changeset = [Mark.remove(1, brand(0), { cellId: atomId })];
				const actual = relevantRemovedRoots(input, noTreeDelegate);
				const array = [...actual];
				assert.deepEqual(array, [deltaId]);
			});
			it("relevant roots from nested changes under a tree being restored by revive", () => {
				const input: Changeset = [Mark.revive(1, atomId, { changes: childChange })];
				const actual = relevantRemovedRoots(input, oneTreeDelegate);
				const array = [...actual];
				assert.deepEqual(array, [deltaId, relevantNestedTree]);
			});
			...
			it("relevant roots from nested changes under a tree being inserted", () => {
				const input: Changeset = [Mark.insert(1, atomId, { changes: childChange })];
				const actual = relevantRemovedRoots(input, oneTreeDelegate);
				const array = [...actual];
				assert.deepEqual(array, [deltaId, relevantNestedTree]);
			});
