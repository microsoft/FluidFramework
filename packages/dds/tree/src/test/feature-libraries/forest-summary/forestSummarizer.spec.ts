...
class FooArray extends sf.arrayAlpha("fooArray", sf.incrementalSummary(sf.string)) {}
...
describe("ForestSummarizer", () => {
	...
	describe("Incremental summarization", () => {
		...
		describe("multi-depth schema", () => {
			 ...
			 */
			class BarArray extends sf.arrayAlpha("barArray", sf.incrementalSummary(BarItem)) {}
		...
		describe("4-depth schema with parameterized incremental summarization", () => {
			...
			class Item extends sf.object("Item", {
				 ...
				 */
				tags: sf.incrementalSummary(sf.map(Tag)),
			}) {}
			...
			class Section extends sf.object("Section", {
				 ...
				 */
				items: sf.incrementalSummary(sf.map(Item)),
			}) {}
			...
			class Workspace extends sf.object("Workspace", {
				 ...
				 */
				documents: sf.incrementalSummary(sf.map(Document)),
			}) {}
