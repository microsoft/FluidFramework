...
describe("Schema Comparison", () => {
	...
	describe("allowsDocumentConcept", () => {
		const createTestTreeWithObjectNodeSchema = (
			fields: [string, TreeFieldStoredSchema][],
			treeName: string,
		) => ({
			name: brand<TreeNodeSchemaIdentifier>(treeName),
			schema: new ObjectNodeStoredSchema(
				new Map(fields.map(([key, schema]) => [brand(key), schema])),
			),
		});

		const createTestTreeWithMapNodeSchema = (
			field: TreeFieldStoredSchema,
			treeName: string,
		) => ({
			name: brand<TreeNodeSchemaIdentifier>(treeName),
			schema: new MapNodeStoredSchema(field),
		});

		it("Fields Schema difference", () => {
			...
			const tree1 = createTestTreeWithObjectNodeSchema(
				[
					[
						"x",
			...
			const tree2 = createTestTreeWithObjectNodeSchema(
				[
					[
						"x",
			...
			const schema1 = {
				...
				nodeSchema: new Map([[tree1.name, tree1.schema]]),
			};
			const schema2 = {
				rootFieldSchema: root2,
				nodeSchema: new Map([[tree2.name, tree2.schema]]),
			};

			const incomaptibilities = allowsDocumentConcepts(schema1, schema2);
			const expected = [
				{
					identifier: "root",
					mismatch: "fieldKind",
			...
			assert.deepEqual(incomaptibilities, expected);

			const tree3 = createTestTreeWithObjectNodeSchema(
				[
					[
						"y",
						fieldSchema(FieldKinds.optional, [
			...
			const schema3 = {
				rootFieldSchema: root2,
				nodeSchema: new Map([[tree3.name, tree3.schema]]),
			};

			const incomaptibilities2 = allowsDocumentConcepts(schema2, schema3);
			const expected2 = [
				{
					identifier: "tree",
					mismatch: "fields",
			...
			assert.deepEqual(incomaptibilities2, expected2);
		});

		it("Node Schema difference", () => {
			const root = fieldSchema(FieldKinds.optional);

			// different identifiers for TreeNodeStoredSchema
			const tree1 = createTestTreeWithObjectNodeSchema(
				[
					[
						"x",
			...
			// MapNodeSchema vs ObjectNodeSchema
			const tree3 = createTestTreeWithMapNodeSchema(
			...
			const incomaptibilities2 = allowsDocumentConcepts(schema1, schema3);
			const expected2 = [
				{
					identifier: "tree",
					mismatch: "nodeKind",
			...
			// Both utilize MapNodeSchema but with differing fieldSchemas
			const tree4 = createTestTreeWithMapNodeSchema(
				fieldSchema(FieldKinds.optional, [
					brand<TreeNodeSchemaIdentifier>("number"),
					brand<TreeNodeSchemaIdentifier>("string"),
				]),
			...
			const schema4 = {
				rootFieldSchema: root,
				nodeSchema: new Map([[tree4.name, tree4.schema]]),
			};

			const incomaptibilities3 = allowsDocumentConcepts(schema3, schema4);
			const expected3 = [
				{
					identifier: "tree",
					mismatch: "allowedTypes",
			...
			assert.deepEqual(incomaptibilities3, expected3);
		});
	});
});
