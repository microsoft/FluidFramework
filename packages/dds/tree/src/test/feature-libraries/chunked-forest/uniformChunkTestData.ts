...
export const emptyShape = new TreeShape(
	brand(EmptyObject.identifier),
	false,
	[],
);
...
const withChildShape = new TreeShape(
	brand(JsonAsTree.JsonObject.identifier),
	false,
	[[xField, numberShape, 1]],
);
const pointShape = new TreeShape(
	brand(JsonAsTree.JsonObject.identifier),
	false,
	[
		[xField, numberShape, 1],
...
const testTrees: {
	...
	},
	{
		name: "withChild",
		dataFactory: () =>
			new UniformChunk(withChildShape.withTopLevelLength(1), [1]),
		reference: [
			{
				type: brand(JsonAsTree.JsonObject.identifier),
				fields: {
	...
	},
	{
		name: "point",
		dataFactory: () =>
			new UniformChunk(pointShape.withTopLevelLength(1), [1, 2]),
		reference: [
			{
				type: brand(JsonAsTree.JsonObject.identifier),
				fields: {
