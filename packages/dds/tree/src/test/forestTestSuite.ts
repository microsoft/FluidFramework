...
export function testForest(config: ForestTestConfiguration): void {
	...
	describe(config.suiteName, () => {
		...
		describe("insert and extract json", () => {
			...
			for (const [name, data] of testCases) {
				it(name, () => {
					...
					initializeForest(forest, [singleJsonCursor(data)], testRevisionTagCodec, testIdCompressor);
		...
		it("can destroy detached fields", () => {
			...
			initializeForest(forest, content.map(singleJsonCursor), testRevisionTagCodec, testIdCompressor);
		...
		describe("can clone", () => {
			...
			it("multiple fields", () => {
				const schema = new TreeStoredSchemaRepository(intoStoredSchema(jsonDocumentSchema));
				const forest = factory(schema);
				initializeForest(forest, [singleJsonCursor(nestedContent)], testRevisionTagCodec, testIdCompressor);
		...
		it("editing a cloned forest does not modify the original", () => {
			...
			initializeForest(forest, content.map(cursorForJsonableTreeNode), testRevisionTagCodec, testIdCompressor);
		...
		describe("can apply deltas with", () => {
			if (!config.skipCursorErrorCheck) {
				it("ensures cursors are cleared before applying deltas", () => {
					...
					initializeForest(forest, [singleJsonCursor(1)], testRevisionTagCodec, testIdCompressor);
					const cursor = forest.allocateCursor();
					moveToDetachedField(forest, cursor);
	...
	testGeneralPurposeTreeCursor(
		...
		(data): ITreeCursor => {
			...
			initializeForest(forest, [cursorForJsonableTreeNode(data)], testRevisionTagCodec, testIdCompressor);
			const cursor = forest.allocateCursor();
			moveToDetachedField(forest, cursor);
			assert(cursor.firstNode());
			return cursor;
