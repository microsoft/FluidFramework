...
// 	cellValue: string;
// 	/**
// 	 * A function that sets up the operation to be performed on the tree and stack.
// 	 */
// 	setupOperation: (tree: TableTreeDefinition, stack: UndoRedoStackManager) => void;
// 	/**
...
// 				localTree.openUndo(stack);
// 				setupOperation(localTree, stack);
				...
				runBenchmark({
					...
					operation: (tree: TableTreeDefinition) => {
						for (let i = 0; i < count; i++) {
							...
							const column = new Column({ id: uuid() });
				...
				runBenchmark({
					...
					operation: (tree: TableTreeDefinition) => {
						for (let i = 0; i < count; i++) {
							...
							const row = new Row({ id: uuid(), cells: {} });
