...
class TestChangeRebaser implements ChangeRebaser<TestChangeset> {
	...
	public static mintChangeset(
		inputContext: readonly number[],
		intentionOpt: number,
	): NonEmptyTestChangeset {
	...
	public compose(changes: TestChangeset[]): TestChangeset {
		...
		for (const change of changes) {
			if (isNonEmptyChange(change)) {
				...
				intentions = TestChangeRebaser.composeIntentions(intentions, change.intentions);
			}
		}
...
function runScenario(scenario: readonly ScenarioStep[]): void {
	...
	for (const step of scenario) {
		// Perform the step
		{
			...
			if (step.type === "Mint") {
				const cs = TestChangeRebaser.mintChangeset(
					getTipContext(client.manager),
					++changeCounter,
				);
