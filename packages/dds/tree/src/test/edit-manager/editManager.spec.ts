...
class TestChangeRebaser implements ChangeRebaser<TestChangeset> {
	...
	public checkChangeList(
		changes: readonly RecursiveReadonly<TestChangeset>[],
		intentions?: Set<number>,
	): void {
		const filtered = changes.filter(isNonEmptyChange);
		const intentionsSeen = new Set<number>();
		const intentionsExpected = new Set<number>(
			intentions ?? makeArray(this.intentionCounter, (i: number) => i + 1),
		);
		let index = 0;
...
function updateIntentionSet(intentions: readonly number[], intentionsSeen: Set<number>) {
...
function editManagerFactory(): {
	...
	const manager = new EditManager<TestChangeset, ChangeFamily<unknown, TestChangeset>>(family);
	...
	for (const step of scenario) {
		// Perform the step
		{
			...
			if (step.type === "Mint") {
				...
				commit.changeset.intentions.forEach((intention) =>
					client.intentions.add(intention),
				);
				client.ref += 1;
			}
		}
