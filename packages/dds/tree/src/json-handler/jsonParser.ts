...
export interface JsonBuilder<ObjectHandle, ArrayHandle> {
    ...
    addPrimitive?(value: JsonPrimitive, context?: JsonBuilderContext<ObjectHandle, ArrayHandle>): void;
...
enum State {
    ...
    InsideObjectAtStart, InsideObjectAfterKey, InsideObjectAfterColon, InsideObjectAfterProperty, InsideObjectAfterComma,
    InsideArrayAtStart, InsideArrayAfterElement, InsideArrayAfterComma, InsideMarkdownAtStart, InsideMarkdownAtEnd,

    // Special states while processing multi-character tokens (which may not arrive all at once)
    InsideKeyword, InsideNumber, InsideKey, InsideString, InsideLeadingMarkdownDelimiter, InsideTrailingMarkdownDelimiter,
...
// prettier-ignore
enum Production { Value, Key, Colon, Comma, CloseBrace, CloseBracket, LeadingMarkdownDelimiter, TrailingMarkdownDelimiter }

type StateTransition = [Production, State];

// prettier-ignore
const stateTransitionTable = new Map<State, StateTransition[]>([
    [State.Start,                      [[Production.Value, State.End], [Production.LeadingMarkdownDelimiter, State.InsideMarkdownAtStart]]],
    [State.InsideObjectAtStart,        [[Production.Key, State.InsideObjectAfterKey], [Production.CloseBrace, State.Pop]]],
    ...
    [State.InsideObjectAfterProperty,  [[Production.Comma, State.InsideObjectAfterComma], [Production.CloseBrace, State.Pop]]],
    [State.InsideObjectAfterComma,     [[Production.Key, State.InsideObjectAfterKey]]],
    [State.InsideArrayAtStart,         [[Production.Value, State.InsideArrayAfterElement], [Production.CloseBracket, State.Pop]]],
    [State.InsideArrayAfterElement,    [[Production.Comma, State.InsideArrayAfterComma], [Production.CloseBracket, State.Pop]]],
...
class JsonParserImpl<ObjectHandle, ArrayHandle> implements StreamedJsonParser {
    public constructor(
        private builder: JsonBuilder<ObjectHandle, ArrayHandle>,
    ...
    private contexts: ParserContext<ObjectHandle, ArrayHandle>[] = [{ state: State.Start, firstToken: '' }];
    ...
    private processJsonText(): boolean {
        ...
        const builderContext = this.builderContextFromParserContext(this.contexts[this.contexts.length - 1])!;
    ...
    private appendText(text: string) {
        assert(this.contexts.length > 1);
        const builderContext = this.builderContextFromParserContext(this.contexts[this.contexts.length - 2])!;
        this.verifyImplemented(this.builder.appendText);
        this.builder.appendText!(text, builderContext);
    }

    private consumeCharAndPush(state: State, parent?: { parentObject?: ObjectHandle; parentArray?: ArrayHandle }) {
    ...
    private completeAndPopContext() {
        ...
        this.builder.completeContext?.(this.builderContextFromParserContext(this.contexts[this.contexts.length - 2]));
        this.popContext();
    }
