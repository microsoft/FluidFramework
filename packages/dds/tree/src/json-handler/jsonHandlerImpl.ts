...
class ResponseHandlerImpl {
    public constructor(private streamedType: StreamedType, abortController: AbortController) {
        if (streamedType instanceof StreamedStringProperty || streamedType instanceof StreamedString) {
            throw new Error('StreamedStringProperty and StreamedString cannot be used as root type');
        }

        const streamedValueHandler = (streamedType as InvocableStreamedType<StreamedValueHandler>).invoke(
            undefined,
            streamedType instanceof StreamedObject ? {} : streamedType instanceof StreamedArray ? [] : undefined,
        );
    ...
    jsonSchema(): JsonObject {
        ...
        const rootIdentity = (this.streamedType as InvocableStreamedType<StreamedValueHandler>).getIdentity();

        // Don't call jsonSchemaFromStreamedType, as we must force the method call on the root
        const schema = (this.streamedType as InvocableStreamedType<StreamedValueHandler>).jsonSchema(
            rootIdentity,
            definitions,
        );
    ...
    private parser: StreamedJsonParser;
}

// The one createResponseHandlerImpl
const createResponseHandlerImpl = (streamedType: StreamedType, abortController: AbortController) => {
...
class JsonHandlerImpl {
    public object<Input>(
        ...
        return function getStreamedObject(getInput?: (partial: PartialArg) => Input): StreamedObject<Input> {
    ...
    public array<Input>(
        ...
        // The function created here serves as the identity of this type's schema,
        // since the schema is independent of the input passed to the handler
        return function getStreamedArray(getInput?: (partial: PartialArg) => Input): StreamedArray<Input> {
            return new StreamedArray(getDescriptor, getStreamedArray, getInput);
        };
    }

    public streamedStringProperty<T extends Record<P, string | undefined>, P extends keyof T>(args: {
        description?: string;
    ...
    public null(args?: { description?: string; complete?: (value: null, partial: PartialArg) => void }): StreamedType {
        return new AtomicNull(args);
    }

    public optional(streamedType: StreamedType): StreamedType {
        if (streamedType instanceof AtomicNull) {
            throw new Error('Cannot have an optional null value');
        }
...
class BuilderDispatcher implements JsonBuilder<StreamedObjectHandler, StreamedArrayHandler> {
    public constructor(private rootHandler: StreamedValueHandler) {}
    ...
    addObject(context?: BuilderContext): StreamedObjectHandler {
        if (!context) {
            ...
            if (!(this.rootHandler instanceof StreamedObjectHandlerImpl)) {
                throw new Error(`Expected object for root`);
            }
    ...
    addArray(context?: BuilderContext): StreamedArrayHandler {
        if (!context) {
            if (!(this.rootHandler instanceof StreamedArrayHandlerImpl)) {
                throw new Error(`Expected array for root`);
            }
            return this.rootHandler;
        } else if (contextIsObject(context)) {
        ...
        if (!context) {
            if (value === null) {
                if (!(this.rootHandler instanceof AtomicNullHandlerImpl)) {
                    throw new Error(`Expected null for root`);
                }
            } else {
                switch (typeof value) {
                    case 'string':
                        if (
                            ...
                            throw new Error(`Expected string or enum for root`);
                        }
                        break;
                    case 'number':
                        if (!(this.rootHandler instanceof AtomicNumberHandlerImpl)) {
                            throw new Error(`Expected number for root`);
                        }
                        break;
                    case 'boolean':
                        if (!(this.rootHandler instanceof AtomicBooleanHandlerImpl)) {
                            throw new Error(`Expected boolean for root`);
                        }
                        break;
                }
            }
...
abstract class SchemaGeneratingStreamedType extends StreamedType {
    ...
    abstract findDefinitions(visited: Set<StreamedTypeIdentity>, definitions: DefinitionMap): void;
    abstract jsonSchema(root: StreamedTypeIdentity, definitions: DefinitionMap): JsonObject;
}

abstract class InvocableStreamedType<T extends StreamedValueHandler | undefined> extends SchemaGeneratingStreamedType {
...
const findDefinitions = (
    ...
    return (streamedType as InvocableStreamedType<StreamedValueHandler>).findDefinitions(visited, definitions);
};

const addDefinition = (streamedType: StreamedTypeIdentity, definitions: DefinitionMap): void => {
...
const jsonSchemaFromStreamedType = (
    ...
    if (root === identity) {
        ...
        return (streamedType as InvocableStreamedType<StreamedValueHandler>).jsonSchema(root, definitions);
    }
};
...
const guaranteedErrorHandler = {
    ...
    set<T, V>(target: T, prop: string, value: V) {
        throw new Error(`Attempted to set property "${prop}" to "${value}" outside a handler body.`);
    },
    has<T>(target: T, prop: string) {
        throw new Error(`Attempted to check existence of property "${prop}" outside a handler body.`);
    },
...
class StreamedObject<Input> extends InvocableStreamedType<StreamedObjectHandler> {
    constructor(
        private getDescriptor: (input: Input) => StreamedObjectDescriptor,
        private identity: StreamedTypeIdentity,
        private getInput?: (partial: PartialArg) => Input,
    ) {
        super();
    }
    ...
    jsonSchema(root: StreamedTypeIdentity, definitions: DefinitionMap): JsonObject {
        ...
        propertyNames.forEach((fieldName) => {
            schemaProperties[fieldName] = jsonSchemaFromStreamedType(properties[fieldName], root, definitions);
        });
    ...
    invoke(parentPartial: PartialArg, partial: PartialArg): StreamedObjectHandler {
        return new StreamedObjectHandlerImpl(partial, this.getDescriptor(this.getInput?.(parentPartial) as Input));
    }
...
class StreamedObjectHandlerImpl implements StreamedObjectHandler {
    constructor(
        ...
        private streamedAnyOf?: StreamedAnyOf,
    ) {}
        ...
        if (!this.descriptor) {
            ...
            if (streamedType instanceof StreamedAnyOf) {
                ...
                if (!(streamedType = streamedType.streamedTypeIfSingleMatch(StreamedObject))) {
                    ...
                    this.handlers[key] = new StreamedObjectHandlerImpl(childPartial, undefined, streamedAnyOf);
        ...
        if (!this.descriptor) {
            ...
            if (streamedType instanceof StreamedAnyOf) {
                ...
                if (!(streamedType = streamedType.streamedTypeIfSingleMatch(StreamedObject))) {
                    ...
                    this.partial[key] = childPartial;
                    this.handlers[key] = new StreamedArrayHandlerImpl(childPartial, undefined, streamedAnyOf);
...
class StreamedArray<Input> extends InvocableStreamedType<StreamedArrayHandler> {
    constructor(
        private getDescriptor: (input: Input) => StreamedArrayDescriptor,
        private identity: StreamedTypeIdentity,
        private getInput?: (partial: PartialArg) => Input,
    ) {
        super();
    ...
    invoke(parentPartial: PartialArg, partial: PartialArg): StreamedArrayHandler {
        return new StreamedArrayHandlerImpl(partial, this.getDescriptor(this.getInput?.(parentPartial) as Input));
    }
...
class StreamedArrayHandlerImpl implements StreamedArrayHandler {
        ...
        if (this.descriptor) {
            ...
            if (streamedType instanceof StreamedAnyOf) {
                ...
                if (!(streamedType = streamedType.streamedTypeIfSingleMatch(StreamedObject))) {
                    const childPartial = {} as PartialObject;
                    this.partial.push(childPartial);
                    this.lastHandler = new StreamedObjectHandlerImpl(childPartial, undefined, streamedAnyOf);
    ...
    completeLast?(): void {
        ...
        if (
            ...
            this.lastHandler.complete(this.partial[this.partial.length - 1] as boolean, this.partial);
...
type StreamedStringPropertyDescriptor<T extends Record<P, string | undefined>, P extends keyof T> = SchemaArgs & {
    target: (partial: PartialArg) => T;
    key: P;
    complete?: (value: string, partial: PartialArg) => void;
};
...
> extends InvocableStreamedType<StreamedStringPropertyHandler> {
    constructor(private args: StreamedStringPropertyDescriptor<T, P>) {
        super();
    }
...
class StreamedStringPropertyHandlerImpl<T extends Record<P, string | undefined>, P extends keyof T>
    implements StreamedStringPropertyHandler
{
    public constructor(
        private onAppend: (chars: string) => void,
        private onComplete?: (value: string, partial: PartialArg) => void,
    ) {}
...
class StreamedString<Parent extends object> extends InvocableStreamedType<StreamedStringHandler> {
    constructor(private args: StreamedStringDescriptor<Parent>) {
        super();
    }
...
class StreamedStringHandlerImpl<Parent extends object> implements StreamedStringHandler {
    public constructor(
        ...
        private onAppend: (chars: string, parent: Parent) => void,
        private onComplete?: (value: string, partial: PartialArg) => void,
    ) {}
...
abstract class AtomicPrimitive<T extends PrimitiveType> extends InvocableStreamedType<AtomicPrimitiveHandler<T>> {
...
class AtomicPrimitiveHandlerImpl<T extends PrimitiveType> implements AtomicPrimitiveHandler<T> {
    public constructor(private onComplete?: (value: T, partial: PartialArg) => void) {}
...
class StreamedAnyOf extends SchemaGeneratingStreamedType {
    ...
    jsonSchema(root: StreamedTypeIdentity, definitions: DefinitionMap): JsonObject {
        return {
            anyOf: this.options.map((streamedType) => jsonSchemaFromStreamedType(streamedType, root, definitions)),
        };
    }
    ...
    public streamedTypeOfFirstMatch(value: string | number | boolean | null): StreamedType | undefined {
        for (const option of this.options) {
            // TODO-AnyOf: Must also consider Optional and AnyOf
