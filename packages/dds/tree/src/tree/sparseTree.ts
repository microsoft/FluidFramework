...
export class SparseNode<TData> implements UpPath {
    ...
    public get parent(): SparseNode<TData> | undefined {
        assert(
            ...
            0x4a3 /* SparseNode.parent is an UpPath API and thus should never be called on the root SparseNode. */,
        );
    ...
    public removeChild(child: SparseNode<TData>): void {
        ...
        assert(childIndex !== -1, 0x4a4 /* child must be parented to be removed */);
...
export function getDescendant<TData>(
    ...
    for (const hop of topDown) {
        ...
        assert(field !== undefined, 0x4a5 /* Field not present in sparse node */);
        // TODO: should do more optimized search (ex: binary search).
        const child = field.find((c) => c.parentIndex === hop.parentIndex);
        assert(child !== undefined, 0x4a6 /* Child not present in sparse node field */);
        curr = child;
    }
    return curr;
}
