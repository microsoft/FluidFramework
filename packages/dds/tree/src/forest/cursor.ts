...
export function mapCursorField<T>(cursor: ITreeCursor, key: FieldKey, f: (cursor: ITreeCursor) => T): T[] {
    ...
    if (result !== TreeNavigationResult.Ok) {
        assert(result === TreeNavigationResult.NotFound, 0x34d /* pending not supported in mapCursorField */);
    ...
    assert(result === TreeNavigationResult.NotFound, 0x34e /* expected enumeration to end at end of field */);
    cursor.up();
    return output;
}
