...
class ExtrinsicRangeManager extends sf.object("Manager", {
	...
	public static fromString(value: string): ExtrinsicRangeManager {
		...
		return new ExtrinsicRangeManager({ content: [...charactersFromString(value)] });
	}
}
...
class MainArray extends sf.array("StringArray", SchemaFactory.string) {
...
class Empty extends sf.object("Empty", {}) {}

class Marker extends sf.object("Marker", {}) {}

/**
 * Tracks a location in a array by making a parallel array.
 * This could be compressed to just a length and index of the marker in the common case (not corrupted).
 * Even simple runs of matching shapes compression we have for persisted format should compress this to O(log(Length)) by compressing it to three runs each with a shape and length.
 */
class ArrayAnchor extends sf.array("ArrayAnchor", [Marker, Empty]) {}

export class ExtrinsicRange2 extends sf.object("ExtrinsicRange", {
	data: [() => Data],
	start: ArrayAnchor,
	end: ArrayAnchor,
}) {}
 ...
 */
const Data = SchemaFactory.string;
