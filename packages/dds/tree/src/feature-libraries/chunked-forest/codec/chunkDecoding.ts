 ...
 */
export function readValue(stream: StreamCursor, shape: EncodedValueShape, idCompressor: IIdCompressor): Value {
	if (shape === undefined) {
		...
		if (shape === true) {
			...
			if(shape === 0){
				return idCompressor.decompress(stream.data[stream.offset] as SessionSpaceCompressedId)
			}
