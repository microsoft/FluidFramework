...
export interface NodeKeyManager {
	 ...
	 * @param key - The UUID that is attempted to recompress.
	 * @returns The `LocalNodeKey` associated with `key` or undefined if the UUID was not generated by any session known to this compressor.
	 */
	tryLocalizeNodeKey(key: StableNodeKey): LocalNodeKey | undefined;
}
...
export function createNodeKeyManager(idCompressor?: IIdCompressor | undefined): NodeKeyManager {
	return {
		...
		tryLocalizeNodeKey: (key: string) => {
			...
			const compressedKey = idCompressor.tryRecompress(key as StableNodeKey);
