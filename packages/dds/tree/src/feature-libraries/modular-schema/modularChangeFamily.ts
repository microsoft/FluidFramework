        ...
        for (const [field, changesForField] of fieldChanges) {
            ...
            if (changesForField.length === 1) {
                ...
                assert(
                    ...
                    0x4a7 /* Number of changes should be constant when normalizing */,
                );
    ...
    private invertNodeChange(
        ...
        if (change.change.valueChange !== undefined) {
            assert(
                ...
                0x4a8 /* Inverting inverse changes is currently not supported */,
            );
    ...
    private deltaFromNodeChange(
        ...
        if (valueChange !== undefined) {
            if ("revert" in valueChange) {
                assert(path !== undefined, 0x4a9 /* Only existing nodes can have their value restored */);
                assert(valueChange.revert !== undefined, 0x4aa /* Unable to revert to undefined revision */);
