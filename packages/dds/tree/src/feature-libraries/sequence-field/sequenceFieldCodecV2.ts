...
export function makeV2CodecHelpers(
	...
	> = {
		...
		attachAndDetach(
			...
			const attach = decoderDispatcher.dispatch(
				encoded.attach,
				context,
			) as Attach;
			const detach = decoderDispatcher.dispatch(
				encoded.detach,
				context,
			) as Detach;
			// In documents generated by clients on release >=2.2 (i.e., running the code from the PR that added this comment),
...
> {
	const { markEffectCodec, changeAtomIdCodec } =
	...
	return {
		encode: (
			...
			for (const mark of changeset) {
				...
				if (mark.cellId !== undefined) {
					encodedMark.cellId = changeAtomIdCodec.encode(
						mark.cellId,
						context.baseContext,
					);
				}
		...
		decode: (
			...
			for (const mark of changeset) {
				...
				if (mark.cellId !== undefined) {
					decodedMark.cellId = changeAtomIdCodec.decode(
						mark.cellId,
						context.baseContext,
					);
				}
				...
				if (
					decodedMark2.cellId !== undefined &&
					decodedMark2.type === "AttachAndDetach"
				) {
