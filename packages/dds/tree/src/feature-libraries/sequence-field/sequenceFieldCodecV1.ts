							...
							idOverride:
								effect.cellRename === undefined
									...
									: {
											...
											id: cellIdCodec.encode(effect.cellRename, context),
										},
							id: effect.id,
						},
					};
	...
	>({
		...
		delete(encoded: Encoded.Remove, context: ChangeEncodingContext): Detach {
			...
			if (idOverride !== undefined) {
				mark.cellRename = cellIdCodec.decode(idOverride.id, context);
			}
			return mark;
		},
		...
		attachAndDetach(encoded: Encoded.AttachAndDetach, context: ChangeEncodingContext): Rename {
			...
			if (attach.id === renameLocalId) {
				assert(detach.cellRename !== undefined, 0x9f7 /* Rename must have idOverride */);
				return {
					type: "Rename",
					idOverride: detach.cellRename,
				};
			}
			return {
				type: "Rename",
