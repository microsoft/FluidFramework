...
export class ObjectForest extends SimpleDependee implements IEditableForest {
	...
	tryGet(
		...
		assert(
			destination instanceof ObjectAnchor,
	...
	up(): TreeNavigationResult {
		...
		this.siblings =
			this.parentStack.length === 0
				...
				: this.parentStack[this.parentStack.length - 1].fields.get(
						this.keyStack[this.keyStack.length - 1],
				  );
		return TreeNavigationResult.Ok;
	}
