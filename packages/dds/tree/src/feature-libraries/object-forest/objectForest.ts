...
class Cursor extends SynchronousCursor implements ITreeSubscriptionCursor {
    ...
    get mode(): CursorLocationType {
        assert(this.innerCursor !== undefined, 0x42b /* Cursor must be current to be used */);
        return this.innerCursor.mode;
    }

    nextField(): boolean {
        assert(this.innerCursor !== undefined, 0x42c /* Cursor must be current to be used */);
        return this.innerCursor.nextField();
    }
    exitField(): void {
        assert(this.innerCursor !== undefined, 0x42d /* Cursor must be current to be used */);
