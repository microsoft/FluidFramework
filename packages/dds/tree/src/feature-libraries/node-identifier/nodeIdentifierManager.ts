...
export interface NodeIdentifierManager {
	...
	/**
	 * Attempts to stabilize a {@link number}.
	 * @param identifier - The identifier that is attempted to stabilize.
	 * @returns The `{@link StableNodeIdentifier}` associated with `identifier` if it is a known {@link LocalNodeIdentifier},
	 * or undefined if the identifier was not generated by any session known to this compressor.
	 */
	tryStabilizeNodeIdentifier(identifier: number): StableNodeIdentifier | undefined
}
...
export function createNodeIdentifierManager(
	...
	return {
		...
		tryStabilizeNodeIdentifier(identifier: number): StableNodeIdentifier | undefined {
			assert(
				idCompressor !== undefined,
				"Runtime IdCompressor must be available to convert node identifiers"
			);
			try {
				return brand(
					// TODO: The assert below is required for type safety but is maybe slow
					assertIsStableId(idCompressor.decompress(extractFromOpaque(identifier as unknown as LocalNodeIdentifier))),
				);
			} catch {
				return undefined
				// throw new UsageError("Unknown local identifier.")
			}
		}
	};
}
