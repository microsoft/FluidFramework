...
export class EditManager<TChangeset, TChangeFamily extends ChangeFamily<any, TChangeset>> {
	...
	public setLocalSessionId(id: SessionId) {
		assert(
			this.localSessionId === undefined || this.localSessionId === id,
			"Local session ID cannot be changed",
		);
	...
	public addSequencedChange(newCommit: Commit<TChangeset>): Delta.Root {
		if (this.trunk.length > 0) {
			...
			assert(
				newCommit.seqNumber === nextSeqNumber,
		...
		const newChangeFullyRebased = this.rebaseChangeFromBranchToTrunk(
			newCommit.changeset,
			branch,
		);
	...
	private rebaseChangeFromBranchToTrunk(
		...
		return this.rebaseOverCommits(
			changeRebasedToRef,
			this.getCommitsBetween(branch.refSeq, undefined),
		);
	}
