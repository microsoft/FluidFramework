...
TODO: Due to floating point precision compose is not quite associative. This may violate our requirements. This could be fixed by making this integer only and handling values past Number.MAX\_SAFE\_INTEGER (ex: via an arbitrarily large integer library) or via modular arithmetic.
