...
This model requires edits are structured as _commands_ that operate on explicit input parameters (often handles to nodes in the tree) that can simply be serialized along with the command when sent over the network.
...
The Shared Tree can use heuristics to adjust the command inputs as necessary when the command is executed, meaning the handles passed to the command may be a best approximation of those originally supplied when the command was created.
