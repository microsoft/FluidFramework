...
-   `noShallowChange`: Requires that a given field be in the same state transaction is applied as it was before the transaction was authored. Note that this only applies to top-level changes; nested properties do not trigger this constraint
