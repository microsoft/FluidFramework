...
To avoid limitations in TypeScript `.d.ts` emissions for recursive schema, and get better IntelliSense and errors, schema must be explicitly named types (like a class or interface) and not simply a `type` declaration.
...
This also allows injection of common functionality for all schema classes (like static implementations of the various schema interfaces for reflection of schema data, and centralized control of the schema scopes and potentially other future settings).
...
An exhaustive list of things which can be done to fix issues related to use of SharedTree schema with `isolatedDeclarations`.
The recommendations above use a subset of these approaches.
