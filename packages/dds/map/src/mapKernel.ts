...
export class MapKernel {
	...
	public rollback(op: unknown, localOpMetadata: unknown): void {
		...
		if (mapOp.type === "clear") {
			...
			// Safe: optional chaining - if pendingClear is undefined, undefined !== "clear" so assert triggers.
			assert(
		...
		messageHandlers.set("delete", {
			process: (
				...
				if (local) {
					...
					const pendingEntry = this.pendingData[pendingEntryIndex];
					// Safe: optional chaining - if pendingEntry is undefined, undefined !== "delete" so assert triggers.
					assert(
		...
		messageHandlers.set("set", {
			process: (
				...
				if (local) {
					...
					const pendingEntry = this.pendingData[pendingEntryIndex];
					// Safe: optional chaining - if pendingEntry is undefined, undefined !== "lifetime" so assert triggers.
					assert(
