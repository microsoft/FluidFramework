...
function makeOperationGenerator(optionsParam?: OperationGenerationConfig): Generator<Operation, FuzzTestState> {
    ...
    const clientOperationGenerator = (state: FuzzTestState) =>
        clientBaseOperationGenerator({ ...state, sharedDirectory: state.random.pick(state.clients).sharedDirectory });
    ...
    performFuzzActions(
        generator,
        {
            createSubDirectory: statefully(({ clients }, { directoryId, path, name }) => {
                const { sharedDirectory } = clients.find((c) => c.sharedDirectory.id === directoryId) ?? {};
...
function assertEventualConsistencyCore(first: SharedDirectory, second: SharedDirectory) {
    ...
    assert.strictEqual(first.size, second.size, `Number of keys not same: Number of keys in ` +
    ...
    for (const key of first.keys()) {
        assert.strictEqual(first.get(key), second.get(key), `Key not found or value not matching ` +
            `key: ${key}, value in dir ${first.id}: ${first.get(key)} and in ${second.id}: ${second.get(key)}`);
    }

    // Check for number of subdirectores with both directories.
    assert.strictEqual(first.countSubDirectory(), second.countSubDirectory(),
        `Number of subDirectories not same: Number of subdirectory in ` +
        `${first.id}: ${first.countSubDirectory()} and in ${second.id}: ${second.countSubDirectory()}`);
    ...
    for (const [name, subDirectory1] of first.subdirectories()) {
        ...
        assertEventualConsistencyCore(subDirectory1 as SharedDirectory, subDirectory2 as SharedDirectory);
    }
}
