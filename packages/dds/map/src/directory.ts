...
export class SharedDirectory extends SharedObject<ISharedDirectoryEvents> implements ISharedDirectory {
     ...
     */
    private readonly root: SubDirectory =
        new SubDirectory(0, new Set(), this, this.runtime, this.serializer, posix.sep);
    ...
    private needProcessSubDirectoryOperation(
        ...
        if (pendingSubDirectoryMessageId !== undefined) {
            ...
            if (op.type === "createSubDirectory") {
                ...
                if (dir !== undefined && !dir.clientIds.has(msg.clientId)
                    && dir.sequenceNumber <= msg.sequenceNumber) {
    ...
    private createSubDirectoryCore(subdirName: string, local: boolean, seq: number, clientId: string): boolean {
        ...
        if (subdir === undefined) {
            ...
            const subDir = new SubDirectory(
                seq, new Set([clientId]), this.directory, this.runtime, this.serializer, absolutePath);
