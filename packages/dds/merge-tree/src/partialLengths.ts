...
export class PartialSequenceLengths {
    ...
    private verifyPartialLengths(partialLengths: PartialSequenceLength[], clientPartials: boolean) {
        ...
        for (const partialLength of partialLengths) {
            ...
            assert(this.minSeq <= partialLength.seq, "sc:0041" /* Sequence number less than minSeq! */);

            // Sequence number should be sorted
            assert(lastSeqNum < partialLength.seq, "sc:0042" /* Sequence number is not sorted! */);
            ...
            if (accumSegLen !== partialLength.len) {
                assert(false, "sc:0043" /* Unexpected total for accumulation of all seglen adjustments! */);
            }
            ...
            if (clientPartials) {
                ...
                if (this.minLength + partialLength.len < 0) {
                    assert(false, "sc:0044" /* Negative length after length adjustment! */);
                }
            }

            if (partialLength.overlapRemoveClients) {
                // Only the flat partialLengths can have overlapRemoveClients, the per client view shouldn't
                assert(!clientPartials, "sc:0045" /* Both overlapRemoveClients and clientPartials are set! */);
    ...
    private verify() {
        if (this.clientSeqNumbers) {
            ...
            assert(!!this.partialLengths, "sc:0046" /* Client view exists but flat view does not! */);
