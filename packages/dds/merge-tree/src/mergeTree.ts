...
export class MergeBlock extends MergeNode implements IMergeBlock {
    ...
    setOrdinal(child: IMergeNode, index: number) {
        ...
        assert((childCount >= 1) && (childCount <= 7), 0x2d /* Child count is not within [1,7] range! */);
        ...
        assert(child.ordinal.length === (this.ordinal.length + 1), 0x2e /* Unexpected child ordinal length! */);
        if (index > 0) {
            assert(child.ordinal > this.children[index - 1].ordinal, 0x2f /* Child ordinal <= previous sibling ordinal! */);
...
export abstract class BaseSegment extends MergeNode implements ISegment {
    ...
    public ack(segmentGroup: SegmentGroup, opArgs: IMergeTreeDeltaOpArgs, mergeTree: MergeTree): boolean {
        ...
        assert(currentSegmentGroup === segmentGroup, 0x30 /* On ack, unexpected segmentGroup! */);
        switch (opArgs.op.type) {
            case ops.MergeTreeDeltaType.ANNOTATE:
                assert(!!this.propertyManager, 0x31 /* On annotate ack, missing segment property manager! */);
                this.propertyManager.ackPendingProperties(opArgs.op);
                return true;

            case ops.MergeTreeDeltaType.INSERT:
                assert(this.seq === UnassignedSequenceNumber, 0x32 /* On insert, seq number already assigned! */);
            ...
            case ops.MergeTreeDeltaType.REMOVE:
                ...
                assert(!!removalInfo, 0x33 /* On remove ack, missing removal info! */);
                assert(!!removalInfo.removedSeq, 0x34 /* On remove ack, missing removed sequence number! */);
...
const minListenerComparer: Collections.Comparer<MinListener> = {
    min: { minRequired: Number.MIN_VALUE, onMinGE: () => { assert(false, 0x35 /* onMinGE() */); } },
...
export class MergeTree {
    ...
    reloadFromSegments(segments: ISegment[]) {
        ...
        assert(!this.collabWindow.collaborating, 0x36 /* Trying to reload from segments while collaborating! */);
    ...
    findHistorialPositionFromClient(pos: number, fromSeq: number, toSeq: number, clientId: number) {
        assert(fromSeq < toSeq, 0x37 /* Invalid range for historical position search! */);
        if (pos < this.getLength(fromSeq, clientId)) {
            assert(toSeq <= this.collabWindow.currentSeq,
                0x38 /* Out-of-bounds end sequence number for historical position search! */);
            const segoff = this.getContainingSegment(pos, fromSeq, clientId);
            assert(segoff.segment !== undefined,
                0x39 /* Containing segment for historical position search is undefined! */);
            const toPos = this.getPosition(segoff.segment, toSeq, clientId);
            const ret = toPos + segoff.offset!;
            assert(ret !== undefined,
                0x3a /* Return value for historical position search is undefined! */);
            return ret;
        } else {
            return pos;
        }
    ...
    setMinSeq(minSeq: number) {
        assert(minSeq <= this.collabWindow.currentSeq, 0x3b /* Trying to set minSeq above currentSeq of collab window! */);

        // Only move forward
        assert(this.collabWindow.minSeq <= minSeq, 0x3c /* minSeq of collab window > target minSeq! */);
    ...
    public insertAtReferencePosition(referencePosition: ReferencePosition, insertSegment: ISegment, opArgs: IMergeTreeDeltaOpArgs): void {
        ...
        if (refOffset !== 0 && refSegLen !== 0) {
            ...
            assert(!!splitSeg.next, 0x3d /* Next segment changes are undefined! */);
    ...
    private insertChildNode(block: IMergeBlock, child: IMergeNode, childIndex: number) {
        assert(block.childCount < MaxNodesInBlock, 0x3e /* Too many children on merge block! */);
    ...
    markRangeRemoved(start: number, end: number, refSeq: number, clientId: number, seq: number, overwrite = false, opArgs: IMergeTreeDeltaOpArgs) {
        ...
        if (savedLocalRefs.length > 0) {
            ...
            if (start < length) {
                ...
                assert(!!refSegment, 0x3f /* Missing reference segment! */);
                ...
                assert(!!refSegment, 0x40 /* Missing reference segment! */);
                if (!refSegment.localRefs) {
                    refSegment.localRefs = new LocalReferenceCollection(refSegment);
                }
