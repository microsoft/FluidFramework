...
for (const incremental of [true, false]) {
	for (const sided of [true, false]) {
		describe(`obliterate partial lengths incremental = ${incremental}, sided = ${sided}`, () => {
			useStrictPartialLengthChecks();

			const options = { mergeTreeEnableSidedObliterate: sided };
			beforeEach(() => {
			...
			it("length of children does not differ from parent when overlapping remove+obliterate", () => {
				const helper = new ReconnectTestHelper(options);
