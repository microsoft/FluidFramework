...
export class PermutationSegment extends BaseSegment {
    ...
    public set start(value: Handle) {
        assert(this._start === Handle.unallocated, 0x11 /* Start of PermutationSegment already allocated! */);
        assert(isHandleValid(value), 0x12 /* Trying to set start of PermutationSegment to invalid handle! */);
    ...
    protected createSplitSegmentAt(pos: number) {
        assert(0 < pos && pos < this.cachedLength, 0x13 /* Trying to split segment at out-of-bounds position! */);
...
export class PermutationVector extends Client {
    ...
    public getMaybeHandle(pos: number): Handle {
        assert(0 <= pos && pos < this.getLength(), 0x14 /* Trying to get handle of out-of-bounds position! */);
    ...
    public handleToPosition(handle: Handle, localSeq = this.mergeTree.collabWindow.localSeq) {
        assert(localSeq <= this.mergeTree.collabWindow.localSeq,
            0x15 /* 'localSeq' for op being resubmitted must be <= the 'localSeq' of the last submitted op. */);
        ...
        assert(isHandleValid(containingSegment.start), 0x16 /* Invalid handle at start of containing segment! */);
