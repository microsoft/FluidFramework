...
async function createMatrixForReconnection(
	...
	const containerRuntime = runtimeFactory.createContainerRuntime(
		dataStoreRuntime,
		overrides,
	);
	const services = {
		...
		objectStorage:
			summary === undefined
				? new MockStorage()
				: MockStorage.createFromSummary(summary),
	};
...
function spyOnContainerRuntimeMessages(
	runtime: MockContainerRuntimeForReconnection,
): {
...
describe("Matrix applyStashedOp", () => {
	it("Can rehydrate a session with stashed ops", async () => {
		...
		} = await createMatrixForReconnection(
			"A",
		...
		const { matrix: matrix2, containerRuntime: containerRuntime2 } =
			await createMatrixForReconnection("B", containerRuntimeFactory, summary, {
				minimumSequenceNumber:
					dataStoreRuntime1.deltaManagerInternal.minimumSequenceNumber,
			});
		...
		const { submittedContent } =
			spyOnContainerRuntimeMessages(containerRuntime1);
		const { processedMessages } =
			spyOnContainerRuntimeMessages(containerRuntime2);
		...
		const { matrix: matrix3, containerRuntime } =
			await createMatrixForReconnection("C", containerRuntimeFactory, summary, {
				minimumSequenceNumber,
			});
