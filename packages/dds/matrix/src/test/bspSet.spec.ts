...
describe("BSP-set tests", () => {
	...
	const intersectRect2D = (
		...
		[Math.max(leftRowMin, rightRowMin), Math.min(leftRowMax, rightRowMax)],
		[Math.max(leftColMin, rightColMin), Math.min(leftColMax, rightColMax)],
	];
