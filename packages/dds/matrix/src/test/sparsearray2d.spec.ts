...
describe("SparseArray2D", () => {
    ...
    describe("clear row/cols", () => {
        function makeCases(clearRows: boolean): IClearTestConfig[] {
            ...
            const cases = [
                makeCase(/* rowStart: */ 0, /* colStart: */ 0, /* rowCount: */ 1, /* colCount: */ 1,
                    /* clearStart: */ 0, /* clearCount: */ 1),

                // Straddle discontinuities in 256 x 256 block.
                makeCase(/* rowStart: */ 0, /* colStart: */ 0, /* rowCount: */ 256, /* colCount: */ 256,
                    /* clearStart: */ 127, /* clearCount: */ 2),
            ];

            // Individually test clearing each row of top-left 16x16 tile.
            for (let i = 0; i < 16; i++) {
                cases.push(makeCase(/* rowStart: */ 0, /* colStart: */ 0, /* rowCount: */ 16, /* colCount: */ 16,
                    /* clearStart: */ i, /* clearCount: */ 1));
            }
            ...
            // Individually test clearing each row of bottom-right 16x16 tile.
            {
                ...
                for (let i = start; i < start + 16; i++) {
                    cases.push(makeCase(
                        /* rowStart: */ start, /* colStart: */ start, /* rowCount: */ count, /* colCount: */ count,
                        /* clearStart: */ i, /* clearCount: */ 1));
                }
            }
        ...
        function testClear(
            { rowStart, colStart, rowCount, colCount,
                rowClearStart, rowClearCount, colClearStart, colClearCount }: IClearTestConfig,
        ) {
