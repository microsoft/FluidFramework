...
import { MapFactory, NewSharedMap } from "../map";
...
function createConnectedMap(id: string, runtimeFactory: MockContainerRuntimeFactory) {
    ...
    const map = new NewSharedMap(id, dataStoreRuntime, MapFactory.Attributes);
    map.connect(services);
    return map;
}

function createLocalMap(id: string) {
    const map = new NewSharedMap(id, new MockFluidDataStoreRuntime(), MapFactory.Attributes);
    return map;
}

describe("Map", () => {
    describe("Local state", () => {
        let map: NewSharedMap;
        ...
        describe("Op processing", () => {
             ...
             *
             * - A NewSharedMap in local state set a key.
             * - A second NewSharedMap is then created from the snapshot of the first one.
             * - The second NewSharedMap sets a new value to the same key.
             * - The expected behavior is that the first NewSharedMap updates the key with the new value. But in the bug
             *   the first NewSharedMap stores the key in its pending state even though it does not send out an op. So,
            ...
            it("should correctly process a set operation sent in local state", async () => {
                ...
                const map1 = new NewSharedMap("testMap1", dataStoreRuntime1, MapFactory.Attributes);
                ...
                // Load a new NewSharedMap in connected state from the snapshot of the first one.
                ...
                const map2 = new NewSharedMap("testMap2", dataStoreRuntime2, MapFactory.Attributes);
                await map2.load(services2);

                // Now connect the first NewSharedMap
                ...
                // Set a new value for the same key in the second NewSharedMap.
    ...
    describe("Connected state", () => {
        ...
        let map1: NewSharedMap;
        let map2: NewSharedMap;
        ...
        describe("API", () => {
            describe(".get()", () => {
                it("Should be able to retrieve a key", () => {
                    ...
                    // Verify the local NewSharedMap
                    assert.equal(map1.get("test"), value, "could not retrieve key");

                    // Verify the remote NewSharedMap
    ...
    describe("Garbage Collection", () => {
        class GCSharedMapProvider implements IGCTestProvider {
            ...
            private readonly map1: NewSharedMap;
            private readonly map2: NewSharedMap;
            ...
            public get sharedObject() {
                // Return the remote NewSharedMap because we want to verify its summary data.
                return this.map2;
            }
