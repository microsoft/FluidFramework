...
export abstract class SharedSegmentSequence<T extends MergeTree.ISegment>
    ...
    protected processCore(message: ISequencedDocumentMessage, local: boolean, localOpMetadata: unknown) {
        ...
        if (this.deferIncomingOps) {
            assert(!local, 0x5e /* Unexpected local op when loading not finished */);
            this.loadedDeferredIncomingOps.push(message);
        } else {
            assert(message.type === MessageType.Operation, 0x5f /* Sequence message not operation */);
    ...
    private snapshotMergeTree(serializer: IFluidSerializer): ITree {
        ...
        assert(this.loadedDeferred.isCompleted, 0x60 /* Snapshot called when not fully loaded */);
