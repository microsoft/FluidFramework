...
import { reservedRangeLabelsKey, ReferenceType } from "@fluidframework/merge-tree";
...
function makeBookmarks(client: MergeTree.TestClient, bookmarkCount: number) {
	...
	for (let i = 0; i < bookmarkCount; i++) {
		...
		if (segoff1?.segment && segoff2?.segment) {
			...
			const lref1 = client.createLocalReferencePosition(
				baseSegment1,
				segoff1.offset,
				ReferenceType.RangeBegin,
				undefined,
			);
			const lref2 = client.createLocalReferencePosition(
				baseSegment2,
				segoff2.offset,
				ReferenceType.RangeEnd,
				undefined,
			);
			lref1.addProperties({ [reservedRangeLabelsKey]: ["bookmark"] });
			lref2.addProperties({ [reservedRangeLabelsKey]: ["bookmark"] });
...
function makeReferences(client: MergeTree.TestClient, referenceCount: number) {
	...
	for (let i = 0; i < referenceCount; i++) {
		...
		if (segoff?.segment) {
			...
			if (i % 2 === 1) {
				lref.refType = ReferenceType.SlideOnRemove;
			}
			refs.push(lref);
		} else {
			i--;
...
export function TestPack(verbose = true) {
	...
	function reportTiming(client: MergeTree.TestClient) {
		if (!verbose) {
			return;
		}
		...
		console.log(
			`${client.longClientId} accum time ${client.accumTime} us ops: ${client.accumOps} ave time ${aveTime} - pack ${avePackTime} ave window ${aveWindow}`,
		);
		console.log(
			`${client.longClientId} accum window time ${client.accumWindowTime} us ave window time not in ops ${aveExtraWindowTime}; max ${client.maxWindowTime}`,
		);
	}
	...
	function clientServer(startFile?: string, initRounds = 1000) {
		...
		let snapClient: MergeTree.TestClient;
		const useGroupOperationsForMoveWord = false;
		let annotateProps: MergeTree.PropertySet | undefined;
		const insertAsRefPos = false;

		const testServer = new MergeTree.TestServer({});
		testServer.measureOps = true;
		if (startFile) {
			MergeTree.loadTextFromFile(startFile, testServer.mergeTree, fileSegCount);
		} else {
			testServer.insertTextLocal(0, initString);
		}

		const clients = new Array<MergeTree.TestClient>(clientCount);
		for (let i = 0; i < clientCount; i++) {
			clients[i] = new MergeTree.TestClient();
			clients[i].measureOps = true;
			if (startFile) {
				MergeTree.loadTextFromFile(startFile, clients[i].mergeTree, fileSegCount);
			} else {
		...
		if (testSyncload) {
			...
			const fromLoadText = new MergeTree.MergeTreeTextHelper(fromLoad).getText(
				MergeTree.UniversalSequenceNumber,
				MergeTree.NonCollabClient,
			);
		...
		if (addSnapClient) {
			snapClient = new MergeTree.TestClient();
			if (startFile) {
				MergeTree.loadTextFromFile(startFile, snapClient.mergeTree, fileSegCount);
			} else {
		...
		function clientProcessSome(client: MergeTree.TestClient, all = false) {
		...
		function serverProcessSome(server: MergeTree.TestClient, all = false) {
		...
		function randomSpateOfInserts(client: MergeTree.TestClient, charIndex: number) {
			...
			if (includeMarkers) {
				const markerOp = client.insertMarkerLocal(pos, ReferenceType.Tile, {
					[MergeTree.reservedTileLabelsKey]: "test",
				});
				testServer.enqueueMsg(
					client.makeOpMessage(markerOp, MergeTree.UnassignedSequenceNumber),
				);
			}
			const textOp = client.insertTextLocal(pos, text);
			testServer.enqueueMsg(client.makeOpMessage(textOp, MergeTree.UnassignedSequenceNumber));
		...
		function randomWordMove(client: MergeTree.TestClient) {
			...
			if (word1) {
				...
				const insertOp =
					!insertAsRefPos && segOff.segment
						? client.insertAtReferencePositionLocal(
								...
								MergeTree.TextSegment.make(word1.text),
						  )
				...
				if (useGroupOperationsForMoveWord) {
					testServer.enqueueMsg(client.makeOpMessage(MergeTree.createGroupOp(...ops)));
					if (MergeTree.TestClient.useCheckQ) {
						client.enqueueTestString();
					}
				}
			...
			if (measureBookmarks) {
				...
				if (measureRanges) {
					...
					for (let i = 0; i < posChecksPerRound; i++) {
						...
						if (segoff1?.segment && segoff2?.segment) {
							const lrefPos1 = testServer.createLocalReferencePosition(
								<MergeTree.BaseSegment>segoff1.segment,
								segoff1.offset,
								ReferenceType.Simple,
								undefined,
							);
							const lrefPos2 = testServer.createLocalReferencePosition(
			...
			if (0 === roundCount % 100) {
				...
				console.log(
					`round: ${roundCount} seq ${
						testServer.seq
					} char count ${testServer.getLength()} height ${stats.maxHeight} lv ${
						stats.leafCount
				...
				if (measureBookmarks) {
					...
					if (measureRanges) {
						...
						const timePerRangeCheck = (rangeOverlapTime / rangeOverlapChecks).toFixed(
							2,
						);
						const resultsRange = (overlapIntervalResults / rangeOverlapChecks).toFixed(
							2,
						);
						console.log(
	...
	function firstTest() {
		let cli = new MergeTree.TestClient();
		...
		const lref1 = cli.createLocalReferencePosition(
...
function checkInsertMergeTree(
	...
	const updatedText = helper.getText(MergeTree.UniversalSequenceNumber, MergeTree.LocalClientId);
...
function checkMarkRemoveMergeTree(
	...
	const helper = new MergeTree.MergeTreeTextHelper(mergeTree);
	const origText = helper.getText(MergeTree.UniversalSequenceNumber, MergeTree.LocalClientId);
