...
export abstract class SharedObjectCore<TEvent extends ISharedObjectEvents = ISharedObjectEvents>
     ...
     */
    public abstract captureRevision(): IChannelRevision;
...
/**
 *  Base class for shared objects with additional functionality for synchronous summarization, serializer, and GC
 */
export abstract class SharedObject<TEvent extends ISharedObjectEvents = ISharedObjectEvents>
    extends SharedObjectCore<TEvent> {
    /**
     * True while we are capturing state to prevent use of regular serializer.
     */
    ...
    protected get serializer(): IFluidSerializer {
        /**
         * During revision capture, only SummarySerializer should be used (instead of this.serializer) to keep track of
         * IFluidHandles that are serialized for GC. These handles represent references to other Fluid objects.
         */
        assert(!this._isCapturing,
    ...
    public captureRevision(): IChannelRevision {
        // Set _isCapturing to true. This flag is used to ensure that we only use SummarySerializer to serialize
        // handles in this object's data.
        // Assertions for re-entrancy are only to ensure _isCapturing value is correct.
        assert(!this._isCapturing, 0x078 /* "Possible re-entrancy! Summary should not already be in progress." */);
        this._isCapturing = true;

        let serializer: SummarySerializer;
        let snapshot: ITree;
        try {
            // We run the full summarize logic to get the revision capture. This may be inefficient if only GC data is
            // used but it's okay for now. DDSs that wish to optimize this can override captureRevision but will still
            // have to capture enough state to produce both summaries and GC data.
            serializer = new SummarySerializer(this.runtime.channelsRoutingContext);
            snapshot = this.snapshotCore(serializer);
            assert(this._isCapturing, 0x079 /* "Possible re-entrancy! Summary should have been in progress." */);
        } finally {
            this._isCapturing = false;
        }

        return {
            summarize: async (fullTree: boolean = false) => {
                return convertToSummaryTreeWithStats(snapshot, fullTree);
            },
            getGCData: async (fullGC: boolean = false): Promise<IGarbageCollectionData> => {
                // The GC data for this shared object contains a single GC node. The outbound routes of this node are
