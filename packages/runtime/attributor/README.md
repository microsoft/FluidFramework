...
To turn on op-stream based attribution in your container, follow this.
To start storing attribution data on new documents and load attribution data available on existing containers, enable
the config flag `"Fluid.Attribution.EnableOnNewFile"`.
...
The current design is therefore motivated by the ability to roll out the feature in Fluid's collaborative environment.
The behavior of `"Fluid.Attribution.EnableOnNewFile"` supports the standard strategy of rolling out code that reads a new format and waiting for it to saturate before beginning to write that new format.
