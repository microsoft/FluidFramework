...
export class IdCompressor implements IIdCompressor, IIdCompressorCore {
	...
	// This is updated to be equal to `localGenCount` + 1 each time it is called.
	...
	public shard(newShardCount: number): SerializedIdCompressorWithOngoingSession[] {
		...
		// Keep parent's localGenCount unchanged - it reflects all IDs generated so far
		// When generating the next ID, we'll calculate the next genCount in the stride sequence
	 ...
	 * This method:
	 * 1. Calculates the next genCount in this shard's stride sequence after current localGenCount
	 * 2. Backfills the normalizer with ALL IDs in the stride cycle containing that genCount
	 * 3. Updates localGenCount to the new genCount
	...
	private generateNextLocalIdWithStride(): LocalCompressedId {
		...
		// We want the smallest value in this sequence that is > localGenCount
		const base = shardOffset + 1;
		let nextGenCount: number;

		if (this.localGenCount < base) {
			// Haven't generated any IDs in this stride yet
			nextGenCount = base;
		} else {
