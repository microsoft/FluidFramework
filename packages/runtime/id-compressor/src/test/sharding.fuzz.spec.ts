...
function handleShard(state: ShardingFuzzTestState, op: ShardOperation): ShardingFuzzTestState {
	...
	// Deserialize and track each shard
	for (const serialized of serializedShards) {
		...
		if (!state.childrenMap.has(compressor)) {
			state.childrenMap.set(compressor, new Set());
		}
		const children = state.childrenMap.get(compressor);
		if (children) {
			children.add(shard);
		}
		...
		// Update active and leaf sets
		state.activeCompressors.add(shard);
		state.leafCompressors.add(shard);

		// Initialize tracking for this shard
		state.compressorGeneratedIds.set(shard, []);
	}

	// Parent is no longer a leaf if it was
	if (state.leafCompressors.has(compressor)) {
...
function handleUnshard(
	...
	// Can't unshard root
	if (leaf === state.rootCompressor) {
		return state;
	}

	// Can't unshard non-leaf
	if (!state.leafCompressors.has(leaf)) {
		return state;
	}

	const parent = state.parentMap.get(leaf);
	if (!parent) {
		return state; // Shouldn't happen, but be defensive
	}

	// Dispose the leaf and get its shard ID
	const shardId = leaf.disposeShard();
	if (!shardId) {
		return state; // Shouldn't happen for a shard
	}
...
function handleGenerateId(
	...
	// Check for local ID collision
	if (state.globalLocalIds.has(localId)) {
		throw new Error(
			`Local ID collision detected! ID ${localId} was generated by multiple compressors.`,
		);
	}
	state.globalLocalIds.add(localId);

	// Decompress to stable ID and check for collision
	...
	// Track this ID for this compressor
	const idsForCompressor = state.compressorGeneratedIds.get(compressor);
	if (idsForCompressor) {
		idsForCompressor.push(localId);
	} else {
		state.compressorGeneratedIds.set(compressor, [localId]);
	}
...
function handleValidate(state: ShardingFuzzTestState): ShardingFuzzTestState {
	// For each compressor, verify it can decompress all IDs up to its last generated
	for (const compressor of state.activeCompressors) {
		const idsGenerated = state.compressorGeneratedIds.get(compressor);
		if (!idsGenerated || idsGenerated.length === 0) {
			continue; // This compressor hasn't generated any IDs yet
		}

		// Get the last ID this compressor generated
		const lastId = idsGenerated.at(-1);
		if (lastId === undefined) {
			continue;
		}

		// Try to decompress all IDs in the range [-1, lastId] that exist in the global set
		// Local IDs are negative, so we iterate from -1 down to lastId
		const startId = -1;
		const endId = lastId; // lastId is negative, e.g., -20

		for (let id = startId; id >= endId; id--) {
			const localId = id as SessionSpaceCompressedId;

			// Only check IDs that were actually generated
			if (!state.globalLocalIds.has(localId)) {
				continue;
			}

			// Try to decompress this ID
			try {
				const stableId = compressor.decompress(localId);
				// Verify the stable ID is in our global set
				if (!state.globalStableIds.has(stableId)) {
					throw new Error(
