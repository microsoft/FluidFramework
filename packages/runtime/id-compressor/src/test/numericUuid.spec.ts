...
import type { StableId } from "../index.js";
...
describe("NumericUuid", () => {
	it("can roundtrip a uuid string", () => {
		...
		assert.equal(
			stableIdFromNumericUuid(numericUuidFromStableId(uuidString)),
			uuidString,
		);
	});
	...
	it("can do general uuid math", () => {
		const uuids = [
			...
			numericUuidFromStableId(
				"00000000-b7c5-4c99-83ff-c1b8e02c09d6" as StableId,
			),
			numericUuidFromStableId(
				"748540ca-b7c5-4c99-83ff-c1b8e02c09d6" as StableId,
			),
			numericUuidFromStableId(
				"748540ca-b7c5-4c99-83ef-c1b8e02c09d6" as StableId,
			),
			numericUuidFromStableId(
				"748540ca-b7c5-4c99-831f-c1b8e02c09d6" as StableId,
			),
			numericUuidFromStableId(
				"0002c79e-b536-4776-b000-000266c252d5" as StableId,
			),
			numericUuidFromStableId(
				"082533b9-6d05-4068-a008-fe2cc43543f7" as StableId,
			),
			numericUuidFromStableId(
				"2c9fa1f8-48d5-4554-a466-000000000000" as StableId,
			),
			numericUuidFromStableId(
				"2c9fa1f8-48d5-4000-a000-000000000000" as StableId,
			),
			numericUuidFromStableId(
				"10000000-0000-4000-b000-000000000000" as StableId,
			),
			numericUuidFromStableId(
				"10000000-0000-4000-b020-000000000000" as StableId,
			),
			numericUuidFromStableId(
				"10000000-0000-4000-b00f-ffffffffffff" as StableId,
			),
			numericUuidFromStableId(
				"10000000-0000-4000-b040-000000000000" as StableId,
			),
			numericUuidFromStableId(
				"f0000000-0000-4000-8000-000000000000" as StableId,
			),
			numericUuidFromStableId(
				"efffffff-ffff-4fff-bfff-ffffffffffff" as StableId,
			),
		];
		uuids.sort();
		...
		for (let i = 0; i < uuids.length; i++) {
			...
			for (const offset of offsets) {
				assertIsSessionId(
					stableIdFromNumericUuid(offsetNumericUuid(uuidA, offset)),
				);
			}
			for (let j = i; j < uuids.length; j++) {
				const uuidB = uuids[j];
				const uuidDelta = addNumericUuids(
					uuidB,
					subtractNumericUuids(uuidA, uuidB),
				);
