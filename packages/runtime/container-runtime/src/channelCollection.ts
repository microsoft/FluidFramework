...
export class ChannelCollection implements IFluidDataStoreChannel, IDisposable {
					...
					[
						{
							...
							localOpMetadata: localMessageMetadata,
						},
					],
					local,
				);
	...
	private processChannelMessages(
		...
		let currentMessageContents: IRuntimeMessageContents[] = [];

		// Helper that sends the current bunch of messages to the data store. It validates that the data stores exists.
		const sendBunchedMessages = () => {
			assert(currentMessageState !== undefined, "message state must exist to send messages");
			...
			currentContext.processMessages(
				{ ...message, type: currentMessageState.type },
		...
		for (const { contents, ...restOfMessageContents } of messageContents) {
			...
			if (
				...
				(currentMessageState.address !== address || currentMessageState.type !== contextType)
			) {
				sendBunchedMessages();
			}
			currentMessageContents.push({
