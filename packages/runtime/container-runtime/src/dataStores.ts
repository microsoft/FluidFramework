...
export class DataStores implements IDisposable {
    ...
    public processAttachMessage(message: ISequencedDocumentMessage, local: boolean) {
        ...
        if (local) {
            assert(this.pendingAttach.has(attachMessage.id),
                "sc:0124" /* Local object does not have matching attach message id */);
    ...
    public bindFluidDataStore(fluidDataStoreRuntime: IFluidDataStoreChannel): void {
        ...
        assert(!!localContext, "sc:0125" /* Could not find unbound context to bind */);
    ...
    public resubmitDataStoreOp(content: any, localOpMetadata: unknown) {
        ...
        assert(!!context, "sc:0126" /* There should be a store context for the op */);
    ...
    public processFluidDataStoreOp(message: ISequencedDocumentMessage, local: boolean, localMessageMetadata: unknown) {
        ...
        const context = this.contexts.get(envelope.address);
        assert(!!context, "sc:0127" /* There should be a store context for the op */);
    ...
    public processSignal(address: string, message: IInboundSignalMessage, local: boolean) {
        ...
        if (!context) {
            ...
            assert(!local, "sc:0128" /* Missing datastore for local signal */);
    ...
    public async snapshot(): Promise<ITreeEntry[]> {
        ...
        const fluidDataStoreSnapshotsP = Array.from(this.contexts).map(async ([fluidDataStoreId, value]) => {
            ...
            assert(
                ...
                "sc:0129" /* summarize should always return a tree when fullTree is true */);
        ...
        await Promise.all(Array.from(this.contexts)
            .filter(([_, context]) => {
                ...
                assert(context.attachState !== AttachState.Attaching,
                    "sc:012a" /* Summarizer cannot work if client has local changes */);
                ...
                .map(([key, value]) => {
                    ...
                    if (value.isLoaded) {
                        ...
                        assert(!!this.baseSnapshot,
                            "sc:012b" /* BaseSnapshot should be there as detached container loaded from snapshot */);
    ...
    public updateUsedRoutes(usedRoutes: string[]) {
        ...
        for (const [id] of usedDataStoreRoutes) {
            assert(this.contexts.has(id), "sc:012c" /* Used route does not belong to any known data store */);
        }
