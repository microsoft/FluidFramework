...
export interface IDocumentSchemaInfo {
	 ...
	 *
	 * @remarks
	 * We use `SemanticVersion` instead of `MinDocumentRuntimeVersion` since we may open future documents with a
	 * minVersionForCollab version that `MinDocumentRuntimeVersion` does not support.
	 * Note that in such a case (where minVersionForCollab is not a valid `MinDocumentRuntimeVersion`),
