...
describe("Outbox", () => {
	...
	const getOutbox = (params: {
		...
		) => LocalBatchMessage | undefined; // Add this optional parameter for tests that need specific behavior
	}) => {
		...
		return new Outbox({
			...
			generateIdAllocationOp: params.generateIdAllocationOp ?? (() => undefined), // Add the mock function here
		});
	};
	...
	it("Sending batches", () => {
		...
		outbox.flush(); // It should be included in this flush

		// Flush 2
		outbox.submit(messages[4]);
		// No ID alloc op for this flush
		outbox.flush();
	...
	it("Batch ID added when applicable (ungrouped batch)", () => {
		...
		idAllocOp = idAllocMessage; // Set the op to be generated by the mock
	...
	it("Uses legacy path for legacy contexts", () => {
		...
		//* Expecting this to be wrong 👇
		// Note the expected CSN here is fixed to the batch's starting CSN
		// Flush 1: ID Alloc [2] gets CSN 1. Main ops [0, 1, 3] get CSN 2.
		// Flush 2: Main op [4] gets CSN 5 (1 + 3 + 1)
		const expectedMessageOrderWithCsn = [
