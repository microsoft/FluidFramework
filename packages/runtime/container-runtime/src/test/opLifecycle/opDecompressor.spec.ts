...
describe("OpDecompressor", () => {
	...
	it("Processes single compressed op", () => {
		...
		assert.strictEqual((result.message.metadata as { compressed?: unknown } | undefined)?.compressed, undefined);
	...
	it("Processes multiple compressed ops", () => {
		...
		assert.strictEqual((firstMessageResult.message.contents as ITestMessageContents).contents, "value0");
		assert.strictEqual((firstMessageResult.message.metadata as { compressed?: unknown } | undefined)?.compressed, undefined);
		...
		for (let i = 1; i < 4; i++) {
			...
			assert.strictEqual((result.message.contents as ITestMessageContents).contents, `value${i}`);
			assert.strictEqual((result.message.metadata as { compressed?: unknown } | undefined)?.compressed, undefined);
			assert.strictEqual(result.message.compression, undefined);
		}
		...
		assert.strictEqual((lastMessageResult.message.contents as ITestMessageContents).contents, "value4");
		assert.strictEqual((lastMessageResult.message.metadata as { compressed?: unknown } | undefined)?.compressed, undefined);
	...
	it("Processes multiple batches of compressed ops", () => {
		...
		assert.strictEqual((nextFirstMessageResult.message.contents as ITestMessageContents).contents, "value0");

		const middleMessageResult = decompressor.processMessage(emptyMessage);
		assert.equal(middleMessageResult.state, "Accepted");
		assert.strictEqual((middleMessageResult.message.contents as ITestMessageContents).contents, "value1");
		...
		assert.strictEqual((endBatchEmptyMessageResult.message.contents as ITestMessageContents).contents, "value2");
	});
