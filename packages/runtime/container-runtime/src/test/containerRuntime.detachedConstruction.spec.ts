...
describe("Detached runtime construction", () => {
	...
	it("preserves projection callbacks with the original derived-runtime constructor arguments", async () => {
		class DerivedRuntime extends ContainerRuntime {}
		const containerRuntimeCtor = new Proxy(DerivedRuntime, {
			construct: (target, args) => {
				const instance: unknown = Reflect.construct(target, args.slice(0, 20));
		...
		const projection: ISummaryTree = {
			type: SummaryType.Tree,
			tree: { text: { type: SummaryType.Blob, content: "constructed content" } },
		};
		const createSummary = Sinon.spy(() => ({ summary: projection }));
		const summarize = Sinon.spy(async () => ({ summary: projection }));
		const { runtime } = await createRuntime({
			containerRuntimeCtor,
			summaryGenerationOptions: {
				fullTreePolicy: "untilFirstAck",
				additionalRootTree: { key: "application", createSummary, summarize },
			},
		});
		assert.equal(runtime.createSummary().tree.application, projection);
		assert.equal(createSummary.callCount, 1);
		assert.equal(summarize.callCount, 0);
		const result = await runtime.summarize({ trackState: false, runGC: false });
		assert.equal(result.summary.tree.application, projection);
		assert.equal(createSummary.callCount, 1);
