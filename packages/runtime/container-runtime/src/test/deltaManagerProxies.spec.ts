...
describe("DeltaManagerSummarizerProxy", () => {
	it("should not wrap non-summarizers", () => {
		...
		const proxy =
			DeltaManagerSummarizerProxy.wrapIfSummarizer(mockDeltaManager);
	...
	it("should not emit readonly event when underlying delta manager emits it", () => {
		...
		assert.equal(
			eventTriggered,
			false,
			"readonly event should not be triggered",
		);
		assert.equal(
			proxy.readOnlyInfo.readonly,
			true,
	...
	it("should handle active state changes correctly", () => {
		...
		assert.equal(
			proxy.active,
			false,
			"active should initially be false for summarizer",
		);
		...
		assert.equal(
			proxy.active,
			false,
			"proxy should alway report active as false",
		);
	});
