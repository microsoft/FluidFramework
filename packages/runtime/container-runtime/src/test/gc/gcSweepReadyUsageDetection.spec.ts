...
describe("Garbage Collection Tests", () => {
	...
	describe("SweepReady Usage Detection", () => {
		...
		const createHandler = (
			uniqueContainerKey: string = "key1",
			forceNoopStorage: boolean = false,
		) =>
					...
					: {
							getItem(key) {
								...
								switch (typeof rawValue) {
									...
									default:
										assert.fail(
											"PRECONDITION: only strings allowed to be accessed via storage",
										);
								}
							},
							setItem(key, value) {
		...
		describe("usageDetectedIninteractiveClient", () => {
			...
			it("setting does not contain 'interactiveClient' - do not close the container", () => {
				...
				assert.equal(
					closeErrors.length,
					0,
					"Shouldn't close if setting doesn't include 'interactiveClient'",
				);
			});
			it("NoopStorage - close the container back-to-back", () => {
				...
				assert.equal(
					closeErrors.length,
					2,
					`Should have closed back-to-back with noopStorage defined. errors:\n${closeErrors}`,
				);
				assert(
					closeErrors.every((e) => e.errorType === sweepReadyUsageErrorType),
					`Expected all SweepReadyUsageErrors. errors:\n${closeErrors}`,
				);
			...
			it("SkipClosure Period undefined - close the container back-to-back", () => {
				...
				assert.equal(
					closeErrors.length,
					2,
					`Should have closed back-to-back with no Skip Closure Period defined. errors:\n${closeErrors}`,
				);
				assert(
					closeErrors.every((e) => e.errorType === sweepReadyUsageErrorType),
					`Expected all SweepReadyUsageErrors. errors:\n${closeErrors}`,
			...
			it("SkipClosure Period set - don't close the container during that period", () => {
				...
				handler.usageDetectedInInteractiveClient({});
				assert.deepEqual(
					JSON.parse(mockLocalStorage[closuresMapLocalStorageKey] as string),
					{ key1: { lastCloseTime: 10 } },
				);
				assert.equal(closeErrors.length, 1, `Expected to close the first time`);
				assert.equal(
					closeErrors[0]?.errorType ?? "",
					sweepReadyUsageErrorType,
					"Expected sweepReadyUsageErrorType the first time",
				);
				...
				assert.equal(
					closeErrors.length,
					1,
					`Should NOT have closed back-to-back due to Skip Closure Period. errors:\n${closeErrors}`,
				);
				...
				assert.equal(
					closeErrors[1]?.errorType ?? "",
					sweepReadyUsageErrorType,
					"Expected sweepReadyUsageErrorType the second time",
				);
			});
			it("Multiple handlers/keys - closure skipping is consistent and independent", () => {
				...
				assert.equal(
					closeErrors.length,
					2,
					`Should have closed other container for the first time. errors:\n${closeErrors}`,
				);
				...
				assert.equal(
					closeErrors.length,
					4,
					`Expected both to close again after waiting 1 day`,
				);
				assert(
					closeErrors.every((e) => e.errorType === sweepReadyUsageErrorType),
					`Expected all SweepReadyUsageErrors. errors:\n${closeErrors}`,
			...
			it("Incorrect JSON schema for closure map value in localStorage - recovers and respects the Skip Closure Period", () => {
				const handler = createHandler();
				mockLocalStorage[closuresMapLocalStorageKey] = JSON.stringify({
					key1: { wrongSchema: "no lastCloseTime member" },
				});
