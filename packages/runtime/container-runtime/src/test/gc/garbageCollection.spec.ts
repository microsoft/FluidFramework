	...
	describe("errors when unreferenced objects are used after they are inactive / deleted", () => {
		...
		describe("GC version changes", () => {
			...
			function getBaseSnapshot() {
				...
				const gcBlobMap: Map<string, IGarbageCollectionState | string[]> = new Map();
			...
			it("reads all GC data from base snapshot when GC version does not change", async () => {
				const { baseSnapshot, gcBlobMap } = getBaseSnapshot();
				const garbageCollector = createGCOverride(baseSnapshot, gcBlobMap, stableGCVersion);
