        ...
        ISummarizerInternalsProvider
{
    ...
    public async refreshLatestSummaryAck(
        ...
        // Tracks whether the summarizer node downloads the snapshot corresponding to the ack.
        let ackedSnapshotDownloaded = false;

        const getSnapshotP = new LazyPromise<ISnapshotTree>(async () => {
            ackedSnapshotDownloaded = true;
        ...
        /**
         * If the acked snapshot was downloaded by the summarizer node and the ack is for a summary that was not
         * generated by this client, it's possible that it had a different GC version than the current. If so, update
         * the summary GC version which will update the last successful summary that this client has seen. We will
         * regenerate the summary and GC data in the next summary to make it up to date to the current version.
         */
         if (ackedSnapshotDownloaded && clientId !== this.clientId) {
            const snapshot = await getSnapshotP;
            const blobId = snapshot.blobs[metadataBlobName];
            if (blobId) {
                const metadata = await readAndParse<IContainerRuntimeMetadata>(this.storage, blobId);
