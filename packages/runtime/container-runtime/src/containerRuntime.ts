...
export function unpackRuntimeMessage(message: ISequencedDocumentMessage) {
    if (message.type === MessageType.Operation) {
        ...
        if (message.contents.address !== undefined && message.contents.type === undefined) {
            ...
            assert(innerContents.type !== undefined, "sc:00ec" /* Undefined inner contents type! */);
            message.type = innerContents.type;
            message.contents = innerContents.contents;
        }
        assert(isRuntimeMessage(message), "sc:00ed" /* Message to unpack is not proper runtime message */);
    } else {
        ...
        ISummarizerInternalsProvider
{
    ...
    private constructor(
        ...
        this.blobManager = new BlobManager(
            ...
            () => {
                assert(this.attachState !== AttachState.Detached, "sc:00ee" /* Blobs NYI in detached container mode */);
                return this.storage;
            },
        ...
        this.deltaManager.on("readonly", (readonly: boolean) => {
            ...
            assert(readonly === this.deltaManager.readonly, "sc:00ef" /* inconsistent readonly property/event state */);
            ...
            assert(!readonly || !this.connected, "sc:00f0" /* Unsafe to transition to read-only state! */);
        ...
        try {
            ...
            if (id === BlobManager.basePath && requestParser.isLeaf(2)) {
                ...
                assert(subRequest.url.startsWith("/"),
                    "sc:00f1" /* Expected createSubRequest url to include a leading slash */);
    ...
    private replayPendingStates() {
        ...
        assert(this.emitDirtyDocumentEvent, "sc:00f2" /* dirty document event not set on replay */);
    ...
    public setConnectionState(connected: boolean, clientId?: string) {
        ...
        if (connected) {
            assert(!!clientId, "sc:00f3" /* Missing clientId */);
        ...
        try {
            ...
            switch (message.type) {
                ...
                case ContainerMessageType.FluidDataStoreOp:
                    ...
                    break;
                case ContainerMessageType.BlobAttach:
                    assert(message?.metadata?.blobId, "sc:00f4" /* Missing blob id on metadata */);
    ...
    public isMessageDirtyable(message: ISequencedDocumentMessage) {
        assert(
            ...
            "sc:00f5" /* Message passed for dirtyable check should be a container runtime message */,
        );
    ...
    public setAttachState(attachState: AttachState.Attaching | AttachState.Attached): void {
        if (attachState === AttachState.Attaching) {
            assert(this.attachState === AttachState.Attaching,
                "sc:00f6" /* Container Context should already be in attaching state */);
        } else {
            assert(this.attachState === AttachState.Attached,
                "sc:00f7" /* Container Context should already be in attached state */);
        }
    ...
    public async summarize(options: {
        ...
        assert(summarizeResult.summary.type === SummaryType.Tree,
            "sc:00f8" /* Container Runtime's summarize should always return a tree */);
    ...
    private addChunk(clientId: string, chunkedContent: IChunkedOp) {
        ...
        assert(chunkedContent.chunkId === map.length + 1,
            "sc:00f9" /* Mismatch between new chunkId and expected chunkMap */); // 1-based indexing
    ...
    private submit(
        ...
        assert(this.attachState !== AttachState.Detached, "sc:00fa" /* sending ops in detached container */);
    ...
    private submitSystemMessage(
        ...
        contents: any) {
        this.verifyNotClosed();
        assert(this.connected, "sc:00fb" /* Container disconnected when trying to submit system message */);
        ...
        if (this.context.clientDetails.capabilities.interactive) {
            this.getScheduler().then((scheduler) => {
                ...
                scheduler.pick(LeaderTaskId, async () => {
                    assert(!this._leader, "sc:00fc" /* Client is already leader */);
                    this.updateLeader(true);
                });

                scheduler.on("lost", (key) => {
                    if (key === LeaderTaskId) {
                        assert(this._leader, "sc:00fd" /* Got leader key but client is not leader */);
                        this._leader = false;
    ...
    private updateLeader(leadership: boolean) {
        ...
        if (this.leader) {
            assert(this.clientId === undefined || this.connected && this.deltaManager && this.deltaManager.active,
                "sc:00fe" /* Leader must either have undefined clientId or be connected with active delta manager! */);
            this.emit("leader");
        } else {
            this.emit("notleader");
        }
    ...
    private async getVersionFromStorage(versionId: string): Promise<IVersion> {
        ...
        assert(!!versions && !!versions[0], "sc:00ff" /* Failed to get version from storage */);
        return versions[0];
    }
    ...
    private async getSnapshotFromStorage(version: IVersion): Promise<ISnapshotTree> {
        ...
        assert(!!snapshot, "sc:0100" /* Failed to get snapshot from storage */);
        return snapshot;
    }
}
