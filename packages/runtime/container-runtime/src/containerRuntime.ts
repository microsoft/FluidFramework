...
implements IContainerRuntime, IContainerRuntimeDirtyable, IRuntime, ISummarizerRuntime {
    ...
    public static async load(
        ...
        if (!context.existing) {
            await runtime.createDataStore(schedulerId, schedulerId)
    ...
    public async request(request: IRequest): Promise<IResponse> {
        ...
        if (requestParser.pathParts.length === 1 && requestParser.pathParts[0] === "_summarizer") {
            ...
            const component = await this.getDataStore(requestParser.pathParts[0], wait) as IComponent;
            if (component) {
     ...
     */
    public async createDataStore(idOrPkg: string, maybePkg: string | string[]) {
    ...
    public createDataStoreContext(pkg: string[], props?: any): IFluidDataStoreContext {
    ...
    createComponent?(idOrPkg: string, maybePkg: string | string[]) {
        ...
        return this.createDataStore(idOrPkg, maybePkg);
    }
    ...
    /** deprecated: backcompat for FDL split */
    createComponentContext?(pkg: string[], props?: any) {
        this.logger.send({
            category: "warning", eventName: "deprecated",
            message: "ContainerRuntime.createComponentContext",
        });
        return this.createDataStoreContext(pkg, props);
    }
