 ...
 * message - Either a "system op" from outside the ContainerRuntime, or the unpacked runtime message, ready for processing
 * isRuntimeMessage - message came from ContainerRuntime and needs to be processed here too (as opposed to a "system op")
 */
		...
		IProvideFluidHandleContext
{
	...
	/**
	 * Processes the op.
	 * @param messageArg - Sequenced message for a distributed document.
	...
	public process({ ...messageArg }: ISequencedDocumentMessage, local: boolean) {
		...
		const hasModernRuntimeMessageEnvelope = messageArg.type === MessageType.Operation;
		...
		ensureContentsDeserialized(messageArg, hasModernRuntimeMessageEnvelope, logLegacyCase);
		if (hasModernRuntimeMessageEnvelope) {
			...
			const processResult = this.remoteMessageProcessor.process(messageArg, logLegacyCase);
			...
			if (isRuntimeMessage(messageArg)) {
				...
				this.ensureNoDataModelChanges(() =>
					this.processRuntimeMessage({
						message: messageArg as InboundSequencedContainerRuntimeMessage,
						local,
						isRuntimeMessage: true,
						savedOp,
					}),
				...
				this.ensureNoDataModelChanges(() =>
					this.observeNonRuntimeMessage({
						message: messageArg as InboundSequencedNonContainerRuntimeMessage,
						local,
						isRuntimeMessage: false,
						savedOp,
					}),
