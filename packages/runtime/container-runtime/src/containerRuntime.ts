		...
		IProvideLayerCompatDetails
{
	...
	private processInboundMessages(
		...
		try {
			...
			const updateSequenceNumbers = (
				message: ISequencedDocumentMessage & { indexInBatch?: number },
						...
						[
							{
								...
								indexInBatch: runtimeMessage.indexInBatch,
							},
						],
						local,
						savedOp,
			...
			for (const { message, localOpMetadata } of messagesWithMetadata) {
				...
				bunchedMessagesContent.push({
					...
					localOpMetadata,
					clientSequenceNumber: message.clientSequenceNumber,
					indexInBatch: currentMessage.indexInBatch,
				});
			}
	 ...
	 * @param messageContents - The contents and per-message metadata, including client sequence
	 * numbers and runtime batch indexes.
	 * @param local - true if the messages were originally generated by the client receiving it.
	 * @param savedOp - true if the message is a replayed saved op.
	 *
	 */
