		...
		IProvideLayerCompatDetails
{
	...
	protected constructor(
		...
		this.outbox = new Outbox({
			...
			getCurrentSequenceNumbers: () => ({
				referenceSequenceNumber: this.deltaManager.lastSequenceNumber, //* Only modified when deltamanager processes
				clientSequenceNumber: this._processedClientSequenceNumber, //* Only modified in process(...)
			}),
	...
	public process(message: ISequencedDocumentMessage, local: boolean): void {
		...
		//* TODO: Maybe incorporate flush into ensureNoDataModelChanges?  Make it more explicit / opinionated?
		this.ensureNoDataModelChanges(() => {
			this.process2(message, local);
		});
	}
	...
	/**
	 * Processes the op.
	 * @param messageCopy - Shallow copy of the sequenced message to process
	 * @param local - true if the message was originally generated by the client receiving it.
	 */
	private process2({ ...messageCopy }: ISequencedDocumentMessage, local: boolean): void {
