        ...
        ISummarizerInternalsProvider
{
    ...
    public async refreshLatestSummaryAck(
        ...
        const getSnapshotP = new LazyPromise<ISnapshotTree>(async () => {
            return this.fetchSnapshotFromStorage(ackHandle, summaryLogger, {
        ...
        const pendingSummaryFound = await this.summarizerNode.refreshLatestSummary(
            proposalHandle,
            async () => getSnapshotP,
            readAndParseBlob,
            summaryLogger,
        ...
        /**
         * Since we just updated the latest summary for this container, we need to update the summary GC version as per
         * the latest summary too.
         * If a pending summary was found for this ack, the corresponding summary op was generated by this client. So,
         * the summary GC version should be updated to the current version this client is running.
         * If a pending summary was not found, the latest summary is based off of the snapshot corresponding to the ack.
         * So, the summary GC version should be updated to the GC version in its metadata.
         */
        if (this.gcEnabled) {
            if (pendingSummaryFound) {
                this.summaryGCVersion = this.currentGCVersion;
            } else {
                await this.updateSummaryGCVersionFromSnapshot(await getSnapshotP);
            }
        }
    }
    ...
    private async refreshLatestSummaryAckFromServer(summaryLogger: ITelemetryLogger): Promise<number> {
        ...
        /**
         * Since the latest summary we are tracking is based off of the snapshot we just fetched, update the summary
         * GC version to the GC version in this snapshot too.
         */
        if (this.gcEnabled) {
            await this.updateSummaryGCVersionFromSnapshot(snapshot);
        }
