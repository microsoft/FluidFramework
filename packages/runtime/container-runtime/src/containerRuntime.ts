        ...
        ISummarizerInternalsProvider
{
    ...
    private constructor(
        ...
        this.datastores = new DataStores(this._logger);
        ...
        for (const [key, value] of fluidDataStores) {
            ...
            this.datastores.setNewContext(key, dataStoreContext);
        }
    ...
    protected async getDataStore(id: string, wait = true): Promise<IFluidRouter> {
        ...
        const deferredContext = this.datastores.ensureContextDeferred(id);
        ...
        id = uuid()): IFluidDataStoreContextDetached
    {
        ...
        this.datastores.setupNewContext(context);
        return context;
    }
    ...
    private processAttachMessage(message: ISequencedDocumentMessage, local: boolean, localMessageMetadata: unknown) {
        ...
        if (local) {
            ...
            this.datastores.contexts.get(attachMessage.id)?.emit("attached");
            this.pendingAttach.delete(attachMessage.id);
            return;
        }

         // If a non-local operation then go and create the object, otherwise mark it as officially attached.
        if (this.datastores.contexts.has(attachMessage.id)) {
       ...
       this.datastores.setNewContext(attachMessage.id, remotedFluidDataStoreContext);
    ...
    private processFluidDataStoreOp(message: ISequencedDocumentMessage, local: boolean, localMessageMetadata: unknown) {
        ...
        const context = this.datastores.getContext(envelope.address);
        context.process(transformed, local, localMessageMetadata);
    }

    private bindFluidDataStore(fluidDataStoreRuntime: IFluidDataStoreChannel): void {
        this.verifyNotClosed();
        assert(this.datastores.notBoundContexts.has(fluidDataStoreRuntime.id),
            "Store to be bound should be in not bounded set");
        this.datastores.notBoundContexts.delete(fluidDataStoreRuntime.id);
        const context = this.datastores.getContext(fluidDataStoreRuntime.id) as LocalFluidDataStoreContext;
        ...
        const deferred = this.datastores.getContextDeferred(fluidDataStoreRuntime.id);
    ...
    public setAttachState(attachState: AttachState.Attaching | AttachState.Attached): void {
        ...
        for (const context of this.datastores.contexts.values()) {
            // Fire only for bounded stores.
            if (!this.datastores.notBoundContexts.has(context.id)) {
    ...
    public createSummary(): ISummaryTree {
        ...
        do {
            ...
            notBoundContextsLength = this.datastores.notBoundContexts.size;
            ...
            Array.from(this.datastores.contexts)
                .filter(([key, _]) =>
                    ...
                    !(this.datastores.notBoundContexts.has(key)
                        || builderTree[key]
        ...
        } while (notBoundContextsLength !== this.datastores.notBoundContexts.size);
