...
import { FluidDataStoreContext, LocalFluidDataStoreContext } from "./dataStoreContext";
 ...
 export class DataStoreContexts implements Iterable<[string,FluidDataStoreContext]>, IDisposable {
    readonly #notBoundContexts = new Set<string>();

    /** Attached and loaded context proxies */
    readonly #contexts = new Map<string, FluidDataStoreContext>();
     ...
     */
    readonly #deferredContexts = new Map<string, Deferred<FluidDataStoreContext>>();

    readonly #disposeOnce = new Lazy<void>(()=>{
    ...
    public getUnbound(id: string): LocalFluidDataStoreContext | undefined {
        ...
        return this.#contexts.get(id) as LocalFluidDataStoreContext;
    }
    ...
    public addUnbound(context: LocalFluidDataStoreContext) {
        const id = context.id;
        assert(!this.#contexts.has(id), 0x158 /* "Creating store with existing ID" */);
    ...
    public addBoundOrRemoted(context: FluidDataStoreContext) {
        const id = context.id;
        assert(!this.#contexts.has(id), 0x15d /* "Creating store with existing ID" */);
