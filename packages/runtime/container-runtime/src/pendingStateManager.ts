...
export class PendingStateManager {
    ...
    public processPendingLocalMessage(message: ISequencedDocumentMessage): unknown {
        ...
        assert(pendingState.type === "message", 0x12d /* No pending message found for this remote message */);
    ...
    private maybeProcessBatchBegin(message: ISequencedDocumentMessage) {
        ...
        if (pendingState.type === "flushMode") {
            assert(pendingState.flushMode === FlushMode.Manual,
                0x12e /* Flush mode should be manual when processing batch begin */);
        }

        // We should not already be processing a batch and there should be no pending batch begin message.
        assert(!this.isProcessingBatch && this.pendingBatchBeginMessage === undefined,
            0x12f /* The pending batch state indicates we are already processing a batch */);
    ...
    private maybeProcessBatchEnd(message: ISequencedDocumentMessage) {
        ...
        if (nextPendingState.type === "flushMode") {
            assert(nextPendingState.flushMode === FlushMode.Automatic,
                0x130 /* Flush mode is set to Manual in the middle of processing a batch */);
            this.pendingStates.shift();
        }

        // There should be a pending batch begin message.
        assert(this.pendingBatchBeginMessage !== undefined, 0x131 /* There is no pending batch begin message */);
        ...
        if (this.pendingBatchBeginMessage === message) {
            assert(batchBeginMetadata === undefined,
                0x132 /* Batch with single message should not have batch metadata */);
        } else {
            // Get the batch metadata from the last message in the batch.
            const batchEndMetadata = message.metadata?.batch;
            assert(batchBeginMetadata === true, 0x133 /* Did not receive batch begin metadata */);
            assert(batchEndMetadata === false, 0x134 /* Did not receive batch end metadata */);
        }
    ...
    private peekNextPendingState(): IPendingState {
        ...
        assert(!!nextPendingState, 0x135 /* No pending state found for the remote message */);
    ...
    public replayPendingStates() {
        assert(this.connected, 0x136 /* The connection state is not consistent with the runtime */);

        // This assert suggests we are about to send same ops twice, which will result in data loss.
        assert(this.clientId !== this.containerRuntime.clientId,
            0x137 /* replayPendingStates called twice for same clientId! */);
