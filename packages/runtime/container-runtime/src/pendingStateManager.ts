...
export class PendingStateManager implements IDisposable {
    ...
    private processRemoteMessage(message: ISequencedDocumentMessage) {
        ...
        if (isOriginalClientId || isNewClientId) {
            assert(this.clientId === undefined, 0x282 /* "multiple clients connected with stashed ops" */);
    ...
    private assertOpMatch(state: IPendingMessage, message: ISequencedDocumentMessage, isOriginalClientId: boolean) {
        assert(message.type === state.messageType, 0x283 /* "different message type" */);
        assert(message.clientSequenceNumber === state.clientSequenceNumber || !isOriginalClientId,
            0x284 /* "client sequence number doesn't match" */);
        switch(message.type) {
            case ContainerMessageType.Attach:
                assert(message.contents.id === state.content.id, 0x285 /* "datastore ID doesn't match" */);
                break;
            case ContainerMessageType.FluidDataStoreOp:
                assert(message.contents.address === state.content.address, 0x286 /* "address doesn't match" */);
                break;
