...
export class PendingStateManager implements IDisposable {
	...
	public getLocalState(): IPendingLocalState | undefined {
		...
		const pendingStates = [...this.savedOps, ...this.pendingMessages.toArray()].map(
			(message) => {
		...
		return pendingStates.length === 0
			? undefined
			: {
					pendingStates,
					nonAckedMessageCount: this.pendingMessages.length,
			  };
	}

	private readonly nonAckedMessageCount: number;
	...
	constructor(
		...
		this.nonAckedMessageCount = initialLocalState?.nonAckedMessageCount ?? 0;
	}
	...
	public async applyStashedOpsAt(seqNum: number) {
		...
		while (!this.initialMessages.isEmpty()) {
			...
			if (nextMessage.referenceSequenceNumber < seqNum) {
				// Only some number of messages in this.initialMessages could be non-acked messages.
				// The rest are already known to be sequenced, and could be cut-off by snapshot refresh logic
				// in Container
				if (this.nonAckedMessageCount <= this.initialMessages.length) {
					continue;
				}
