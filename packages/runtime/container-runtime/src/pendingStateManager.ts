...
export class PendingStateManager {
        ...
        if (this.hasPendingMessages()) {
            return {
                ...
                pendingStates: this.pendingStates.toArray().map(
                    // delete localOpMetadata since it may not be serializable and will be regenerated by rebaseOp()
    ...
    constructor(
        ...
        private readonly rebaseOp: (type, content) => Promise<unknown>,
    ...
    public async rebaseAt(seqNum: number) {
        ...
        while (!this.initialStates.isEmpty()) {
            ...
            if (nextState.type === "message") {
                ...
                const localOpMetadata = await this.rebaseOp(nextState.messageType, nextState.content);
