...
export class PendingStateManager implements IDisposable {
	...
	public get pendingMessagesCount(): number {
		return (
			...
			this.unappliedStashedMessages.length +
			this.appliedStashedMessages.length
		);
	}
	...
	public getLocalState(): IPendingLocalState | undefined {
		...
		if (this.hasPendingMessages()) {
			return {
				pendingStates: [
					...this.savedOps,
					...this.appliedStashedMessages.toArray(),
	...
	public processPendingLocalMessage(message: SequencedContainerRuntimeMessage): unknown {
		...
		const pendingMessage = !this.appliedStashedMessages.isEmpty()
			? this.appliedStashedMessages.peekFront()
			: this.pendingMessages.peekFront();
		assert(
		...
		(!this.appliedStashedMessages.isEmpty()
			? this.appliedStashedMessages
	...
	private maybeProcessBatchEnd(message: ISequencedDocumentMessage) {
		...
		if (
			(this.pendingMessages.isEmpty() && this.appliedStashedMessages.isEmpty()) ||
			batchEndMetadata === false
		) {
