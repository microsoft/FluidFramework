	...
	implements IFluidDataStoreContextInternal, IFluidParentContext, IDisposable
{
	 ...
	 * the channel in 2.4.0. For channels before that, call `process` for each message.
	 */
	private processMessagesCompat(
		...
		if (channel.processMessages !== undefined) {
			return channel.processMessages(message, messageContents, local);
		}
		for (const { contents, localOpMetadata, clientSequenceNumber } of messageContents) {
			channel.process({ ...message, contents, clientSequenceNumber }, local, localOpMetadata);
		}
	}
	...
	public processMessages(
		...
		if (this.loaded) {
			...
			this.processMessagesCompat(this.channel, message, messageContents, local);
		} else {
	...
	protected processPendingOps(channel: IFluidDataStoreChannel) {
		...
		for (const { message, messageContents } of pendingMessages) {
			if (message.sequenceNumber > baseSequenceNumber) {
				this.processMessagesCompat(channel, message, messageContents, false /* local */);
			}
		}
