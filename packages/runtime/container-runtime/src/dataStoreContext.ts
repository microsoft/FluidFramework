	...
	implements IFluidDataStoreContextInternal, IFluidParentContext, IDisposable
{
	...
	protected processPendingOps(channel: IFluidDataStoreChannel) {
		...
		for (const messagesWithMetadata of pendingMessagesWithMetadata) {
			...
			// Each bunch of messages sent to the data store are part of a batch. They either have the same
			// sequence number for grouped batches or continuous sequence numbers for non-grouped batches.
			// So, only checking the first message's sequence number is sufficient as batches are processed
