...
export class BlobManager {
    ...
    public async getBlob(blobId: string): Promise<ArrayBufferLike> {
        ...
        if (this.runtime.attachState === AttachState.Detached) {
            assert(this.redirectTable.has(blobId), 0x36c /* requesting unknown blobs */);
    ...
    private async onUploadReject(localId: string, error) {
        const entry = this.pendingBlobs.get(localId);
        assert(!!entry, 0x370 /* Must have pending blob entry for blob which failed to upload */);
        if (!this.runtime.connected) {
            if (entry.status === PendingBlobStatus.OnlinePendingUpload) {
                this.transitionToOffline(localId);
            }
    ...
    private transitionToOffline(localId: string) {
        assert(!this.runtime.connected, 0x371 /* Must only transition to offline flow while runtime is disconnected */);
        const entry = this.pendingBlobs.get(localId);
        assert(!!entry, 0x372 /* No pending blob entry */);
    ...
    public reSubmit(metadata: Record<string, unknown> | undefined) {
        assert(!!metadata, 0x374 /* Resubmitted ops must have metadata */);
        const { blobId, localId }: { blobId?: string; localId?: string; } = metadata;
        if (!blobId) {
            assert(!!localId, 0x375 /* Submitted BlobAttach ops must have a blobId or localId */);
            // We submitted this op while offline. The blob should have been uploaded by now.
            const pendingEntry = this.pendingBlobs.get(localId);
            assert(pendingEntry?.status === PendingBlobStatus.OfflinePendingOp &&
                !!pendingEntry?.storageId, 0x376 /* blob must be uploaded before resubmitting BlobAttach op */);
    ...
    public processBlobAttachOp(message: ISequencedDocumentMessage, local: boolean) {
        ...
        if (local) {
            if (message.metadata.localId === undefined) {
                ...
                assert(!!waitingBlobs, 0x377 /* local online BlobAttach op with no pending blob */);
                waitingBlobs.forEach((localId) => {
                    const pendingBlobEntry = this.pendingBlobs.get(localId);
                    assert(pendingBlobEntry !== undefined, 0x378 /* local online BlobAttach op with no pending blob entry */);
    ...
    public getGCData(fullGC: boolean = false): IGarbageCollectionData {
        ...
        for (const [localId, storageId] of this.redirectTable) {
            assert(!!storageId, 0x379 /* Must be attached to get GC data */);
    ...
    public setRedirectTable(table: Map<string, string>) {
        ...
        assert(this.redirectTable.size === table.size, 0x37a /* Redirect table size must match BlobManager's local ID count */);
