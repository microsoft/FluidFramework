...
export abstract class LocalChannelContextBase implements IChannelContext {
	...
	processMessages(
		...
		if (this.isLoaded) {
			this.services.value.deltaConnection.processMessages(message, messageContents, local);
		} else {
			assert(
				!local,
		...
		super(
			...
			new LazyPromise<IChannel>(async () => {
				try {
					...
					for (const { message, messageContents } of this.pendingMessagesState.messages) {
