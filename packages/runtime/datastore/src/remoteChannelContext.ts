...
export class RemoteChannelContext implements IChannelContext {
	...
	constructor(
		...
		this.channelP = new LazyPromise<IChannel>(async () => {
			...
			const pendingMessages = this.pendingMessagesState.messages;
			for (const { message, messageContents } of pendingMessages) {
				this.services.deltaConnection.processMessages(message, messageContents, false);
			}
	...
	public processMessages(
		...
		if (this.isLoaded) {
			this.services.deltaConnection.processMessages(message, messageContents, local);
		} else {
			assert(!local, 0x195 /* "Remote channel must not be local when processing op" */);
			assert(this.pendingMessagesState !== undefined, "pending messages queue is undefined");
			this.pendingMessagesState.messages.push({
				message,
				messageContents: Array.from(messageContents),
			});
			this.pendingMessagesState.pendingCount += messageContents.length;
