...
export class RemoteChannelContext implements IChannelContext {
    ...
    public processOp(message: ISequencedDocumentMessage, local: boolean, localOpMetadata: unknown): void {
        ...
        if (this.isLoaded) {
            ...
            assert(!local, 0x155 /* Remote channel must not be local when processing op */);
    ...
    public reSubmit(content: any, localOpMetadata: unknown) {
        assert(this.isLoaded, 0x156 /* Remote channel must be loaded when resubmitting op */);
    ...
    private async loadChannel(): Promise<IChannel> {
        assert(!this.isLoaded, 0x157 /* Remote channel must not already be loaded when loading */);
