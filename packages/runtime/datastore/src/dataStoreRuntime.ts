...
IFluidDataStoreChannel, IFluidDataStoreRuntime, IFluidHandleContext {
     ...
     */
    public async summarize(fullTree: boolean = false, trackState: boolean = true, summaryTelemetryData?: Map<string, string>): Promise<ISummaryTreeWithStats> {
        ...
        await Promise.all(Array.from(this.contexts)
            .filter(([contextId, _]) => {
                ...
                const contextSummary = await context.summarize(true /* fullTree */, trackState, summaryTelemetryData);
    ...
    public getAttachSummary(summaryTelemetryData?: Map<string, string>): ISummaryTreeWithStats {
        /**
        ...
        for (const [contextId, context] of this.contexts) {
            ...
            if (!this.notBoundedChannelContextSet.has(contextId)) {
                ...
                if (context.isLoaded) {
                    const contextSummary = context.getAttachSummary(summaryTelemetryData);
                    assert(
