	...
	implements IFluidDataStoreChannel, IFluidDataStoreRuntime, IFluidHandleContext
{
	 ...
	 * @param messageContents - The contents of the channel messages and their metadata.
	...
	private processChannelMessages(
		...
		const sendBunchedMessages = () => {
			assert(currentAddress !== undefined, "an address must exist to send messages");
		...
		for (const { contents, ...restOfMessageContents } of messageContents) {
			...
			if (currentAddress !== undefined && currentAddress !== envelope.address) {
				sendBunchedMessages();
			}
