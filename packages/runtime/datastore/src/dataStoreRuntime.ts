	...
	implements IFluidDataStoreChannel, IFluidDataStoreRuntime, IFluidHandleContext
{
	...
	/**
	 * Process a bunch of messages. The messages here are contiguous messages of the same type in a batch for this
	 * data store.
	 * @param messagesWithMetadata - A bunch of contiguous messages of the same type and their metadata.
	 * @param local - true if the messages were originally generated by the client receiving it.
	 */
	private processBunchedMessages(
		...
		/**
		 * Channel ops are sent as a bunch so that DDSes can process them together.
		...
		if (messagesType === DataStoreMessageType.ChannelOp) {
			this.processChannelMessages(messagesWithMetadata, local);
		}
	...
	/**
	 * Process data store messages. The messages here are contiguous messages for this data store in a batch.
	 * @param messagesWithMetadata - A bunch of contiguous messages for this data store and their metadata.
	 * @param local - true if the messages were originally generated by the client receiving it.
	 */
	public processMessages(
		...
		/** Bunch contiguous messages of the same type and process them together. */
		...
		messagesWithMetadata.forEach(({ message, localOpMetadata }) => {
			...
			const maybeProcessBunch = (messageType: DataStoreMessageType) => {
				...
				if (bunchedOps.length > 0) {
					this.processBunchedMessages(bunchedOps, local);
					bunchedOps = [];
				}
			};
	...
	private processChannelMessages(
		...
		messagesWithMetadata.forEach((messageWithMetadata) => {
			...
			if (previousAddress !== undefined && previousAddress !== envelope.address) {
				...
				channelContext.processMessages(contextMessagesWithMetadata, local);
