...
export class SummarizerNodeWithGC extends SummarizerNode implements IRootSummarizerNodeWithGC {
    ...
    public async summarize(fullTree: boolean, trackState: boolean = true): Promise<IContextSummarizeResult> {
        ...
        if (!this.gcDisabled && this.isTrackingInProgress()) {
            assert(this.wipSerializedUsedRoutes !== undefined,
                0x16f /* wip used routes should be set if tracking a summary */);
        }
    ...
    public async getGCData(fullGC: boolean = false): Promise<IGarbageCollectionData> {
        assert(!this.gcDisabled, 0x170 /* Getting GC data should not be called when GC is disabled! */);
        assert(this.getGCDataFn !== undefined, 0x171 /* GC data cannot be retrieved without getGCDataFn */);
    ...
    public startSummary(referenceSequenceNumber: number, summaryLogger: ITelemetryLogger) {
        ...
        if (!this.gcDisabled) {
            assert(
                ...
                0x172 /* We should not already be tracking used routes when to track a new summary */);
    ...
    protected completeSummaryCore(
        ...
        if (!this.gcDisabled) {
            ...
            assert(wipSerializedUsedRoutes !== undefined, 0x173 /* We should have been tracking used routes */);
        }
    ...
    public createChild(
        ...
        assert(!this.children.has(id), 0x174 /* Create SummarizerNode child already exists */);
