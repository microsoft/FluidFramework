...
export class SummarizerNode implements IRootSummarizerNode {
    ...
    public startSummary(referenceSequenceNumber: number, summaryLogger: ITelemetryLogger) {
        assert(this.wipSummaryLogger === undefined,
            "s_9r" /* wipSummaryLogger should not be set yet in startSummary */);
        assert(this.wipReferenceSequenceNumber === undefined, "s_9s" /* Already tracking a summary */);
    ...
    public async summarize(fullTree: boolean): Promise<ISummarizeResult> {
        assert(this.isTrackingInProgress(),
            "s_9t" /* summarize should not be called when not tracking the summary */);
        assert(this.wipSummaryLogger !== undefined,
            "s_9u" /* wipSummaryLogger should have been set in startSummary or ctor */);
    ...
    protected completeSummaryCore(
        ...
        assert(this.wipSummaryLogger !== undefined,
            "s_9v" /* wipSummaryLogger should have been set in startSummary or ctor */);
        assert(this.wipReferenceSequenceNumber !== undefined, "s_9w" /* Not tracking a summary */);
        ...
        assert(!!localPathsToUse, "s_9x" /* Tracked summary local paths not set */);
    ...
    protected refreshLatestSummaryFromPending(
        ...
        if (summaryNode === undefined) {
            ...
            assert(
                ...
                "s_9y" /* Not found pending summary, but this node has previously completed a summary */,
            );
            return;
        } else {
            assert(
    ...
    public async loadBaseSummary(
        ...
        if (decodedSummary.pathParts.length > 0) {
            assert(!!this.latestSummary, "s_9z" /* Should have latest summary defined during loadBaseSummary */);
        ...
        if (outstandingOps.length > 0) {
            ...
            assert(
                ...
                "s_a0" /* When loading base summary, expected outstanding ops <= tracking sequence number */,
            );
        }
    ...
    public createChild(
        ...
        assert(!this.children.has(id), "s_a1" /* Create SummarizerNode child already exists */);
        ...
        switch (createParam.type) {
            ...
            case CreateSummarizerNodeSource.FromSummary: {
                if (this.initialSummary === undefined) {
                    assert(
                        !!parentLatestSummary,
                        "s_a2" /* Cannot create child from summary if parent does not have latest summary */);
                }
