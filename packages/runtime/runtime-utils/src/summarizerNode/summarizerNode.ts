...
export class SummarizerNode implements IRootSummarizerNode {
    ...
    public startSummary(referenceSequenceNumber: number, summaryLogger: ITelemetryLogger) {
        assert(this.wipSummaryLogger === undefined,
            0x15f /* wipSummaryLogger should not be set yet in startSummary */);
        assert(this.wipReferenceSequenceNumber === undefined, 0x160 /* Already tracking a summary */);
    ...
    public async summarize(fullTree: boolean): Promise<ISummarizeResult> {
        assert(this.isTrackingInProgress(),
            0x161 /* summarize should not be called when not tracking the summary */);
        assert(this.wipSummaryLogger !== undefined,
            0x162 /* wipSummaryLogger should have been set in startSummary or ctor */);
    ...
    protected completeSummaryCore(
        ...
        assert(this.wipSummaryLogger !== undefined,
            0x163 /* wipSummaryLogger should have been set in startSummary or ctor */);
        assert(this.wipReferenceSequenceNumber !== undefined, 0x164 /* Not tracking a summary */);
        ...
        assert(!!localPathsToUse, 0x165 /* Tracked summary local paths not set */);
    ...
    protected refreshLatestSummaryFromPending(
        ...
        if (summaryNode === undefined) {
            ...
            assert(
                ...
                0x166 /* Not found pending summary, but this node has previously completed a summary */,
            );
            return;
        } else {
            assert(
    ...
    public async loadBaseSummary(
        ...
        if (decodedSummary.pathParts.length > 0) {
            assert(!!this.latestSummary, 0x167 /* Should have latest summary defined during loadBaseSummary */);
        ...
        if (outstandingOps.length > 0) {
            ...
            assert(
                ...
                0x168 /* When loading base summary, expected outstanding ops <= tracking sequence number */,
            );
        }
    ...
    public createChild(
        ...
        assert(!this.children.has(id), 0x169 /* Create SummarizerNode child already exists */);
        ...
        switch (createParam.type) {
            ...
            case CreateSummarizerNodeSource.FromSummary: {
                if (this.initialSummary === undefined) {
                    assert(
                        !!parentLatestSummary,
                        0x16a /* Cannot create child from summary if parent does not have latest summary */);
                }
