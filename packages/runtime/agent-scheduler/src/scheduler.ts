...
class AgentScheduler extends EventEmitter implements IAgentScheduler {
    public static async load(runtime: IFluidDataStoreRuntime, context: IFluidDataStoreContext) {
        ...
        if (!runtime.existing) {
            ...
            assert(handle !== undefined, 0xe2 /* Missing handle on scheduler load */);
    ...
    private get clientId(): string {
        ...
        assert(!!clientId, 0xe3 /* Trying to get missing clientId! */);
        return clientId;
    }
    ...
    public async pick(taskId: string, worker: () => Promise<void>): Promise<void> {
        ...
        assert(this.context.deltaManager.clientDetails.capabilities.interactive,
            0xe4 /* Bad client interactive check */);
    ...
    public async release(...taskUrls: string[]): Promise<void> {
        ...
        for (const taskUrl of taskUrls) {
            ...
            assert(active, 0xe5 /* This agent became inactive while releasing */);
    ...
    private async clearTasks(taskUrls: string[]) {
        assert(this.isActive(), 0xe6 /* Trying to clear tasks on inactive agent */);
    ...
    private initialize() {
        ...
        quorum.on("removeMember", async (clientId: string) => {
            assert(this.runtime.objectsRoutingContext.isAttached, 0xe7 /* Detached object routing context */);
    ...
    private onNewTaskAssigned(key: string) {
        assert(!this.runningTasks.has(key), 0xe8 /* task is already running */);
    ...
    private async onTaskReassigned(key: string, currentClient: string | null) {
        ...
        assert(currentClient !== undefined, 0xe9 /* client is undefined */);
        if (this.isActive()) {
