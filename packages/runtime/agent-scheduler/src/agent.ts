...
export interface IAgentSchedulerEvents {
    /**
     * The task has been assigned to this client, in response to pick() being called
     ...
     */
    picked: [taskId: string];
    /**
     * The task was successfully released back to the pool. Client will not attempt to
     * re-acquire the task, unless pick() is called.
     */
    released: [taskId: string];
    /**
     * Task is lost due to disconnect or data store / container being attached.
