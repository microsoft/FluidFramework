 ...
 * @sealed
 * @privateRemarks
 * After partners move to the alpha interfaces this interface should be renamed and tagged to be internal.
 */
export interface CommitStagedChangesOptionsExperimental {
	 ...
	 * Defaults to false.
	 * @remarks
	 * The squash parameter is analogous to `git squash` but differs in a notable way: ops created by a client
	 * exiting staging mode are not necessarily coalesced into a single op or something like it.
	 * It still does have the desirable property that "unnecessary changes" (such as inserting some content
	 * then removing it) will be removed from the set of submitted ops, which means it helps reduce network
	 * traffic and the chance of unwanted data being persisted--even if only temporarily--in the document.
	 *
	 * By not attempting to reduce the set of changes to a single op a la `git squash`,
	 * we can better preserve the ordering of changes that remote clients see
	 * such that they better align with the client which submitted the changes.
	 */
	squash?: boolean;
}
 ...
 * @legacy @beta
 * @sealed
 * @privateRemarks After partners move to the alpha interfaces this interface should be
 * renamed and tagged to be internal.
 */
 ...
 * @legacy @beta
 * @sealed
 * @privateRemarks After partners move to the alpha interfaces this interface should be renamed
 * and tagged to be internal.
 */
