...
```bash
npm i @fluidframework/runtime-definitions
```
...
Import the `public` ([Semantic Versioning (SemVer)](https://semver.org/)) APIs from `@fluidframework/runtime-definitions`.

Import the `beta` APIs from `@fluidframework/runtime-definitions/beta`.

Import the `legacy` APIs from `@fluidframework/runtime-definitions/legacy`.
