...
export class MergeTree {
    ...
    blockClone(block: IMergeBlock, segments?: ISegment[]) {
        ...
        for (let i = 0; i < block.childCount; i++) {
            ...
            if (child.isLeaf()) {
                const segment = this.segmentClone(<ISegment>child);
    ...
    pack(block: IMergeBlock) {
        ...
        const packedBlocks = <IMergeBlock[]>new Array(MaxNodesInBlock);
        let readCount = 0;
    ...
    getStats() {
        const nodeGetStats = (block: IMergeBlock): MergeTreeStats => {
            ...
            for (let i = 0; i < block.childCount; i++) {
                ...
                let height = 1;
                if (!child.isLeaf()) {
                    const childStats = nodeGetStats(<IMergeBlock>child);
    ...
    leftExcursion<TClientData>(node: IMergeNode, leafAction: ISegmentAction<TClientData>) {
        ...
        while (parent) {
            ...
            for (childIndex = parent.childCount - 1; childIndex >= 0; childIndex--) {
                ...
                if (matchedStart) {
                    if (!node.isLeaf()) {
                        const childBlock = <IMergeBlock>node;
    ...
    private insertingWalk(
        ...
        for (childIndex = 0; childIndex < block.childCount; childIndex++) {
            ...
            if ((pos < len) || ((pos === len) && this.breakTie(pos, len, seq, child, refSeq, clientId, context.candidateSegment))) {
                ...
                found = true;
                if (!child.isLeaf()) {
                    const childBlock = <IMergeBlock>child;
                    // internal node
        ...
        while (!stateStack.empty()) {
            ...
            if ((state.op === IncrementalExecOp.Go) && (state.childIndex < state.block.childCount)) {
                ...
                if ((len > 0) && (state.start < len) && (state.end > 0)) {
                    if (!child.isLeaf()) {
                        const childState = new IncrementalMapState(<IMergeBlock>child, state.actions, state.pos,
