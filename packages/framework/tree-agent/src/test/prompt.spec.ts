...
describe("Prompt generation", () => {
	...
	it("sanitizes schema names that contain invalid characters", () => {
		...
		assert.ok(!prompt.includes("Test-Object!"), "The unsanitized identifier should not show up in the prompt")
	});
	...
	it("sanitizes schema names that have leading digit", () => {
		...
		assert.ok(!prompt.includes("test.1TestObject"), "The unsanitized identifier should not show up in the prompt")
	});
});
