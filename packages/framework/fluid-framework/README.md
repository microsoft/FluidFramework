...
The `fluid-framework` package bundles a collection of Fluid Framework client libraries for easy use when paired with a corresponding service client package (e.g. `@fluidframework/azure-client`, `@fluidframework/tinylicious-client`, or `@fluidframework/odsp-client`).
...
-   A service client (e.g. `@fluidframework/azure-client`, `@fluidframework/odsp-client`, or `@fluidframework/tinylicious-client` for local development) to allow connecting to a Fluid service.
