...
export class TextSummarizeAction {
	public static async execute(openAi: OpenAI, text: string, textContext?: string) {
		const prompt = TextSummarizeAction.buildFunctionPrompt(text, textContext);
		...
		const functionCall: TextSummarizeActionResponse =
...
export interface TextSummarizeActionResponse {
	summary: {
		description: string;
	};
}
