...
export class PresenceDatastoreManagerImpl implements PresenceDatastoreManager {
	...
	/**
	 * Tracks whether another client has responded to our join request.
	 *
	 * @remarks
	 ...
	 */
	private haveCompleteSnapshot = false;
	...
	public joinSession(
		...
		forceEvenWhenAlone: boolean = false,
	): void {
		// Before broadcasting the join message, check that there is at
		// least one other audience member present. This is useful to
		...
		this.logger?.sendTelemetryEvent({
			...
			details: {
				...
				// Note that empty updateProviders is indicative of forced join
				// even when alone.
	...
	private deferJoinUntilAudienceMember(selfClientId: string, audience: IAudience): void {
		this.logger?.sendTelemetryEvent({
		...
		// Prepare to join once another audience member joins.
		// Alternatively, processSignal may force a join when a presence
		// signal is received even without audience members.
		const joinWhenAudienceMemberJoined = (
			addedClientId: ClientConnectionId,
			addedClient: IClient,
		): void => {
			if (!addedClient.details.capabilities.interactive || addedClientId === selfClientId) {
				// Keep listening
				return;
			}
			this.stopWaitingAndJoin(selfClientId, /* forceEvenWhenAlone */ false);
		};
		audience.on("addMember", joinWhenAudienceMemberJoined);
		this.stopWaitingForAudienceMembers = () => {
			audience.off("addMember", joinWhenAudienceMemberJoined);
		};
	}

	private stopWaitingAndJoin(
		selfClientId: ClientConnectionId,
		forceEvenWhenAlone: boolean,
	): void {
		this.stopWaitingForAudienceMembers?.();
		this.stopWaitingForAudienceMembers = undefined;
		// Confirm not currently disconnected
		if (this.runtime.getJoinedStatus() !== "disconnected") {
			this.joinSession(selfClientId, forceEvenWhenAlone);
		}
	}

	public onDisconnected(): void {
		this.haveCompleteSnapshot = false;
		this.stopWaitingForAudienceMembers?.();
		this.stopWaitingForAudienceMembers = undefined;
	}
	...
	public processSignal(
		...
		// Check for unexpected case of receiving a remote presence signal
		// without having been alerted to audience join. (Perhaps join signal
		// was dropped.)
		if (this.stopWaitingForAudienceMembers !== undefined) {
			this.stopWaitingAndJoin(selfClientId, /* forceEvenWhenAlone */ true);
		}
		...
		if (message.type === joinMessageType) {
			...
			if (joinResponseFor) {
				...
				if (joinResponseFor.includes(selfClientId)) {
					...
					justGainedCompleteSnapshot = !this.haveCompleteSnapshot;
					this.haveCompleteSnapshot = true;
				}
				if (this.broadcastRequests.size > 0) {
	...
	private prepareJoinResponse(
		...
		if (this.haveCompleteSnapshot && updateProviders.includes(selfClientId)) {
