...
type ABorCD = { a: number; b: number } | { c: number; d: number };
	...
	describe("Runtime schema validation", () => {
		...
		describe("response to Join signal", () => {
			it("does not contain validation metadata for remote clients", () => {
				...
				const ABOrCDValidatorFunction = createSpiedValidator<ABorCD>((d: unknown) => {
					if (typeof d !== "object" || d === null) {
						return undefined;
					}
					const ab = "a" in d && "b" in d;
					const cd = "c" in d && "d" in d;
					return ab === cd ? undefined : (d as ABorCD);
				});
				const statesWorkspace = presence.states.getWorkspace("name:testWorkspace", {
					...
					latestMap: StateFactory.latestMap({
						...
						validator: ABOrCDValidatorFunction,
					}),
				});
				const latest = statesWorkspace.states.latest;
				const latestMap = statesWorkspace.states.latestMap;
				const attendee1 = presence.attendees.getAttendee(attendeeId1);
				latest.getRemote(attendee1)?.value();
				const attendee1Map = latestMap.getRemote(attendee1);
				const dataForKey1 = attendee1Map.get("key1");
				assert.ok(dataForKey1);
				assert.ok(dataForKey1.value());
				const dataForKey2 = attendee1Map.get("key2");
				assert.ok(dataForKey2);
				assert.equal(dataForKey2.value(), undefined);
