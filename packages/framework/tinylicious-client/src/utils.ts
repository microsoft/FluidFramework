/**
* Fixed size circular Queue implementation (FIFO replacement)
*/
export class CircularBuffer<T> {
	 ...
	 */
	/**
	 * Adds one or more items to the circular buffer.
	...
	public remove(index: number, count = 1): T[] {
		...
		*/
		...
		/**
		* The array representation of this buffer.
		*/
		const array = this.toArray();
		/**
		* Removed items from the buffer.
		*/
		const removedItems = array.splice(new_index, count);
	 ...
	 */
	public toArray(): T[] {
		return [...this.buffer.slice(this.pos), ...this.buffer.slice(0, this.pos)];
	}
