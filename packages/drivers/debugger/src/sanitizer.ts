...
class ChunkedOpProcessor {
    ...
    writeSanitizedContents(contents: any): void {
        ...
        try {
            ...
            assert(stringified.length <= this.concatenatedLength,
                "sc:0070" /* Stringified length of chunk contents > total starting length */);
        } catch (e) {
            this.debugMsg(e);
            throw e;
        }
    ...
    reset(): void {
        assert(this.writtenBack, "sc:0071" /* resetting ChunkedOpProcessor that never wrote back its contents */);
...
export class Sanitizer {
    ...
    fixAttachContents(contents: any): any {
        assert(typeof contents === "object", "sc:0072" /* Unexpected type on contents for fix of an attach! */);
    ...
    fixOpContentsObject(contents: any) {
        ...
        if (!this.objectMatchesSchema(contents, opContentsSchema)) {
            ...
            assert(innerContent !== undefined, "sc:0073" /* innerContent for fixing op contents is undefined! */);
    ...
    sanitize(): ISequencedDocumentMessage[] {
        ...
        try {
            ...
            assert(!this.chunkProcessor.isPendingProcessing(), "sc:0074" /* After sanitize, pending incomplete ops! */);
        } catch (error) {
