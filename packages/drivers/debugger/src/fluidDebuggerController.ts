...
export class DebugReplayController extends ReplayController implements IDebuggerController {
    ...
    public async initStorage(documentService: IDocumentService): Promise<boolean> {
        ...
        assert(!!documentService, "sc:0067" /* Invalid document service! */);
        assert(!this.documentService, "sc:0068" /* Document service already set! */);
        assert(!this.documentStorageService, "sc:0069" /* Document storage service already set! */);
        ...
        assert(this.isSelectionMade() === this.shouldUseController,
            "sc:006a" /* User selection status does not match replay controller use status! */);
        return this.shouldUseController;
    }
    ...
    protected resolveStorage(
        ...
        assert(!this.isSelectionMade(), "sc:006b" /* On storage resolve, user selection already made! */);
        assert(!!storage, "sc:006c" /* On storage resolve, missing storage! */);
        this.storage = storage;
        assert(this.isSelectionMade(), "sc:006d" /* After storage resolve, user selection status still false! */);
    ...
    while (true) {
        ...
        if (messages.length === 0) {
            assert(!partialResult,
                ...
                "sc:006e" /* No messages loaded from chunk, but nonzero number of partial results loaded from chunk! */);
            break;
        }
        yield messages;
