...
describe("Routerlicious Url Resolver", () => {
    ...
    it("Should resolve the Routerlicious urls correctly", async () => {
        ...
        const url: string = "https://www.wu2.prague.office-int.com/loader/fluid/thinkable-list?component=@fluid-example/shared-text@0.11.14146";
        ...
        assert.equal(resolved.url, "fluid://wu2.prague.office-int.com/fluid/thinkable-list?component=@fluid-example/shared-text@0.11.14146", "FluidUrl does not match");
    });

    it("Should resolve the localhost urls correctly", async () => {
        const urlResolver = new RouterliciousUrlResolver(undefined, async () => Promise.resolve(token), []);
        const url: string = "http://localhost:3000/loader/fluid/damp-competition?component=@fluid-example/shared-text@^0.11.0";
        const resolved = (await urlResolver.resolve({ url })) as IFluidResolvedUrl;
        assert.equal(resolved.tokens.jwt, token, "Token does not match");
        ...
        assert.equal(resolved.url, "fluid://localhost:3003/fluid/damp-competition?component=@fluid-example/shared-text@^0.11.0", "FluidUrl does not match");
    });

    it("Should handle local External request", async () => {
        const request: IRequest = {
            url: "/loader/fluid/damp-competition?component=@fluid-example/shared-text@^0.11.0",
        };
        ...
        assert.equal(url, "fluid://localhost:3003/fluid/damp-competition?component=@fluid-example/shared-text@^0.11.0", "Improperly formed FluidURL");
    });

    it("Should handle local Internal request", async () => {
        const request: IRequest = {
            url: "http://gateway:3000/loader/fluid/damp-competition?component=@fluid-example/shared-text@^0.11.0",
        };
    ...
    it("Should handle Deployed Internal request", async () => {
        const request: IRequest = {
            url: "http://angry-dog-gateway:3000/loader/fluid/damp-competition?component=@fluid-example/shared-text@^0.11.0",
        };
        const provider = new Provider({}).defaults({
            worker: {
                serverUrl: "http://localhost:3003",
    ...
    it("Should handle deployed External request", async () => {
        ...
        assert.equal(url, "fluid://alfred.wu2-ppe.prague.office-int.com/fluid/damp-competition?component=@fluid-example/shared-text@^0.11.0", "FluidUrl does not match");
    });
});
