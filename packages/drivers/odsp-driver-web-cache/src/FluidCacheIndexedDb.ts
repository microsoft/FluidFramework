  ...
  return new Promise((resolve, reject) => {
    openDB<FluidCacheDBSchema>(FluidDriverCacheDBName, CurrentCacheVersion, {
      upgrade: (db, oldVersion) => {
        try {
          // We changed the format of the object store, so we delete the old stores to create a new one in V3
          ...
          // Catch any error done when attempting to delete the older version. If the object does not exist
...
/** Deletes the indexed DB instance. Warning this can throw an error in Firefox incognito, where accessing storage is prohibited.  */
...
export interface FluidCacheDBSchema extends DBSchema {
  [FluidDriverObjectStoreName]: {
    ...
    value: {
      ...
      /**
       * Files may have multiple cached items associated with them, this property uniquely identifies a specific cache entry for a file.
      ...
      /**
       * The last time the cache entry was used. This is initially set to the time the cache entry was created Measured as ms since unix epoch.
       */
      lastAccessTimeMs: number;
    };

    indexes: { createdTimeMs: number; partitionKey: string; lastAccessTimeMs: number; fileId: string };
  };
}
