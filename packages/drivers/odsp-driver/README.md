...
| fluid:telemetry:OdspDriver:createNewEmptyFile_cancel | `/_api/v2.1/drives/DRIVEID/items/root://FILEPATH/FILENAME:/content?@name.conflictBehavior=rename&select=id,name,parentReference&ump=1` | Error when creating a new empty Fluid file. By default, a detached container with attachment blobs first creates an empty file, uploads the blobs, and then uploads a summary that references them. When the inlined detached-blob document format is enabled, the blobs are instead sent as summary blobs in the create request, so this empty-file path is not used. (Storage Call) |
