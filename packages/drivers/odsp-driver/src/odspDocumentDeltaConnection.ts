...
class SocketReference extends TypedEventEmitter<ISocketEvents> {
	...
	private static readonly socketIoSockets: Map<string, SocketReference> =
		new Map();
	...
	public constructor(
		...
		this._socket = socket;
		assert(
			!SocketReference.socketIoSockets.has(key),
...
export class OdspDocumentDeltaConnection extends DocumentDeltaConnection {
	...
	public static async create(
		...
		const enableMultiplexing =
			!parsedUrl.searchParams.has("documentId") &&
		...
		const key = socketReferenceKeyPrefix
			? `${socketReferenceKeyPrefix},${url}`
			: url;
		const socketReferenceKey = enableMultiplexing
			? key
			: `${key},${tenantId},${documentId}`;

		const socketReference =
			OdspDocumentDeltaConnection.getOrCreateSocketIoReference(
				timeoutMs,
				socketReferenceKey,
				url,
				enableMultiplexing,
	...
	private readonly getOpsMap: Map<
		string,
	...
	protected async initialize(
		...
		if (this.enableMultiplexing) {
			...
			this.earlySignalHandler = (
				...
				if (
					messageDocumentId === undefined ||
					messageDocumentId === this.documentId
				) {
		...
		this.addTrackedListener("get_ops_response", (result: IGetOpsResponse) => {
			...
			if (
				data !== undefined ||
				result.nonce.startsWith(this.requestOpsNoncePrefix)
			) {
	...
	protected addTrackedListener(
		...
		switch (event) {
				...
				super.addTrackedListener(
					...
					(msg: ISignalMessage | ISignalMessage[], documentId?: string) => {
						...
						if (filteredMsgs.length > 0) {
							...
							listener(
								filteredMsgs.length === 1 ? filteredMsgs[0] : filteredMsgs,
								documentId,
							);
						}
					},
			...
			case "nack": {
				...
				super.addTrackedListener(
					event,
					(clientIdOrDocumentId: string, nacks: INack[]) => {
						const handle =
