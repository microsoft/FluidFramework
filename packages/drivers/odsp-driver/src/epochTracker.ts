...
export class EpochTracker {
    ...
    public set fileEntry(fileEntry: IFileEntry | undefined) {
        assert(this._fileEntry === undefined, 0x81 /* File Entry should be set only once */);
        assert(fileEntry !== undefined, 0x82 /* Passed file entry should not be undefined */);
    ...
    private async checkForEpochError(
        ...
        if (error.errorType === OdspErrorType.epochVersionMismatch) {
            try {
                ...
                assert(!!this.fileEntry, 0x83 /* File Entry should be set to clear the cached entries!! */);
