 ...
 */
export function getQueryStringLength(url: string): number {
...
export function getUrlAndHeadersAndOptionsWithAuth(url: string, token: string | null): { url: string; headers: { [index: string]: string; }; isOptionsCall: boolean; } {
    let isOptionsCall = false;
    if (!token || token.length === 0) {
        return { url, headers: {}, isOptionsCall };
    }
