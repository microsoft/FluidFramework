...
describe("OdspPointInTimeDocumentServiceFactory lineage guard", () => {
	...
	it("fails with a UsageError (before creating any service) when no file version exists at or before the target", async () => {
		const factory = new OdspPointInTimeDocumentServiceFactory(getStorageToken, undefined);
		const resolvedUrl = await makeResolvedUrl();
		const internals = factory as unknown as FactoryInternals;

		// A REAL version manager whose oldest recoverable version ("42.0") is at seq 5, so a target of
		// 3 predates all retained history: findBaseForSeq returns `noBaseVersion`, which the factory
		// must translate into a UsageError naming the target and the oldest resolved sequence number.
		const realManager = new OdspVersionManager(
			fakeFetcher({ liveEpoch: "epoch-A", versionEpoch: "epoch-A" }),
		);
		stub(internals, "createVersionManager").returns(realManager);
		...
		await assert.rejects(
			async () => factory.createPointInTimeDocumentService(resolvedUrl, 3),
			(error: Error) => {
				assert.ok(
					error instanceof UsageError,
					"a target with no recoverable base is caller misuse (UsageError)",
				);
				assert.match(
					error.message,
					/No ODSP file version is available at or before sequence number 3/,
				);
				assert.match(
					error.message,
					/oldest resolved file version is at sequence number 5/,
					"the error surfaces the oldest resolved sequence number to aid the caller",
				);
				return true;
			},
			"a target predating all versions must fail the load with a UsageError",
		);
		// The load must short-circuit: no base resolution and no service creation once there is no base.
		assert.ok(
			resolveFileVersion.notCalled,
			"the base version must not be resolved when there is no base",
		);
		assert.ok(
			createDocumentServiceCore.notCalled,
			"no document service should be created when there is no base",
		);
	});

	describe("resolveFileVersion: builds the recoverable base's resolved URL", () => {
		/** Reach the private `resolveFileVersion` seam to exercise its real URL/query construction. */
		async function resolveFileVersion(
			factory: OdspPointInTimeDocumentServiceFactory,
			resolvedUrl: IResolvedUrl,
			fileVersion: string,
		): Promise<IResolvedUrl> {
			return (factory as unknown as FactoryInternals).resolveFileVersion(
				resolvedUrl,
				fileVersion,
			);
		}

		/** A resolved URL carrying an explicit data-store path and container package name. */
		async function makeResolvedUrlWithHints(config: {
			...
			return {
				...
				codeHint:
					config.containerPackageName === undefined
						? undefined
						: { containerPackageName: config.containerPackageName },
			} as unknown as IOdspResolvedUrl;
		}

		it("threads the target version, drive/item ids, data-store path, and package name into the resolved URL", async () => {
			const factory = new OdspPointInTimeDocumentServiceFactory(getStorageToken, undefined);
			const resolvedUrl = await makeResolvedUrlWithHints({
				dataStorePath: "/dataStore",
				containerPackageName: "my-package",
			});

			const resolved = (await resolveFileVersion(
				factory,
				resolvedUrl,
				"42.0",
			)) as IOdspResolvedUrl;
			...
			assert.equal(
				...
				"42.0",
				"the requested file version is threaded through",
			);
			assert.equal(resolved.driveId, driveId);
			assert.equal(resolved.itemId, itemId);
			assert.equal(resolved.dataStorePath, "/dataStore", "the data-store path is preserved");
			assert.equal(
				resolved.codeHint?.containerPackageName,
				"my-package",
				"the container package name is preserved",
			);
			assert.ok(
				resolved.endpoints.snapshotStorageUrl.includes("versions/42.0/"),
				`the snapshot endpoint targets the requested version, got ${resolved.endpoints.snapshotStorageUrl}`,
			);
		});

		it("omits the container package name when the source URL has none", async () => {
			const factory = new OdspPointInTimeDocumentServiceFactory(getStorageToken, undefined);
			const resolvedUrl = await makeResolvedUrlWithHints({ dataStorePath: "/" });
			...
			assert.equal(resolved.fileVersion, "40.0");
			assert.equal(
				resolved.codeHint?.containerPackageName,
				undefined,
				"no container package name is set when the source URL omits one",
			);
		});
	});
});
