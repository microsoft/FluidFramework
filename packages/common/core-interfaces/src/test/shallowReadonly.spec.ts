...
/**
 * Result is defined using `ShallowReadonly` type generator.
 *
 * @param v - value whose type is passed through `ShallowReadonly`
 * @returns the original value with modified type
 */
function makeReadonly<const T>(v: T) {
	return v as ShallowReadonly<T>;
}
...
function makeReadonlyNoGenericsDeepening<const T>(v: T) {
	return v as ShallowReadonly<T, { DeepenedGenerics: never }>;
}
...
describe("ShallowReadonly", () => {
	...
	describe("arrays become immutable", () => {
		...
		it("array of bigint or basic object", () => {
			...
			assertIdenticalTypes(
				result,
				createInstanceOf<readonly (bigint | { property: string })[]>(),
			);
		});
		it("array of supported types (symbols or basic object)", () => {
			...
			assertIdenticalTypes(
				result,
				createInstanceOf<readonly (symbol | { property: string })[]>(),
			);
		});
		it("array of bigint", () => {
		...
		it("array of functions with properties", () => {
			...
			assertIdenticalTypes(
				result,
				createInstanceOf<readonly ((() => number) & { property: number })[]>(),
			);
		});
		it("array of objects and functions", () => {
			...
			assertIdenticalTypes(
				result,
				createInstanceOf<readonly ({ property: number } & (() => number))[]>(),
			);
		});
	...
	describe("read-only arrays are preserved", () => {
		...
		it("readonly array of mutable object is preserved", () => {
			const result = makeReadonly(readonlyArrayOfObjects);
			assertIdenticalTypes(result, readonlyArrayOfObjects);
		});
	});
	...
	describe("object properties become immutable", () => {
		...
		it("object with array of `number | undefined`", () => {
			...
			assertIdenticalTypes(
				result,
				createInstanceOf<{
					readonly arrayOfNumbersOrUndefined: (number | undefined)[];
				}>(),
			);
		});
		...
		it("object with array of functions", () => {
			...
			assertIdenticalTypes(
				result,
				createInstanceOf<{ readonly arrayOfFunctions: (typeof aFunction)[] }>(),
			);
		});
		it("object with array of functions with properties", () => {
			...
			assertIdenticalTypes(
				result,
				createInstanceOf<{
					readonly arrayOfFunctionsWithProperties: ((() => number) & {
						property: number;
					})[];
				}>(),
			);
		...
		it("object with array of objects and functions", () => {
			...
			assertIdenticalTypes(
				result,
				createInstanceOf<{
					readonly arrayOfObjectAndFunctions: ((() => number) & {
						property: number;
					})[];
				}>(),
			);
		...
		it("object with array of `bigint`s or objects", () => {
			...
			assertIdenticalTypes(
				result,
				createInstanceOf<{
					readonly arrayOfBigintOrObjects: (
						| bigint
						| {
								property: string;
						  }
		...
		it("object with array of `symbol`s or objects", () => {
			...
			assertIdenticalTypes(
				result,
				createInstanceOf<{
					readonly arrayOfSymbolOrObjects: (
						| symbol
						| {
								property: string;
						  }
		...
		it("object with recursion and `symbol`", () => {
			...
			assertIdenticalTypes(
				result,
				createInstanceOf<Readonly<typeof objectWithSymbolOrRecursion>>(),
			);
		});
		...
		it("object with recursion and handle", () => {
			...
			assertIdenticalTypes(
				result,
				createInstanceOf<Readonly<typeof objectWithFluidHandleOrRecursion>>(),
			);
		});
		...
		it("object with possible type recursion through union", () => {
			...
			assertIdenticalTypes(
				result,
				createInstanceOf<Readonly<typeof objectWithPossibleRecursion>>(),
			);
		});
		it("object with optional type recursion", () => {
			...
			assertIdenticalTypes(
				result,
				createInstanceOf<Readonly<typeof objectWithOptionalRecursion>>(),
			);
		});
		it("object with deep type recursion", () => {
			...
			assertIdenticalTypes(
				result,
				createInstanceOf<Readonly<typeof objectWithEmbeddedRecursion>>(),
			);
		});
		it("object with alternating type recursion", () => {
			...
			assertIdenticalTypes(
				result,
				createInstanceOf<Readonly<typeof objectWithAlternatingRecursion>>(),
			);
		});
		...
		it("object with inherited recursion and extended with mutable properties", () => {
			...
			assertIdenticalTypes(
				result,
				createInstanceOf<{
					readonly outer: typeof objectInheritingOptionalRecursionAndWithNestedSymbol;
				}>(),
			);
		});
		...
		it("`string` indexed record of recursion or `number`", () => {
			...
			assertIdenticalTypes(
				result,
				createInstanceOf<Readonly<typeof stringRecordWithRecursionOrNumber>>(),
			);
		});
	...
	describe("built-in or common class instances", () => {
		...
		describe("`IFluidHandle<T>` becomes `Readonly<IFluidHandle<T>>`", () => {
			...
			it("`IFluidHandle<{...}>` generic becomes shallowly immutable by default", () => {
				...
				assertIdenticalTypes(
					...
					createInstanceOf<
						Readonly<
							IFluidHandle<{
								readonly [p: string]: { x: number; y: number };
							}>
						>
					>(),
				);
			...
			it("object with `IFluidHandle<number>`", () => {
				...
				assertIdenticalTypes(
					result,
					createInstanceOf<{
						readonly handle: IFluidHandle<number>;
					}>(),
				);
			});
		...
		describe("that are mutable become immutable version", () => {
			describe("Map becomes ReadonlyMap", () => {
				...
				it("Map<object, object> generics are intact by default", () => {
			...
			describe("Set becomes ReadonlySet", () => {
				...
				it("Set<object> generics are intact by default", () => {
					const result = makeReadonly(setOfRecords);
					assertIdenticalTypes(result, readonlySetOfRecords);
				});
			});
		});
		describe("that are immutable make generics shallowly immutable when enabled", () => {
		...
		describe("that are immutable keep generics intact by default", () => {
			it("ReadonlyMap<object, object>", () => {
