...
export interface IContainer extends IEventProvider<IContainerEvents> {
	 ...
	 *
	 * To finish the creation of a container and make it available for other users to open and collaborate with
	 * it must be attached, which means it is uploaded to server storage. This function accomplishes that.
	 *
	 ...
	 * If the container is not closed, it is safe to continue editing the container, calling serialize to capture
	 * the container's state, or try calling attach again.
	 *
	 */
	attach(
		request: IRequest,
	 ...
	 * This is useful in draft-like scenarios. For example, think of an email draft, where a user can make changes
	 * over time, but no other users can see or have access to that draft.
	 *
	 ...
	 * @remarks
	 * Fluid supports rehydrating from strings generated by previous versions, but does not support the content
	 * itself being leveraged.
	 *
	 * Consider storing this string somewhere durable (e.g. localStorage or IndexedDB) to enable your application
	 * to recover it after tab close.
	 *
	 * To reload a container from a string your application previously stored, use the Loader method
	 * rehydrateDetachedContainerFromSnapshot. That method will create a new detached container which matches
	 * the state of the previously serialized container.
	 */
	serialize(): string;
	 ...
	 * @remarks
	 *
	 * {@link IContainer.connectionState} will be set to {@link (ConnectionState:namespace).(Connected:variable)},
	 * and the "connected" event will be fired if/when connection succeeds.
	 */
	connect(): void;
	 ...
	 * @remarks
	 *
	 * {@link IContainer.connectionState} will be set to {@link (ConnectionState:namespace).(Disconnected:variable)},
	 * and the "disconnected" event will be fired when disconnection completes.
	 */
	disconnect(): void;
	 ...
	 * This is full set of metadata props which the container wants to expose.
	 * Whenever container receives updates from `IContainerEvents.metadataUpdate` event, we overwrite only those
