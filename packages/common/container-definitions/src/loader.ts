...
export interface IContainer extends IEventProvider<IContainerEvents> {
	 ...
	 *
	 * By default an attach failure will result in the closure of the container.
	 ...
	 *
	 * In the event of this method throwing an exception, you should check if the container is closed via the closed property.
	 ...
	 *
	 * Before calling this function you should check the closed property and the attachState property of the container.
	 * The container's closed property should be false, and the attachState should not be AttachState.Attached.
	 *
	 * If those conditions are met, this function will return a string that includes the full serialized state of the
	 * container. This string should not be inspected or modified, as we make no guarantees about the stability of its content.
	 * That said, we will support strings generated by previous versions, we just do not support the content itself being leveraged.
	 *
	 * You should store this string somewhere your application can recover it, like local storage or index db.
	 *
	 * To reload a container from a string your application previously stored you should use the Loader method rehydrateDetachedContainerFromSnapshot.
	 * This method will create a new detached container which matches the state of the previously serialized container.
	 */
	serialize(): string;
