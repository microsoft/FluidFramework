...
export interface IContainerContextInternal extends IContainerContext {
	 ...
	 * identity to a sequence number). The range is `[from, to)`: `from` is inclusive and `to` is exclusive;
	 * omitting `to` leaves the range unbounded. `abortSignal` cancels an in-flight fetch when the runtime
	 * stops reading early. Optional: absent means the runtime has no historical-op read.
	 */
	readonly fetchOps?: (
		from: number,
		to?: number,
