...
describe("DoublyLinkedList", () => {
	...
	describe(".splice", () => {
		for (const splicePos of [
			-1,
			0,
			1,
			Math.floor(listCount / 2),
			listCount,
			listCount - 1,
		]) {
			it(`splice at position ${splicePos}`, () => {
				...
				const spliceIndex =
					splicePos < 0 ? nodesArray.length + splicePos : splicePos;
				...
				const listSplice =
					spliceNode === undefined
						...
						: list.splice(spliceNode);

				assert.equal(
					list.length,
					nodesArray.length,
					"remaining lengths don't match",
				);
				assert.equal(
					listSplice.length,
					arraySplice.length,
					"spliced lengths don't match",
				);
