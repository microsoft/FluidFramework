 ...
 */
export const defaultLoadModelOptions: Required<
	Omit<LoadModelOptions, "modelDirectoryPath">
> = {
...
export async function loadModel(options: LoadModelOptions): Promise<ApiModel> {
	...
	if (!(await FileSystem.existsAsync(modelDirectoryPath))) {
		throw new Error(
			`Provided directory does not exist: "${modelDirectoryPath}".`,
		);
	}
...
function applyInheritDoc(
	...
	if (apiItem instanceof ApiDocumentedItem && apiItem.tsdocComment) {
		// eslint-disable-next-line unicorn/prevent-abbreviations
		const inheritDocTag: DocInheritDocTag | undefined =
