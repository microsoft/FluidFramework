 ...
 *
 * @param tempDirectoryPath - Resolved path to the directory containing the freshly generated test output.
...
export async function compareDocumentationSuiteSnapshot(
	...
	await FileSystem.ensureFolderAsync(tempDirectoryPath);
	await FileSystem.ensureFolderAsync(snapshotDirectoryPath);

	// Clear any existing test_temp data
	await FileSystem.ensureEmptyFolderAsync(tempDirectoryPath);
	...
	const result = await compare(tempDirectoryPath, snapshotDirectoryPath, {
		compareContent: true,
	});
