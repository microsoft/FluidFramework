...
-   Improved error reporting: if a benchmark function exits before data collection is complete (e.g. by never calling `state.time()`, or by breaking out of the timing loop while it is still returning `true`), an error is now thrown with a descriptive message.
