...
module.exports = class extends Generator {
    ...
    moveBuildFiles() {
        ...
        this.fs.copy(
            this.templatePath("tsconfig.esnext.json"), // FROM
            this.destinationPath("tsconfig.esnext.json") // TO
        );
    ...
    _copyComponent() {
        const componentFilePath =
            this.answers.template === "react" ? "src/main.tsx" : "src/main.ts";
        const fileString = this.fs.read(this.templatePath(componentFilePath));

        const project = new Project({});

        const file = project.createSourceFile(this.destinationPath(componentFilePath), fileString);
        ...
        if (this.answers.container === "no") {
            ...
            file.getVariableDeclaration("DiceRollerInstantiationFactory").rename(
                this._componentFactoryClassName()
            );
        }
    ...
    async end() {
        ...
        this.log("       Publishes the component to https://packages.wu2.prague.office-int.com/#/");
        this.log("\n");

        this.log(
            "We suggest you open your component with your favorite IDE.\n Then start by typing:"
        );
