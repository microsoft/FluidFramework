...
test("loads a complete configuration", async () => {
	...
	try {
		await writeJson(directory, "inventory.json", {
			tenants: {
				azureFluidRelayTenant: { selfHostTenantId: "selfHostTenant", documents: ["document"] },
			},
			errors: [],
		});
		...
		assert.equal(
			result.inventory.tenants.azureFluidRelayTenant.selfHostTenantId,
			"selfHostTenant",
		);
...
test("uses the Azure Fluid Relay tenant ID when selfHostTenantId is missing", async () => {
	...
	try {
		await writeJson(directory, "inventory.json", {
			tenants: { azureFluidRelayTenant: { documents: ["document"] } },
			errors: [],
		});
		await writeJson(directory, "config.json", {
			inventoryPath: "inventory.json",
		...
		const result = await loadConfiguration(path.join(directory, "config.json"));
		assert.deepEqual(result.warnings, [
			"Inventory tenant azureFluidRelayTenant has no selfHostTenantId; using the Azure Fluid Relay tenant ID",
		]);
	} finally {
		await rm(directory, { recursive: true, force: true });
	}
	...
	try {
		...
		await assert.rejects(loadConfiguration(path.join(directory, "config.json")), (error) => {
			...
			return error.message.includes(
				"azureFluidRelayTenants must include inventory tenant azureFluidRelayTenant",
			);
		});
	} finally {
		await rm(directory, { recursive: true, force: true });
