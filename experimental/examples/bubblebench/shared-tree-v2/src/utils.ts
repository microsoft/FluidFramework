...
} from "@fluid-internal/tree/dist/dependency-tracking";
import { Delta, ISharedTree, SharedTreeFactory } from "@fluid-internal/tree";
import {
    mapFieldMarks,
    mapMarkList,
    mapTreeFromCursor,
...
export class TestTreeProvider {
    ...
    public static async create(
        ...
        // The on-demand summarizer shares a container with the first tree, so at least one tree
        // and container must be created right away.
        assert(
    ...
    public async createTree(): Promise<ISharedTree> {
        ...
        return (this._trees[this.trees.length] =
            await dataObject.getSharedObject<ISharedTree>(
                TestTreeProvider.treeId,
            ));
    }
 ...
 */
export function assertDeltaEqual(
    a: Delta.FieldMarks,
    b: Delta.FieldMarks,
): void {
