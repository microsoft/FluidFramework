...
import { bubbleSchema, clientSchema, numberSchema, stringSchema } from "./schema";
...
export class AppState implements IAppState {
	...
	public get localClient() {
		...
		const localClientNode = treeNodes.filter(
			(treeNode) => treeNode.getFieldValue(Client.clientIdFieldKey) === this.localClientId,
		);
	...
	makeClientInitialJsonTree(numBubbles: number) {
		...
		const clientInitialJsonTree: JsonableTree = {
			...
			fields: {
				clientId: [{ type: stringSchema.name, value: clientId }],
	...
	public get clients() {
		...
		return treeNodes.map(
			(treeNode) => new Client(this.tree, treeNode.anchor, this.shortTermSpawnedAnchors),
		);
	}
