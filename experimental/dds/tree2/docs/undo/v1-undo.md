...
There are many ways this could be accomplished, and will be the subject of another document.
We propose the following approach for now.

Do not garbage-collect repair data associated with edits that...

1. are still in the collaboration window
2. were created by the local session

The repair data associated with any other edit should be garbage-collected.
...
The one drawback,
is that a client will not have the required repair data to apply an undo edits generated by a peer
