...
export function runExhaustiveComposeRebaseSuite<TContent, TChangeset>(
	...
	describe.skip("Transaction rebase over single edit", () => {
		 ...
		 *
		 * Given a trunk edit T and a series of local edits L1, ..., Ln, this test
		 */
		for (const initialState of initialStates) {
			outerFixture(`starting with contents ${JSON.stringify(initialState.content)}`, () => {
				const intentionMinter = makeIntentionMinter();
				...
				for (const namedSourceEdits of localEdits) {
					for (const [
						...
						function rebaseEdits(
							...
							const rebasedSourceEdits: TaggedChange<TChangeset>[] = [];
							...
							for (let i = 0; i < sourceEdits.length; i++) {
								...
								rebasedSourceEdits.push(rebaseTagged(edit, ...editsToRebaseOver));
							}

							return rebasedSourceEdits;
						}
						...
						innerFixture(title, () => {
							...
							const rebasedSourceEdits: TaggedChange<TChangeset>[] = rebaseEdits(
								sourceEdits,
								editToRebaseOver,
							);

							const transactStrategies: ((
								...
								(edits) => {
									...
									const firstEdit = tagChange(
										compose([
											tagChange(edits[0].change, undefined),
											tagChange(edits[1].change, undefined),
										]),
										`[${edits[0].revision}, ${edits[1].revision}]` as RevisionTag,
									);
									const transactedEdits: TaggedChange<TChangeset>[] = [
										firstEdit,
										...edits.slice(2),
									];
									return transactedEdits;
								},
							];

							for (const transact of transactStrategies) {
								const transactionifiedEdits = transact(sourceEdits);
								const transactionifyThenRebase = rebaseEdits(
									transactionifiedEdits,
									editToRebaseOver,
								);
								const rebaseThenTransactionify = transact(rebasedSourceEdits);

								for (let i = 0; i < transactionifyThenRebase.length; i++) {
									assertDeepEqual(
										transactionifyThenRebase[i],
										rebaseThenTransactionify[i],
									);
								}

								verifyComposeAssociativity(transactionifyThenRebase);
							}
						});
					}
				}
