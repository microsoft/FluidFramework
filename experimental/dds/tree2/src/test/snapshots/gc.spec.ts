...
function getNewTreeView(
  tree: ISharedTree,
): TreeView<TreeField<typeof schema.rootFieldSchema>> {
  ...
  class GCSequenceProvider implements IGCTestProvider {
    ...
    public async addOutboundRoutes() {
      ...
      this._expectedRoutes.push(
        subtree1.handle.absolutePath,
        subtree2.handle.absolutePath,
      );
    ...
    public async deleteOutboundRoutes() {
      assert(
        this.tree1View.handles.length > 0,
