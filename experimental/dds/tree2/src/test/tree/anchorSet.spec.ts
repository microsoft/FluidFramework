...
describe("AnchorSet", () => {
	...
	it("can move within field", () => {
		...
		visitDelta(delta, anchors.getVisitor());
	...
	it("can rebase over insert", () => {
		...
		visitDelta(makeDelta(insert, makePath([fieldFoo, 4])), anchors.getVisitor());
	...
	it("can rebase over delete", () => {
		...
		visitDelta(makeDelta(deleteMark, makePath([fieldFoo, 4])), anchors.getVisitor());
	...
	it("can rebase over delete of parent node", () => {
		...
		visitDelta(makeDelta(deleteMark, makePath([fieldFoo, 5])), anchors.getVisitor());
		...
		checkEquality(anchors.locate(anchor2), path2);
		visitDelta(makeDelta(deleteMark, makePath([fieldFoo, 3])), anchors.getVisitor());
	...
	it("triggers events", () => {
		...
		log.expect([]);
		visitDelta(new Map([[rootFieldKey, [0, deleteMark]]]), anchors.getVisitor());
		...
		visitDelta(new Map([[rootFieldKey, [deleteMark, insertMark]]]), anchors.getVisitor());
		...
		visitDelta(makeDelta(insertMark, makePath([rootFieldKey, 0], [fieldFoo, 5])), anchors.getVisitor());
	...
	it("triggers path visitor callbacks", () => {
		...
		const log = new UnorderedTestLogger();
		const anchors = new AnchorSet();
		visitDelta(makeDelta(insertMark, makePath([rootFieldKey, 0], [fieldFoo, 3])), anchors.getVisitor());
		const anchor0 = anchors.track(makePath([rootFieldKey, 0]));
		const node0 = anchors.locate(anchor0) ?? assert.fail();
		...
		visitDelta(makeDelta(insertMark, makePath([rootFieldKey, 0], [fieldFoo, 4])), anchors.getVisitor());
		log.expect([["visitSubtreeChange.onInsert-foo-4", 1]]);
		log.clear();
		visitDelta(makeDelta(deleteMark, makePath([rootFieldKey, 0], [fieldFoo, 5])), anchors.getVisitor());
