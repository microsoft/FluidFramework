...
import { setupTest, pretty } from "./utils";
...
describe("SharedTreeObject", () => {
	...
	it("can read and write handles", async () => {
		const root = setupTest(schema, initialTree);
		assert.equal(await root.handle.get(), 42);

		root.handle = new MockHandle(43);
		assert.equal(await root.handle.get(), 43);
	});
...
describe("SharedTreeList", () => {
	describe("inserting nodes created by factory", () => {
		...
		it("insertAtStart()", () => {
			const list = setupTest(schema, [{ id: "B" }]);
		...
		it("insertAtEnd()", () => {
			const list = setupTest(schema, [{ id: "A" }]);
		...
		it("insertAt()", () => {
			const list = setupTest(schema, [{ id: "A" }, { id: "C" }]);
	...
	describe("removing items", () => {
		...
		it("removeAt()", () => {
			const list = setupTest(schema, [0, 1, 2]);
			assert.deepEqual(list, [0, 1, 2]);
			list.removeAt(1);
		...
		it("removeRange()", () => {
			const list = setupTest(schema, [0, 1, 2, 3]);
	...
	describe("moving items", () => {
		describe("within the same list", () => {
			...
			it("moveToStart()", () => {
				const list = setupTest(schema, initialTree);
				assert.deepEqual(list, [0, 1, 2, 3]);
				list.moveToStart(1);
		...
		describe("between different lists", () => {
			...
			it("moveToStart()", () => {
				const { listA, listB } = setupTest(schema, initialTree);
