...
describe("SequenceField - Compose", () => {
	...
	it("move1, move2, return2", () => {
		for (const [a, b, c] of [
			...
			const move1: TaggedChange<SF.Changeset<never>> = tagChange(
				Change.move(a, 1, b > a ? b + 1 : b),
				tag1,
			);
			const move2: TaggedChange<SF.Changeset<never>> = tagChange(
				Change.move(b, 1, c > b ? c + 1 : c),
				tag2,
			);
			const return2: TaggedChange<SF.Changeset<never>> = tagRollbackInverse(
	...
	it("move1 ○ [return1, move2]", () => {
		for (const [a, b, c] of [
			...
			const return1: TaggedChange<SF.Changeset<never>> = tagRollbackInverse(
				Change.return(b, 1, a > b ? a + 1 : a, { revision: tag1, localId: brand(0) }),
				tag2,
				tag1,
			);
			const move2: TaggedChange<SF.Changeset<never>> = tagChange(
				Change.move(a, 1, c > a ? c + 1 : c),
				tag3,
			);
			const part2 = shallowCompose([return1, move2]);
			const composed: SF.Changeset<never> = shallowCompose(
	...
	it("[move1, move2] ○ [return2, move3]", () => {
		for (const [a, b, c, d] of [
			...
			const move3: TaggedChange<SF.Changeset<never>> = tagChange(
				Change.move(b, 1, d > b ? d + 1 : d),
				tag4,
			);
