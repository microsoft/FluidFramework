...
describe("SharedTree", () => {
	...
	describe("can tolerate incomplete transactions when attaching", () => {
		...
		it("when aborted", async () => {
			...
			tree1.editableTree.insertAt(0, ["D"]);
			// Check that the joining tree was initialized with data from the attach summary
			assert.deepEqual(tree2.editableTree.asArray, []);

			await provider.ensureSynchronized();
			assert.deepEqual(tree1.editableTree.asArray, ["D"]);
			assert.deepEqual(tree2.editableTree.asArray, ["D"]);

			// Check that further edits are interpreted properly
			tree1.editableTree.insertAt(1, ["B"]);
			await provider.ensureSynchronized();
			assert.deepEqual(tree1.editableTree.asArray, ["D", "B"]);
			assert.deepEqual(tree2.editableTree.asArray, ["D", "B"]);
		});
