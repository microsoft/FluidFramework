...
export function setAdjacentCells(cellId: CellId, adjacentCells: IdRange[]): void {
	assert(cellId.adjacentCells === undefined, 0x74e /* Should not overwrite adjacentCells */);
	// Only set adjacentCells if there actually are adjacent cells.
	if (adjacentCells.length > 0 && (adjacentCells.length > 1 || adjacentCells[0].count > 1)) {
