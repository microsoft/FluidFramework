...
import { CursorLocationType, FieldKey, FieldUpPath, moveToDetachedField, UpPath } from "../core";
import { ISharedTreeView, SharedTreeFactory } from "../shared-tree";
import { brand, fail } from "../util";
import { AsyncGenerator, Generator, done, Weights, IRandom } from "./types";
...
import { DDSFuzzTestState } from "./ddsFuzzHarness";
...
export const makeFieldEditGenerator = (
	...
	function fieldEditGenerator(state: FuzzTestState): FieldEditTypes {
		...
		assert(fieldPath.parent !== undefined, "parent of fieldpath must be defined.");
...
function getExistingFieldPath(tree: ISharedTreeView, random: IRandom): FieldPathWithCount {
	...
	assert(cursor.mode === CursorLocationType.Fields, "cursor must be in Fields mode.");
