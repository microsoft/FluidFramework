...
describe("EditLog", () => {
	...
	it("can be constructed from sequenced edits", () => {
		...
		expect(log.numberOfLocalEdits).to.equal(
			0,
			"Newly initialized log should not have local edits.",
		);
		...
		expect(log.getIdAtIndex(0)).to.equal(
			id0,
			"Log should have edits in order of construction.",
		);
		expect(log.getIdAtIndex(1)).to.equal(
			id1,
			"Log should have edits in order of construction.",
		);
	});
	...
	describe("tryGetIndexOfId", () => {
		...
		it("returns undefined when queried with a nonexistent edit", () => {
			...
			const log = new EditLog({ editChunks, editIds });
			expect(log.tryGetIndexOfId("aa203fc3-bc28-437d-b01c-f9398dc859ef" as EditId)).to.equal(
				undefined,
			);
		});
	});
	...
	it("can sequence a local edit", async () => {
		...
		expect(editFromId0).to.not.be.undefined;
		expect(assertNotUndefined(editFromId0).id).equals(
			edit0.id,
			"Log should contain local edit.",
		);
		...
		expect(assertNotUndefined(editFromId0).id).equals(
			edit0.id,
			"Log should contain sequenced edit.",
		);
	});
	...
	it("can sequence multiple local edits", async () => {
		...
		for (let i = 0; i < numEdits; i++) {
			...
			ids.push(edit.id);
			expect(log.getIndexOfId(edit.id)).equals(
				i,
				"Local edits should be appended to the end of the log.",
			);
		}
		expect(log.length)
			.equals(log.numberOfLocalEdits)
			.and.equals(numEdits, "Only local edits should be present.");

		log.sequenceLocalEdits();

		expect(log.editIds).to.deep.equal(
			ids,
	...
	it("can correctly compare equality to other edit logs", () => {
		...
		const editChunks = [{ startRevision: 0, chunk: [editWithoutId0, editWithoutId1] }];
		const editChunksCopy = [
			{ startRevision: 0, chunk: [editWithoutId0Copy, editWithoutId1Copy] },
		];
		const editIds = [id0, id1];
	...
	it("can handle sparse sequence numbers with a minimum sequence number that's not in memory", () => {
		...
		const minimumSequenceNumber =
			sequenceNumber - 1 - extraEditsToKeep * sequenceNumberInterval;
		for (let i = 0; i < targetEditLogSize; i++) {
			const edit = newEdit([]);
			log.addSequencedEdit(edit, {
				sequenceNumber,
