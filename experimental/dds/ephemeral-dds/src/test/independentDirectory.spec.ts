...
const dirNode = {} as IndependentDirectoryNode<IndependentValue<{ x: number }>>;
declare function createLatestValueManager<T>(
	intitial: Serializable<T>,
): IndependentValue<Serializable<T>>;

const dirImplX = createEphemeralIndependentDirectory(
	// eslint-disable-next-line @typescript-eslint/consistent-type-assertions
	{} as IFluidDataStoreRuntime,
	{
		cursor: createLatestValueManager({ x: 0, y: 0 }),
		// eslint-disable-next-line @typescript-eslint/consistent-type-assertions
		camera: {} as IndependentValue<{ x: number; y: number; z: number }>,
	},
);
// Workaround ts(2775): Assertions require every name in the call target to be declared with an explicit type annotation.
const dirImpl: typeof dirImplX = dirImplX;
dirImpl.add("caret", createLatestValueManager({ id: "", pos: 0 }));
