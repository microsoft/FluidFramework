...
let syntheticOptions = (yargs) => {
    ...
    yargs.describe(
        "sizePerAsset",
        "Size of an individual asset in a commit [KB]"
    );
...
let argv = require("yargs")
    ...
    .command(
        "synthetic",
        "Create a branch with synthetic test data",
        syntheticOptions
    )
...
require("./server.js").then(async (materializedHistoryServer) => {
    ...
    let generateGUID = require("@fluid-experimental/property-common").GuidUtils
        .generateGUID;
    ...
    if (command === "synthetic" || command === "all") {
        ...
        let dummyString = _.range(100)
            .map(() => "x")
            .join("");
        let desiredPayLoadSize =
            argv.sizePerAsset - (argv.useGuids ? 154 : 130);
        ...
        await createBranchWithCommits({
            ...
            commitFunction: async (i) => {
                if (i <= numCommits) {
                    ...
                    while (
                        insertedAssets <
                        argv.sizePerCommit / argv.sizePerAsset
                    ) {
                        let assetName =
                            "asset_" + padNumber(globalAssetCounter++, 5);
            ...
            console.log(
                "Size of final commit CS: " + Math.floor(totalFinalSize / 1024)
            );
            console.log(
                "Average leaf size: " +
                    Math.floor(totalFinalSize / 1024 / leafs.length)
            );
        });
    }
});
