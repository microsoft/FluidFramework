 ...
 */
const RedisBranchAssignationsMH =
    require("hfdm-redis-client").RedisBranchAssignationsMH;
...
const processArgs = function () {
    const printUsageAndExit = function () {
        console.log(
            'Usage: node get_mh_instance_for_branch.js "<BRANCH_GUID>"'
        );
        process.exit(1);
    };
...
const doWork = async function () {
    const instance = await branchAssignationsMH.getMhInstanceForBranch(
        branchGuid
    );
