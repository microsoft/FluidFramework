...
const processArgs = () => {
    const argv = yargs
        ...
        .option("power", {
            ...
            default: 3,
            description:
                "Order of magnitude of the amount of commits to import",
            type: "number",
        })
        .option("folder", {
            alias: "f",
...
const doWork = async (a) => {
    ...
    let i = 1;
    do {
        commitFilePath = path.join(
            a.folder,
            `${i.toString().padStart(a.power, "0")}.json`
        );
        ...
        if (fileExists) {
            ...
            fetched = await Fixtures.fetchMaterializedView(
                a.branchGuid,
                commit.guid,
                headers
            );
            expect(fetched.changeSet).to.eql(
                JSON.parse(resultChangeSet.toString())
            );
