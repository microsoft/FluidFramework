...
class RepairManager {
    ...
    async scanAndRepairBranches(params) {
        ...
        do {
            ...
            for (const branchGuid of filteredResult.branches) {
                const branchInfo = await this._hfdmClassicClient.getBranch(
                    branchGuid
                );
                if (branchInfo.branch.parent) {
                    logger.trace(
                        `Pushing back branch '${branchGuid}' because it has a parent`
                    );
    ...
    async _makeBranchConsistent({ branchGuid, since }) {
        ...
        if (!mhEnabled) {
            logger.trace(
                `Branch '${branchGuid}' does not have Materialized History enabled`
            );
            return;
        }
        ...
        localBranch.headCommitGuid = await this._getLatestConsistentCommit(
            localBranch,
            since
        );
        logger.trace(
    ...
    async _updateBranchHead(branch) {
        ...
        this._mhService._storageManager.update(
            batch,
            "branch:" + branch.guid,
            branch
        );
    ...
    async _forceRepairBranch(branch, commitGuid) {
        ...
        while (!finished) {
            ...
            for (const commit of response.commits) {
                const createCommitReq = _.pick(commit, [
                    "guid",
                    "meta",
                    "changeSet",
                ]);
                ...
                await this._mhService._commitManager.createCommit(
                    createCommitReq
                );
                commitsApplied++;
        ...
        logger.trace(
            `Applied ${commitsApplied} commits on branch ${branch.guid}`
        );
    }
    ...
    async _getLatestConsistentCommit(branch, since) {
        ...
        while (
            ...
            (consecutiveConsistentCommits < CONSISTENT_COMMIT_LIMIT &&
                commitDate >= since)
        ) {
