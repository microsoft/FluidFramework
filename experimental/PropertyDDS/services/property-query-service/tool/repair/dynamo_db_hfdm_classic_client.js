...
class DynamoDBHfdmClassicClient {
    ...
    constructor() {
                ...
                systemMonitor: pm
                    ...
                    .createInstance(
                        ...
                        ModuleLogger.getLogger(
                            "HFDM.MaterializedHistoryRepair.SystemMonitor"
                        ),
                        "mr",
        ...
        this._propertyGraph = PropertyGraphManager.getPropertyGraph(
            storeSettings,
            dependencies
        );
    }
    ...
    async getBranch(branchGuid) {
        const branchesResult = await this._propertyGraph.getBranches([
            branchGuid,
        ]);
        ...
        result.repository.rootCommit =
            repositoriesResult.repositories[0].rootCommit;
        return result;
    }
    ...
    async scanBranches(params) {
        ...
        if (params.lastEvaluatedKey) {
            scanParams.ExclusiveStartKey = AWS.DynamoDB.Converter.marshall(
                params.lastEvaluatedKey
            );
        }
        const scanResult = await new Promise((resolve, reject) => {
            CredentialRotation.ddbClient._dynamodb.scan(
                scanParams,
                (err, result) => {
                    if (err) {
                        reject(err);
        ...
        result.branches = scanResult.Items.filter((branchRow) => {
            ...
            const meta =
                uRow.meta && uRow.meta.data && JSON.parse(uRow.meta.data);
            return !!(
                meta &&
                meta.materializedHistory &&
                meta.materializedHistory.enabled
            );
