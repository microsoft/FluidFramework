...
} = require("@fluidframework/local-driver");
const { requestFluidObject } = require("@fluidframework/runtime-utils");
const {
    LocalDeltaConnectionServer,
} = require("@fluidframework/server-local-server");
const {
...
function createDerivedGuid(referenceGuid, identifier) {
    ...
    return (
        ...
        `${hexHash.substr(12, 4)}-${hexHash.substr(16, 4)}-${hexHash.substr(
            20,
            12
        )}`
    );
...
describe("Rebasing", () => {
            ...
            it(`Generated Test Case #${i} (seed: ${seed})`, async () => {
                ...
                for (const operation of operations) {
                    operationCumSums.push(
                        (operationCumSums[operationCumSums.length - 1] != null
                            ? operationCumSums[operationCumSums.length - 1]
                            : 0) + operation.probability
                    );
                }
                ...
                try {
                    ...
                    for (const j of _.range(numOperations)) {
                        ...
                        const selectedOperation = _.sortedIndex(
                            operationCumSums,
                            operationId
                        );
    ...
    beforeEach(async () => {
        ...
        const old_applyRemoteChangeSet1 =
            sharedPropertyTree1._applyRemoteChangeSet.bind(sharedPropertyTree1);
    ...
    describe("with non overlapping inserts", () => {
        ...
        afterEach(async () => {
            ...
            const mv = await mhService.getCommitMV({
                ...
                [
                    {
                        getParameters: (random) => {
                            ...
                            return {
                                position:
                                    random.irandom(array.getLength()) || 0,
