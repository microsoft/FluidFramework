...
describe("Commit creation integration test", function () {
        ...
        describe("with two commits", () => {
            ...
            describe("Materialized checkout", () => {
                it("should fail with an invalid request signature", () => {
                    return expect(
                        Fixtures.fetchMaterializedView(
                            createdBranchGuid,
                            secondCommitGuid,
                            {},
                            {}
                        ...
                        changeSet: {
                            insert: {
                                String: {
                                    ...
                                    aLargeString: Array(LARGE_STRING_SIZE)
                                        .fill("c")
                                        .join(""),
                                    aThirdLargeString: Array(LARGE_STRING_SIZE)
                                        .fill("d")
                                        .join(""),
                                },
                                "Reference<String>": {
                    ...
                    expect(fetchResult).to.eql({
                        changeSet: {
                            ...
                            modify: {
                                String: {
                                    aLargeString: {
                                        oldValue: Array(LARGE_STRING_SIZE)
                                            .fill("a")
                                            .join(""),
                                        value: Array(LARGE_STRING_SIZE)
                                            .fill("c")
                                            .join(""),
                                    },
                                },
                            ...
                            remove: {
                                String: {
                                    aSecondLargeString: Array(LARGE_STRING_SIZE)
                                        .fill("b")
                                        .join(""),
                                },
                            },
            ...
            return expect(
                Fixtures.createCommit(createdBranchGuid, headers, {
                    ...
                    changeSet: JSON.stringify(
                        chunkableChangeSetWithEmptyProperty
                    ),
                    meta: {},
                    created: Date.now(),
                })
    ...
    describe("On a separate branch", () => {
        ...
        it("should not hang when a change set modifies non-existing paths", async () => {
            ...
            await expect(
                ...
                "Invalid path in ChangeSet: assets[FolderAsset-0001].entries"
            );
        });
    });
    ...
    describe("On a separate branch", () => {
        ...
        it("should correctly process enums, even in chunk boundaries", async () => {
            ...
            for (let i = 0; i < 4000; i++) {
                ...
                insertEnum.modify.NodeProperty.container.insert[
                    "enum<my.sample:status-1.0.0>"
                ][key] = i % 3;
            }
            ...
            const mv = await Fixtures.fetchMaterializedView(
                createdBranchGuid,
                commitGuid,
                headers
            );
    ...
    describe("On a separate branch", () => {
        ...
        it("should support querying and sorting paths with special characters", async () => {
            ...
            for (let i = 0; i < 10; i++) {
                insertChangeSet.insert.String[`/does: "this=work?${i}`] =
                    "I hope so";
            }
            let headers =
                Fixtures.getRequestSignatureHeaders(createdBranchGuid);
