...
describe("Branch deletion integration test", function () {
    ...
    for (let i = 0; i < 1000; i++) {
        firstChangeSet.insert.String["aString" + i] = Array(1024)
            .fill("a")
            .join("");
    }
            ...
            describe("with two commits", () => {
                ...
                describe("with an invalid request signature", () => {
                    it("should reject the creation request", () =>
                        expect(
                            Fixtures.deleteBranches([createdBranchGuid], {})
                        ).to.be.rejectedWith(
                            Error,
                ...
                describe("Branch deletion", () => {
                    ...
                    it("should call the branch deletion", async () => {
                        let headers =
                            Fixtures.getRequestSignatureHeaders(
                                createdBranchGuid
                            );
                        taskInfo = await Fixtures.deleteBranches(
                            [createdBranchGuid],
                            headers
                        );
                    });
                    ...
                    it("should poll the task until completion", async () => {
                        do {
                            ...
                            taskInfo = await Fixtures.fetchDeleteTask(
                                taskInfo.taskUrl,
                                headers
                            );
                    ...
                    it("should reject the poll without a signature", () =>
                        expect(
                            Fixtures.fetchDeleteTask(taskInfo.taskUrl, {})
                        ).to.be.rejectedWith(
                            Error,
                            "Request signature algorithm not supported"
                        ));
    ...
    describe("For a branch not existing", () => {
        ...
        describe("Branch deletion", () => {
            ...
            it("should poll the task until completion", async () => {
                do {
                    let headers =
                        Fixtures.getRequestSignatureHeaders(branchGuid);
                    taskInfo = await Fixtures.fetchDeleteTask(
                        taskInfo.taskUrl,
                        headers
                    );
            ...
            it("should return a 404 when trying to fetch the branch", function () {
                let headers = Fixtures.getRequestSignatureHeaders(branchGuid);
                return expect(
                    Fixtures.fetchBranch(branchGuid, headers)
                ).to.be.rejectedWith(Error, "Not Found");
            });
        });
    });
        ...
        describe("On a properly created branch", () => {
            ...
            describe("Branch deletion", () => {
                ...
                it("should modify the branch to a non-existent head commit", async () => {
                    let storageManager =
                        server._materializedHistoryService._storageManager;
                    let batch = storageManager.startWriteBatch();
                    let branch = await storageManager.get(
                        `branch:${branchGuid}`
                    );
    ...
    describe("With an existing task for a branch", () => {
        describe("On a properly created branch", () => {
            before(async () => {
                let headers = Fixtures.getRequestSignatureHeaders(
            ...
            describe("with two commits", () => {
                ...
                describe("Branch deletion", () => {
                    ...
                    it("should populate a task object in DynamoDB", async () => {
                        let deletionManager =
                            server._materializedHistoryService._deletionManager;
                        await deletionManager._writeTaskStatus(theFailedTask);
                        await deletionManager._writeTaskPerBranch(
                            theFailedTask
                        );
                    });
            ...
            describe("with two commits", () => {
                ...
                describe("Branch deletion retry", () => {
                    ...
                    it("should have deleted all the nodes", () => {
                        let storageManager =
                            server._materializedHistoryService._storageManager;
                        let fetchPromises = theFailedTask.nodesToDelete.map(
                            (nodeRef) =>
                                expect(
                                    storageManager.get(nodeRef)
    ...
    describe("when taking a closer look at what actually gets deleted", () => {
        const compareRefSets = (expected, actual) => {
            ...
            expectedUS = new Set(
                expected.filter((ref) => !ref.startsWith("delete"))
            );
        ...
        before(() => {
            ...
            sandbox
                .stub(backend, "store")
                .callsFake(async (batch, ref, value) => {
                    storedRefs.push(ref);
            ...
            sandbox
                .stub(backend, "update")
                .callsFake((batch, ref, value, extra) => {
