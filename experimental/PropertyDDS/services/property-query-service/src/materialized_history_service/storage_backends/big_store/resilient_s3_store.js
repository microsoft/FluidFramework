...
class ResilientS3Store extends BigStore {
    ...
    constructor(params) {
        super(
            params,
            StoreLocation.S3,
            ModuleLogger.getLogger("HFDM.BigStore.ResilientS3Store")
        );
    ...
    async delete(key) {
        ...
        this._regionalS3s.forEach((regionalStore) => {
            deletePromises.push(
                this._regionalCall(regionalStore, "delete", [key])
            );
        });
    ...
    async deleteAll(keys) {
        ...
        this._regionalS3s.forEach((regionalStore) => {
            deletePromises.push(
                this._regionalCall(regionalStore, "deleteAll", [keys])
            );
        });
    ...
    async exists(key) {
        ...
        return Promise.mapSeries(this._regionalS3s, async (regionalStore) => {
            if (!existsResult.result) {
                try {
                    existsResult.result = await this._regionalCall(
                        regionalStore,
                        "exists",
                        [key]
                    );
    ...
    async getObject(key) {
        ...
        return Promise.mapSeries(this._regionalS3s, async (regionalStore) => {
            if (!getObjectResult.result) {
                try {
                    getObjectResult.result = await this._regionalCall(
                        regionalStore,
                        "getObject",
                        [key]
                    );
    ...
    async putObject(key, buffer, tags) {
        ...
        let putPromise = async (regionalStore) => {
            ...
            try {
                await this._regionalCall(regionalStore, "putObject", [
                    key,
                    buffer,
                    tags,
                ]);
        ...
        if (firstError) {
            if (successfulRegions.length > 0) {
                try {
                    await Promise.all(
                        _.map(successfulRegions, (regionalStore) =>
                            rollbackPromise(regionalStore)
                        )
                    );
                } catch (rollbackError) {
                    // intentionally ignored
    ...
    async getObjectUrl(key, expiry) {
        ...
        return Promise.mapSeries(this._regionalS3s, async (regionalStore) => {
            if (!getObjectUrlResult.result) {
                try {
                    let exists = await this._regionalCall(
                        regionalStore,
                        "exists",
                        [key]
                    );
