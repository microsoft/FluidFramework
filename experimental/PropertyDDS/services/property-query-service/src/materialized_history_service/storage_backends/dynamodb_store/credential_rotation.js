...
class CredentialRotation {
    ...
    constructor() {
        ...
        if (
            ...
            logger.info(
                "Credentials will rotate using the configured AWS role"
            );
        }
    ...
    async start() {
        if (this._role) {
            ...
            logger.info(
                "Using static AWS config: credentials will not rotate."
            );
        }
    }
    ...
    async _onRotation(error, creds) {
        ...
        try {
            ...
            logger.error(
                `Failed to connect with renewed credentials using ${JSON.stringify(
                    this._role
                )}:`,
                connectError
            );
