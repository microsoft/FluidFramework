...
class RetryTask {
    ...
    async start() {
        ...
        if (this._config.firstTimeoutMilliSec) {
            ...
            this._currentRetryTimeout = Math.floor(
                this._config.intervalFn(that._retryCount + 1)
            );
        }
...
function _parseConfig() {
    if (typeof this._config === "object") {
        ...
        if (!this._config.maxRetryCount && this._config.taskTimeoutMilliSec) {
            ...
            this._maxRetryCount =
                Number(this._config.maxRetryCount) || DEFAULT_MAX_RETRYCOUNT;
        }
    } else {
...
function _isMaxTimeoutConfigured() {
    return (
        ...
        this._config.maxSingleTimeoutMilliSec > 0
    );
}
...
async function _onError(error) {
    ...
    const errorFilterAbort =
        typeof this._config.errorFilter === "function" &&
    ...
    if (errorFilterAbort || isFatalError) {
        ...
        logger.debug(
            `${msgPrefix}Error is not retryable. Aborting task.`,
            diags
        );
        throw error;
    }
    ...
    if (this._maxRetryCount && this._retryCount >= this._maxRetryCount) {
        ...
        logger.debug(
            `${msgPrefix}Max retry count reached. Aborting task.`,
            diags
        );
        throw error;
    }
    ...
    if (
        this._config.taskTimeoutMilliSec &&
        this._config.taskTimeoutMilliSec > 0
    ) {
            ...
            if (logger.isLevelEnabled(ModuleLogger.levels.DEBUG)) {
                logger.debug(
                    `${msgPrefix}next retry in ` +
                        ...
                        `the task timeout of ${this._config.taskTimeoutMilliSec.toFixed(
                            0
                        )} ms. ` +
                        `${JSON.stringify({
                            retryCount: this._retryCount,
    ...
    diags.taskName = this._taskName;
    logger.info(
        `${msgPrefix}Transient failure. Retry in ` +
            `${jitteredTimeoutMilliSec.toFixed(0)} ms.`,
        diags
    );
...
function _setNextRetryTimeout() {
    this._currentRetryTimeout = Math.floor(
        this._config.intervalFn(this._retryCount + 1)
    );
    if (_isMaxTimeoutConfigured.call(this)) {
