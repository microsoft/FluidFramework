...
class PagedQuery {
    ...
    hasNext() {
        const hasNext =
            !this._isCancelled &&
            (this._pageIndex === 0 ||
                !!this._params.ddbParams.ExclusiveStartKey);

        if (logger.isLevelEnabled(ModuleLogger.levels.TRACE)) {
            this._log(
                `hasNext(${this._params.queryName}), page: ${this._pageIndex}, ${hasNext}`
            );
        }
    ...
    async next(next) {
        ...
        const taskCb = (error) =>
            credsRotation.ddbClient._ddbApiCall(this._queryType, ddbParams);
        ...
        return [
            ...
            queryResult.ConsumedCapacity
                ? queryResult.ConsumedCapacity.CapacityUnits
                : 0,
        ];
    }
