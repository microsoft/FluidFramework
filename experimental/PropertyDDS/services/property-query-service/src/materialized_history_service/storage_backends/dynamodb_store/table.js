...
const getTableName = (keyspace, tableId, tableNames) => {
    if (tableNames && tableNames[tableId]) {
        logger.info(
            `Table name override: { ${tableId}: ${tableNames[tableId]} }`
        );
...
exported.init = function (keyspace, tableNames) {
    ...
    exported.ACCESS_CONTROL = createInstance(
        "accessControl",
        keyspace,
        tableNames,
        ["pk", "sk"],
        { byPolicyId: { name: "byPolicyId" } }
    );
    exported.BINARY_OBJECTS = createInstance(
        "binaryObjects",
        keyspace,
        tableNames,
        ["objectKey"],
        { byRepoPartition: { name: "byRepoPartition" } }
    );
    exported.BINARY_MERGES = createInstance(
        "binaryMerges",
        keyspace,
        tableNames,
        ["id"],
        { byBranchPage: { name: "byBranchPage" } }
    );
    exported.BRANCHES = createInstance(
        "branches",
        keyspace,
        tableNames,
        ["branch"],
    ...
    exported.BUSINESS = createInstance("business", keyspace, tableNames, [
        "id",
    ]);
    exported.COMMITS = createInstance(
        "commits",
        keyspace,
        tableNames,
        ["commit"],
        { byTopology: { name: "byTopology2" } }
    );
    exported.LIFECYCLE = createInstance("lifecycle", keyspace, tableNames, [
        "id",
        "transitionTime",
    ]);
    exported.MAPS = createInstance("maps", keyspace, tableNames, [
        "map",
        "key",
    ]);
    exported.MATERIALIZED_HISTORY = createInstance(
        "materializedHistory",
        keyspace,
        tableNames,
        ["PK", "SK"]
    );
    exported.METRICS = createInstance("metrics", keyspace, tableNames, [
        "id",
        "created",
    ]);
    exported.REPOSITORIES = createInstance(
        "repositories",
        keyspace,
        tableNames,
        ["repository"],
        { byCreatorId: { name: "byCreatorId" } }
    );
    exported.TOPOLOGY = createInstance("topology", keyspace, tableNames, [
        "id",
        "created",
    ]);
};
