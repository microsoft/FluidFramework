...
exported.init = function (keyspace, tableNames) {
	...
	exported.ACCESS_CONTROL = createInstance("accessControl", keyspace, tableNames, ["pk", "sk"], {
		byPolicyId: { name: "byPolicyId" },
	});
	exported.BINARY_OBJECTS = createInstance("binaryObjects", keyspace, tableNames, ["objectKey"], {
		byRepoPartition: { name: "byRepoPartition" },
	});
	exported.BINARY_MERGES = createInstance("binaryMerges", keyspace, tableNames, ["id"], {
		byBranchPage: { name: "byBranchPage" },
	});
	exported.BRANCHES = createInstance("branches", keyspace, tableNames, ["branch"], {
		byRepo: { name: "byRepo" },
	});
	exported.BUSINESS = createInstance("business", keyspace, tableNames, ["id"]);
	exported.COMMITS = createInstance("commits", keyspace, tableNames, ["commit"], {
	...
	exported.MATERIALIZED_HISTORY = createInstance("materializedHistory", keyspace, tableNames, [
		"PK",
		"SK",
	]);
	exported.METRICS = createInstance("metrics", keyspace, tableNames, ["id", "created"]);
	exported.REPOSITORIES = createInstance("repositories", keyspace, tableNames, ["repository"], {
