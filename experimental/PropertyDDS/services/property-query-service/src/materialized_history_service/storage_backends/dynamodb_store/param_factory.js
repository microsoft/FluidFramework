...
class ParamFactory {
    ...
    static getUpdateExpression(updates, removeArray, condition) {
        ...
        if (updates) {
            ...
            result.expressionAttributeValues =
                AWS.DynamoDB.Converter.marshall(attributeValues);
    ...
    createDeleteItem(tableName, pk, options) {
        options.keyMustExist = _.isUndefined(options.keyMustExist)
            ? true
        ...
        if (this._config.returnConsumedCapacity) {
            ddbParams.ReturnConsumedCapacity =
                this._config.returnConsumedCapacity;
        }
        ...
        if (options.keyMustExist) {
            ...
            ddbParams.ExpressionAttributeNames =
                ParamFactory.escapeReservedNames(pkColumnNames);
            const attrs = _.map(
                ddbParams.ExpressionAttributeNames,
                (columnName, escapedName) => {
    ...
    createPutItem(tableName, params, protectedAttrs) {
        ...
        if (protectedAttrs) {
            ddbParams.ExpressionAttributeNames =
                ParamFactory.escapeReservedNames(protectedAttrs);
            const attrs = _.map(
                ddbParams.ExpressionAttributeNames,
                (value, attr) => {
    ...
    createUpdateItem(tableName, pk, updates, condition, removeArray) {
        ...
        const updateExp = ParamFactory.getUpdateExpression(
            updates,
            removeArray,
            condition
        );
        ...
        ddbParams.ExpressionAttributeValues =
            updateExp.expressionAttributeValues;
