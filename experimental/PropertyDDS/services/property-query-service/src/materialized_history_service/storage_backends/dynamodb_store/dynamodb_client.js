...
const newGuid = require("@fluid-experimental/property-common").GuidUtils
    .generateGUID;
...
class DynamoDBClient {
    ...
    constructor(awsConfig, storeConfig) {
        ...
        if (!BufferedPagedQuery) {
            BufferedPagedQuery = require(path.join(
                __dirname,
                "buffered_paged_query"
            ));
        }
        ...
        if (!WriteTransaction) {
            WriteTransaction = require(path.join(
                __dirname,
                "write_transaction"
            ));
        }
        ...
        const isHttps =
            _.isUndefined(awsConfigCopy.endpoint) ||
            regEx.test(awsConfigCopy.endpoint);
    ...
    async _ddbApiCall(fnName) {
        ...
        const tableName = DynamoDBClient.getTableNameFromArgs.apply(
            null,
            arguments
        );
        ...
        if (logger.isLevelEnabled(ModuleLogger.levels.TRACE)) {
            ...
            let filteredLog = Logging.filterLogObject(
                args,
                Logging.filterKeys.input
            );
            filteredLog.opId = opId;
            logger.trace(
                `${fnName}${
                    tableName ? ":" + tableName : ""
                } ${Logging.getDbInStyle()} ${JsonUtils.stringify(
                    filteredLog,
                    null,
                    2
                )}`
        ...
        const timedResult = await _timedApiCall.call(
            this,
            tableName,
            fnName,
            args
        ...
        if (logger.isLevelEnabled(ModuleLogger.levels.TRACE)) {
            ...
            filteredLog.opId = opId;
            filteredLog.elapsedMilliSec =
                timedResult.elapsedMilliSec.toFixed(0);
            logger.trace(
                `${fnName}${
                    tableName ? ":" + tableName : ""
                } ${Logging.getDbOutStyle()} ${JsonUtils.stringify(
                    filteredLog,
                    null,
                    2
                )}`
    ...
    async batchGetItem(batchParams, options = { consistentRead: false }) {
        ...
        _.mapValues(batchParams, (pks, tableName) => {
            if (!rq[tableName]) {
                ...
                if (options.columns) {
                    ...
                    rq[tableName].ProjectionExpression =
                        vars.projectionExpression;
                    rq[tableName].ExpressionAttributeNames =
                        vars.expressionAttributeNames;
                }
            }
        ...
        const taskCb = async (retryCount) => {
            try {
                const result = await this._ddbApiCall(
                    "batchGetItem",
                    ddbParams
                );
                _.mapValues(result.Responses, (marshalledRows, tableName) => {
                    ...
                    _.each(marshalledRows, (marshalledRow) => {
                        unmarshalledRows.push(
                            AWS.DynamoDB.Converter.unmarshall(marshalledRow)
                        );
                    });
                });
        ...
        const tableName = DynamoDBClient.getTableNameFromArgs(
            fnName,
            ddbParams
        );
        const task = new MetricsRetryTask(
            this._config.retry,
            taskCb,
            fnName,
            tableName
    ...
    async deleteItem(tableName, pk, options = { keyMustExist: true }) {
        const ddbParams = this._paramFactory.createDeleteItem(
            tableName,
            pk,
            options
        );
        ...
        try {
            ...
            if (
                options.keyMustExist &&
                error.code === "ConditionalCheckFailedException"
            ) {
                throw new OperationError(
                    `Failed to delete ${JSON.stringify(
                        pk
                    )} from table ${tableName}: not found.`,
                    operation,
                    HttpStatus.NOT_FOUND
                );
            }
    ...
    async describeTable(tableName) {
        ...
        if (!data || !data.Table) {
            throw new OperationError(
                "Unexpected describeTable reply: " +
                    JsonUtils.stringify(data, null, 2),
                operation,
    ...
    async getBusinessField(fieldName) {
        ...
        this.expectResults(
            `Business field not found: ${fieldName}`,
            result,
            options.columns
        );
        return result.value;
    ...
    async getItem(tableName, pk, options) {
        ...
        let result = await _retryableDdbApiCall.call(
            this,
            "getItem",
            ddbParams
        );
        if (!result.Item && !options.consistentRead) {
            logger.info(
                `Retrying getItem ${JSON.stringify(
                    pk
                )} on table ${tableName} with consistent read`
            );
            ddbParams.ConsistentRead = true;
            result = await _retryableDdbApiCall.call(
                this,
                "getItem",
                ddbParams
            );
        ...
        return result.Item
            ? AWS.DynamoDB.Converter.unmarshall(result.Item)
            : undefined;
    }
    ...
    async getPKColumns(tableName) {
        ...
        return _.map(
            ...
            (attributeObj) => attributeObj.AttributeName
        );
    }
    ...
    static getTableNameFromArgs(fnName, ddbParams) {
        ...
        if (ddbParams) {
            ...
            if (!tableName) {
                tableName =
                    ddbParams.RequestItems &&
                    Object.keys(ddbParams.RequestItems)[0];
            }
        }
    ...
    async putItem(tableName, params, protectedAttrs) {
        const ddbParams = this._paramFactory.createPutItem(
            tableName,
            params,
            protectedAttrs
        );
    ...
    setQueryParams(tableName, pk, params) {
        ...
        params.ddbParams.KeyConditionExpression =
            conditionalExpressions.join(" AND ");
    }
    ...
    async updateItem(tableName, pk, updates, condition, removeArray) {
        ...
        return result.Attributes
            ? AWS.DynamoDB.Converter.unmarshall(result.Attributes)
            : undefined;
    }
    ...
    async createIndex(
        tableName,
        indexName,
        ddbAttributeDefinitions,
        ddbIndexCreateParams
    ) {
        const ddbParams = {
            TableName: tableName,
        ...
        _.defaults(
            ddbParams.GlobalSecondaryIndexUpdates[0].Create,
            ddbIndexCreateParams
        );
    ...
    async waitForIndex(
        ...
        throw new OperationError(
            `Timeout waiting for index to be ${
                action === "delete" ? "deleted" : "created"
            }: ` +
                `${tableName}.${indexName} ` +
        ...
        if (DynamoDBException.isDuplicateKey(error)) {
            ...
            for (let tableName of tableNames) {
                ...
                try {
                    ...
                    pkColumns = table
                        ? table.pkColumns
                        : await this.getPKColumns(tableName);
                    ...
                    const itemHandler = (marshalledItem) => {
                        const item =
                            AWS.DynamoDB.Converter.unmarshall(marshalledItem);
                    ...
                    if (dupPKs.size < 1) {
                        // Should not happen:
                        throw new Error(
                            "Failed to find duplicate keys in ddbParams"
                        );
                    }

                    dupItemsPerTable[tableName] = _.map([...dupPKs], (dupPK) =>
                        JSON.parse(dupPK)
                    );
                } catch (e2) {
                    // Unable to describe the table. Dump all keys without analyzing the duplicates:
                    logger.warn(e2);
                    error.message += `. ddbParams: ${JSON.stringify(
                        ddbParams
                    )}`;
                    throw e2;
                }
...
async function _timedApiCall(tableName, fnName, args) {
    ...
    await PluginManager.instance.systemMonitor.startSegment(
        segmentName,
        true,
        async () => {
            ...
            try {
                result = await this._dynamodb[fnName]
                    .apply(this._dynamodb, args)
                    .promise();
            } finally {
                ...
                if (
                    elapsedMilliSec >=
                    this._config.elapsedWarningThresholdMilliSec
                ) {
