...
class DynamoDBBackend {
    ...
    constructor(in_params) {
        ...
        this._maxConcurrentS3Uploads =
            this._settings.maxConcurrentS3Uploads || 10;
     ...
     */
    async init() {
        this._credsRotation.init(
            this._ddbSettings.aws,
            this._ddbSettings.config
        );
    ...
    async _writeBatch(tasks) {
        ...
        tasks.forEach((t) => {
            let s3Key =
                t.value.length > this._largeNodeThreshold
                    ? `${t.key}:rev:${generateGUID()}`
                    : null;
            ...
            writeItems.push({
                ...
                SK: ".",
                value:
                    t.value.length > this._largeNodeThreshold ? null : t.value,
                s3Key: s3Key,
            });
        });
    ...
    async _fetchBatch(consistent, tasks) {
        ...
        try {
            let fetched = await this._credsRotation.ddbClient.batchGetItem(
                getParams,
                options
            );
            tasks.forEach((t) => {
                let fetchResult = fetched[this._tableName].find(
                    (f) => f.PK === t.key.PK
                );
            ...
            (err) => {
                ...
                if (in_value.length > this._largeNodeThreshold) {
                    ...
                    AsyncContext.incrementInContext(
                        "wcuUsed",
                        Math.ceil(in_value.length / 1024)
                    );
                }
            }
        );
                    ...
                    resolve: async (result) => {
                        if (!result) {
                            ...
                            if (result.s3Key) {
                                ...
                                try {
                                    let s3Result =
                                        await this._bigStore.getObject(
                                            result.s3Key
                                        );
    ...
    async _wrapStampede(fnName, args) {
        ...
        if (this._stampedePromises[fnName][cacheKey]) {
            ...
            this._stampedePromises[fnName][cacheKey] = this[fnName].apply(
                this,
                args
            );
