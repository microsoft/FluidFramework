...
class BranchesController extends BaseController {
    ...
    constructor(params) {
         ...
         */
        this._app.post(
            this.getV1Paths("/branchDeletion").concat([
                "/mhs/v1/branchDeletion",
            ]),
            bodyParser.json({ limit: "100mb" }),
            this._requestSignatureValidator.validateSignature(
         ...
         */
        this._app.get(
            this.getV1Paths("/branch/:branchGuid").concat([
                "/mhs/v1/branch/:branchGuid",
            ]),
            this._requestSignatureValidator.validateSignature(
        ...
        this._app.get(
            this.getV1Paths(
                "/branch/:branchGuid/commit/:commitGuid/materializedView"
            ).concat([
        ...
        this._app.get(
            this.getV1Paths(
                "/branch/:branchGuid/commit/:commitGuid/changeSet"
            ).concat([
    ...
    async getBranch(req, res, next) {
        try {
            const branch = await this._materializedHistoryService.getBranch(
    ...
    async createCommit(req, res, next) {
        try {
            const commit = await this._materializedHistoryService.createCommit(
                req.body
            );
    ...
    async createCommitTask(req, res, next) {
        try {
            ...
            this._systemMonitor.startBackgroundTransaction(
                "CreateCommit",
                async () => {
                    this._systemMonitor.extract(tracer);
                    await this._materializedHistoryService.createCommit(
                        req.body
                    );
                }
            );
    ...
    async getCommit(req, res, next) {
        try {
            const commit = await this._materializedHistoryService.getCommit(
    ...
    async getCommitMV(req, res, next) {
        ...
        try {
            ...
            await this._systemMonitor.startWebTransaction(
                "GetCommitMV",
                async () => {
                    [branchInfo, mvAtCommit] =
                        await this._mvQueryExecutor.execute(
                            req.params.branchGuid,
                            req.params.commitGuid,
                            req.query
                        );
    ...
    async createDeleteBranchTask(req, res, next) {
        ...
        try {
            ...
            if (existingTaskForBranches.length > 0) {
                this.render(
                    res,
                    { existingTasks: existingTaskForBranches },
                    HTTPStatus.CREATED
                );
                return;
            }
    ...
    async getDeleteBranchTask(req, res, next) {
        ...
        try {
            const [task, taskPromise] =
                await this._materializedHistoryService.getDeleteBranchTask({
    ...
    async retryDeleteBranchTask(req, res, next) {
        ...
        try {
            ...
            await this._materializedHistoryService._deletionManager._writeTaskStatus(
                task
            );
            const taskPromise =
                this._materializedHistoryService.retryDeleteBranchTask(task);
    ...
    _handleErrors(err, req, res, next) {
        ...
        this.render(
            res,
            { errors },
            errors[0].statusCode || HTTPStatus.INTERNAL_SERVER_ERROR
        );
    }
}
