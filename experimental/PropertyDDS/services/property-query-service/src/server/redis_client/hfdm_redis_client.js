...
(function () {
    ...
    const _checkConnected = function () {
        return this.redis
            ? Promise.resolve()
            : Promise.reject(new Error("Not connected to redis"));
    };
    ...
    HfdmRedisClient.prototype.connect = function () {
        ...
        if (this.redis && this.redis.status === "connect") {
            return Promise.reject(
                new Error(
                    "Can't connect, a connection is already established to Redis"
                )
            );
        }
     ...
     */
    HfdmRedisClient.prototype._handlePubSubMessage = function (
        channel,
        message
    ) {
     ...
     * @param {string} message - Contents of the message
     */
    HfdmRedisClient.prototype._handlePubSubPMessage = function (
        pattern,
        channel,
        message
    ) {
     ...
     */
    HfdmRedisClient.prototype._addSubscriptionCallback = function (
        channel,
        callbackKey,
        callback
    ) {
     ...
     * @param {function} callbackKey - A unique identifier for the callback
     */
    HfdmRedisClient.prototype._removeSubscriptionCallback = function (
        channel,
        callbackKey
    ) {
        if (this._pubSubSubscriptions[channel]) {
            const index = this._pubSubSubscriptions[channel].findIndex(
                function (item) {
    ...
    const _subscribe = function (verb, channel, callbackKey, callback) {
        const that = this;
        const mustSubscribe =
            !that._pubSubSubscriptions[channel] ||
            that._pubSubSubscriptions[channel].length <= 0;
    ...
    const _unsubscribe = function (verb, channel, callbackKey) {
        ...
        if (
            !this._pubSubSubscriptions[channel] ||
            this._pubSubSubscriptions[channel].length <= 0
        ) {
            delete this._pubSubSubscriptions[channel];
     ...
     * @return {Promise} - A promise that resolves when the subscription is successfully created or rejects otherwise
     */
    HfdmRedisClient.prototype.subscribe = function (
        channel,
        callbackKey,
        callback
    ) {
     ...
     * @return {Promise} - A promise that resolves when the subscription is successfully created or rejects otherwise
     */
    HfdmRedisClient.prototype.psubscribe = function (
        pattern,
        callbackKey,
        callback
    ) {
     ...
     * @return {Promise} - A promise that resolves when the subscription is successfully created or rejects otherwise
     */
    HfdmRedisClient.prototype.psubscribeAll = function (
        pattern,
        callbackKey,
        callback
    ) {
     ...
     * @return {Promise} - A promise that resolves when the subscription is successfully removed or rejects otherwise
     */
    HfdmRedisClient.prototype.punsubscribeAll = function (
        pattern,
        callbackKey
    ) {
        if (this._cluster) {
     ...
     */
    HfdmRedisClient.prototype._definedCommand = function (
        name,
        ...restOfArguments
    ) {
    ...
    HfdmRedisClient.prototype.disconnect = function () {
        ...
        if (!this.redis) {
            promise = Promise.reject(
                new Error("The redis client is not instanciated")
            );
        } else if (
            ...
            promise = Promise.reject(
                new Error(
                    `Can't disconnect. Current status: ${this.redis.status}`
                )
            );
