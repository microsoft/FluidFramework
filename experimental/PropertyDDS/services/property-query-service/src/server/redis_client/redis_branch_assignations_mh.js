...
const luaProcs = {
    ...
    removeMhInstance: fs.readFileSync(
        path.join(luaProcFolder, "remove_mh_instance.lua")
    ),
    reportMhFailure: fs.readFileSync(
        path.join(luaProcFolder, "report_mh_failure.lua")
    ),
    confirmMhEviction: fs.readFileSync(
        path.join(luaProcFolder, "confirm_mh_eviction.lua")
    ),
    upsertMhInstance: fs.readFileSync(
        path.join(luaProcFolder, "upsert_mh_instance.lua")
    ),
    upsertMhUserInstance: fs.readFileSync(
        path.join(luaProcFolder, "upsert_mh_user_instance.lua")
    ),
 ...
 */
RedisBranchAssignationsMH.prototype.markMhInstanceShuttingDown = function (
    host
) {
 ...
 */
RedisBranchAssignationsMH.prototype.upsertMhUserInstance = async function (
    mhUserId
) {
    ...
    await this.client.upsertMhUserInstance(
        SHARDING_PREFIX,
        theKey,
        mhUserId,
        expiry
 ...
 */
RedisBranchAssignationsMH.prototype.getMhInstanceForBranch = function (
    branchId
) {
    // Try and hit out of the stored proc.  This will allow us using a replica,
    // as LUA scripts need to hit the master at all times.
    return this.client
        .hget(MH_BRANCH_ASSIGNATIONS_HMAP, branchId)
        .then((result) => {
            if (result === null) {
                return this.client
                    .getMhInstanceForBranch(SHARDING_PREFIX, branchId)
                    .then((mhId) => {
                        if (!mhId) {
                            throw new Error(
                                "No MH instance available. Try again later."
                            );
                        }
                        logger.debug(
            ...
            logger.debug(
                `getMhInstanceForBranch returns branch ${branchId} -> ${result} from map`
            );
            return result;
        });
};
 ...
 */
RedisBranchAssignationsMH.prototype.setMhInstanceForBranch = function (
    branchId,
    mhId
) {
 ...
 */
RedisBranchAssignationsMH.prototype.removeMhInstanceForBranch = function (
    branchId,
    mhId
) {
    if (!mhId) {
        ...
        logger.debug(
            `removeMhInstanceForBranch removes branch ${branchId} for ${mhId}`
        );
        return this.client.removeMhInstanceForBranch(
            SHARDING_PREFIX,
            branchId,
            mhId
        );
...
RedisBranchAssignationsMH.prototype.getMhBranchesForInstance = function (mhId) {
    return this.client
        .hgetall(MH_BRANCH_ASSIGNATIONS_HMAP)
        .then((allEntries) => {
            const result = [];
...
RedisBranchAssignationsMH.prototype.reportFailure = function (
    ...
    if (deploy.isGreen()) {
        logger.error(
            `server ${mhId} reported as failing in green mode for branch ${branchId}`
        );
        return Promise.resolve();
    }

    thisMinute =
        thisMinute !== undefined
            ? thisMinute
...
RedisBranchAssignationsMH.prototype.resetFailure = function (mhId, mhUserId) {
    if (
        this._currentFailureMaps[mhUserId] &&
        this._currentFailureMaps[mhUserId][mhId]
    ) {
