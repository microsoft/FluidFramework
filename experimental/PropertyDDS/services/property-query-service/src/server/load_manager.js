...
let logger = ModuleLogger.getLogger(
...
class LoadManager {
    ...
    _resetLoadShedCooldown() {
        this._loadShedCooldown = Math.floor(
            Math.random() *
                this._loadShedding.cooldownFactor *
    ...
    async _updateLoad() {
        ...
        logger.trace(
            "Updating the load for this instance, setting load as",
            load
        );
        ...
        if (this._loadShedCooldown === 0) {
            ...
            if (average > this._loadShedding.cpuThreshold) {
                ...
                try {
                    ...
                    const sheddable = await this._branchTracker.getSheddable(
                        minActiveTime
                    );
                    const branchesToShed = this._getBranchesToShed(
                        overload,
                        sheddable
                    );
                    if (branchesToShed.length > 0) {
                        ...
                        await Promise.all(
                            _.map(branchesToShed, (branchGuid) => {
                                this._storageManager.clearCacheForBranch(
                                    branchGuid
                                );
                                return this._branchAssignations.removeMhInstanceForBranch(
                                    branchGuid,
    ...
    _getBranchesToShed(overload, sheddable) {
        ...
        const totalUsage = sheddable.reduce(
    ...
    _calculateLoad() {
        ...
        const cpuPercentUsed =
            (cpuTimeUsed * 100) / (this._loadUpdateIntervalMs / MILLIS);
    ...
    async tearDown() {
        logger.info(
            "Unsubscribing this instance from the MH availability registry"
        );
