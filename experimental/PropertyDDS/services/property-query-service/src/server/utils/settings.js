...
function Settings(in_configJSONPaths, in_dynamicDefaults, in_options) {
            ...
            if (fs.existsSync(masterFiles[itr])) {
                try {
                    const obj = JSON.parse(
                        fs.readFileSync(masterFiles[itr], "utf8")
                    );
                    const overrideKeys = Object.keys(obj);
                    for (
                        let overrideItr = 0;
                        overrideItr < overrideKeys.length;
                        overrideItr++
                    ) {
                        overrides[overrideKeys[overrideItr]] =
                            obj[overrideKeys[overrideItr]];
                    }
                } catch (e) {
                    console.log(
                        "ERROR(ignored): failed parsing/reading:",
                        masterFiles[itr]
                    );
                }
            }
    ...
    if (process.env.OVERRIDES_AS_ENV) {
        try {
            ...
            console.log(
                "ERROR(ignored): failed parsing/reading OVERRIDES_AS_ENV."
            );
        }
    }
    ...
    for (var i = 0; i < configKeys.length; i++) {
        whitelistKeys.push(
            configKeys[i].replace(separator, utils.ENV_CLASS_SEPARATOR)
        );
    }
