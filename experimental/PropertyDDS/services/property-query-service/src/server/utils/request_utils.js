...
(function () {
    ...
    const HTTPError =
        require("@fluid-experimental/property-common").HTTPErrorNoStack;
    const request = require("request");
    const generateGUID = require("@fluid-experimental/property-common")
    ...
    RequestUtils.buildErrorMessage = function (in_error, in_response) {
        ...
        response.request.uri.format =
            response.request.uri.format || function () {};
        ...
        try {
            if (in_error) {
                ...
                const isQuietError = _.every(bodyErrors, function (err) {
                    return (
                        (err.flags & HTTPError.FLAGS.QUIET) ===
     ...
     */
    RequestUtils.passthroughCallback = function (
        in_callback,
        in_error,
        in_response,
        in_body
     ...
     * @param {object} in_response Response object (may contain errors)
     */
    RequestUtils.handleRequestCallback = function (
        in_callback,
        in_error,
        in_response
    ) {
        try {
            const errorMsg = RequestUtils.buildErrorMessage(
                in_error,
                in_response
            );
     ...
     * @param {object} in_response Response object (may contain errors)
     */
    RequestUtils.handleRequestCallbackWithResponse = function (
        in_callback,
        in_error,
        in_response
    ) {
    ...
    RequestUtils.requestWithRetries = function (in_params, in_callback) {
        ...
        const regexp =
            in_params.logFilterRegExp && new RegExp(in_params.logFilterRegExp);
        let startTime;
        let lastStartTime;
        ...
        in_params.maximumRetryInterval =
            in_params.maximumRetryInterval || 120000;
        in_params.logMessage =
            in_params.logMessage || "Error requesting " + uri + " ";
        in_params.requestCallbackHandler =
            in_params.requestCallbackHandler ||
            RequestUtils.handleRequestCallback;
        in_params.requestParams.timeout =
            in_params.requestParams.timeout || 5000;

        // Retry indefinitely if a retry condition is present
        in_params.retries = in_params.errorFilter
            ...
            if (Array.isArray(in_params.retryStatusCodes)) {
                ...
                if (results) {
                    ...
                    if (in_params.retryStatusCodes.indexOf(httpStatus) !== -1) {
                        ...
                        if (!regexp || !(err.message || "").match(regexp)) {
                            logger.warn(
                                `${in_params.logMessage} ${err.message}, ${tracking}`
                            );
                        }
                        if (in_params.prepareRetryCallback) {
                            try {
            ...
            // Retry options
            {
                ...
                interval: function (retryCount) {
                    ...
                    if (in_params.useExponentialBackoff) {
                        ...
                        if (
                            tentativeInterval > in_params.maximumRetryInterval
                        ) {
            ...
            function (err, res1, res2) {
                ...
                if (tracking) {
                    ...
                    if (err) {
                        ...
                        logger.info(
                            `${uri} (${duration}): succeded, ${tracking}`
                        );
                    }
                }
