...
const basicAuthMiddlewareFactory =
    require("./middlewares/basicauth").middlewareFactory;
...
(function () {
    ...
    var BaseServer = function (in_params) {
        ...
        this._shortServiceName =
            this._params.shortServiceName ||
            process.env.SHORT_SERVICE_NAME ||
            "BS";
        ...
        this._instanceMonitor =
            this._params.instanceMonitor || defaultInstanceMonitor;
        this._authenticator = this._params.authenticator;

        let systemMonitor;
        if (
            in_params.systemMonitor &&
            process.env.SYSTEM_MONITOR_ENABLED === "true"
        ) {
        ...
        if (this._params.initializeServer) {
            if (this._authenticator) {
                if (this._params.urlWhiteList) {
                    this._authenticator.setUrlWhiteList(
                        this._params.urlWhiteList
                    );
                }
            ...
            this._app.use(function (req, res, next) {
                ...
                if (
                    that._logger &&
                    that._logger.isLevelEnabled(ModuleLogger.levels.TRACE)
                ) {
                    // Skip logging some high-frequency, diagnostic routes
                    var pathname = url.parse(req.url).pathname;
                    if (
                        _.indexOf(that._logHTTPRequests.skip, pathname) === -1
                    ) {
    ...
    BaseServer.prototype.start = function (options) {
        ...
        if (options && _.isFinite(options.keepAliveTimeout)) {
            ...
            that._logger.debug(
                `New keepAliveTimeout: ${options.keepAliveTimeout}`
            );
        }
        ...
        return that._instanceMonitor
            ...
            .then(function () {
                ...
                listen =
                    that._params.listen &&
                    (options && _.isBoolean(options.listen)
                        ? options.listen
                        : true);
                if (listen) {
                    return that.listen();
            ...
            .then(function () {
                var getEventLoopTime = function () {
                    ...
                    setImmediate(function () {
                        ...
                        var metric = (diff[0] + (diff[1] / 1e9) * 1000).toFixed(
                            5
                        );
                        that.emit("eventloop.latency", {
                            latency: metric,
                        });
                        that._eventLoopTimeTimeout = setTimeout(
                            getEventLoopTime,
                            5000
                        );
                    });
    ...
    BaseServer.prototype.stop = function () {
        ...
        return this.close()
            ...
            .then(() =>
                this._logger.info("Calling terminate handler for:", this._name)
            )
            .then(() => this.term())
            .catch((e) => this._logger.warn(e))
            .then(() => {
        ...
        if (this._server.listening) {
            ...
            return isPortFree(this._port).then(() => {
                return new Promise((resolve, reject) => {
                    this._server.listen(
                        this._port,
                        settings.get("baseServer:backlog"),
                        (err) => {
                            if (err) {
                                reject(err);
                            } else {
    ...
    BaseServer.prototype.close = function () {
        if (this._server.listening) {
            ...
            const displayCount = () => {
                that._server.getConnections((err, count) => {
                    if (err) {
                        ...
                        that._logger.info(
                            `Active connection count(${this._name}): ${count}`
                        );
            ...
            const intervalLength = settings.get(
                "baseServer:connectionCountInterval"
            );
            const displayInterval = setInterval(
                displayCount.bind(this),
                intervalLength
            );
    ...
    BaseServer.prototype.installSignalHandler = function () {
        ...
        let sigHandler = function (signal) {
            ...
            this._terminating++;
            this._logger.warn(
                `Received signal: ${signal}: ${this._terminating}/${maxSignals}`
            );
            if (this._terminating > 1) {
                ...
                const termTimeout =
                    (this._params.termTimeout ||
                        settings.get("baseServer:termTimeout")) * 1000;
                setTimeout(() => {
                    const duration = Date.now() - this._termStartTime;
                    this._logger.error(
                        `Too long to terminate: ${duration} ms. Aborting...`
                    );
                    process.exit(1);
                }, termTimeout);
                this.stop()
                    ...
                    .then((res) => {
                        const duration = Date.now() - this._termStartTime;
                        this._logger.info(
                            `Terminated in ${duration} ms. Exiting...`
                        );
                        signals.forEach((sig) =>
