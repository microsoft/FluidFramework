...
class Rotation {
    ...
    constructor(role, params) {
        if (!role || !role.arn || !role.prefix) {
            throw new Error(
                `Failed to instantiate new Rotation: ${JSON.stringify(role)}`
            );
        }
    ...
    addCallback(onRotation) {
        ...
        // retrying
        if (this._error) {
            const cbResult = this._creds
                ? Object.assign({}, this._creds)
                : null;
    ...
    async _startRotation() {
        ...
        try {
            ...
            logger.info(
                `Successfully rotated credentials for: ${JSON.stringify(
                    this._role
                )}.`
            );
            ...
            logger.info(
                `Credentials for ${JSON.stringify(
                    this._role
                )} will rotate on: ` + new Date(this._nextTime).toISOString()
            );
        } catch (error) {
            this._error = error;
            this._promise = null;
            ...
            logger.error(
                `Failed to obtain temporary credentials for ${JSON.stringify(
                    this._role
                )}:`,
                error
            );
    ...
    async _stopRotation() {
        try {
            ...
            if (this._callbacks.size === 0) {
                ...
                this._error = null;
                logger.info(
                    `Credentials rotation was stopped for: ${JSON.stringify(
                        this._role
                    )}.`
                );
...
const _getNormalizedKey = (role, params) => {
    if (!role || !role.arn || !role.prefix) {
        throw new Error(
            `Failed to get rotation for the role: ${JSON.stringify(role)}`
        );
    }
