...
class BasicAuthController extends BaseController {
    ...
    constructor(baseServer, params) {
        const basicAuthItem = (params && params.basicAuth) || {
            username:
                settings.get("hfrs:basicAuthName") ||
                settings.get("basicAuthName"),
            password:
                settings.get("hfrs:basicAuthPassword") ||
                settings.get("basicAuthPassword"),
            passwordList:
                settings.get("hfrs:basicAuthPasswordList") ||
                settings.get("basicAuthPasswordList"),
        };
        ...
        this._isBasicAuthConfigured =
            !process.env.CLOUDOS_MONIKER || !!basicAuthMiddleware;
    ...
    setupRoutes(routes) {
        ...
        _.each(routes.get, (callback, route) => {
            if (this._basicAuthMiddleware) {
                this._baseServer
                    .getExpressApp()
                    .get(route, this._basicAuthMiddleware);
            }
        ...
        _.each(routes.post, (callback, route) => {
            if (this._basicAuthMiddleware) {
                this._baseServer
                    .getExpressApp()
                    .post(route, this._basicAuthMiddleware);
            }
