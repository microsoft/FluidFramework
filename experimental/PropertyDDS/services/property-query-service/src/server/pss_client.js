...
const logger = ModuleLogger.getLogger(
    "HFDM.MaterializedHistoryService.PSSClient"
);
...
class PSSClient {
    ...
    registerBroker() {
        if (this._enableEviction) {
            ...
            setTimeout(
                this._initializeRegistrationInterval.bind(this),
                whenToStartInterval
            );

            return this._branchAssignations.upsertBrokerInstance(
                this._brokerId
            );
        }
    ...
    async getCommitRange(params) {
        ...
        try {
            ...
            if (
                ...
                this._branchAssignations
                    ...
                    .catch((ex) =>
                        logger.warn(
                            `Failed to report failure for instance ${pssHost}`,
                            ex
                        )
                    );
            }
        ...
        if (this._enableEviction) {
            this._branchAssignations
                .resetFailure(pssHost, this._brokerId)
                .catch(() => {});
        }
        return result;
    }
    ...
    async _getPSSInstance(branchGuid) {
        let pss = await this._branchAssignations.getPSSInstanceForBranch(
            branchGuid
        );
