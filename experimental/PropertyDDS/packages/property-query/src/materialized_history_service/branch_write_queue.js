...
const logger = ModuleLogger.getLogger(
    "MaterializedHistoryService.BranchWriteQueue"
);
const {
    ChangeSet,
...
class BranchWriteQueue {
    ...
    async _addPendingCommitPromise(branchGuid, commitGuid, promise) {
        if (!this._pendingCommitPromises[branchGuid]) {
            logger.trace(
                `Creating pending commit promises collection for branch ${branchGuid}`
            );
        ...
        let cleanUp = () => {
            if (this._pendingCommitPromises[branchGuid]) {
                logger.trace(
                    `Deleting pending commit promise for commit ${commitGuid}`
                );
                delete this._pendingCommitPromises[branchGuid][commitGuid];
                if (
                    Object.keys(this._pendingCommitPromises[branchGuid])
                        .length === 0
                ) {
                    logger.trace(
        ...
        this._pendingCommitPromises[branchGuid][commitGuid].then(
            cleanUp,
            cleanUp
        );
        ...
        if (
            ...
            try {
                ...
                if (ex.statusCode === HTTPStatus.NOT_FOUND) {
                    let lastKnownCommitGuid = await this._getLastCommitGuid(
                        branchGuid
                    );
            ...
            try {
                ...
                if (ex.statusCode === HTTPStatus.NOT_FOUND) {
                    ...
                    if (this._pendingBranchPromises[branchGuid]) {
                        ...
                        let branchInfo = await this._pssClient.getBranch(
                            branchGuid
                        );
    ...
    async _validateTopologyWorker({ task, taskDp }) {
        const _tryToFetchMissingAndQueue = async (lastKnownCommit) => {
            ...
            let topologyWorkerQueue = [
                ...this._validateTopologyQueue[task.branchGuid],
            ];
            do {
                // Let's try to find if the hole isn't in the queue already
                let taskAndDp = topologyWorkerQueue.find(
                    ({ task: queueItem }) =>
                        queueItem.guid === itemSearchingParent
                );

                if (taskAndDp) {
                    let { task: expectedParent, taskDp: expectedDp } =
                        taskAndDp;
            ...
            if (itemSearchingParent !== lastKnownCommit) {
                ...
                do {
                    ...
                    allCommitsFound =
                        fetchedResults.find(
                            (fr) => fr.guid === itemSearchingParent
                        ) !== undefined;
                    ...
                    fetchedResults.forEach((fr) => {
                        ...
                        let correspondingInTopologyQueue =
                            topologyWorkerQueue.find(
                                ({ task: queueItem }) =>
                                    queueItem.guid === fr.guid
                            );

                        if (correspondingInTopologyQueue) {
                            // Queue already and bind the promise
                            let { task: foundTask, taskDp: foundDp } =
                                correspondingInTopologyQueue;
                            foundTask.previouslyCompleted = true;
                            lastPromise = foundDp;
                            this._queueCommit(foundTask)
                                .then((res) => {
                                    logger.trace(
                                        `Done writing ${foundTask.guid}`
                                    );
                                    foundDp.resolve(res);
                                })
                                .catch((ex2) => {
                                    logger.trace(
                                        `Failed writing ${foundTask.guid}`,
                                        ex2
                                    );
                                    foundDp.reject(ex2);
                                });
        ...
        if (task.previouslyCompleted) {
            logger.trace(
                `Item ${task.guid} was processed as part of compensation, skipping`
            );
            return;
        }
        ...
        try {
            ...
            logger.trace(
                `Uniqueness of ${task.guid} failed.  Resolving with existing`
            );
            ...
            if (ex.statusCode === HTTPStatus.NOT_FOUND) {
                let lastCommitGuid = await this._getLastCommitGuid(
                    task.branchGuid
                );

                if (
                    lastCommitGuid !== task.parentGuid &&
                    lastCommitGuid !== task.guid
                ) {
                    // Not on tip!
                    if (task.rebase) {
                        // If rebase mode is active, we have to check, whether the parent commit exists
                        try {
                            await this._commitManager.getCommit(
                                task.parentGuid
                            );
                            ...
                            this._queueCommit(task).then(
                                (res) => {
                                    logger.trace(
                                        `Done writing ${lastCommitGuid}`
                                    );
                                    taskDp.resolve(res);
                                },
                                (ex2) => {
                                    logger.trace(
                                        `Failed writing ${lastCommitGuid}`
                                    );
                                    taskDp.reject(ex2);
                                }
                            );
                            ...
                            throw new Error(
                                "Failed to fetch parent commit: " + e.message
                            );
                        }
                    } else {
                        // Fetch missing and queue
                        logger.trace(
                            `Fetching hole between ${lastCommitGuid} and ${task.guid}`
                        );
                        await _tryToFetchMissingAndQueue(lastCommitGuid).catch(
                            (ex2) => {
                                logger.warn(
                                    "Failed compensating for a hole",
                                    ex2
                                );
                                taskDp.reject(ex2);
                            }
    ...
    async _getLastCommitGuid(branchGuid) {
        ...
        if (
            ...
            if (this._currentlyWritingCommit[branchGuid]) {
                ...
                try {
                    let theBranch = await this._branchManager.getBranch(
                        branchGuid
                    );
    ...
    queueCommitGracefully(commitReq) {
        ...
        if (
            ...
            logger.trace(
                `Commit ${commitReq.guid} already queued, skipping the processing`
            );
            return this._pendingCommitPromises[commitReq.branchGuid][
                commitReq.guid
            ];
        }
        ...
        let work = async () => {
            if (!this._validateTopologyQueue[commitReq.branchGuid]) {
                ...
                this._validateTopologyQueue[commitReq.branchGuid]
                    .drain()
                    .then(() => {
                        delete this._validateTopologyQueue[
                            commitReq.branchGuid
                        ];
                    });
            }
        ...
        return this._addPendingCommitPromise(
            commitReq.branchGuid,
            commitReq.guid,
            work()
        );
    ...
    async queueBranchGracefully(branchReq) {
        ...
        if (this._pendingBranchPromises[branchReq.guid]) {
            logger.trace(
                `Branch ${branchReq.guid} already queued, skipping the processing`
            );
    ...
    _flushBranchQueue(branchGuid, ex) {
        ...
        if (this._validateTopologyQueue[branchGuid]) {
            this._validateTopologyQueue[branchGuid].drain().then(() => {
                if (
                    !this._writeQueue[branchGuid] ||
                    this._writeQueue[branchGuid].idle()
                ) {
                ...
                (bg) =>
                    new Promise(async (res, rej) => {
                        ...
                        if (!branchResolved) {
                            ...
                            this._pendingBranchPromises[bg].then(
                                afterBranch,
                                afterBranch
                            );
                        }
