...
class BranchManager {
    ...
    async createBranch(in_params) {
        ...
        if (in_params.parentBranchGuid) {
            const parentBranch = await this.getBranch(
                in_params.parentBranchGuid
            );
            ...
            bTreeParameters = {
                ...
                initialChunkSizeFactor: this._settings.get(
                ...
                nodesPerHierarchicalHistoryLevel: this._settings.get(
        ...
        if (!rootCommit) {
            ...
            if (
                in_params.meta &&
                in_params.meta.materializedHistory &&
                in_params.meta.materializedHistory.indices
            ) {
                for (const indexName of Object.keys(
                    ...
                    await this._indexManager.createIndex({
                        ...
                        name: indexName,
                        def: in_params.meta.materializedHistory.indices[
                            indexName
                        ],
                        inBatch: {
                            batch,
            ...
            const guidGenerator = new DeterministicGuidGenerator(
                in_params.guid,
                in_params.rootCommitGuid
            );
            ...
            const templateRootNodeRef = this._btreeManager.createBTree(
                batch,
                {},
                in_params.guid,
                guidGenerator
            ...
            this._storage.store(
                batch,
                "commitTemplates:" + in_params.rootCommitGuid,
                {
                    guid: in_params.rootCommitGuid,
                    branchGuid: in_params.guid,
