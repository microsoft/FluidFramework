...
class InMemoryByValue {
    ...
    static async filterByValue(from, changeSet, limitedPaths) {
        ...
        if (retainedPaths.length > 0) {
            let filtered = PropertyUtils.getFilteredChangeSetByPaths(
                changeSet,
                retainedPaths
            );
            ...
            CLAUSES.forEach((clauseName) => {
                ...
                if (clause) {
                    ...
                    clauseFields.forEach((cf) => {
                        ...
                        tokenizedFieldsToGather[cf] =
                            PathHelper.tokenizePathString(cf);
                    });
                }
            });
            ...
            PropertyUtils.traverseChangeSetRecursivelyAsync(
                changeSet,
                {
                    preCallback: (context, cb) => {
                        ...
                        const keyForContext =
                            InMemoryByValue._getWhereClauseKey(
                                tokenizedPagingPath,
                        ...
                        if (keyForContext) {
                            try {
                                InMemoryByValue._validatePrimitiveType(
                                    context.getTypeid()
                                );
                            } catch (ex) {
                                reject(ex);
                            ...
                            paths[
                                _getPath(
                                    context._parentStack,
                                    -1 *
                                        tokenizedFieldsToGather[keyForContext]
                                            .length
                                )
                            ][keyForContext] = {
                ...
                () => {
                    let matchingPaths =
                        InMemoryByValue._matchGatheredFieldsAgainstWhere(
                            paths,
                            where
                        );
    ...
    static _getWhereClauseKey(
        ...
        let found = Object.keys(tokenizedFieldsToGather).find((f) => {
            ...
            const expectedStackLength =
                tokenizedPagingPath.length + v.length + depthLimit;
    ...
    static _evaluateEq(expected, actual, typeId) {
        const cv = ComparatorFactory.getComparator(typeId).compare(
            expected,
            actual
        );
        return cv === 0;
    ...
    static _evaluateIn(expected, actual, typeId) {
        ...
        if (typeId === "Uint64" || typeId === "Int64") {
            ...
            return expected
                .map((exp) =>
                    InMemoryByValue._castToNumberIfNeeded(typeId, exp)
                )
                .includes(actual);
        }
    }
    ...
    static _matchGatheredFieldsAgainstWhere(paths, where) {
        return Object.keys(paths).filter((p) =>
            InMemoryByValue._evaluateWhere(where, paths[p])
        );
    }
}
