...
class ScanTraversalUtils {
    ...
    static shouldStopTraversing(
        ...
        if (tokenizedPagingPath.length >= context._parentStack.length) {
            let tppSlicedToDepth = tokenizedPagingPath.slice(
                0,
                ...
                for (let tokenizedSubPath of tokenizedFieldsToGather) {
                    ...
                    if (len >= context._parentStack.length) {
                        let comparable = context._parentStack
                            .slice(0, tokenizedPagingPath.length + depthLimit)
    ...
    static isItemContext(tokenizedPagingPath, depthLimit, context) {
        ...
        if (
            expectedStackLength < context._parentStack.length ||
            tokenizedPagingPath.length >= context._parentStack.length
        ) {
            return false;
        }

        const beginPart = context._parentStack.slice(
            0,
            tokenizedPagingPath.length
        );
