...
} = require("../change_set_processing/merge_chunked_changeset");
...
class MultipleQueriesExecution {
    ...
    async execute(branchInfo, commitGuid, queryString) {
                    ...
                    case QUERY_VERSIONS.V1:
                        return new QueryV1Execution({
                            materializedHistoryService:
                                this._materializedHistoryService,
                        }).execute(branchInfo, commitGuid, q);
                    default:
                        throw new OperationError(
                            "Unsupported query language " + q.queryLanguage,
                            "GetCommit",
        ...
        return {
            changeSet: mergeChunkedChangeSet(
                multipleQueriesResults.map((qr) => qr.changeSet)
            ),
            queryPaths: _.uniq(
                _.flatten(multipleQueriesResults.map((qr) => qr.queryPaths))
            ),
        };
    }
    ...
    _validateAndParseQueryString(queryString) {
        ...
        if (result.error) {
            throw new OperationError(
                `Invalid query ${result.error.details
                    .map((e) => `${e.path}: ${e.message}`)
                    .join(", ")}`,
                "_validateAndParseQueryString",
                HTTPStatus.BAD_REQUEST,
                OperationError.FLAGS.QUIET
            );
        }
