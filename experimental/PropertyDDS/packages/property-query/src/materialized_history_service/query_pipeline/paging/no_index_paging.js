...
class NoIndexPaging {
    ...
    static async doPaging(query, changeSet, limitedPaths) {
        const tokenizedPagingPath = PathHelper.tokenizePathString(
            query.from[0].pathPrefix
        );
        ...
        if (limitedPaths) {
            ...
            memIndex = memIndex.filter((mi) =>
                standardizedLimitedPaths.includes(mi.path)
            );
        }

        const partialCheckoutPaths = memIndex
            .sort(
                NoIndexPaging._sortingComparator.bind(
                    this,
                    tokenizedOrderClauses,
                    0
                )
            ...
            .slice(
                query.paging.offset,
                query.paging.offset + query.paging.limit
            )
            .map((i) => i.path);
        ...
        if (partialCheckoutPaths.length > 0) {
            ...
            if (memIndex.length > 0) {
                return {
                    changeSet: NoIndexPaging._getEmptySetChangeSet(
                        tokenizedPagingPath,
                        changeSet
                    ),
                    queryPaths: [],
                    ...
                    preCallback: (context, cb) => {
                        ...
                        if (
                            ...
                            if (
                                ...
                                ] = Array(tokenizedOrderClauses.length).fill({
                                    value: undefined,
                                    typeId: undefined,
                                });
                            }
                        ...
                        const orderClauseIndex =
                            NoIndexPaging._getOrderClauseIndex(
                                tokenizedPagingPath,
                                tokenizedOrderClauses,
                                depthLimit,
                                context
                        ...
                        if (orderClauseIndex > -1) {
                            try {
                                NoIndexPaging._validatePrimitiveType(
                                    context.getTypeid()
                                );
                                knownSortTypes[orderClauseIndex] =
                                    NoIndexPaging._validateSameType(
                                        knownSortTypes[orderClauseIndex],
                                        context.getTypeid()
                                    );
                            } catch (ex) {
                            ...
                            idx[
                                _getPath(
                                    context,
                                    -1 *
                                        tokenizedOrderClauses[orderClauseIndex]
                                            .by.length
                                )
                            ][orderClauseIndex] = {
    ...
    static _getOrderClauseIndex(
        ...
        return tokenizedOrderClauses.findIndex((oc) => {
            const expectedStackLength =
                tokenizedPagingPath.length + oc.by.length + depthLimit;
            ...
            return (
                _.isEqual(beginPart, tokenizedPagingPath) &&
                _.isEqual(endPart, oc.by)
            );
        });
    }
    ...
    static _sortingComparator(tokenizedOrderClauses, level, a, b) {
        ...
        if (
            tokenizedOrderClauses.length === 1 &&
            tokenizedOrderClauses[0].by.length === 0
        ) {
            comparisonValue = ComparatorFactory.getKeyComparator().compare(
                a.path,
                b.path
            );
        } else {
            comparisonValue = ComparatorFactory.getComparator(typeId).compare(
                aValue,
                bValue
            );
        }
        ...
        if (comparisonValue !== 0) {
            ...
            if (tokenizedOrderClauses.length > level + 1) {
                return NoIndexPaging._sortingComparator(
                    tokenizedOrderClauses,
                    level + 1,
                    a,
                    b
