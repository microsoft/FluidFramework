...
class IndexUtils {
    ...
    static getIndexTraversalParamsFromFilter(params) {
        ...
        if (params.where) {
            ...
            const getValueFromConditions = (conditions) => {
                const value = _.times(
                    params.indexDef.fields.length,
                    _.constant(NOT_PROVIDED)
                );
    ...
    static getTraversalOrder(params) {
        ...
        if (params.order) {
            for (let i = 0; i < params.order.length; i++) {
                ...
                if (
                    params.indexDef.fields[i] &&
                    params.indexDef.fields[i].name === by
                ) {
                ...
                for (const key of sortedKeys) {
                    ...
                    if (key.startsWith(pathPrefix)) {
                        if (depthLimit < Infinity) {
                            const prefixParts =
                                PathHelper.tokenizePathString(pathPrefix);
                            const pathParts =
                                PathHelper.tokenizePathString(key);
                            if (
                                pathParts.length <=
                                prefixParts.length + depthLimit
                            ) {
                                paths.push(key);
                            }
                        } else {
    ...
    static applyRemoveInMV(mv, keys) {
        ...
        for (const key of parents) {
            if (
                mvAt.insert &&
                mvAt.insert.NodeProperty &&
                mvAt.insert.NodeProperty[key]
            ) {
