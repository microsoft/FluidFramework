...
const WHERE_CLAUSE_SCHEMA = Joi.object({
	...
	in: Joi.array().items(ALTERNATIVES_FOR_MOST_CLAUSES),
	gt: Joi.object().pattern(Joi.string(), ALTERNATIVES_FOR_MOST_CLAUSES),
	lt: Joi.object().pattern(Joi.string(), ALTERNATIVES_FOR_MOST_CLAUSES),
...
const QUERY_SCHEMA = Joi.object({
	from: Joi.array()
		...
		.max(1)
		.required(), // Currently only a single from is supported
	paging: Joi.object({
...
class QueryV1Execution {
	...
	async execute(branchInfo, commitGuid, query) {
		...
		if (query.from[0].useIndex) {
			...
			const indexDef =
				branchInfo.indices &&
				branchInfo.indices[indexName] &&
				branchInfo.indices[indexName].def;
			if (!indexDef) {
				throw new OperationError(
					`Index '${indexName}' used in query does not exist on branch ` +
						`'${branchInfo.guid}'`,
					"Query",
			...
			const { values, valueRanges, excludeValues } =
				IndexUtils.getIndexTraversalParamsFromFilter({
		...
		if (query.paging && !indexPaging) {
			// If the index is not good, fall back to in-memory paging
			let limitedPaths;
			if (result.queryPaths) {
				limitedPaths = result.queryPaths;
			}
			result = await NoIndexPaging.doPaging(query, result.changeSet, limitedPaths);
		}
	...
	_validateAndParseQueryString(query) {
		...
		if (
			result.value.from[0].typeId &&
			TypeIdHelper.isPrimitiveType(result.value.from[0].typeId)
		) {
			throw new OperationError(
		...
		if (result.value.from[0].depthLimit === -1) {
			...
			throw new OperationError(
				"from.depthLimit must not be zero",
				"GetCommit",
				HTTPStatus.BAD_REQUEST,
				OperationError.FLAGS.QUIET,
			);
