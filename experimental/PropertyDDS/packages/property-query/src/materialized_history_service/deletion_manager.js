...
const logger = ModuleLogger.getLogger(
    "HFDM.MaterializedHistoryService.DeletionManager"
);
...
class DeletionManager {
    ...
    async createDeleteBranchTask(in_params) {
        ...
        logger.debug(
            `Creating delete task ${taskGuid}, for branchGuids`,
            in_params.branchGuids
        );
        await this._writeTaskStatus(task);
        await this._writeTaskPerBranch(task);
        logger.debug(
            `Done creating delete task ${taskGuid}, beginning to process`
        );
    ...
    async _writeTaskPerBranch(task) {
        ...
        task.branchGuids.map((bg) =>
            this._storage.store(batch, `deleteTaskPerBranch:${bg}`, {
    ...
    async _deleteTaskStatus(task) {
        ...
        let deletePromises = task.branchGuids.map((bg) =>
            this._storage.delete(`deleteTaskPerBranch:${bg}`, {
                taskGuid: task.taskGuid,
            })
        );
    ...
    async getDeleteBranchTaskByBranches(in_params) {
        ...
        return await Promise.all(
            _.uniq(
                tbb.filter((t) => t !== undefined).map((t) => t.taskGuid)
            ).map(async (tg) => {
    ...
    async getDeleteBranchTask(in_params) {
        let task = await this._storage.get(
            "deleteTask:" + in_params.taskGuid,
            true
        );
    ...
    async _startProcessingTask(task) {
        try {
            ...
            logger.trace(
                `[${task.taskGuid}] Done scanning nodes`,
                task.nodesToDelete
            );
            ...
            logger.trace(
                `[${task.taskGuid}] Beginning to delete`,
                task.nodesToDelete
            );
            await this._doDeletion(task);
            logger.trace(
                `[${task.taskGuid}] Done deleting`,
                task.nodesToDelete
            );
    ...
    async retryDeleteBranchTask(task) {
        if (!task.nodesToDelete) {
            this._promisesForTasks[task.taskGuid] =
                this._startProcessingTask(task);
    ...
    async _traverseCommitChain(branchGuid, commitGuid, nodesMarkedForDeletion) {
        ...
        let fetchCommitNodes = commitNodeIds.map(async (cni) => {
            ...
            logger.trace(
                `[${branchGuid}] Beginning to fetch commitNode ${cni}`
            );
            ...
            logger.trace(
                `[${branchGuid}] Done fetching commitNode ${cni}`,
                commitNode
            );
            ...
            if (commitNode) {
                ...
                if (
                    commitNode.branchGuid &&
                    commitNode.branchGuid !== branchGuid
                ) {
                    logger.trace(
        ...
        if (node) {
            ...
            if (!node.branchGuid || node.branchGuid === branchGuid) {
                for (const delta of node.deltas) {
                    ...
                    if (nodeChangeSet) {
                        let childrenNodes =
                            nodeChangeSet["array<String>"].children
                                .insert[0][1];
                        let subTraversals = childrenNodes.map(async (cn) => {
                            let indexOfHash = cn.indexOf("#");
                            let trimmedCn =
                                indexOfHash > -1
                                    ? cn.substring(0, indexOfHash)
                                    : cn;
                            if (cn.substring(0, 2) === "l:") {
                                nodesMarkedForDeletion.add(
                                    getBaseNodeRef(trimmedCn)
                                );
    ...
    async _traverseHierarchicalHistory(nodeRef, nodesMarkedForDeletion) {
        ...
        logger.trace(
            "Beginning to traverse hierarchical history node",
            hiNodeRef
        );
        ...
        if (hiNode) {
            const addHRefs = (array) => {
                for (let i = 1; i < array.length; i++) {
                    const hNodeRef =
                        "h:" +
                        parseNodeReference(array[i].ref).guid +
                        "#" +
