...
class MaterializedHistoryService {
    ...
    async createDeleteBranchTask(params) {
        ...
        try {
            ...
            await this._branchWriteQueue.clearQueuesForDeletion(
                params.branchGuids
            );
        }
        return result;
    ...
    waitUntilCommitApplied(branchGuid, commitGuid) {
        return this._branchWriteQueue.waitUntilCommitApplied(
            branchGuid,
            commitGuid
        );
    }
    ...
    async getIndexMV(params) {
        ...
        const indexDef =
            branch.indices &&
            branch.indices[params.indexName] &&
            branch.indices[params.indexName].def;
        if (!indexDef) {
            throw new OperationError(
                `Index '${params.indexName}' does not exist on branch '${params.branchGuid}'!`,
                "GetIndexMV",
