...
(function () {
    let _ = require("lodash"),
        chunkChangeSet =
            require("./change_set_processing/chunk_change_set").chunkChangeSet,
        ...
        if (this.children.length === 0) {
            ...
            if (chunks.length > 1) {
                let lastChangeSetSize = JSON.stringify(
                    chunks[chunks.length - 1].changeSet
                ).length;
                if (
                    lastChangeSetSize <
                    this._session.bTreeParameters.chunkSize *
            ...
            if (changeSet !== undefined) {
                ...
                childrenToProcess = chunkChangeSet(
                    changeSet,
                    undefined,
                    boundaries,
                    {
                ...
                if (
                    childrenToProcess.length > 0 &&
                    childrenToProcess[0].correspondingChunkIndex === 0
                ) {
            ...
            childrenToProcess.forEach((x, i) => {
                let nodeData = this._startChildNodeProcessing(
                    x.correspondingChunkIndex,
                    x
                );

                if (
                    x.correspondingChunkIndex === 0 ||
    ...
    InternalNode.prototype._outermostChildProcessedHandler = async function () {
        await Promise.all([
            this._getSibling(
                false,
                false,
                NodeStatus.INTERNAL_OUTERTMOST_CHILDREN_PROCESSED
            ),
            this._getSibling(
                false,
                true,
    ...
    InternalNode.prototype._childrenProcessedHandler = async function () {
        ...
        let insertOperations =
            childrenArrayDiffChangeSet["array<String>"].children.insert;
        let removeOperations =
            childrenArrayDiffChangeSet["array<String>"].children.remove;
        ...
        for (let i = 0; i < this.modifiedChildIndices.length; i++) {
            ...
            if (newChildNodes) {
                ...
                insertOperations.push([
                    ...
                    newChildNodes.map(
                        InternalNode.prototype._encodeCsChildEntry
                    ),
                ]);
        ...
        childrenArrayDiffChangeSet._toReversibleChangeSet(
            nodeChangeSet.getSerializedChangeSet()
        );
        ...
        let d = (order - 1) / 2 + 1;
        let childrenInCs = this._getChildrenArrayFromCS(
            nodeChangeSet.getSerializedChangeSet()
        );
        if (childrenInCs.length > order) {
            ...
            if (!parentNode && this.newNodes.length > 1) {
                ...
                this._setNewNodes([
                    {
                        ...
                        startPath:
                            childrenInCs.length > 0
                                ? this._decodeCsChildEntry(childrenInCs[0])
                                      .startPath
                                : undefined,
                        deltaCS: !this.forceNew
                            ? childrenArrayDiffChangeSet.getSerializedChangeSet()
                            : undefined,
            ...
            if (parentNode && childrenInCs.length < d) {
    ...
    InternalNode.prototype._mergeRequiredHandler = function () {
        return this._baseMergeRequiredHandler((previousSiblingNode) => {
            ...
            if (previousSiblingNode.newNodes === undefined) {
                ...
                previousNodeCS = this._encodeChildrenCS(
                    previousSiblingNode.children
                );
                previousSiblingNode
                    ._getParent()
                ...
                let previousNodeEntry = previousSiblingNode.newNodes.splice(
                    previousSiblingNode.newNodes.length - 1,
                    1
                )[0];
                previousNodeCS = previousNodeEntry.changeSet;
            }
            childrenFromPreviousNode =
                this._getChildrenArrayFromCS(previousNodeCS);
                ...
                this._setNewNodes([
                    {
                        ...
                        startPath:
                            this.children.length > 0
                                ? this.children[0].startPath
                                : undefined,
                    },
                ]);
            }

            let children = this._getChildrenArrayFromCS(
                this.newNodes.splice(0, 1)[0].changeSet
            );

            let splitNodeList = this._splitInternalNodesList(
                childrenFromPreviousNode.concat(children)
            );
            ...
            if (this.newNodes && this.newNodes.length > 0) {
                this.mergeNeeded =
                    this._getChildrenArrayFromCS(
                        this.newNodes[this.newNodes.length - 1].changeSet
                    ).length < d;
            } else {
    ...
    InternalNode.prototype._mergedHandler = async function () {
        ...
        if (this.newNodes && this.newNodes.length === 1) {
            ...
            let subNodeChildren = this._getChildrenArrayFromCS(
        ...
        if (!noLongerNeededRoot) {
            ...
            this.newParentNode =
                childNodeData.newParentNode || childNodeData.newChildNodes[0];
    ...
    BaseNode.prototype._getChild = function (in_index) {
        ...
        return (
            childEntry &&
            this.nodeDependencyManager().getNode(
                childEntry.nodeRef,
                this._session
            )
        );
    ...
    InternalNode.prototype._splitInternalNodesList = function (in_childList) {
            ...
            return [
                {
                    ...
                    startPath:
                        in_childList.length > 0
                            ? this._decodeCsChildEntry(in_childList[0])
                                  .startPath
                            : undefined,
                },
            ];
        ...
        for (let i = 0; i < numSubNodes; i++) {
            let usedRemainder = Math.min(
                remainderPerNode,
                remainder - summedRemainder
            );
            summedRemainder += usedRemainder;
            let size = d + usedRemainder;
            if (size > order) {
                throw new OperationError(
                    "Size was greater than order!",
                    "CreateCommit",
    ...
    InternalNode.prototype._stateHandlers = {
        ...
        [NodeStatus.INTERNAL_CHILDREN_STARTED]:
            InternalNode.prototype._childrenStartedHandler,
        [NodeStatus.INTERNAL_OUTERTMOST_CHILDREN_PROCESSED]:
            InternalNode.prototype._outermostChildProcessedHandler,
        [NodeStatus.INTERNAL_CHILDREN_PROCESSED]:
            InternalNode.prototype._childrenProcessedHandler,
        [NodeStatus.MERGE_REQUIRED]:
            InternalNode.prototype._mergeRequiredHandler,
        [NodeStatus.WAITING_FOR_MERGE]:
            InternalNode.prototype._waitingForMergeHandler,
        [NodeStatus.MERGED]: InternalNode.prototype._mergedHandler,
        [NodeStatus.WAITING_FOR_DELETE]:
            InternalNode.prototype._waitingForDeleteHandler,
    };

    /**
     * @inheritdoc
     */
    BaseNode.prototype._siblingWaitStatus =
        NodeStatus.INTERNAL_CHILDREN_STARTED;
