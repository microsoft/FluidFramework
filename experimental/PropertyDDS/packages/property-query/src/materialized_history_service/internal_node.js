...
(function () {
	...
	InternalNode.prototype._fetchedHandler = async function () {
		...
		if (this.children.length === 0) {
			...
			for (let i = 0; i < chunks.length; i++) {
				...
				storePromises.push(
					this._btreeManager._storage.store(this.batch, "l:" + chunkInfo.guid, chunkInfo),
				);
				this.children.push({
				...
				this._setNewNodes([
					{
						...
						startPath:
							childrenInCs.length > 0
								? this._decodeCsChildEntry(childrenInCs[0]).startPath
								: undefined,
	...
	InternalNode.prototype._mergeRequiredHandler = function () {
		return this._baseMergeRequiredHandler((previousSiblingNode) => {
			...
			if (this.newNodes && this.newNodes.length > 0) {
				this.mergeNeeded =
					this._getChildrenArrayFromCS(this.newNodes[this.newNodes.length - 1].changeSet)
						.length < d;
			} else {
	...
	BaseNode.prototype._getChild = function (in_index) {
		...
		return (
			childEntry && this.nodeDependencyManager().getNode(childEntry.nodeRef, this._session)
		);
	};
			...
			return [
				{
					...
					startPath:
						in_childList.length > 0
							? this._decodeCsChildEntry(in_childList[0]).startPath
							: undefined,
				},
			];
		}
	...
	InternalNode.prototype._encodeChildrenCS = function (in_children) {
		return {
			"array<String>": {
				children: {
					insert: [[0, in_children.map(InternalNode.prototype._encodeCsChildEntry)]],
				},
			},
		};
	};
