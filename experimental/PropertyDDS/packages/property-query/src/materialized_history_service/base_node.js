...
(function () {
	...
	let BaseNode = function (in_params) {
		...
		if (in_params.parent) {
			this.indexInLefmostSubtree =
				in_params.parent.indexInLefmostSubtree === 0 ? in_params.indexInParent : undefined;
		} else {
	...
	BaseNode.prototype._getSibling = async function (
		...
		if (siblingIndex < 0 || siblingIndex >= parent.children.length) {
			...
			if (in_skipCompleted && parent._getStatus() >= NodeStatus.INTERNAL_CHILDREN_PROCESSED) {
				return undefined;
			}
	...
	BaseNode.prototype._writeCSNode = async function (in_nodeDefinition, in_newNodeCallback) {
		...
		if (
			...
			let limitOfSubNodesNotExceeded =
				originalNode.deltas.length < this._session.bTreeParameters.maxNodeSubEntries;
			...
			if (
				...
				this._btreeManager._storage.update(this.batch, originalNodeRef, newNode, {
					originalNodeSize,
				});
		...
		if (!deltaNodeCreated) {
			...
			this._btreeManager._storage.store(this.batch, baseNodeRef, internalNodeData);
		}
