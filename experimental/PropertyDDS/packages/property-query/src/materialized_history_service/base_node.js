...
(function () {
    const _ = require("lodash"),
        ...
        logger = ModuleLogger.getLogger(
            "HFDM.MaterializedHistoryService.BaseNode"
        );
    ...
    let BaseNode = function (in_params) {
        ...
        if (in_params.parent) {
            this.indexInLefmostSubtree =
                in_params.parent.indexInLefmostSubtree === 0
                    ? in_params.indexInParent
                    : undefined;
        } else {
        ...
        this._btreeManager =
            in_params.btreeManager || in_params.parent._btreeManager;
        ...
        if (this._session.branchGuid && this._session.commitGuid) {
            this._guidGenerator = new DeterministicGuidGenerator(
                this._session.branchGuid,
                this._session.commitGuid
            );
        }
    };
    ...
    BaseNode.startPathComparison = function (chunk1, chunk2) {
        return compareChangeSetBoundaries(
            chunk1.startPath,
            chunk2.startPath,
            true,
            true
    ...
    BaseNode.prototype._startedHandler = async function () {
        ...
        if (this._session.deleteWindow) {
            ...
            if (nodeDefinition) {
                this.nodeChangeSet =
                    await this._btreeManager._storage.getNodeChangeset(
                        this._ref
                    );
            ...
            this.nodeChangeSet =
                await this._btreeManager._storage.getNodeChangeset(this._ref);
            ...
            if (this.subtreeModification.changeSet !== undefined) {
                ...
                if (this._session.branchGuid !== nodeDefinition.branchGuid) {
                    ...
                    logger.trace(
                        `Copying node '${this._ref}' as it's modified in branch '${this._session.branchGuid}'`
                    );
                    this._setNewNodes([
                        {
     ...
     *    The status of the node after processing this handler has finished
     */
    BaseNode.prototype._baseMergeRequiredHandler = async function (
        in_mergeCallback
    ) {
        // Has this node itself been modified, or has it only been loaded to be merged
        // with a sibling?
        let directModification =
            this.subtreeModification.changeSet !== undefined;

        let previousSiblingNode = await this._getSibling(
            this.mergeNeeded && directModification,
            false,
        ...
        if (previousSiblingNode === undefined) {
            if (this.indexInLefmostSubtree === 0) {
                ...
                await this._getSibling(
                    this.mergeNeeded,
                    true,
                    NodeStatus.MERGE_REQUIRED
                );
            ...
            let mergeNeeded =
                this.mergeNeeded || previousSiblingNode.mergeNeeded;
            ...
            if (mergeNeeded) {
                if (
                    previousSiblingNode._getStatus() >
                    NodeStatus.WAITING_FOR_MERGE
                ) {
                    throw new OperationError(
                        "Merge needed but sibling node was past WAITING_FOR_MERGE",
                        "CreateCommit",
    ...
    BaseNode.prototype._baseDelete = async function () {
        ...
        const nodeDefinition = await this._btreeManager._storage.get(
            baseNodeRef
        );
        const deltaIndex = _.findIndex(
            nodeDefinition.deltas,
            (x) => x.id === nodeSubId
        );
        const previousDeltaIndex = _.findIndex(
            nodeDefinition.deltas,
            (x) => x.previousDeltaIndex === deltaIndex
        );
    ...
    BaseNode.prototype._getSibling = async function (
        ...
        if (parent._getStatus() >= NodeStatus.INTERNAL_CHILDREN_PROCESSED) {
            throw new OperationError(
                "Tried getting sibling but parent was past INTERNAL_CHILDREN_PROCESSED",
                "CreateCommit",
                HTTPStatus.INTERNAL_SERVER_ERROR
            );
        }
        ...
        if (siblingIndex < 0 || siblingIndex >= parent.children.length) {
            parent = await parent._getSibling(
                in_loadNodesIfNotAvailable,
                in_nextSibling,
                parent._siblingWaitStatus,
                in_skipCompleted,
                in_nextSibling ? 0 : -1
            );
        ...
        if (siblingNode === undefined && in_loadNodesIfNotAvailable) {
            siblingNode = parent._startChildNodeProcessing(
                siblingIndex,
                undefined,
                in_childToWaitFor
            );
    ...
    BaseNode.prototype._writeCSNode = async function (
        in_nodeDefinition,
        in_newNodeCallback
    ) {
        let nodeRef;
        ...
        if (
            in_nodeDefinition.deltaCS &&
            in_nodeDefinition.previousNodeRefs.length === 1 &&
            !in_nodeDefinition.splitNode
        ) {
            // console.log('deltaCS was available');
            let originalNodeRef = getBaseNodeRef(
                in_nodeDefinition.previousNodeRefs[0]
            );
            let originalNode = await this._btreeManager._storage.get(
                originalNodeRef
            );

            if (originalNode === undefined) {
                throw new OperationError(
                    "Original node was undefined!",
                    "CreateCommit",
                    HTTPStatus.INTERNAL_SERVER_ERROR
                );
            }
            ...
            let limitOfSubNodesNotExceeded =
                originalNode.deltas.length <
                this._session.bTreeParameters.maxNodeSubEntries;
            let nodeSizeDoesNotExceedLimit =
                originalNodeSize + deltaCS <
                this._session.bTreeParameters.chunkSize *
                    this._session.bTreeParameters.maxNodeSizeFactor;
            if (
                nodeSizeDoesNotExceedLimit &&
                limitOfSubNodesNotExceeded &&
                originalNode.branchGuid === this._session.branchGuid
            ) {
                let newNode = originalNode;
                let previousSubId = parseNodeReference(
                    in_nodeDefinition.previousNodeRefs[0]
                ).subId;
                ...
                let lastDeltaChangeSetIndex = _.findIndex(
                    newNode.deltas,
                    (x) => x.id === previousSubId
                );
                ...
                const subId = this._guidGenerator.getNextGuid(
                    previousSubId,
                    true
                );
                const delta = {
                ...
                newNode.deltas[lastDeltaChangeSetIndex].changeSet =
                    reversedDeltaCs.getSerializedChangeSet();
                newNode.deltas[lastDeltaChangeSetIndex].previousDeltaIndex =
                    deltaIndex;
        ...
        if (!deltaNodeCreated) {
            let parsedRef, guid;
            if (
                in_nodeDefinition.previousNodeRefs &&
                in_nodeDefinition.previousNodeRefs.length > 0
            ) {
                parsedRef = parseNodeReference(
                    _.last(in_nodeDefinition.previousNodeRefs)
                );
            } else {
    ...
    BaseNode.prototype._getParent = function () {
        let nodeParentRef = this.nodeDependencyManager().getNodeParent(
            this._ref,
            this._session
        );
        if (nodeParentRef) {
            let parentData = this.nodeDependencyManager().getNode(
                nodeParentRef,
                this._session
            );
            return parentData;
    ...
    BaseNode.prototype._getStatus = function () {
        return this.nodeDependencyManager().getNodeStatus(
            this._ref,
            this._session
        );
    };
     ...
     */
    BaseNode.prototype._startChildNodeProcessing = function (
        in_nodeIndexInParent,
        ...
        let nodeData = this.nodeDependencyManager().getNode(
            childNodeRef,
            this._session
        );
        if (!nodeData) {
            ...
            return this.nodeDependencyManager()._startNodeProcessing(
                childNodeRef,
                this._session,
                newNodeData,
                this
    ...
    BaseNode.prototype._waitForAllChildrenCompleted = function () {
        return this.nodeDependencyManager().waitForAllChildrenCompleted(
            this._ref,
            this._session
        );
    };
    ...
    BaseNode.prototype._waitForStatus = function (in_status) {
        return this.nodeDependencyManager().waitForNodeStatus(
            this._ref,
            this._session,
            in_status
        );
