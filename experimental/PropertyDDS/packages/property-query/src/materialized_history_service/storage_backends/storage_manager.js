...
const logger = ModuleLogger.getLogger(
    "HFDM.MaterializedHistoryService.StorageManager"
);
...
class StorageManager {
    ...
    store(in_batch, in_nodeRef, in_value) {
        ...
        if (batchCache) {
            ...
            logger.warn(
                "Attempted to set an item in the batch cache, but there was no batch cache",
                in_batch
            );
        }
        ...
        logger.trace(
            `Cache set and queued ${in_nodeRef}.  Batch ${in_batch.guid}`
        );
    }
    ...
    async get(in_nodeRef, in_consistent, in_bypassCache) {
        ...
        if (in_bypassCache) {
            let fetchedRecord = await this._backend.get(
                baseNodeReference,
                in_consistent
            );
            ...
            if (fetchedRecord) {
                return Promise.resolve(
                    this._serializer.deserialize(fetchedRecord)
                );
            }
        ...
        let getFromBackend = this._backend.get(
            baseNodeReference,
            in_consistent
        );
        this._asyncContext.incrementInContext("nodesReadFromBackend", 1);
        ...
        let racePromise = Promise.race([
            ...
            getFromBackend.then((fetchedRecord) => {
                ...
                if (fetchedRecord) {
                    ...
                    logger.trace(
                        `Tried returning ${in_nodeRef} from storage but it was empty!`
                    );
                    return undefined;
                }
            }),
    ...
    async getNodeExpectingDelta(nodeRef, expectedDelta, consistent) {
        ...
        do {
            node = await this.get(
                nodeRef,
                consistent || shouldBypassCache,
                shouldBypassCache
            );
            deltaIndex = !node
                ? -1
                : node.deltas.findIndex((x) => x.id === expectedDelta);
            if (deltaIndex === -1) {
        ...
        if (deltaIndex === -1) {
            throw new OperationError(
                `Delta Index was not found! ${nodeRef}/${expectedDelta}`,
                "GetCommit",
    ...
    async getNodeChangeset(in_nodeRef, in_consistent, in_changeSetType) {
        ...
        let { node, deltaIndex } = await this.getNodeExpectingDelta(
            in_nodeRef,
            nodeSubId,
            in_consistent
        );
        ...
        let accumulatedCS = new ChangeSet(
            in_changeSetType !== StorageManager.ChangeSetType.FULL_CHANGESET
        ...
        if (
            ...
            let previousEntry = node.deltas.find(
            ...
            deltaCS._toReversibleChangeSet(
                accumulatedCS.getSerializedChangeSet()
            );
    ...
    _serializeAndCache(key, value) {
        ...
        let storeValue = !_.isString(value)
