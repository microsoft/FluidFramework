...
(function () {
     ...
     */
    InMemoryBackend.prototype.store = function (
        in_batch,
        in_nodeRef,
        in_value
    ) {
     ...
     * @param {String} in_value - The value under which the record is stored
     */
    InMemoryBackend.prototype.update = function (
        in_batch,
        in_nodeRef,
        in_value
    ) {
    ...
    InMemoryBackend.prototype._dumpStatistics = function () {
        ...
        for (let key of Object.keys(this._data)) {
            ...
            if (type === "l") {
                dataForCS += JSON.stringify(
                    JSON.parse(this._data[key]).changeSet
                ).length;
                dataForDelta += JSON.stringify(
                    JSON.parse(this._data[key]).deltas
                ).length;
            }
        }
        ...
        for (let key of Object.keys(statistics)) {
            console.log(
                key +
                    ...
                    " size: " +
                    Math.ceil(statistics[key].size / 1024) +
                    " KB"
            );
        }
