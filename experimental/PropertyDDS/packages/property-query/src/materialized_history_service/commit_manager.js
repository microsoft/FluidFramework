		...
		return this._systemMonitor.startBackgroundTransaction("CreateCommit", async () => {
			...
			return this._asyncContext.runInNewContext(async () => {
				...
				return Promise.all(requests)
					.then(([in_branch, in_parentCommit, in_parentCommitTemplates]) => {
					...
					.then(async ([rootNode, templatesRootNode]) => {
						...
						const newTemplatesRootNode =
							templatesRootNode.newParentNode || templatesRootNode.newChildNodes[0];
						...
						let results = await this._indexManager.updateBranchIndices({
							commit: {
								...
								branchGuid: in_params.branchGuid,
								parentGuid: parentCommit.guid,
								changeSet: _.isString(in_params.changeSet)
		...
		if (in_params.onlyChanges) {
			...
			if (in_params.fetchSchemas) {
				previousCSPromises.push(
					this._storage.getNodeChangeset(previousCommitTemplatesNode.rootNodeRef),
				);
			}
		}
				...
				for (let i = 0; i < nodes.length - 1; i++) {
					...
					if (nodes[i].endPath === chunkBoundaries[i + 1]) {
						...
						let path = PathHelper.tokenizePathString(
							getPathFromChunkBoundaryFormat(chunkBoundaries[i + 1]),
						);
			...
			if (in_params.additionalPaths.length > 0) {
				for (let i = 0; i < in_params.additionalPaths.length; i++) {
					...
					let propertyPrefixExists =
						in_params.paths[indexInPaths] === in_params.additionalPaths[i] ||
						(indexInPaths > 0 &&
			...
			.then((rootNodeCS) => {
				return this._btreeManager.traverseBTree(
					...
					],
					{
						...
						nodeCallback: (node) => {
							nodes[node.ref] = node;
							nodes[node.ref].children = InternalNode.prototype.decodeCsChildren(
								node.changeSet,
							);
							if (!rootNode) {
								rootNode = node;
