...
const logger = ModuleLogger.getLogger(
    "MaterializedHistoryService.CommitManager"
);
const {
...
const {
    getEncodedTemplates,
    getDecodedTemplates,
} = require("../utils/template_conversion");
...
class CommitManager {
    ...
    createCommit(in_params) {
        ...
        return this._systemMonitor.startBackgroundTransaction(
            "CreateCommit",
            async () => {
                this._systemMonitor.addCustomAttributes({
                    ...
                    byteSize: _.isString(in_params.changeSet)
                        ...
                        : Buffer.byteLength(
                              JSON.stringify(in_params.changeSet)
                          ),
                });
                ...
                return this._asyncContext.runInNewContext(async () => {
                    ...
                    let requests = [
                        ...
                        this._storage.get(
                            "commit:" + in_params.parentGuid,
                            true
                        ),
                        this._storage.get(
                            "commitTemplates:" + in_params.parentGuid,
                            true
                        ),
                    ];
                    ...
                    return Promise.all(requests)
                        .then(
                            ([
                                in_branch,
                                in_parentCommit,
                                in_parentCommitTemplates,
                            ]) => {
                                logger.trace(
                                    "Done fetching branch for commit",
                                    in_branch,
                                    in_params.guid
                                );
                                logger.trace(
                                    "Done fetching parent commit for commit",
                                    in_parentCommit,
                                    in_params.guid
                                );
                                branch = in_branch;
                                parentCommit = in_parentCommit;
                                parentCommitTemplates =
                                    in_parentCommitTemplates;
                                ...
                                if (
                                    branch.headCommitGuid !==
                                        in_params.parentGuid &&
                                    !force
                                ) {
                                    throw new OperationError(
                                        `Parent is not the head of the branch. Expected ${branch.headCommitGuid}, got ` +
                                            in_params.parentGuid,
                                        "CreateCommit",
                                ...
                                if (!parentCommitTemplates) {
                                    throw new OperationError(
                                        "Branch does not define template information",
                                        "CreateCommit",
                                        HTTPStatus.BAD_REQUEST,
                                        OperationError.FLAGS.QUIET
                                    );
                        ...
                        .then(async () => {
                            ...
                            if (_.isString(in_params.changeSet)) {
                                ...
                                if (
                                    branch.indices &&
                                    Object.keys(branch.indices).length > 0
                                ) {
                                ...
                                async () => {
                                    return await Promise.all([
                                        this._btreeManager.updateBTree(
                                            {
                                                branchGuid:
                                                    in_params.branchGuid,
                                                commitGuid: in_params.guid,
                                                bTreeParameters:
                                                    branch.bTreeParameters,
                                            },
                        ...
                        .then(async ([rootNode, templatesRootNode]) => {
                            ...
                            await this._systemMonitor.startSegment(
                                "Storage: Finish B-tree update batches",
                                true,
                                async () => {
                            ...
                            let newCommit = {
                                ...
                                treeLevels:
                                    parentCommit.treeLevels + levelChange,
                            };

                            // Create the commit templates object
                            if (!templatesRootNode) {
                                throw new OperationError(
                                    `Error writing commit ${in_params.guid} in branch ${in_params.branchGuid}, ` +
                                        "templatesRootNode did not exist",
                                    "GetCommit",
                                    HTTPStatus.NOT_FOUND,
                                    OperationError.FLAGS.QUIET
                                );
                            ...
                            const newTemplatesRootNode =
                                templatesRootNode.newParentNode ||
                                templatesRootNode.newChildNodes[0];
                            const templatesLevelChange =
                                templatesRootNode.levelChange || 0;
                            const newCommitTemplates = {
                                branchGuid: in_params.branchGuid,
                                parentGuid: parentCommit.guid,
                                rootNodeRef: newTemplatesRootNode.nodeRef,
                                treeLevels:
                                    parentCommitTemplates.treeLevels +
                                    templatesLevelChange,
                            };
                            this._storage.store(
                                batch,
                                "commitTemplates:" + newCommit.guid,
                                newCommitTemplates
                            );
                            this._storage.store(
                                batch,
                                "commit:" + newCommit.guid,
                                newCommit
                            );
                            ...
                            let results =
                                await this._indexManager.updateBranchIndices({
                                    commit: {
                                        guid: in_params.guid,
                                        branchGuid: in_params.branchGuid,
                                        parentGuid: parentCommit.guid,
                                        changeSet: _.isString(
                                            in_params.changeSet
                                        )
                                            ? JSON.parse(in_params.changeSet)
                                            : in_params.changeSet,
                                    },
                                    inBatch: {
                                        branch,
                            ...
                            await this._systemMonitor.startSegment(
                                "Storage: Finish commit and branch update batches",
                                true,
                                async () => {
                                    await this._storage.finishWriteBatch(batch);
                                }
                            ...
                            logger.trace(
                                "Done creating commit",
                                in_params.guid
                            );

                            this._systemMonitor.addCustomAttributes(
                                this._asyncContext.getDBStats()
                            );
    ...
    _joinPaths(in_path1, in_path2) {
        ...
        while (
            tokens2.length > 0 &&
            types2[0] === PathHelper.TOKEN_TYPES.RAISE_LEVEL_TOKEN
        ) {
        ...
        for (let i = 0; i < finalTokens.length; i++) {
            switch (finalTypes[i]) {
                case PathHelper.TOKEN_TYPES.PATH_SEGMENT_TOKEN:
                    if (result.length === 0) {
                        result = PathHelper.quotePathSegmentIfNeeded(
                            finalTokens[i]
                        );
                    } else {
                        result +=
                            "." +
                            PathHelper.quotePathSegmentIfNeeded(finalTokens[i]);
                    }
                    break;
                ...
                default:
                    throw new OperationError(
                        `Unsupported token type ${finalTypes[i]}`,
                        "GetCommit",
                        HTTPStatus.BAD_REQUEST,
                        OperationError.FLAGS.QUIET
                    );
    ...
    async getCommitCS(in_params) {
        ...
        params.bTreeParameters =
            in_params.bTreeParameters || BTreeManager.ORIGINAL_BTREE_PARAMETERS;

        let changeSet;
        await this._systemMonitor.startSegment(
            "Commit Manager: Get Commit CS",
            true,
            async () => {
    ...
    async getCommitMV(in_params) {
        ...
        await this._systemMonitor.startSegment(
            "Commit Manager: Get Materialized View",
            true,
            async () => {
                materializedView = await this._getCommitChangeSetInternal(
                    params
                );
            }
        );
    ...
    async getSquashedCommitRange(in_params) {
        ...
        await this._systemMonitor.startSegment(
            "Commit Manager: Get Squashed CS",
            true,
            async () => {
                squashedChangeSet = await this._getCommitChangeSetInternal(
                    params
                );
            }
        );
    ...
    async deleteCommit(in_branchGuid, in_commitGuid) {
        ...
        if (branch.headCommitGuid !== headCommit.guid) {
            throw new OperationError(
                `Only the head commit of a branch may be deleted. '${in_commitGuid}' is not the head ` +
                    `of '${in_branchGuid}'`,
                "DeleteCommit",
                HTTPStatus.BAD_REQUEST,
                OperationError.FLAGS.QUIET
        ...
        const parentCommit = await this._storage.get(
            "commit:" + headCommit.parentGuid
        );
        const deleteWindow = {
            from: parentCommit.timestamp,
            to: headCommit.timestamp,
        };
            ...
            async () => {
                await Promise.all([
                    ...
                    this._btreeManager.updateBTree(
                        ...
                        headCommitTemplates.rootNodeRef,
                        undefined,
                        batch
                    ),
                ]);
        ...
        await this._systemMonitor.startSegment(
            "Storage: Finish B-tree update batch",
            true,
            async () => {
                await this._storage.finishWriteBatch(batch);
            }
        ...
        await this._systemMonitor.startSegment(
            "Storage: Finish Finish commit and branch update batch",
            true,
            async () => {
                await this._storage.finishWriteBatch(batch);
            }
    ...
    async _getCommitChangeSetInternal(in_params) {
        ...
        if (in_params.fetchSchemas) {
            nodeFetchPromises.push(
                this._storage.get("commitTemplates:" + in_params.guid)
            );
        }

        const [commitNode, commitTemplatesNode] = await Promise.all(
            nodeFetchPromises
        );
        ...
        let currentCSPromises = [
            this._storage.getNodeChangeset(commitNode.rootNodeRef),
        ];
        if (in_params.fetchSchemas) {
            currentCSPromises.push(
                this._storage.getNodeChangeset(commitTemplatesNode.rootNodeRef)
            );
        }
        ...
        if (in_params.onlyChanges) {
            let previousCommitGuid =
                in_params.oldCommitGuid || commitNode.parentGuid;
            if (previousCommitGuid === undefined) {
                // Should be HTTPStatus.NOT_IMPLEMENTED, but server errors could make this server get killed
                throw new OperationError(
                    "Only changes is not yet supported for the root commit",
                    "GetCommit",
                    HTTPStatus.BAD_REQUEST,
                    OperationError.FLAGS.QUIET
                );
            ...
            let previousNodePromises = [
                this._storage.get("commit:" + previousCommitGuid),
            ];
            if (in_params.fetchSchemas) {
            ...
            previousCSPromises.push(
                this._storage.getNodeChangeset(previousCommitNode.rootNodeRef)
            );
            if (in_params.fetchSchemas) {
                previousCSPromises.push(
                    this._storage.getNodeChangeset(
                        previousCommitTemplatesNode.rootNodeRef
                    )
                );
            }
        }

        const [rootNodeCS, rootNodeTemplatesCS] = await Promise.all(
            currentCSPromises
        );
        const [previousNodeCS, previousNodeTemplatesCS] = await Promise.all(
            previousCSPromises
        );
        ...
        traversalPromises.push(
            this._getEntriesFromChangeset(propertiesTraversal)
        );
        ...
        if (in_params.fetchSchemas) {
            templatesTraversal = {
                ...
                previousNode: in_params.onlyChanges
                    ...
                    : undefined,
                treeLevelDifference: in_params.onlyChanges
                    ? commitTemplatesNode.treeLevels -
                      previousCommitTemplatesNode.treeLevels
                    : undefined,
            ...
            traversalPromises.push(
                this._getEntriesFromChangeset(templatesTraversal)
            );
        }
        ...
        if (in_params.fetchSchemas) {
            _.assign(
                changeSet,
                getDecodedTemplates(templatesTraversal.mergedResultChangeSet)
            );
        }
        ...
        return {
            changeSet,
            nextPagePath:
                propertiesTraversal.finalNextPagePath !== undefined
                    ? getPathFromChunkBoundaryFormat(
    ...
    _addPathsToChangeSet(in_params) {
        ...
        return this._btreeManager
            .traverseBTree([in_params.rootNode], {
                ...
                onlyChanges: in_params.onlyChanges,
                previousNodes: in_params.previousNode
                    ? [in_params.previousNode]
                    : undefined,
            ...
            .then(({ nodes, chunkBoundaries, nextPagePath }) => {
                ...
                for (let i = 0; i < nodes.length - 1; i++) {
                    ...
                    if (nodes[i].endPath === chunkBoundaries[i + 1]) {
                        ...
                        let path = PathHelper.tokenizePathString(
                            getPathFromChunkBoundaryFormat(
                                chunkBoundaries[i + 1]
                            )
                        );
                        ...
                        if (path.length > 1) {
                            ...
                            if (!changeSetsMatch) {
                                logger.error(
                                    "Non-matching changesets found at chunk boundary (" +
                                        "branch: " +
                                        in_params.branchGuid +
                                        ", " +
                                        "commit: " +
                                        in_params.commitGuid +
                                        ", " +
                                        'path: "' +
                                        getPathFromChunkBoundaryFormat(
                                            chunkBoundaries[i + 1]
                ...
                in_params.mergedResultChangeSet = mergeChunkedChangeSet(
                    _.map(nodes, (x, i) => {
                        ...
                        if (in_params.paths.length !== 0) {
                            filteredChangeSet =
                                Utils.getFilteredChangeSetByPaths(
                                    changeSet,
                                    filterPathsTree
                                );
                        }
                        ...
                        if (in_params.ranges) {
                            ...
                            for (let j = 0; j < in_params.ranges.length; j++) {
                                const convertedRange = [
                                    convertPathToChunkBoundaryFormat(
                                        in_params.ranges[j][0]
                                    ),
                                    convertPathToChunkBoundaryFormat(
                                        in_params.ranges[j][1]
                                    ),
                                ];
                                ...
                                for (let k = 0; k < chunks.length; k++) {
                                    if (
                                        chunks[k].correspondingChunkIndex === 1
                                    ) {
                                        changeSetsToMerge.push(
                                            chunks[k].changeSet
                                        );
                                    }
                                }
                            ...
                            filteredChangeSet =
                                mergeChunkedChangeSet(changeSetsToMerge);
                        }
                        ...
                        if (in_params.pagingLimit !== undefined) {
                            ...
                            let changeSetSize =
                                this._serializer.serialize(changeSet).length;
                            if (
                                in_params.summedChangeSetSize <
                                in_params.pagingLimit
                            ) {
                                in_params.summedChangeSetSize += changeSetSize;
                            } else {
                                if (in_params.finalNextPagePath === undefined) {
                                    in_params.finalNextPagePath =
                                        chunkBoundaries[i];
                                }
                                changeSet = {};
                            }
                        ...
                        if (in_params.followReferences) {
                            ...
                            Utils.traverseChangeSetRecursively(changeSet, {
                                preCallback: (in_context) => {
                                    ...
                                    if (
                                        in_context.getTypeid().substr(0, 9) ===
                                        "Reference"
                                    ) {
        ...
        return this._addPathsToChangeSet(in_params).then(() => {
            ...
            if (in_params.additionalPaths.length > 0) {
                for (let i = 0; i < in_params.additionalPaths.length; i++) {
                    let indexInPaths = _.sortedIndex(
                        in_params.paths,
                        in_params.additionalPaths[i]
                    );
                    let propertyPrefixExists =
                        in_params.paths[indexInPaths] ===
                            in_params.additionalPaths[i] ||
                        (indexInPaths > 0 &&
                            in_params.additionalPaths[i].substr(
                                0,
                                in_params.paths[indexInPaths - 1].length
                            ) === in_params.paths[indexInPaths - 1]);

                    if (!propertyPrefixExists) {
                        filteredPaths.push(in_params.additionalPaths[i]);
                        in_params.paths.splice(
                            indexInPaths,
                            0,
                            in_params.additionalPaths[i]
                        );
    ...
    _getAllLeafsForCommit(in_params) {
        let requests = [this._storage.get("commit:" + in_params.guid)];
        let commitNode;
        return Promise.all(requests)
            .then(([commit]) => {
                if (commit === undefined) {
            ...
            .then((rootNodeCS) => {
                return this._btreeManager.traverseBTree(
                    ...
                    ],
                    {
                        ...
                        nodeCallback: (node) => {
                            nodes[node.ref] = node;
                            nodes[node.ref].children =
                                InternalNode.prototype.decodeCsChildren(
                                    node.changeSet
                                );
                            if (!rootNode) {
                                rootNode = node;
   ...
   */
                    ...
                    .traverseBTree(
                        ...
                        ],
                        {
                            ...
                            nodeCallback: (node, level) => {
                                ...
                                if (
                                    (!root && d > children.length) ||
                                    children.length > order
                                ) {
                                    throw new OperationError(
                                        `Invalid B-Tree node with ${children.length} children`,
                                        "ValidateCommit",
    ...
    _plotTree(in_commit) {
        ...
        return (
            `file://${__dirname}'/../../VisualizeTree.html?data=` +
            encodeURIComponent(
                JSON.stringify({ name: "root", children: extractTree(root) })
            )
        );
    }
}
