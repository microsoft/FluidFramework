...
class BTreeManager {
				...
				do {
					...
					if (commonPreviousLeafNode) {
						let previousNodesStartIndex =
							commonPreviousLeafNode.ref === "l:" + startLeafNodeGuid
								? commonPreviousLeafNode.endIdx
								: commonPreviousLeafNode.endIdx + 1;

						let previousNodes =
							endCommitHistoryNode.levels[commonPreviousLeafNode.level].previous;
					...
					if (!converged) {
						endCommitHistoryNode = await this._storage.get(
							"hi:" + parseNodeReference(historySequence[0].ref).guid,
						);
	...
	async updateBTree(sessionParams, rootNodeRef, changeSet, batch) {
		sessionParams.bTreeParameters =
			sessionParams.bTreeParameters || BTreeManager.ORIGINAL_BTREE_PARAMETERS;
