...
class BTreeManager {
    ...
    traverseBTree(in_startNodes, in_params = {}) {
        ...
        if (pagingLimit) {
            let minChunkSize =
                bTreeParameters.chunkSize * bTreeParameters.mergeLimitFactor;
                    ...
                    for (; j < in_nodes.length; j++) {
                        ...
                        if (nodeChildren.length > 0) {
                            ...
                            if (endPath === undefined) {
                                if (
                                    firstStartPath > startPath &&
                                    firstStartPath.substr(
                                        0,
                                        startPath.length
                                    ) !== startPath
                                ) {
                            ...
                            if (firstChildIndex < nodeChildren.length) {
                                currentPath =
                                    nodeChildren[firstChildIndex].startPath ||
                                    "";
                            }
                            while (
                                firstChildIndex < nodeChildren.length &&
                                (endPath !== undefined
                                    ? currentPath < endPath
                                    : currentPath.substr(0, startPath.length) <=
                                      startPath)
                            ) {
                                nodeRefsMap.set(
                                    nodeChildren[firstChildIndex].nodeRef,
                                    {
                                        nodeIndex: j,
                                        childIndex: firstChildIndex,
                                        startPath:
                                            nodeChildren[firstChildIndex]
                                                .startPath,
                                        endPath: nodeChildren[
                                            firstChildIndex + 1
                                        ]
                                            ? nodeChildren[firstChildIndex + 1]
                                                  .startPath
                                            : in_nodes[j].endPath,
                                        nodeRef:
                                            nodeChildren[firstChildIndex]
        ...
        const traversalFunction = (
            ...
            if (previousNodes && thisTreeLevelDifference <= 0) {
                ...
                for (let i = 0; i < childNodes.length; i++) {
                    let previousNodeIdx = _.findIndex(
                        previousChildNodes,
                        (x) => x.nodeRef === childNodes[i].nodeRef
                    );
            ...
            if (pagingLimit !== undefined) {
                if (previousNodes) {
                    ...
                    throw new OperationError(
                        "Chunking is currently not supported when requesting a changeSet",
                        "GetCommit",
                ...
                let maxNeededNodes = Math.ceil(
                    maxLeafsToFetch / minLeafsPerChild
                );
                ...
                let nodesToKeep =
                    Math.min(maxNeededNodes, childNodes.length - chunkStart) +
                    1;

                if (chunkStart + nodesToKeep < childNodes.length) {
                    if (childNodes[chunkStart + nodesToKeep]) {
                        nextPagePath =
                            childNodes[chunkStart + nodesToKeep].startPath;
                    } else {
                        nextPagePath =
                            childNodes[chunkStart + nodesToKeep - 1].endPath;
                    }
                }
                childNodes = childNodes.slice(
                    chunkStart,
                    chunkStart + nodesToKeep
                );
            }
            ...
            if (previousChildNodes.length > 0) {
                let fetchingLeafs =
                    parseNodeReference(previousChildNodes[0].nodeRef).type ===
                    "l";
                if (!fetchingLeafs) {
                    childNodesToFetch =
                        childNodesToFetch.concat(previousChildNodes);
                }
            }

            // Get all child nodes
            return Promise.all(
                childNodesToFetch.map((x) => {
                    if (
                        previousNodes &&
                        parseNodeReference(x.nodeRef).type === "l"
                    ) {
                        return this._getNodeDeltaChangeset(
                            x,
                            previousChildNodes,
                            directParent
                        );
    ...
    async _getNodeDeltaChangeset(
        ...
        if (
            startIndex === in_previousNodes.length ||
        ...
        while (
            in_previousNodes[endIndex].endPath !== undefined &&
            in_previousNodes[endIndex].endPath < in_nodeInfo.endPath
        ) {
            endIndex++;
        }
        endIndex++;
        let overlappingPreviousNodes = in_previousNodes.slice(
            startIndex,
            endIndex
        );
        ...
        let correspondingPreviousNode = _.find(
            overlappingPreviousNodes,
            (x) => getBaseNodeRef(x.nodeRef) === nodeBaseRef
        );
        if (correspondingPreviousNode) {
            // The changeSet is contained inside of one node, so we can fetch that node
            return await this._storage.getNodeChangeset(
                in_nodeInfo.nodeRef,
                false,
            ...
            if (in_directParent) {
                ...
                let historyNodeRef =
                    "h" +
                    nodeBaseRef.substr(1) +
                    "#" +
                    parseNodeReference(overlappingPreviousNodes[0].nodeRef)
                        .guid;
                ...
                if (overlappingPreviousNodes.length !== 1) {
                    throw new OperationError(
                        "There must be exactly one overlapping previous node!",
                        "GetNodeDeltaCS",
                ...
                let endLeafNodeGuid = parseNodeReference(
                    in_nodeInfo.nodeRef
                ).guid;
                ...
                let [startCommitHistoryNode, endCommitHistoryNode] =
                    await Promise.all([
                ...
                if (!startCommitHistoryNode || !endCommitHistoryNode) {
                    throw new OperationError(
                        "Invalid hierarchical history. Start and end node must exist.",
                        "GetNodeDeltaCS",
                        HTTPStatus.INTERNAL_SERVER_ERROR
                    );
                }
                ...
                let historySequence = [
                    _.last(endCommitHistoryNode.levels[0].current),
                ];
                let converged = true;
                do {
                    ...
                    for (
                        let i = 0;
                        i < endCommitHistoryNode.levels.length &&
                        !commonLeafNode;
                        i++
                    ) {
                        let startLevel = Math.min(
                            i,
                            startCommitHistoryNode.levels.length - 1
                        );

                        let currentEndEntries =
                            endCommitHistoryNode.levels[i].current;
                        let currentStartEntries =
                            startCommitHistoryNode.levels[startLevel].current;
                        for (
                            let j = currentEndEntries.length - 1;
                            j >= 0 && !commonLeafNode;
                            j--
                        ) {
                    ...
                    if (!commonLeafNode) {
                        throw new OperationError(
                            "Invalid hierarchical history. Could not find common leaf node.",
                            "CreateCommit",
                            HTTPStatus.INTERNAL_SERVER_ERROR
                        );
                    }
                    ...
                    for (let i = 0; i < commonLeafNode.level; i++) {
                        ...
                        historySequence.unshift.apply(
                            historySequence,
                            currentNodes
                        );
                    }
                    let currentNodesStartIndex =
                        commonLeafNode.ref === "l:" + startLeafNodeGuid
                            ? commonLeafNode.endIdx
                            : commonLeafNode.endIdx + 1;

                    currentNodes =
                        endCommitHistoryNode.levels[commonLeafNode.level]
                            .current;
                    currentNodes = currentNodes.slice(
                    ...
                    for (
                        let i = 0;
                        i < commonLeafNode.level && !commonPreviousLeafNode;
                        i++
                    ) {
                        let startLevel = Math.min(
                            i,
                        ...
                        let previousEndEntries =
                            endCommitHistoryNode.levels[i].previous;
                        let currentStartEntries =
                            startCommitHistoryNode.levels[startLevel].current;
                        for (
                            let j = previousEndEntries.length - 1;
                            j >= 0 && !commonPreviousLeafNode;
                            j--
                        ) {
                    ...
                    if (commonPreviousLeafNode) {
                        let previousNodesStartIndex =
                            commonPreviousLeafNode.ref ===
                            "l:" + startLeafNodeGuid
                                ? commonPreviousLeafNode.endIdx
                                : commonPreviousLeafNode.endIdx + 1;

                        let previousNodes =
                            endCommitHistoryNode.levels[
                                commonPreviousLeafNode.level
                            ].previous;
                        previousNodes = previousNodes.slice(
                            previousNodesStartIndex,
                    ...
                    converged =
                        historySequence[0].ref === "l:" + startLeafNodeGuid;

                    if (!converged) {
                        endCommitHistoryNode = await this._storage.get(
                            "hi:" +
                                parseNodeReference(historySequence[0].ref).guid
                        );
                        if (!endCommitHistoryNode) {
                            throw new OperationError(
                                "Invalid history information. Could not find history info node.",
                                "CreateCommit",
                                HTTPStatus.INTERNAL_SERVER_ERROR
                            );
                        }
                ...
                let nodes = await Promise.all(
                    _.map(historySequence.slice(1), (endNode, idx) => {
                        let startNode = historySequence[idx];
                        return this._storage.get(
                            "h:" +
                                parseNodeReference(endNode.ref).guid +
                                "#" +
