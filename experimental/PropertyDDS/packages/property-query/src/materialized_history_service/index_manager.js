...
const logger = ModuleLogger.getLogger(
    "HFDM.MaterializedHistoryService.IndexManager"
);
...
class IndexManager {
    ...
    async updateBranchIndices(params) {
        ...
        let indexHeads = _.mapValues(
            branch.indices,
            (index) => index.head.guid
        );
        ...
        const indicesToProcess = _.pick(
            branch.indices,
            Object.keys(indexHeads)
        );
        ...
        Utils.traverseChangeSetRecursively(changeSet, {
            preCallback: (context) => {
                if (context.getOperationType() === "remove") {
                    ...
                    const indicesWithSchema = filterByIncludeSchema(
                        context.getTypeid()
                    );
                    ...
                    const matchedParent = matchedChangeSets.get(
                        context.getParentNestedChangeSet()
                    );
                    if (matchedParent) {
                ...
                while (!iterator.atEnd()) {
                    switch (iterator.opDescription.type) {
                        ...
                        case ArrayChangeSetIterator.types.MODIFY:
                            offset =
                                iterator.currentOffset +
                                iterator.opDescription.operation[0];
                            value =
                                value.substring(0, offset) +
                                iterator.opDescription.operation[1] +
                                value.substring(
                        ...
                        case ArrayChangeSetIterator.types.REMOVE:
                            ...
                            value =
                                value.substring(0, offset) +
                                value.substring(
                                    offset + iterator.lastOperationOffset * -1
                                );
                            break;
                        default:
                            break;
                ...
                return await this._btreeManager.updateBTree(
                    {
                        ...
                        pathBuilder:
                            IndexUtils.indexChunkPathBuilder.bind(null),
                        sortKeyEncoder:
                            IndexUtils.normalizedToBinaryEncoder.bind(
                                null,
                                indexDef.fields,
                                keyEncoder
                            ),
    ...
    async _getChangeSetFromIndexUpdates(params) {
        ...
        const applyInsertInMV = (keys) => {
            const { keysFoundCount } = traverseAndCreate(
                indexMV,
                keys,
                "insert"
            );
        ...
        const applyRemoveInChangeSet = (keys, keysToKeepCount) => {
            ...
            let { csAt, keysFoundCount } = traverseAndCreate(
                changeSet,
                keysToModify,
                "modify"
            );
        ...
        if (indexUpdates.insert) {
            for (const path of Object.keys(indexUpdates.insert)) {
                ...
                for (const field of indexDef.fields) {
                    keys.push(
                        IndexUtils.normalizeValue(
                            indexUpdates.insert[path][field.name]
                        )
                    );
                }
                keys.push(path);
        ...
        if (indexUpdates.modify) {
            for (const path of Object.keys(indexUpdates.modify)) {
                ...
                for (const field of indexDef.fields) {
                    keys.push(
                        IndexUtils.normalizeValue(
                            indexUpdates.modify[path][field.name].newValue
                        )
                    );
                }
                keys.push(path);
            ...
            for (const path of Object.keys(indexUpdates.modify)) {
                ...
                for (const field of indexDef.fields) {
                    keys.push(
                        IndexUtils.normalizeValue(
                            indexUpdates.modify[path][field.name].oldValue
                        )
                    );
                }
                keys.push(path);
                const keysFoundCount = IndexUtils.applyRemoveInMV(
                    indexMV,
                    keys
                );
        ...
        if (indexUpdates.remove) {
            for (const path of Object.keys(indexUpdates.remove)) {
                ...
                for (const field of indexDef.fields) {
                    keys.push(
                        IndexUtils.normalizeValue(
                            indexUpdates.remove[path][field.name]
                        )
                    );
                }
                keys.push(path);
    ...
    async getIndexMV(params) {
        ...
        if (filtering) {
            ({ values, valueRanges, excludeValues, pathPrefix, depthLimit } =
                filtering);
        }
        ...
        const rootNodeCS = await this._storage.getNodeChangeset(
        ...
        const keyEncoder = new IndexKeyEncoder(indexDef.fields);
        const paths = values
            ? values.map((valueArray) => keyEncoder.encode(valueArray))
            : [];
        paths.sort();
        let ranges;
        if (valueRanges) {
        ...
        const convertValueToChangeSetPath = (valueArray) =>
            valueArray
                .map((part) => {
                    return PathHelper.quotePathSegmentIfNeeded(
                        IndexUtils.normalizeValue(part)
                    );
                })
                .join(".");
        const valuesPaths = values
            ? values.map(convertValueToChangeSetPath)
            : [];
            ...
            _.map(nodes, (x, i) => {
                ...
                if (excludeValues) {
                    for (const value of excludeValues) {
                        ...
                        IndexUtils.applyRemoveInMV(
                            changeSet,
                            value.map(IndexUtils.normalizeValue)
                        );
                    }
                }
