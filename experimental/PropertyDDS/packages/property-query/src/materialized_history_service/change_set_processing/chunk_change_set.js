...
(function () {
	...
	let _addPropertyToChunk = function (
		...
		let stackIndex =
			in_stackIndex === undefined ? in_traversalState.stack.length - 1 : in_stackIndex;
	...
	let _descendIntoSubChangesetForKey = function (
		...
		if (newChunkCS === CHUNK_SIZE_EXCEEDED) {
			...
			if (
				(in_key.operation === MODIFY_PROPERTY || in_key.operation === STATIC_PROPERTY) &&
				in_traversalState.firstModifyWithoutContent === undefined
			) {
			...
			if (in_key.operation === INSERT_PROPERTY || in_key.operation === REMOVE_PROPERTY) {
				in_traversalState.firstModifyWithoutContent = undefined;
			...
			for (; state.currentIndex < state.keys.length; state.currentIndex++) {
				...
				if (traversalState.currentPathRightBoundary) {
					...
					let pathOverlapsWithRightBoundary =
						...
						traversalState.currentPathRightBoundary.substr(0, propertyPath.length) ===
							propertyPath &&
					...
					if (
						...
						(traversalState.currentPathRightBoundary === undefined ||
							(propertyPath < traversalState.currentPathRightBoundary &&
