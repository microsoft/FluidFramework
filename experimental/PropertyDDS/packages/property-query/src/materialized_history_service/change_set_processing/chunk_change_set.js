...
(function () {
    const _ = require("lodash"),
        ...
        TypeIdHelper =
            require("@fluid-experimental/property-changeset").TypeIdHelper,
        PathHelper =
            require("@fluid-experimental/property-changeset").PathHelper;
            ...
            if (_.isArray(in_changeSet.remove)) {
                for (let k = 0; k < in_changeSet.remove.length; k++) {
                    result.push({
                        ...
                        sortKey: in_sortKeyEncoder(
                            in_changeSet.remove[k],
                            in_stackDepth
                        ),
    ...
    let _isLeaf = function (in_typeid, in_data) {
        ...
        if (
            context.context === "map" &&
            TypeIdHelper.isPrimitiveType(context.typeid)
        ) {
            return true;
        }
        ...
        if (TypeIdHelper.isTemplateTypeid(context.typeid)) {
            return (
                (_.isObject(in_data) && _.isEmpty(in_data)) ||
                context.isEnum || // Enum
                TypeIdHelper.isPrimitiveType(context.typeid)
            ); // Reference
        } else {
            return (
                context.typeid !== "NamedNodeProperty" &&
     ...
     */
    let estimateSizeForInsertionInObject = function (
        in_object,
        in_key,
        in_value,
        in_isEmpty,
        ...
        let valueSize =
            in_knownSize !== undefined ? in_knownSize : _len(in_value);
     ...
     */
    let _addPropertyToChunk = function (
        in_traversalState,
        in_key,
        in_initialize,
        in_forceInsertion,
        ...
        if (in_key.operation === REMOVE_PROPERTY) {
            ...
            if (in_traversalState.maxChunkSize !== Infinity) {
                insertSize += estimateSizeForInsertionInObject(
                    chunkCS,
                    "remove",
                    [],
                    chunkCSIsEmpty,
            ...
            if (in_traversalState.maxChunkSize !== Infinity) {
                ...
                insertSize += estimateSizeForInsertionInObject(
                    subChunk[in_key.typeid] || {},
                    in_key.key,
                    !in_initialize ? in_key.data : {},
                    subChunk[in_key.typeid] === undefined,
            ...
            if (
                in_key.operation === INSERT_PROPERTY ||
            ...
            if (
                in_key.operation !== STATIC_PROPERTY &&
                in_key.operation !== MODIFY_PROPERTY
            ) {
                in_traversalState.firstModifyWithoutContent = undefined;
            }
     ...
     * @return {boolean} - ???
     */
    let _descendIntoSubChangesetForKey = function (
        in_traversalState,
        in_key,
        in_pathBuilder,
        in_sortKeyEncoder
        ...
        let newChunkCS = _addPropertyToChunk(
            in_traversalState,
            in_key,
            true,
            in_traversalState.firstInsertInChunk
        );
        ...
        if (newChunkCS === CHUNK_SIZE_EXCEEDED) {
            ...
            if (!in_traversalState.firstInsertInChunk) {
                ...
                in_traversalState.stack[in_traversalState.stack.length - 1]
                    .currentIndex--;
                return chunkFull;
            }
        } else {
            ...
            if (
                ...
                in_traversalState.firstModifyWithoutContent =
                    in_traversalState.stack.length;
                in_traversalState.chunkSizeBeforeFirstModifyWithoutContent =
                    oldChunkSize;
            }
        ...
        in_traversalState.stack.push({
            ...
            useSquareBrackets:
                context === "map" || context === "set" || context === "array",
            currentIndex: 0,
            currentKey: in_key,
        });
    ...
    let _updatePathBoundaries = function (in_traversalState, in_propertyPath) {
        ...
        if (
            !_.some(
                in_traversalState.stack,
                (x) => x.insertOrRemoveOverlapsWithRightBoundary
            )
        ) {
            ...
            chunkIndex = _.sortedIndex(
                in_traversalState.pathBoundaries,
                in_propertyPath
            );
            ...
            if (
                in_traversalState.pathBoundaries[chunkIndex] === in_propertyPath
            ) {
                chunkIndex++;
            }
        } else {
        ...
        in_traversalState.currentPathRightBoundary =
            in_traversalState.pathBoundaries[chunkIndex];
    };
    ...
    let chunkChangeSet = function (
        ...
        let pathBuilder =
            (in_options && in_options.pathBuilder) || _joinKeyWithPath;
        let sortKeyEncoder =
            (in_options && in_options.sortKeyEncoder) || _escapeKeyForSorting;
        let startChunk = {};
        let traversalState = {
            stack: [
                {
        ...
        while (traversalState.stack.length !== 0) {
            ...
            for (
                ...
                if (traversalState.currentPathRightBoundary) {
                    ...
                    let pathOverlapsWithRightBoundary =
                        ...
                        traversalState.currentPathRightBoundary.substr(
                            0,
                            propertyPath.length
                        ) === propertyPath &&
                        traversalState.currentPathRightBoundary !==
                    ...
                    state.insertOrRemoveOverlapsWithRightBoundary =
                        (key.operation === INSERT_PROPERTY ||
                            key.operation === REMOVE_REVERSIBLE_PROPERTY ||
                            key.operation === REMOVE_PROPERTY) &&
                        pathOverlapsWithRightBoundary;
                    ...
                    if (
                        (!firstDescent || !traversalState.firstInsertInChunk) &&
                        (traversalState.currentPathRightBoundary <=
                            propertyPath ||
                            (state.insertOrRemoveOverlapsWithRightBoundary &&
                                key.operation === REMOVE_PROPERTY))
                ...
                if (
                    traversalState.chunks.length > 1 &&
                    traversalState.currentChunk.startPath === undefined
                ) {
                    traversalState.currentChunk.startPath = pathBuilder(
                        state.path,
                        key,
                        traversalState.stack.length - 1
                    );
                ...
                if (leaf) {
                    ...
                    chunkFull = _descendIntoSubChangesetForKey(
                        traversalState,
                        key,
                        pathBuilder,
                        sortKeyEncoder
            ...
            let addNewChunk = (updatePathBoundaries) => {
                ...
                for (let i = 1; i < traversalState.stack.length; i++) {
                    traversalState.stack[i].chunkCS = _addPropertyToChunk(
                        traversalState,
            ...
            if (chunkFull) {
                ...
                if (traversalState.firstModifyWithoutContent !== undefined) {
                    let parentCS =
                        traversalState.stack[
                            traversalState.firstModifyWithoutContent - 1
                        ].chunkCS;
                    let key =
                        traversalState.stack[
                            traversalState.firstModifyWithoutContent
                        ].currentKey;
                    ...
                    traversalState.currentChunk.size =
                        traversalState.chunkSizeBeforeFirstModifyWithoutContent;
            ...
            if (
                state === traversalState.stack[traversalState.stack.length - 1]
            ) {
     ...
     */
    let compareChangeSetBoundaries = function (
        path1,
        path2,
        startPath1,
        startPath2
        ...
        if (path1 === path2) {
            if (path1 === undefined && startPath1 !== startPath2) {
