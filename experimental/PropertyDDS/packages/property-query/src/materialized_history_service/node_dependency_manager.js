...
const _ = require("lodash"),
    ...
    logger = ModuleLogger.getLogger(
        "HFDM.MaterializedHistoryService.NodeDependencyManager"
    ),
 ...
 */
NodeDependencyManager.prototype.registerNodeType = function (
    in_nodeTypeName,
    in_constructor
) {
        ...
        if (in_parentRef) {
            ...
            if (isCreateCommitSession) {
                if (
                    parent.status >=
                        NodeDependencyManager.NodeStatus
                            .INTERNAL_CHILDREN_PROCESSED &&
                    parent.status <= NodeDependencyManager.NodeStatus.COMPLETED
                ) {
                    throw new OperationError(
                        "Parent's status must be before INTERNAL_CHILDREN_PROCESSED",
                        "CreateCommit",
        ...
        if (isCreateCommitSession) {
            ...
            if (
                ...
                for (let child of nodeStatus.children) {
                    if (
                        child.status <
                        NodeDependencyManager.NodeStatus.COMPLETED
                    ) {
                        throw new OperationError(
                            "Child's status must be COMPLETED",
                            "CreateCommit",
                            HTTPStatus.INTERNAL_SERVER_ERROR
                        );
                    }
        ...
        this._eventQueue.push(() => {
            return Promise.resolve()
                .then(() => {
                    if (session.failed) {
                        logger.trace(
                            `Early cancellation for ${in_node._ref} to ${in_newStatus}`
                        );
                        return Promise.resolve(
                            NodeDependencyManager.NodeStatus.ERROR
                        );
                    } else {
                ...
                .then((nextState) => {
                    let promiseIndex =
                        session.pendingPromises.indexOf(currentPromise);
                    session.pendingPromises.splice(promiseIndex, 1);

                    logger.trace(
                        `Node ${in_node._ref} to ${in_newStatus} was succesful`
                    );
                    logger.trace(
                        `Pending promise count: ${session.pendingPromises.length}`
                    );
                    // Even if we succeded, the session might have failed already
                    if (
                        session.pendingPromises.length === 0 &&
                        session.failed
                    ) {
                ...
                .catch((error) => {
                    ...
                    logger.trace(
                        `Node ${in_node._ref} to ${in_newStatus} failed!`
                    );
                    logger.trace(error);
                    logger.trace(
                        `Pending promise count: ${session.pendingPromises.length}`
 ...
 */
NodeDependencyManager.prototype.__getNodeName = function (
    in_nodeRef,
    in_session
) {
    let pathToNode = [];
 ...
 */
NodeDependencyManager.prototype.waitForNodeStatus = function (
    in_nodeRef,
    in_session,
    in_newStatus
) {
        ...
        if (nodeStatus.status >= in_newStatus) {
            if (nodeStatus.status >= NodeDependencyManager.NodeStatus.ERROR) {
                if (nodeStatus.session.failed) {
                    ...
                    reject(
                        new OperationError(
                            `Awaited node '${in_nodeRef}' has failed`,
                            "CreateCommit",
                            HTTPStatus.INTERNAL_SERVER_ERROR
                        )
                    );
 ...
 *     The current session
 */
NodeDependencyManager.prototype._removeNodeFromSession = function (
    in_nodeRef,
    in_session
) {
...
NodeDependencyManager.prototype.getNode = function (in_nodeRef, in_session) {
    return (
        in_session.nodeStatus[in_nodeRef] &&
 ...
 */
NodeDependencyManager.prototype.getNodeParent = function (
    in_nodeRef,
    in_session
) {
    return (
 ...
 */
NodeDependencyManager.prototype.getNodeStatus = function (
    in_nodeRef,
    in_session
) {
    return (
...
NodeDependencyManager.prototype.waitForAllChildrenCompleted = function (
    ...
    const waitFunction = () => {
        ...
        let pendingChildren = _.filter(
            node.children,
            (child) => child.status < NodeDependencyManager.NodeStatus.COMPLETED
        );
 ...
 */
NodeDependencyManager.prototype._startNodeProcessing = function (
    in_nodeRef,
    in_session,
    in_nodeData,
    in_parentNode
    ...
    this._setNodeStatus(
        node,
        NodeDependencyManager.NodeStatus.STARTED,
        in_parentNode && in_parentNode._ref
    );
