...
(function () {
    let _ = require("lodash"),
        ...
        mergeChunkedChangeSet =
            require("./change_set_processing/merge_chunked_changeset").mergeChunkedChangeSet,
        ...
        ConsoleUtils =
            require("@fluid-experimental/property-common").ConsoleUtils,
        OperationError =
    ...
    LeafNode.prototype._fetchedHandler = function () {
        ...
        let modificationPerformed =
            this.subtreeModification.changeSet !== undefined;
        ...
        if (modificationPerformed) {
            let changeSetSize = JSON.stringify(
                newChangeset.getSerializedChangeSet()
            ).length;
            if (
                changeSetSize >
          ...
          newSize = Math.ceil(changeSetSize / 2); // Some leeway to prevent a tiny last chunk
                ...
                let chunkedDeltaCS = chunkChangeSet(
                    deltaCS.getSerializedChangeSet(),
                    undefined,
                ...
                if (newChunks.length > 1) {
                    let lastChangeSetSize = JSON.stringify(
                        newChunks[newChunks.length - 1].changeSet
                    ).length;
                    if (
                        lastChangeSetSize <
                        this._session.bTreeParameters.chunkSize *
                        ...
                        let lastTwoChunks = newChunks.splice(
                            newChunks.length - 2
                        );
                        let mergedCS = mergeChunkedChangeSet(
                            lastTwoChunks.map((x) => x.changeSet)
                        );
                        newChunks.push({
                            changeSet: mergedCS,
                            startPath: lastTwoChunks[0].startPath,
                            deltaCS: mergeChunkedChangeSet(
                                lastTwoChunks.map((x) => x.deltaCS)
                            ),
                            splitNode: true,
    ...
    LeafNode.prototype._mergeRequiredHandler = function () {
        return this._baseMergeRequiredHandler((previousSiblingNode) => {
            ...
            if (previousSiblingNode.newNodes === undefined) {
                ...
                previousSiblingNode
                    ._getParent()
                    .modifiedChildIndices.push(
                        previousSiblingNode.indexInParent
                    );
                ...
                let lastPreviousChunk = previousSiblingNode.newNodes.splice(
                    previousSiblingNode.newNodes.length - 1,
                    1
                )[0];
            ...
            let mergedCS = mergeChunkedChangeSet([
                this.newNodes[0].changeSet,
                changeSet,
            ]);
            this.newNodes[0].changeSet = mergedCS;
            this.newNodes[0].startPath = startPath;
            this.newNodes[0].previousNodeRefs = previousNodeRefs.concat(
                this.newNodes[0].previousNodeRefs
            );
            this.newNodes[0].deltaCS = mergeChunkedChangeSet([
                this.newNodes[0].deltaCS,
                previousDeltaCS,
            ]);
    ...
    LeafNode.prototype._mergedHandler = async function () {
        await this._writeNewNodes(
            async (baseNodeRef, nodeData, nodeDefinition) => {
                ...
                if (nodeDefinition.deltaCS) {
                    ...
                    let newHistoryNodeLevels = deepCopy(
                        previousHistoryNode.levels
                    );
                    ...
                    for (
                        currentLevelIndex = 0, nextLevelNeedsUpdate = true;
                        currentLevelIndex < newHistoryNodeLevels.length;
                        currentLevelIndex++
                    ) {
                        let currentLevel =
                            newHistoryNodeLevels[currentLevelIndex];
                        if (currentLevel.current.length >= 1) {
                            let lastEntry =
                                currentLevel.current[
                                    currentLevel.current.length - 1
                                ];
                    ...
                    for (
                        ...
                        if (currentLevel.current.length > 1) {
                            historyNodesToAdd.push({
                                from: currentLevel.current[
                                    currentLevel.current.length - 2
                                ],
                                to: currentLevel.current[
                                    currentLevel.current.length - 1
                                ],
                        ...
                        if (
                            currentLevel.current.length >
                            this._session.bTreeParameters
                                .nodesPerHierarchicalHistoryLevel
                        ) {
                    ...
                    for (let k = 0; k < historyNodesToAdd.length; k++) {
                        ...
                        if (connectionInfo.level > 0) {
                            let previousLevelCommits =
                                newHistoryNodeLevels[connectionInfo.level - 1]
                                    .previous;
                            let changeSets = await Promise.all(
                                _.map(
                                    previousLevelCommits.slice(1),
                                    async (toCommitInfo, i) => {
                ...
                return {};
                                        ...
                                        let fromCommitInfo =
                                            previousLevelCommits[i];
                                        if (connectionInfo.level === 1) {
                                            let historyNodePromise = undefined;
                                            if (
                                                toCommitInfo.ref === baseNodeRef
                                            ) {
                                                historyNodePromise =
                                                    Promise.resolve({
                                                        changeSet:
                                                            nodeDefinition.deltaCS,
                                                    });
                                            } else {
                                                historyNodePromise =
                                                    this._btreeManager._storage.get(
                                                        "h:" +
                                                            parseNodeReference(
                                                                toCommitInfo.ref
                                                            ).guid +
                                            ...
                                            let [
                                                ...
                                                historyNodePromise,
                                                this._btreeManager._storage.getNodeChangeset(
                                                    fromCommitInfo.ref +
                                                        ":" +
                                                        fromCommitInfo.lastSubId,
                                                    false,
    ...
    LeafNode.prototype._stateHandlers = {
        ...
        [NodeStatus.WAITING_FOR_MERGE]:
            LeafNode.prototype._waitingForMergeHandler,
        [NodeStatus.MERGED]: LeafNode.prototype._mergedHandler,
        [NodeStatus.WAITING_FOR_DELETE]:
            LeafNode.prototype._waitingForDeleteHandler,
    };
