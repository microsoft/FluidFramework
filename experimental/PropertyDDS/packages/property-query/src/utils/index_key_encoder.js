...
class IndexKeyEncoder {
    ...
    encode(values) {
        if (values.length !== this._fields.length) {
            throw new OperationError(
                `Wrong number of values. Provided ${values.length}, expected ${this._fields.length}`,
                "IndexEncoding",
        ...
        return values
            .map((value, index) =>
                this.encodeSingleValue(value, this._fields[index].typeId)
            )
            .join("");
    }
    ...
    decode(encodedValue) {
        ...
        for (let i = 0; i < encodedValue.length; i++) {
            ...
            if (!decoder) {
                throw new Error(
                    `Type code '0x${encodedValue[i]
                        .charCodeAt(0)
                        .toString(16)}' not recognized!`
                );
            }
    ...
    _initializeEncoders() {
        ...
        this._encoders[IndexKeyEncoder.Type.String] =
            this._encodeString.bind(this);
        this._encoders[IndexKeyEncoder.Type.Integer] =
            this._encodeInteger.bind(this);
        this._encoders[IndexKeyEncoder.Type.Boolean] =
            this._encodeBoolean.bind(this);
        this._encoders[IndexKeyEncoder.Type.Single] = (value) =>
            this._encodeFloat(value, false);
        this._encoders[IndexKeyEncoder.Type.Double] = (value) =>
            this._encodeFloat(value, true);
    }
    ...
    _initializeDecoders() {
        ...
        this._decoders[CODE.FALSE] = (encodedValue, i) => ({
            value: false,
            endIndex: i,
        });
        this._decoders[CODE.TRUE] = (encodedValue, i) => ({
            value: true,
            endIndex: i,
        });
        ...
        for (
            let i = CODE.INT_MIN.charCodeAt(0);
            i <= CODE.INT_MAX.charCodeAt(0);
            i++
        ) {
    ...
    _initializeByteArrays() {
        ...
        this._byteArrays = {};
        this._byteArrays["Single"] = new Uint8Array(
            this._typeArrays["Single"].buffer
        );
        this._byteArrays["Double"] = new Uint8Array(
            this._typeArrays["Double"].buffer
        );
    }
    ...
    _encodePath(path) {
        ...
        return (
            CODE.PATH +
            tokenizedPath.map(this._encodeString.bind(this)).join("") +
            CODE.NULL
        );
    }
    ...
    _encodeString(text) {
        return (
            CODE.STRING + text.replace(CODE.NULL, CODE.ESCAPED_NULL) + CODE.NULL
        );
    }
    ...
    _encodeInteger(number) {
        if (_.isNumber(number)) {
            ...
            const code = String.fromCharCode(
                CODE.INT_ZERO.charCodeAt(0) + byteLength * Math.sign(number)
            );
            ...
            const code = String.fromCharCode(
                CODE.INT_ZERO.charCodeAt(0) + byteLength * (isNegative ? -1 : 1)
            );
            return (
                code +
                this._numberToBigEndianBytes(high, isNegative) +
    ...
    _numberToBigEndianBytes(number, isNegative) {
        ...
        while (number > 0) {
            ...
            littleEndianBytes.push(
                ((normalizedNumber >>> 4) & 0x0f).toString(16)
            );
            number = number >>> 8;
        }
    ...
    _decodePath(encodedValue, startIndex) {
        startIndex++;
        const partsToPath = (parts) =>
            parts
                .map((part) => PathHelper.quotePathSegmentIfNeeded(part))
                .join(".");
        let pathParts = [];
        let value, endIndex;
        ...
        return {
            value: partsToPath(pathParts),
            endIndex: encodedValue.length - 1,
        };
    }
    ...
    _decodeString(encodedValue, startIndex) {
        startIndex++;
        const extractValue = (end) =>
            encodedValue
                .substring(startIndex, end)
                .replace(CODE.ESCAPED_NULL, CODE.NULL);
        let foundNull = false;
        for (let i = startIndex; i < encodedValue.length; i++) {
        ...
        return {
            value: extractValue(encodedValue.length - 1),
            endIndex: encodedValue.length - 1,
        };
    }
    ...
    _decodeInteger(encodedValue, startIndex) {
        ...
        if (byteLength <= 4) {
            let value = this._bigEndianBytesToNumber(
            ...
            return {
                value: [low, high, !(codeDiff === 8 && high & 0x80000000)],
                endIndex: startIndex + hexLength,
            };
        }
    }
    ...
    _decodeFloat(encodedValue, startIndex) {
        ...
        const hex = encodedValue.substring(
            startIndex + 1,
