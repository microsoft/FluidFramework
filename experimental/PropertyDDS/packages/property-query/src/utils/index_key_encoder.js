...
class IndexKeyEncoder {
	...
	encode(values) {
		...
		return values
			.map((value, index) => this.encodeSingleValue(value, this._fields[index].typeId))
			.join("");
	}
	...
	decode(encodedValue) {
		...
		for (let i = 0; i < encodedValue.length; i++) {
			...
			if (!decoder) {
				throw new Error(
					`Type code '0x${encodedValue[i].charCodeAt(0).toString(16)}' not recognized!`,
				);
			}
	...
	_decodePath(encodedValue, startIndex) {
		...
		const partsToPath = (parts) =>
			parts.map((part) => PathHelper.quotePathSegmentIfNeeded(part)).join(".");
		let pathParts = [];
		let value, endIndex;
	...
	_decodeString(encodedValue, startIndex) {
		...
		const extractValue = (end) =>
			encodedValue.substring(startIndex, end).replace(CODE.ESCAPED_NULL, CODE.NULL);
		let foundNull = false;
		for (let i = startIndex; i < encodedValue.length; i++) {
	...
	_decodeInteger(encodedValue, startIndex) {
		...
		if (byteLength <= 4) {
			let value = this._bigEndianBytesToNumber(
				encodedValue.substring(startIndex + 1, startIndex + 1 + hexLength),
			);
			if (codeDiff < 0) {
				value = ~value * -1;
