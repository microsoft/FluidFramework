...
describe("Storage Manager", () => {
	...
	describe("A consistent read that ends up yielding no result", () => {
		...
		before(() => {
			mockBackendGet = sandbox.stub(mockBackend, "get").callsFake(() => {
				callCount++;
		...
		it("should return undefined", async function () {
			...
			await expect(aStorageManager.get("this-will-never-exist", true)).to.eventually.eql(
				undefined,
			);
	...
	describe("A long batch where an item falls off the LRU before the write is completed", () => {
		...
		before(() => {
			mockStartWriteBatch = sandbox
				.stub(mockBackend, "startWriteBatch")
		...
		it("should keep the node in a special cache until the batch is committed", () =>
			expect(aStorageManager.get("aKey:aSubId")).to.eventually.eql({
				a: "value",
			}));

		it("should finish the batch", () => aStorageManager.finishWriteBatch(aBatch));
	...
	describe("A concurrent read and write where the backend read ends after the write begins", () => {
		...
		before(() => {
			...
			mockBackendGet.callsFake(async () => {
				await delay(100);
		...
		it("should write an entry", () => aStorageManager.store(aBatch, theKey, valueWritten));
	...
	describe("Flushing the cache for a branch", () => {
		...
		before(async () => {
			...
			aStorageManager.store(batch, `branch:${branchGuid}`, {
				a: "something",
				guid: branchGuid,
			});
			aStorageManager.store(batch, `l:${anotherGuid}`, {
				a: "something",
