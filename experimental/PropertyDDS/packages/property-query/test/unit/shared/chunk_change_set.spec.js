	...
	let modifySimpleTestProperty = function (in_root) {
		...
		for (let i = 0; i < ids.length; i++) {
			...
			if (i % 3 === 0) {
				...
				entry
					.get(["nested", "string"])
					.setValue("+" + entry.get(["nested", "string"]).getValue() + "+");
				entry
					.get(["subProperty", "string1"])
					.setValue("+" + entry.get(["subProperty", "string1"]).getValue() + "+");
			}
		}
					...
					testChangeSetModification(
						() => {
							...
							for (let i = 10; i < 90; i++) {
								result.insert(
									String(i),
									PropertyFactory.create("adsk.test:subTemplate-1.0.0"),
								);
							}
							return result;
						},
		...
		for (let [dataTypeDescription, type, dataCS] of [
			...
			describe(dataTypeDescription, () => {
				for (let [operation, article] of [
					...
					it(
						"should correctly create " + operation + " chunks for ranges in the middle",
						() => {
							testReversibleChangeSetChunking(
								operation,
								["cc1", "cc2", "cc3"],
								[{ bbb: dataCS }, undefined, undefined, { ddd: dataCS }],
								type,
								dataCS,
							);
						},
