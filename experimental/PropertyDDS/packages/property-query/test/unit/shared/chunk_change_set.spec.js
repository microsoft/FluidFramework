...
const generateDeterministicGuid =
    require("./test_utils").generateDeterministicGuid;
...
describe("chunkChangeSet", function () {
        ...
        PropertyFactory._reregister({
            ...
            properties: [
                ...
                },
                {
                    ...
                    typeid: "String",
                    context: "map",
                    default: { key1: "value1", key2: "value" },
                },
            ],
        });
    });
    beforeEach(() => {
        deterministicRandom = new DeterministicRandomGenerator(
            "fcfaa9c7-8483-85ca-04ee-c20458f86532"
        );
    });
     ...
     */
    let createSimpleTestProperty = function (
        in_root = PropertyFactory.create("NodeProperty"),
        in_count = 10
    ) {
        for (let i = 0; i < in_count; i++) {
            ...
            entry
                .get("guid")
                .setValue(generateDeterministicGuid(deterministicRandom));
            for (let j = 0; j < 3; j++) {
                let subEntry = PropertyFactory.create(
                    "adsk.test:mixedTemplate-1.0.0"
                );
                subEntry
                    .get("guid")
                    .setValue(generateDeterministicGuid(deterministicRandom));
            ...
            if (i % 3 === 0) {
                ...
                for (let j = 0; j < 3; j++) {
                    ...
                    subEntry
                        .get("guid")
                        .setValue(
                            generateDeterministicGuid(deterministicRandom)
                        );
                    entry.get("subProperty").insert(subEntry.getId(), subEntry);
                }
            } else {
                ...
                entry
                    .get("string1")
                    .setValue("+" + entry.get("string1").getValue() + "+");
                entry
                    .get(["nested", "string"])
                    .setValue(
                        "+" + entry.get(["nested", "string"]).getValue() + "+"
                    );
                entry
                    .get(["subProperty", "string1"])
                    .setValue(
                        "+" +
                            entry.get(["subProperty", "string1"]).getValue() +
                            "+"
                    );
            }
        }
    ...
    let testChangeSetChunking = function (
        ...
        for (let i = 0; i < in_chunks.length; i++) {
            ...
            if (in_chunkSize !== undefined) {
                expect(JSON.stringify(chunk.changeSet).length).to.equal(
                    chunk.size
                );
            }
            ...
            if (chunk !== in_chunks[0] && in_testFirstPath) {
                let path = chunk.startPath.substr(
                    0,
                    chunk.startPath.length - 1
                );
                path = path.replace(/\x00/g, ".");
                let firstPathChangeset = Utils.getChangesByPath(
                    path,
                    null,
                    chunk.changeSet
                );
            ...
            Utils.traverseChangeSetRecursively(chunk.changeSet, {
                postCallback: (context) => {
                    ...
                    if (chunk.startPath !== undefined) {
                        expect(
                            path >= chunk.startPath,
                            `Expected '${path}' to be at least '${chunk.startPath}'`
                        );
                    }
                    if (i < in_chunks.length - 1) {
                        let endPath = in_chunks[i + 1].startPath;
                        const contextPath = convertPathToChunkBoundaryFormat(
                            context.getFullPath()
                        );
                        expect(
                            contextPath < endPath,
                            `Expected '${contextPath}' to be below '${endPath}'`
                        );
                    }
                },
            });
    ...
    describe("should support chunking a modification CS on path boundaries", () => {
        let testChangeSetModification = function (
            in_createCallback,
            in_modifyCallbacks,
            in_chunkSize
        ) {
            ...
            for (let callback of in_modifyCallbacks) {
                ...
                let chunks2 = chunkChangeSet(
                    CS2,
                    undefined,
                    chunks.slice(1).map((x) => x.startPath)
                );
            ...
            for (let size of [256, 1024]) {
                it("into " + size + " bytes chunks", function () {
                    testChangeSetModification(
                        createSimpleTestProperty,
                        [modifySimpleTestProperty],
                        size
                    );
                });
            }
        ...
        it("with static properties", () => {
            ...
            const create = () => {
                ...
                myProperty.insert(
                    "aString",
                    PropertyFactory.create("String", "single", "i".repeat(10))
                );
                ...
                subProperty.insert(
                    "zDynamicString",
                    PropertyFactory.create("String", "single", "i".repeat(50))
                );
                return root;
            };
    ...
    describe("escaping and unescaping paths", () => {
        let cases = [
            ...
            "\x00\x00\x00",
            "1.2.3\x00",
            "aaaa\x01\x01.12.cccc",
            "𐎦おѨ\x01\x01.12.cccc",
        ];

        cases.forEach((c) => {
            it(`should escape and unescape a path properly ${encodeURI(
                c
            )}`, () => {
    ...
    describe("chunking of inserts and removes ", () => {
        let testReversibleChangeSetChunking = (
            operation,
            boundaries,
            expectedChunkCS,
            nestedType,
            ...
            boundaries = boundaries.map((x) => "data\u0000" + x + "\u0000");
                ...
                for (let [operation, article] of [
                    ...
                    it(
                        "should correctly chunk " +
                            ...
                            "s",
                        () => {
                            testReversibleChangeSetChunking(
                                operation,
                                ["ccc"],
                    ...
                    it(
                        "should correctly chunk " +
                            ...
                            " " +
                            operation +
                            ", leaving an empty initial " +
                            operation,
                        () => {
                            testReversibleChangeSetChunking(
                                operation,
                                ["aaa"],
                                [undefined, { bbb: dataCS, ddd: dataCS }],
                                type,
                                dataCS
                            );
                        }
                    ...
                    it(
                        "should correctly chunk " +
                            ...
                            " " +
                            operation +
                            ", leaving multiple empty initial " +
                            operation +
                            "s",
                        () => {
                            testReversibleChangeSetChunking(
                    ...
                    it(
                        "should correctly chunk " +
                            ...
                            " " +
                            operation +
                            ", leaving an empty final " +
                            operation,
                        () => {
                            testReversibleChangeSetChunking(
                                operation,
                                ["eee"],
                                [{ bbb: dataCS, ddd: dataCS }, undefined],
                                type,
                                dataCS
                            );
                        }
                    ...
                    it(
                        "should correctly create " +
                            operation +
                            " chunks for ranges in the middle",
                        () => {
                            testReversibleChangeSetChunking(
                                operation,
                                ["cc1", "cc2", "cc3"],
