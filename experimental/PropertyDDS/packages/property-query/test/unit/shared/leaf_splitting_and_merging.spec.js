...
describe("Node", function () {
    ...
    before(() => {
        let outerRandom = new DeterministicRandomGenerator(
    ...
    let runMhsTest = async function (
        in_modificationFunctions,
        in_settings,
        in_options = {}
    ) {
                ...
                .then(([leafs, tree]) => {
                    ...
                    if (leafs.length > 1) {
                        for (let i = 0; i < leafs.length; i++) {
                            ...
                            let leafSize = JSON.stringify(
                                leafs[i].changeSet
                            ).length;
                            expect(leafSize).to.be.gte(
                                settings.get("mh:chunkSize") *
                                    settings.get("mh:mergeLimitFactor")
                            );
                    ...
                    return Promise.resolve(
                        fun(root, { leafs, service, commitInformation, tree })
                    );
                })
                .then(async () => {
                    ...
                    if (root.hasPendingChanges()) {
                        ...
                        let lastChangeSet = new ChangeSet(
                            deepCopy(
                                commitInformation[commitInformation.length - 1]
                                    .fullChangeSet
                            )
                        );
                    ...
                    .then(([materializedViewCS, commitCS]) => {
                        ...
                        if (index > 0) {
                            ...
                            if (!in_options.useReversibleChangeSets) {
                                ...
                                strippedCS =
                                    strippedCS.getSerializedChangeSet();
                            ...
                            expect(strippedCS).to.deep.equal(
                                commitInfo.changeSet
                            );
                        }
        ...
        for (
            let i = 0;
            i < inSettings["mhTest:numRandomizedSquashedChangesetTests"];
            i++
        ) {
            let random = new DeterministicRandomGenerator(randomGuids[i]);
            let startCommitIdx =
                random.irandom() % (commitInformation.length - 1);
            let endCommitIdx =
                startCommitIdx +
                (random.irandom() %
                    (commitInformation.length - startCommitIdx)) +
                1;

            let expectSquashedCS = new ChangeSet();
            _.forEach(
                commitInformation.slice(startCommitIdx + 1, endCommitIdx),
            ...
            expect(computedSquashedCS).to.deep.equal(
                expectSquashedCS.getSerializedChangeSet()
            );
        }
    };
        ...
        let leafMergeTestCode = function (in_params) {
            ...
            expectedNumLeafs =
                expectedNumLeafs !== undefined
                    ...
                    : Math.max(
                          1,
                          settings.resultingNumLeafs - leafsToDelete.length
                      );
            ...
            return runMhsTest(
                [
                    ...
                    (root, { leafs }) => {
                        for (let leafNumber of leafsToDelete.concat(
                            leafsToModify
                        )) {
                            ...
                            let leafsInChunk2 = leafs[leafNumber + 1]
                                ? Object.keys(
                                      leafs[leafNumber + 1].changeSet.insert
                                          .String
                                  )
                                : [];
                            for (let i = 0; i < leafsInChunk1.length; i++) {
                                if (
                                    !_.includes(leafsInChunk2, leafsInChunk1[i])
                                ) {
            ...
            return runMhsTest(
                [
                    ...
                    (root, { leafs, tree }) => {
                        ...
                        let limit2 = getStartPathFromTree(
                            [0, 2, 1, 1, 1],
                            tree
                        );
                        insertSuccessiveProperties(
            ...
            return runMhsTest(
                [
                    ...
                    (root, { leafs, tree }) => {
                        ...
                        expect(leafs[1].startPath).to.equal(
                            "testProperty\x00stringB\x00"
                        );
            ...
            return runMhsTest(
                [
                    ...
                    (root, { leafs, tree }) => {
                        ...
                        expect(leafs[2].startPath).to.equal(
                            "testProperty\x00stringC\x00"
                        );
                    ...
                    async (root, { leafs, commitInformation, service }) => {
                        ...
                        let initalMV = await service.getCommitMV({
                            guid: commitInformation[
                                commitInformation.length - 1
                            ].guid,
            ...
            let fetchMVWithPaging = function (service, guid, tree, branchGuid) {
                ...
                return service
                    ...
                    .then(({ changeSet, nextPagePath }) => {
                        fetchedMV = mergeChunkedChangeSet([
                            fetchedMV,
                            changeSet,
                        ]);
                        if (nextPagePath) {
                    ...
                    (root, { service, commitInformation, tree }) => {
                        return fetchMVWithPaging(
                            ...
                            let keys = Object.keys(
                                materializedView.insert.NodeProperty
                            );
                            keys.sort();
                            ...
                            if (options.ranges) {
                                for (
                                    let i = 0;
                                    i < options.ranges.length;
                                    i++
                                ) {
                                    let range = options.ranges[i];
                                    let pathsInRange = _.filter(
                                        allPaths,
                                        (x) => {
                                            return (
                                                x >= range[0] && x < range[1]
                            ...
                            expectedKeys = expectedKeys.map((x) => {
                                let tokenizedPath =
                                    PathHelper.tokenizePathString(x);
                                if (tokenizedPath.length > 1) {
                                    expectedSubKeys[tokenizedPath[0]] =
                                        expectedSubKeys[tokenizedPath[0]] || [];
                                    expectedSubKeys[tokenizedPath[0]].push(
                                        tokenizedPath[1]
                                    );
                                }
                            ...
                            for (let i = 0; i < keys.length; i++) {
                                ...
                                if (expectedSubKeys[keys[i]]) {
                                    let expectedSubKeyList =
                                        expectedSubKeys[keys[i]];
                                    expectedSubKeyList.sort();
                                    let actualKeys = Object.keys(
                                        CS.insert.String
                                    );
                                    actualKeys.sort();
                ...
                [
                    ...
                    (root, { service, commitInformation }) => {
                        return service
                            ...
                            .then((materializedView) => {
                                let Properties =
                                    materializedView.changeSet.insert;

                                expect(Properties.Reference.Ref).to.exist;
                                expect(Properties.NodeProperty.Prop1).to.not
                                    .exist;
    ...
    describe("random Tests", () => {
        ...
        after(() => {
            console.log(
                "Total time: " + Math.ceil(summedTime / 10) / 100 + "s"
            );
        });
        ...
        for (let i = 0; i < numRandomTests; i++) {
            (() => {
                let j = i;
                it("Test " + j, () => {
                    let random = new DeterministicRandomGenerator(
                        randomGuids[j]
                    );
                    ...
                    let randomPermutationFunction = (root) => {
                        ...
                        for (let k = 0; k < rangesToRemove; k++) {
                            if (sortedKeys.length > 0) {
                                let startIndex =
                                    random.irandom() % sortedKeys.length;
                                let remainingKeys =
                                    sortedKeys.length - startIndex - 1;
                                let endIndex =
                                    startIndex +
                                    1 +
                                    (random.irandom() % remainingKeys);
                                for (let l = startIndex; l < endIndex; l++) {
                        ...
                        for (let k = 0; k < rangesToInsert; k++) {
                            ...
                            if (sortedKeys.length < 3) {
                                ...
                                let startIndex =
                                    random.irandom() % (sortedKeys.length - 1);

                                rangeStart =
                                    Number(sortedKeys[startIndex]) / 1e16;
                                rangeEnd =
                                    Number(sortedKeys[startIndex + 1]) / 1e16;
            ...
            return runMhsTest(
                [
                    (root) => {
                        root.insert(
                            "array",
                            PropertyFactory.create("Float64", "array")
                        );
