...
describe("Branch write queue", () => {
    ...
    describe("A commit, in-band, without other commit queued, on tip of the branch", () => {
        ...
        it("should resolve the promise with a success response", () =>
            expect(
                branchWriteQueue.queueCommitGracefully(theCommit)
    ...
    describe("A commit, in-band, with the same commit queued, on tip of the branch", () => {
        ...
        it("should create the commit only once", () =>
            expect(createCommitStub.withArgs(theCommit)).to.have.been
                .calledOnce);
    ...
    describe("A commit, in-band, alone in the queue, already existing the database", () => {
        ...
        it("should resolve the promise with an existing response", () =>
            expect(
                branchWriteQueue.queueCommitGracefully(theCommit)
            ).to.eventually.eql(SUCCESS_COMMIT_EXISTING_RESPONSE));

        it("should check that the commit already exists", () =>
            expect(getCommitStub.withArgs(commitGuid)).to.have.been.calledOnce);

        it("should not create the commit", () =>
            expect(createCommitStub.withArgs(theCommit)).not.to.have.been
                .called);
    ...
    describe("Two commits, respecting the commit topology, getting queued together", () => {
        ...
        it("should resolve the promise with success for both", () =>
            Promise.all([
                expect(
                    branchWriteQueue.queueCommitGracefully(theFirstCommit)
                ).to.eventually.eql(SUCCESS_COMMIT_CREATE_RESPONSE),
                expect(
                    branchWriteQueue.queueCommitGracefully(theSecondCommit)
                ).to.eventually.eql(SUCCESS_COMMIT_CREATE_RESPONSE),
            ]));
        ...
        it("should check that the commits doesn't already exist", () =>
            Promise.all([
                expect(getCommitStub.withArgs(firstCommitGuid)).to.have.been
                    .calledOnce,
                expect(getCommitStub.withArgs(secondCommitGuid)).to.have.been
                    .calledOnce,
            ]));

        it("should create each commit", () =>
            Promise.all([
                expect(createCommitStub.withArgs(theFirstCommit)).to.have.been
                    .calledOnce,
                expect(createCommitStub.withArgs(theSecondCommit)).to.have.been
                    .calledOnce,
            ]));
    ...
    describe("Three commits out-of-order, all in the queue", () => {
        ...
        it("should resolve the promise with success for both", () =>
            Promise.all([
                expect(
                    branchWriteQueue.queueCommitGracefully(theThirdCommit)
                ).to.eventually.eql(SUCCESS_COMMIT_CREATE_RESPONSE),
                expect(
                    branchWriteQueue.queueCommitGracefully(theFirstCommit)
                ).to.eventually.eql(SUCCESS_COMMIT_CREATE_RESPONSE),
        ...
        it("should check that the third commit doesn't already exist", () =>
            expect(getCommitStub.withArgs(thirdCommitGuid)).to.have.been
                .calledOnce);
        ...
        it("should create each commit", () =>
            Promise.all([
                ...
                expect(createCommitStub.withArgs(theSecondCommit)).to.have.been
                    .calledOnce,
                expect(createCommitStub.withArgs(theThirdCommit)).to.have.been
                    .calledOnce,
            ]));

        it("should have created the commits in the right order", () => {
            expect(callTimes[firstCommitGuid]).to.be.below(
                callTimes[secondCommitGuid]
            );
            expect(callTimes[firstCommitGuid]).to.be.below(
                callTimes[thirdCommitGuid]
            );
            expect(callTimes[secondCommitGuid]).to.be.below(
                callTimes[thirdCommitGuid]
            );
        });
    ...
    describe("Four commits out-of-order, with a hole", () => {
        ...
        it("should resolve the promise with success for all", () =>
            Promise.all([
                expect(
                    branchWriteQueue.queueCommitGracefully(theFourthCommit)
                ).to.eventually.eql(SUCCESS_COMMIT_CREATE_RESPONSE),
                expect(
                    branchWriteQueue.queueCommitGracefully(theThirdCommit)
                ).to.eventually.eql(SUCCESS_COMMIT_CREATE_RESPONSE),
        ...
        it("should check that the fourth commit doesn't already exist", () =>
            expect(getCommitStub.withArgs(fourthCommitGuid)).to.have.been
                .calledOnce);
        ...
        it("should create each commit", () =>
            Promise.all([
                ...
                expect(createCommitStub.withArgs(theThirdCommit)).to.have.been
                    .calledOnce,
                expect(createCommitStub.withArgs(theFourthCommit)).to.have.been
                    .calledOnce,
            ]));
        ...
        it("should have created the commits in the right order", () => {
            ...
            expect(callTimes[thirdCommitGuid]).to.be.below(
                callTimes[fourthCommitGuid]
            );
        });
    ...
    describe("Compensation fetching multiple pages", () => {
        ...
        it.skip("should resolve the promise with success for all", () =>
            Promise.all([
                expect(
                    branchWriteQueue.queueCommitGracefully(commits[14])
                ).to.eventually.eql(SUCCESS_COMMIT_CREATE_RESPONSE),
                expect(
                    branchWriteQueue.queueCommitGracefully(commits[15])
                ).to.eventually.eql(SUCCESS_COMMIT_CREATE_RESPONSE),
            ]));
        // https://github.com/microsoft/FluidFramework/pull/13220
        ...
        it.skip("should check that the last commit doesn't already exist", () =>
            Promise.all([
                expect(getCommitStub.withArgs(commitGuids[14])).to.have.been
                    .calledOnce,
                expect(getCommitStub.withArgs(commitGuids[15])).to.have.been
                    .calledOnce,
            ]));
        // https://github.com/microsoft/FluidFramework/pull/13220
        it.skip("should create each commit", () =>
            Promise.all(
                commits.map(
                    (c) =>
                        expect(createCommitStub.withArgs(c)).to.have.been
                            .calledOnce
                )
            ));
        // https://github.com/microsoft/FluidFramework/pull/13220
        it.skip("should have created the commits in the right order", () => {
            for (let i = 0; i < commitGuids.length - 1; i++) {
                for (let j = i + 1; j < commitGuids.length; j++) {
                    expect(callTimes[commitGuids[i]]).to.be.below(
                        callTimes[commitGuids[j]]
                    );
                }
            }
        });
    ...
    describe("Waiting for a commit in concurrency with creating it", () => {
        ...
        it("should resolve the promise with a success response", () =>
            Promise.all([
                ...
                expect(
                    branchWriteQueue.waitUntilCommitApplied(
                        branchGuid,
                        commitGuid
                    )
    ...
    describe("Queuing two branches concurrently", () => {
        ...
        it("should resolve all calls with success", () =>
            Promise.all([
                expect(
                    branchWriteQueue.queueBranchGracefully(theBranch)
                ).to.eventually.eql(SUCCESS_COMMIT_CREATE_RESPONSE),
                expect(
                    branchWriteQueue.queueBranchGracefully(theBranch)
                ).to.eventually.eql(SUCCESS_COMMIT_CREATE_RESPONSE),
        ...
        it("should create the branch once", () =>
            expect(createBranchStub.withArgs(theBranch)).to.have.been
                .calledOnce);
    ...
    describe("Waiting for an existing branch", () => {
        ...
        it("should be resolved", () =>
            expect(
                branchWriteQueue.waitUntilBranchCreated(branchGuid)
            ).to.eventually.eql(SUCCESS_COMMIT_EXISTING_RESPONSE));
    ...
    describe("Creating a branch while queued for deletion", () => {
        ...
        it("should reject the branch creation", () =>
            expect(
                branchWriteQueue.queueBranchGracefully({ guid: branchGuid })
            ).to.be.rejectedWith(`Branch ${branchGuid} locked for deletion`));
    ...
    describe("Locking a branch while it is applying commits", () => {
        ...
        before(() => {
            ...
            sandbox
                .stub(mockCommitManager, "createCommit")
                .callsFake(async () => {
                    lockPromise = branchWriteQueue.lockQueuesForDeletion([
                        branchGuid,
                    ]);
                    return SUCCESS_COMMIT_CREATE_RESPONSE;
                });
