...
describe("Branch write queue", () => {
	...
	describe("A commit, in-band, without other commit queued, on tip of the branch", () => {
		...
		before(() => {
			getBranchStub = sandbox
				.stub(mockBranchManager, "getBranch")
				.withArgs(branchGuid)
				.resolves({
					guid: branchGuid,
			...
			createCommitStub = sandbox
				.stub(mockCommitManager, "createCommit")
				.callsFake(async () => {
					return SUCCESS_COMMIT_CREATE_RESPONSE;
				});

			getCommitStub = sandbox
				.stub(mockCommitManager, "getCommit")
				.withArgs(commitGuid)
				.rejects(
					new OperationError(
		...
		it("should resolve the promise with a success response", () =>
			expect(branchWriteQueue.queueCommitGracefully(theCommit)).to.eventually.eql(
				SUCCESS_COMMIT_CREATE_RESPONSE,
			));
	...
	describe("A commit, in-band, alone in the queue, already existing the database", () => {
		...
		it("should resolve the promise with an existing response", () =>
			expect(branchWriteQueue.queueCommitGracefully(theCommit)).to.eventually.eql(
				SUCCESS_COMMIT_EXISTING_RESPONSE,
			));
	...
	describe("Two commits, respecting the commit topology, getting queued together", () => {
		...
		before(() => {
			...
			getCommitStub = sandbox
				...
				.rejects(
					new OperationError(
						`Commit ${firstCommitGuid} not found!`,
						"somewhere",
						HTTPStatus.NOT_FOUND,
						OperationError.FLAGS.QUIET,
					),
				...
				.rejects(
					new OperationError(
						`Commit ${secondCommitGuid} not found!`,
						"somewhere",
						HTTPStatus.NOT_FOUND,
						OperationError.FLAGS.QUIET,
					),
		...
		it("should resolve the promise with success for both", () =>
			Promise.all([
				expect(branchWriteQueue.queueCommitGracefully(theFirstCommit)).to.eventually.eql(
					SUCCESS_COMMIT_CREATE_RESPONSE,
				),
				expect(branchWriteQueue.queueCommitGracefully(theSecondCommit)).to.eventually.eql(
					SUCCESS_COMMIT_CREATE_RESPONSE,
				),
			]));
	...
	describe("Three commits out-of-order, all in the queue", () => {
		...
		it("should resolve the promise with success for both", () =>
			Promise.all([
				expect(branchWriteQueue.queueCommitGracefully(theThirdCommit)).to.eventually.eql(
					SUCCESS_COMMIT_CREATE_RESPONSE,
				),
				expect(branchWriteQueue.queueCommitGracefully(theFirstCommit)).to.eventually.eql(
					SUCCESS_COMMIT_CREATE_RESPONSE,
	...
	describe("Four commits out-of-order, with a hole", () => {
		...
		before(() => {
			...
			getCommitStub = sandbox
				...
				.rejects(
					new OperationError(
						`Commit ${thirdCommitGuid} not found!`,
						"somewhere",
						HTTPStatus.NOT_FOUND,
						OperationError.FLAGS.QUIET,
					),
				...
				.rejects(
					new OperationError(
						`Commit ${fourthCommitGuid} not found!`,
						"somewhere",
						HTTPStatus.NOT_FOUND,
						OperationError.FLAGS.QUIET,
					),
			...
			getCommitRangeStub = sandbox
				.stub(mockPssClient, "getCommitRange")
				.withArgs({
					branchGuid: branchGuid,
		...
		it("should resolve the promise with success for all", () =>
			Promise.all([
				expect(branchWriteQueue.queueCommitGracefully(theFourthCommit)).to.eventually.eql(
					SUCCESS_COMMIT_CREATE_RESPONSE,
				),
				expect(branchWriteQueue.queueCommitGracefully(theThirdCommit)).to.eventually.eql(
					SUCCESS_COMMIT_CREATE_RESPONSE,
	...
	describe("Compensation fetching multiple pages", () => {
		...
		it.skip("should resolve the promise with success for all", () =>
			Promise.all([
				expect(branchWriteQueue.queueCommitGracefully(commits[14])).to.eventually.eql(
					SUCCESS_COMMIT_CREATE_RESPONSE,
				),
				expect(branchWriteQueue.queueCommitGracefully(commits[15])).to.eventually.eql(
					SUCCESS_COMMIT_CREATE_RESPONSE,
				),
			]));
		// https://github.com/microsoft/FluidFramework/pull/13220
		...
		it.skip("should create each commit", () =>
			Promise.all(
				commits.map((c) => expect(createCommitStub.withArgs(c)).to.have.been.calledOnce),
			));
		// https://github.com/microsoft/FluidFramework/pull/13220
	...
	describe("An out-of-band commit that needs to be fetched", () => {
		...
		before(() => {
			...
			getPssCommitStub = sandbox
				.stub(mockPssClient, "getCommit")
				.withArgs({
					branchGuid: branchGuid,
	...
	describe("Awaiting for a commit applied, already in the database", () => {
		...
		it("should resolve", () =>
			expect(
				branchWriteQueue.waitUntilCommitApplied(branchGuid, commitGuid),
	...
	describe("Creating a commit, with a hole, replied with a 403 by the pss", () => {
		...
		it("should be rejected", () =>
			expect(branchWriteQueue.queueCommitGracefully(theCommit)).to.be.rejectedWith(
				OperationError,
				"Authorization failed.",
			));
	...
	describe("Creating a commit, with a hole, replied with an empty list by the pss", () => {
		...
		before(() => {
			...
			getPssCommitStub = sandbox.stub(mockPssClient, "getCommitRange").resolves({
				commits: [],
			});
	...
	describe("Waiting for a commit that exists in the PSS, concurrently with ingesting its parent", () => {
		...
		before(() => {
			getCommitStub = sandbox.stub(mockCommitManager, "getCommit").callsFake(async (p) => {
		...
		it("should be resolved", function () {
			...
			return Promise.all([
				...
				expect(
					branchWriteQueue.waitUntilCommitApplied(branchGuid, thirdCommitGuid),
				).to.eventually.eql({ status: "ok" }),
			]);
		});

		it("should have called getCommit", () => expect(getCommitStub).to.have.been.called);

		it("should have called createCommit", () =>
			Promise.all([
				expect(
					createCommitStub.withArgs({
						guid: commitGuid,
						parentGuid: parentGuid,
						branchGuid: branchGuid,
						meta: {},
	...
	describe("Queuing two branches concurrently", () => {
		...
		before(() => {
			...
			createBranchStub = sandbox
				.stub(mockBranchManager, "createBranch")
				.callsFake(async () => {
					return SUCCESS_COMMIT_CREATE_RESPONSE;
				});
		});
		...
		it("should resolve all calls with success", () =>
			Promise.all([
				expect(branchWriteQueue.queueBranchGracefully(theBranch)).to.eventually.eql(
					SUCCESS_COMMIT_CREATE_RESPONSE,
				),
				expect(branchWriteQueue.queueBranchGracefully(theBranch)).to.eventually.eql(
					SUCCESS_COMMIT_CREATE_RESPONSE,
	...
	describe("A commit on top of a missing branch, concurrently", () => {
		...
		before(() => {
			...
			sandbox.stub(mockBranchManager, "getBranch").callsFake(() => {
				if (!branchCreated) {
					return Promise.reject(
						new OperationError(
							`Branch ${branchGuid} not found!`,
	...
	describe("Waiting for an existing branch", () => {
		...
		before(() => {
			sandbox.stub(mockBranchManager, "getBranch").resolves({
				guid: branchGuid,
		...
		it("should be resolved", () =>
			expect(branchWriteQueue.waitUntilBranchCreated(branchGuid)).to.eventually.eql(
				SUCCESS_COMMIT_EXISTING_RESPONSE,
			));
	...
	describe("A commit on top of a missing branch, opted-out, with branch repair", () => {
		...
		before(() => {
			...
			sandbox.stub(mockBranchManager, "createBranch").callsFake(async () => {
				await delay(100);
				branchCreated = true;
				return SUCCESS_COMMIT_CREATE_RESPONSE;
			});

			sandbox.stub(mockCommitManager, "createCommit").callsFake(async () => {
				return SUCCESS_COMMIT_CREATE_RESPONSE;
			});
		...
		it("should be rejected", () =>
			expect(branchWriteQueue.queueCommitGracefully(theCommit)).to.be.eventually.rejectedWith(
				Error,
	...
	describe("waiting for a commit on top of a missing branch", () => {
		...
		before(async () => {
			...
			sandbox.stub(mockBranchManager, "getBranch").callsFake(() => {
				if (!branchCreated) {
					return Promise.reject(
						new OperationError(
							`Branch ${missingParentBranchGuid} not found!`,
							"_fetchAndQueue",
							HTTPStatus.NOT_FOUND,
							OperationError.FLAGS.QUIET,
						),
			...
			createBranchStub = sandbox.stub(mockBranchManager, "createBranch").callsFake(() => {
				branchCreated = true;
			...
			sandbox
				...
				.rejects(
					new OperationError(
						`Commit ${missingCommitGuid} not found!`,
						"_fetchAndQueue",
						HTTPStatus.NOT_FOUND,
						OperationError.FLAGS.QUIET,
					),
		...
		it("should succeed", () =>
			expect(
				branchWriteQueue.waitUntilCommitApplied(missingParentBranchGuid, missingCommitGuid),
	...
	describe("Waiting for a branch while a branch is locked for deletion", () => {
		...
		it("should reject the create commit", () =>
			expect(branchWriteQueue.waitUntilBranchCreated(branchGuid)).to.be.rejectedWith(
	...
	describe("Creating a branch while queued for deletion", () => {
		...
		it("should reject the branch creation", () =>
			expect(branchWriteQueue.queueBranchGracefully({ guid: branchGuid })).to.be.rejectedWith(
				`Branch ${branchGuid} locked for deletion`,
			));
	...
	describe("Committing on a branch locked for deletion", () => {
		...
		it("should reject the branch creation", () =>
			expect(
				branchWriteQueue.queueCommitGracefully({
					guid: commitGuid,
					branchGuid: branchGuid,
				}),
			).to.be.rejectedWith(`Branch ${branchGuid} locked for deletion`));
	...
	describe("Locking a branch while it is applying commits", () => {
		...
		it.skip("should resolve the first commit, reject the second one", () =>
			Promise.all([
				...
				expect(branchWriteQueue.queueCommitGracefully(theSecondCommit)).to.be.rejectedWith(
					`Branch ${branchGuid} locked for deletion`,
				),
			]));
		// https://github.com/microsoft/FluidFramework/pull/13220
	...
	describe("A branch, on top of a missing commit, while the branches gets locked", () => {
		...
		it("should reject the promise with a locked response", () =>
			expect(branchWriteQueue.queueBranchGracefully(theBranch)).to.be.rejectedWith(
				`Branch ${branchGuid} locked for deletion`,
			));
