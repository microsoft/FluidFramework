...
describe("Branch Manager", () => {
    ...
    describe("Creating a branch without a parent", () => {
        ...
        const expectedBtreeParameters = {
            ...
            nodesPerHierarchicalHistoryLevel: settings.get(
        ...
        before(() => {
            sandbox
                .stub(mockStorageManager, "get")
                .withArgs(`branch:${branchGuid}`)
                .resolves(undefined);

            sandbox.stub(mockStorageManager, "store").callsFake((b, k, v) => {
        ...
        it("should populate the branch data with the configured BTree parameters", () =>
            expect(createdBranchPayload.bTreeParameters).to.eql(
                expectedBtreeParameters
            ));
