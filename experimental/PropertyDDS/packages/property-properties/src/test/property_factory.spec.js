...
describe('PropertyFactory', function () {
    ...
    it('should validate a simple file', function () {
        var testFile1 = require('./validation/good_point_id');
        var result = PropertyFactory.validate(testFile1);
        expect(result.isValid).to.equal(true);
    });

    it('should fail an invalid file', function () {
        var testFile1 = require('./validation/bad_primitive_typeid');
        var result = PropertyFactory.validate(testFile1);
    ...
    it('should throw on unknown Template', function () {
        var insertUnknowPropertyChangeSet = {
            insert: {
                'UnknownProperty': {
                    'test': {
                        String: {
        ...
        expect(
            function () {
                root.deserialize(insertUnknowPropertyChangeSet);
            }).to.throw();
    });
    ...
    it('should support the creation of a polymorphic collection', function () {
        ...
        expect(PropertyFactory.inheritsFrom(instance._properties.testSet.getTypeid(), 'NamedProperty')).to.equal(true);
    });

    it('should return correct template based on typeid', function () {
        PropertyFactory.register(SimplePoint);
        var returnedTemplate = JSON.parse(JSON.stringify(PropertyFactory.getTemplate(SimplePoint.typeid)));
        ...
        expect(PropertyFactory.getTemplate('Adsk.Library:Colors.ColorPalette-1.0.0')).to.be.undefined;
    });
    ...
    describe('Inheritance', function () {
        ...
        it('Will throw because of a type mismatch when creating template', function () {
            ...
            try {
                ...
                expect(error.message).to.include(
                    'PF-001: Id already exists: const1'
                );
            }
        });
        ...
        it('should fail when inheriting from multiple types that have the same property name', function () {
            ...
            expect(PropertyFactory.create.bind(PropertyFactory, 'SimpleTest:ShapeWithInheritanceError-1.0.0'))
            ...
            it('should allow overriding default values', function () {
                ...
                var ShapeWithOverriddenDefaults = {
                    ...
                    properties: [
                        ...
                        { id: 'array', typeid: 'String', value: ['array_string_1', 'array_string_2'], context: 'array' },
                        {
                ...
                expect(instance.resolvePath('array_poly').getValues()[1].string).to.eql('entry_string_poly_2_inherited');
                expect(instance.resolvePath('enum').getValue()).and.eql(2);
                expect(instance.resolvePath('untyped.num').getValue()).and.eql(2);
                expect(instance.resolvePath('untyped.string').getValue()).and.eql('untyped_string_inherited');
                expect(instance.resolvePath('template.num').getValue()).and.eql(2);
                expect(instance.resolvePath('template.dynamic.dynamic_string')
                    .getValue()).and.eql('dynamic_string_inherited');
                ...
                expect(instance.resolvePath('map').getEntriesReadOnly().key2.get('string').getValue())
                    .to.equal('map_string_2_inherited');
                expect(instance.resolvePath('map').getEntriesReadOnly().key3.get('string').getValue())
                    .to.equal('map_string_3_inherited');
                expect(instance.resolvePath('map_poly').getIds().length).to.equal(2);
                expect(instance.resolvePath('map_poly').getEntriesReadOnly().key2.get('string').getValue())
                    .to.equal('map_poly_string_2_inherited');
                expect(instance.resolvePath('map_poly').getEntriesReadOnly().key3.get('string').getValue())
                    .to.equal('map_poly_string_3_inherited');
                expect(instance.resolvePath('map_nest').getIds().length).to.equal(2);
                expect(instance.resolvePath('map_nest').getEntriesReadOnly().key2.get('map_nested')
                    .getIds().length).to.equal(2);
                expect(instance.resolvePath('map_nest').getEntriesReadOnly().key2.get('map_nested')
                    .getAsArray().length).to.equal(2);
                expect(instance.resolvePath('map_nest').getEntriesReadOnly().key2.get('map_nested')
                    .getAsArray()[0].get('string').getValue()).to.equal('map_nested_2_string_2_inherited');
                expect(instance.resolvePath('map_nest').getEntriesReadOnly().key2.get('map_nested')
                    .getAsArray()[1].get('string').getValue()).to.equal('map_nested_2_string_3_inherited');
                expect(instance.resolvePath('map_nest').getEntriesReadOnly().key3.get('map_nested')
                    .getIds().length).to.equal(1);
                expect(instance.resolvePath('map_nest').getEntriesReadOnly().key3.get('map_nested')
                    .getAsArray()[0].get('string').getValue()).to.equal('map_nested_3_string_1_inherited');
                expect(instance.resolvePath('set').getIds().length).to.equal(2);
                expect(instance.resolvePath('set').getAsArray()[0].get('string')
                    .getValue()).to.equal('set_string_2_inherited');
                expect(instance.resolvePath('set').getAsArray()[1].get('string')
                    .getValue()).to.equal('set_string_3_inherited');
                expect(instance.resolvePath('set_poly').getIds().length).to.equal(2);
                expect(instance.resolvePath('set_poly').getAsArray()[0].get('string')
                    .getValue()).to.equal('set_poly_string_2_inherited');
                expect(instance.resolvePath('set_poly').getAsArray()[1].get('string')
                    .getValue()).to.equal('set_poly_string_3_inherited');
                expect(instance.resolvePath('set_nest').getIds().length).to.equal(1);
                expect(instance.resolvePath('set_nest').getAsArray()[0].get('set_nested').getIds().length).to.equal(2);
                expect(instance.resolvePath('set_nest').getAsArray()[0].get('set_nested')
                    .getAsArray()[0].get('string').getValue()).to.equal('set_nested_2_string_1_inherited');
                expect(instance.resolvePath('set_nest').getAsArray()[0].get('set_nested')
                    .getAsArray()[1].get('string').getValue()).to.equal('set_nested_2_string_2_inherited');
            });
        ...
        describe('Overriding constants', function () {
            it('should allow overriding constants', function () {
                ...
                var instance = PropertyFactory.create('SimpleTest:SquareWithOverriddenConstants-1.0.0');
            ...
            it('should allow overriding constants with typed values', function () {
                ...
                var instance = PropertyFactory.create('SimpleTest:SquareWithTypedOverriddenConstants-1.0.0');

                expect(instance._getChildrenCount()).to.equal(5);
                expect(instance.resolvePath('entry1').getTypeid())
                    .to.eql('SimpleTest:EntryWithTypedOverriddenConstants-1.0.0');
                expect(instance.resolvePath('entry1.string').getValue()).to.eql('entry_string_1');
                expect(instance.resolvePath('entry2').getTypeid())
                    .to.eql('SimpleTest:EntryWithTypedOverriddenPolyConstants-1.0.0');
                ...
                expect(instance.resolvePath('array').getValues()[0].string).to.eql('entry_string_1_inherited');
                expect(instance.resolvePath('array').getValues()[1].name).to.eql('entry_string_2');
                expect(instance.resolvePath('array').getValues()[1].string).to.eql('entry_string_2_inherited');
                expect(instance.resolvePath('map').getIds().length).to.equal(2);
                expect(instance.resolvePath('map').getEntriesReadOnly().key2.get('name').getValue())
                    .to.equal('map_string_2');
                expect(instance.resolvePath('map').getEntriesReadOnly().key2.get('string').getValue())
                    .to.equal('map_string_2_inherited');
                expect(instance.resolvePath('map').getEntriesReadOnly().key3.get('name').getValue())
                    .to.equal('map_string_3');
                expect(instance.resolvePath('map').getEntriesReadOnly().key3.get('string').getValue())
                    .to.equal('map_string_3_inherited');
                expect(instance.resolvePath('set').getIds().length).to.equal(2);
                expect(instance.resolvePath('set').getAsArray()[0].get('name')
                    .getValue()).to.equal('set_string_2');
                expect(instance.resolvePath('set').getAsArray()[0].get('string')
                    .getValue()).to.equal('set_string_2_inherited');
                expect(instance.resolvePath('set').getAsArray()[1].get('name')
                    .getValue()).to.equal('set_string_3');
                expect(instance.resolvePath('set').getAsArray()[1].get('string')
                    .getValue()).to.equal('set_string_3_inherited');
            });
            ...
            it('should not allow overriding constants with a typedValue that does not inherit from base type', function () {
                ...
                expect(PropertyFactory.create.bind(PropertyFactory, 'SimpleTest:SquareWithDiffTypeidConstants-1.0.0'))
                    .to.throw(MSG.TYPED_VALUES_MUST_DERIVE_FROM_BASE_TYPE +
    ...
    describe('Default values', function () {
        ...
        it('should set a default value for a template, parent default value has precedence', function () {
            ...
            expect(instance.get('template').get('dynamic').get('dynamic_string').getValue()).to.equal('I am a string');
        });
        ...
        it('should set a default value for a typed map', function () {
            ...
            expect(instance.get('map').getEntriesReadOnly().key1.get('string').getValue()).to.equal('I am a string 1');
            expect(instance.get('map').getEntriesReadOnly().key2.get('string').getValue()).to.equal('I am a string 2');
        });
        ...
        describe('#Polymorphic', function () {
            ...
            it('should fail when setting a default typedValue that doesnt have a typeid.', function () {
                ...
                expect(PropertyFactory.create.bind(PropertyFactory, 'SimpleTest:DefaultPolyNoTypeIdContainer-1.0.0'))
                    .to.throw(MSG.FIELD_TYPEID_IS_REQUIRED + 'typedValue SimpleTest:DefaultPolyBase-1.0.0');
            });
            ...
            it('should set a default polymorphic value for a non-primitive set', function () {
                ...
                expect(instance.get('set').getAsArray()[2].get('str').getValue()).to.equal('ArrSubSub');
            ...
            it('should set a default polymorphic value for a non-primitive map', function () {
                ...
                expect(instance.get('map').getEntriesReadOnly().key1.get('num').getValue()).to.equal(222);
                expect(instance.get('map').getEntriesReadOnly().key1.get('str').getValue()).to.equal('Sub');
                expect(instance.get('map').getEntriesReadOnly().key2.get('num').getValue()).to.equal(222);
                expect(instance.get('map').getEntriesReadOnly().key2.get('str').getValue()).to.equal('ArrSub');
                expect(instance.get('map').getEntriesReadOnly().key3.get('num').getValue()).to.equal(333);
                expect(instance.get('map').getEntriesReadOnly().key3.get('str').getValue()).to.equal('ArrSubSub');
                expect(instance.get('map').getEntriesReadOnly().key3.get('num2').getValue()).to.equal(111);
            });
            ...
            it('should fail when setting default polymorphic values not derived from base type', function () {
                ...
                expect(PropertyFactory.create.bind(PropertyFactory, 'SimpleTest:DefaultPolyUnderivedContainer-1.0.0'))
                    .to.throw(MSG.TYPED_VALUES_MUST_DERIVE_FROM_BASE_TYPE + 'SimpleTest:DefaultPoly-1.0.0');
            });
    ...
    describe('Constants', function () {
        it('should set constant properties as readonly', function () {
            ...
            expect(instance.get('num')._checkIsNotReadOnly.bind(
                instance.get('num'), true)).to.throw(MSG.MODIFICATION_OF_CONSTANT_PROPERTY);
            expect(instance.get('num').setValue.bind(
                instance.get('num'), 1111)).to.throw(MSG.MODIFICATION_OF_CONSTANT_PROPERTY);
        });
        ...
        it('should set constant child properties as readonly', function () {
            ...
            expect(instance.get('template').get('dynamic').get('dynamic_string')._isConstant).to.equal(true);
            expect(instance.get('template').get('num')._checkIsNotReadOnly.bind(
                instance.get('template').get('num'), true)).to.throw(MSG.MODIFICATION_OF_CONSTANT_PROPERTY);
            expect(instance.get('template').get('num').setValue.bind(
                instance.get('template').get('num'), 1111)).to.throw(MSG.MODIFICATION_OF_CONSTANT_PROPERTY);
            expect(instance.get('template').get('dynamic').get('dynamic_string')._checkIsNotReadOnly.bind(
                instance.get('template').get('dynamic').get('dynamic_string'), true)).to.throw(
                    MSG.MODIFICATION_OF_CONSTANT_PROPERTY);
            expect(instance.get('template').get('dynamic').get('dynamic_string').setValue.bind(
                instance.get('template').get('dynamic').get('dynamic_string'), 'should throw')).to.throw(
                    MSG.MODIFICATION_OF_CONSTANT_PROPERTY);
        });
        ...
        it('should support constants with no value defined', function () {
            ...
            var instanceInherited = PropertyFactory.create('SimpleTest:ConstantNoValueInherited-1.0.0');
        ...
        describe('#Polymorphic', function () {
            ...
            it('should fail when setting a constant typedValue that doesnt have a typeid.', function () {
                ...
                expect(PropertyFactory.create.bind(PropertyFactory, 'SimpleTest:ConstantPolyNoTypeIdContainer-1.0.0'))
                    .to.throw(MSG.FIELD_TYPEID_IS_REQUIRED + 'typedValue SimpleTest:ConstantPolyBase-1.0.0');
            });
            ...
            it('should fail when setting constant polymorphic values not derived from base type', function () {
                ...
                expect(PropertyFactory.create.bind(PropertyFactory, 'SimpleTest:ConstantPolyUnderivedContainer-1.0.0'))
                    .to.throw(MSG.TYPED_VALUES_MUST_DERIVE_FROM_BASE_TYPE + 'SimpleTest:ConstantPoly-1.0.0');
            });
    ...
    describe('InstanceOf Type Checking', function () {
        it('Native types pass instanceOf check', function () {
            ...
            var propTypes = ['String', 'Bool', 'Int8', 'Uint8', 'Int16', 'Uint16', 'Int32', 'Int64', 'Uint64', 'Uint32',
                'Float32', 'Float64', 'Reference'];
            ...
            for (var i = 0; i < contextSingleTypes.length; ++i) {
                ...
                expect(PropertyFactory.instanceOf(PropertyFactory.create(propType), propType)).to.be.true;
            }

            for (var j = 0; j < propTypes.length; ++j) {
                var propType = propTypes[j];
                expect(PropertyFactory.instanceOf(PropertyFactory.create(propType, 'map'), propType, 'map')).to.be.true;
            }

            var contextArrayTypes = propTypes.concat(['Enum']);
            for (var k = 0; k < contextArrayTypes.length; ++k) {
                var propType = contextArrayTypes[k];
                expect(PropertyFactory.instanceOf(PropertyFactory.create(propType, 'array'), propType, 'array')).to.be.true;
            }
        });

        it('instanceOf check succeeds for schema based properties and native typeids', function () {
            PropertyFactory.register(SimplePoint);
            expect(PropertyFactory.instanceOf(PropertyFactory.create(SimplePoint.typeid), 'BaseProperty')).to.be.true;
            ...
            expect(PropertyFactory.instanceOf(
                PropertyFactory.create(testEnum.typeid, 'array'), 'Enum', 'array')).to.be.true;
        });

        it('instanceOf check fails for non native typeids', function () {
            PropertyFactory.register(SimplePoint);
            expect(PropertyFactory.instanceOf(PropertyFactory.create(SimplePoint.typeid), SimplePoint.typeid)).to.be.false;
        });
    });
});
...
describe('Template registration', function () {
    ...
    before(function () {
        ColorID = require('./validation/good_color_id');
    });
    ...
    it(
        'should print a warning when registering an existing template that is not different from what is in the registry',
        function () {
            myPropertyFactory.register(ColorID['1-0-0'].original);
            myPropertyFactory.register(ColorID['1-0-0'].original);
    ...
    it(
        'should accept registering a different template from what is in the registry if it is semantically equivalent',
        function () {
    ...
    it(
        'should throw when registering an existing template version that is different from what is in the registry',
        function () {
            myPropertyFactory.register(ColorID['1-0-0'].original);
            expect(myPropertyFactory.register.bind(
                myPropertyFactory,
    ...
    it('should throw when registering an unversioned template', function () {
        expect(myPropertyFactory.register.bind(
            myPropertyFactory,
            require('./validation/bad_missing_semver_in_typeid')
        )).to.throw(Error);
    });
    ...
    it('should throw when registering an invalid versioned template', function () {
        expect(myPropertyFactory.register.bind(
            myPropertyFactory,
            require('./validation/bad_primitive_typeid')
        )).to.throw(Error);
    });
    ...
    it(
        ...
        function () {
            myPropertyFactory.register(ColorID['1-0-0'].original);
            expect(myPropertyFactory._registerRemoteTemplate.bind(
                myPropertyFactory,
                ColorID['1-0-0'].modified,
                generateGUID())
            ).to.throw(Error);
    ...
    it(
        'should register a remote template even when there are other versions of the same template in the local registry',
        function () {
    ...
    it(
        'should register a local template even when there are other versions of the same template in the remote registry',
        function () {
            var scope = generateGUID();
            var scope2 = generateGUID();
            myPropertyFactory._registerRemoteTemplate(ColorID['1-1-0'].goodSemver, scope);
            myPropertyFactory._registerRemoteTemplate(ColorID['1-0-1'].goodSemver, scope);
            myPropertyFactory._registerRemoteTemplate(require('./validation/good_point_id'), scope2);
            myPropertyFactory._registerRemoteTemplate(require('./validation/good_color_palette'), scope2);
    ...
    describe('Constants', () => {
        describe('should throw when trying to register a set not inheriting from a namedProperty', () => {
            ...
            it('@bugfix should support constant which inhirets from a polymorphic array', () => {
                ...
                expect(areaProp.get('units').get(0).get('name').value).to.equal(centimeterProp.get('name').value);
            });
        });
    });
});
...
describe('Async validation', function () {
    ...
    it('can validate asynchronously', function () {
        ...
        var templatePrevious = JSON.parse(JSON.stringify(require('./validation/good_point_id')));
    ...
    it('can perform context validation asynchronously', function (done) {
        ...
        templateValidator.validateAsync(childSchema).then(function (result) {
...
describe('inheritsFrom() method', () => {
    ...
    it('should recognize that the test set that inherits from NamedNodeProperty also inherits from NodeProperty',
        () => {
            PropertyFactory._reregister({ typeid: 'autodesk.examples:test.set-1.0.0', inherits: 'NamedNodeProperty' });
            const result = PropertyFactory.inheritsFrom('autodesk.examples:test.set-1.0.0', 'NodeProperty');
    ...
    it('should recognize that the test set that inherits from NamedNodeProperty also inherits from NamedProperty',
        () => {
            PropertyFactory._reregister({ typeid: 'autodesk.examples:test.set-1.0.0', inherits: 'NamedNodeProperty' });
            const result = PropertyFactory.inheritsFrom('autodesk.examples:test.set-1.0.0', 'NamedProperty');
    ...
    describe('Caching', () => {
        ...
        it('should refresh the cache when _reregister() is called', () => {
            ...
            PropertyFactory._reregister({ typeid: 'autodesk.examples:test.set-1.0.0', inherits: 'NodeProperty' });
            expect(PropertyFactory.inheritsFrom('autodesk.examples:test.set-1.0.0', 'NodeProperty')).to.be.true;

            PropertyFactory._reregister({ typeid: 'autodesk.examples:test.set-2.0.0', inherits: 'NodeProperty' });
        ...
        it('should flush the cache when _clear() is called', () => {
            expect(PropertyFactory.inheritsFrom('autodesk.examples:test.set-1.0.0', 'NamedProperty')).to.be.true;
            expect(PropertyFactory.inheritsFrom('autodesk.examples:test.set-2.0.0', 'NamedProperty')).to.be.true;
...
describe('Remote template scope collection', () => {
    ...
    it('should collect registered remote templates in the scope.', () => {
        ...
        const templateA = Object.assign({}, ColorID['1-0-0'].original, { typeid: 'testA:ColorID-1.0.0' });
        const templateB = Object.assign({}, ColorID['1-0-0'].original, { typeid: 'testB:ColorID-1.0.0' });
        ...
        expect(scope().item(scopeGuid).item('testA:ColorID').getKeys())
            .to.have.members(['1.0.0']);

        expect(scope().item(scopeGuid).has('testB:ColorID')).to.be.true;
        expect(scope().item(scopeGuid).item('testB:ColorID').getKeys())
            .to.have.members(['1.0.0']);
    });
    ...
    it('should be able to get a specific template from the scope collection.', () => {
        ...
        expect(PropertyFactory._get('testA:ColorID-1.0.0', undefined, scopeGuidA).typeid)
            .to.be.equal('testA:ColorID-1.0.0');

        expect(PropertyFactory._get('testB:ColorID-1.0.0', undefined, scopeGuidB).typeid)
            .to.be.equal('testB:ColorID-1.0.0');
    });

    it('should be cleared when the PropertyFactory is.', () => {
        const scopeGuid = generateGUID();
        const template = Object.assign({}, ColorID['1-0-0'].original, { typeid: 'test:ColorID-1.0.0' });
