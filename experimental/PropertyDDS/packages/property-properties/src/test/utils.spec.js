...
describe('Utils', function() {
    ...
    describe('Utils.traverseChangeSetRecursively', function() {
        ...
        it('should report correctly for inserts', function() {
            ...
            testRoot.resolvePath('"test\\"Templated\\"Property".containedMap').insert('entry',
                PropertyFactory.create('autodesk.test:utilsTestContained-1.0.0'));
            ...
            // add members
            // Simple string
            testRoot._properties.map.insert('string',
                PropertyFactory.create('String'));

            // String with name that has to be escaped
            testRoot._properties.map.insert('s"tr"ing',
                PropertyFactory.create('String'));

            // A map in a map
            testRoot._properties.map.insert('map',
                PropertyFactory.create('map<>'));
            testRoot._properties.map.get('map').insert('string',
                PropertyFactory.create('String'));

            // An array in a map
            testRoot._properties.map.insert('array',
                PropertyFactory.create('array<>'));
            testRoot._properties.map.get('array').insert(0,
                PropertyFactory.create('NodeProperty'));
            testRoot._properties.map.get('array').get(0).insert('string',
                PropertyFactory.create('String'));

            // A composed property
            testRoot._properties.map.insert('testParent',
                PropertyFactory.create('autodesk.test:utilsTestParent-1.0.0'));

            // A NodeProperty
            testRoot._properties.map.insert('nodeProperty',
                PropertyFactory.create('NodeProperty'));
            testRoot._properties.map.get('nodeProperty').insert('string',
                PropertyFactory.create('String'));
            ...
            namedNodePropForSet.insert('string',
                PropertyFactory.create('String'));
    ...
    describe('Utils.traverseChangeSetRecursivelyAsync', function() {
        ...
        it('@regression should work when replacing a map element', function(done) {
            ...
            testRoot._properties.map.set('testParent', PropertyFactory.create('autodesk.test:utilsTestParent-1.0.0',
                'single', { errorMsg: 'another element', progress: 99 }));
    ...
    describe('Change set helper functions', function() {
        ...
        it('should work correctly for modifies', function() {
            ...
            // Test map insertion
            root.resolvePath('map').insert('entry2',
                PropertyFactory.create('autodesk.test:utils.spec.task.subject-1.0.0'));
            ...
            for (var i = 0; i < 5; i++) {
                assert(Utils.getChangesByPath(_.keys(modifiedResults.modify)[i], root,
                    root.serialize({ 'dirtyOnly': true }), false).modify !== undefined);
            }
            for (var i = 0; i < 5; i++) {
                assert(Utils.getChangesByPath(_.keys(modifiedResults.insert)[i], root,
                    root.serialize({ 'dirtyOnly': true }), false).insert !== undefined);
            }
        });
        ...
        it('should correctly work for removes', function() {
            ...
            assert(Utils.getChangesByPath(sim.getId() + '.' + subject2.getId() + '.' + subject3.getId(),
                root, root.serialize({ 'dirtyOnly': true }), false).removed === true);

            root.remove(sim.getId());
            assert(Utils.getChangesByPath(sim.getId() + '.' + subject2.getId() + '.' + subject3.getId(),
                root, root.serialize({ 'dirtyOnly': true }), false).removed === true);
            assert(Utils.getChangesByPath(sim.getId() + '.' + subject2.getId(), root,
                root.serialize({ 'dirtyOnly': true }), false).removed === true);
        ...
        it('should work for nested templates', function() {
            ...
            expect(Utils.getChangesByPath('c.myNestedProp.errorMsg',
        ...
        it('should work for an object with characters that have to be quoted', function() {
            ...
            var changes = Utils.getChangesByType('String', changeSet);
            expect(changes.modify).to.have.keys('simple_property',
                '"test.property"',
                '"test\\"property\\""',
            ...
            expect(Utils.getChangesByPath('"test\\"property\\""', node, changeSet, false)).to.have.keys('modify');
            expect(Utils.getChangesByPath('"test[property]".".property.".test',
        ...
        it('should work for a node property with characters that have to be quoted', function() {
            ...
            node.get('test[property]').get('.property.').insert('test',
                PropertyFactory.create('String', undefined, 'test'));

            var changeSet = node._serialize(true);
            var changes = Utils.getChangesByType('String', changeSet);
            expect(changes.insert).to.have.keys('simple_property',
                '"test.property"',
                '"test\\"property\\""',
                '"test[property]".".property.".test');
            ...
            expect(Utils.getChangesByPath('"test\\"property\\""', node, changeSet, false)).to.have.keys('insert');
            expect(Utils.getChangesByPath('"test[property]".".property.".test',
                node, changeSet, false)).to.have.keys('insert');
        });
    });
