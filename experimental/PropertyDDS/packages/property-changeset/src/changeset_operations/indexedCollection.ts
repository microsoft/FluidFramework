 ...
 * @private
 */
const _fastIsEmptyObject = (in_object: any): boolean => {
...
export namespace ChangeSetIndexedCollectionFunctions {
	...
	export const _performApplyAfterOnPropertyIndexedCollection = function (
		...
		if (in_appliedPropertyChanges.remove) {
			...
			const baseInserted = io_basePropertyChanges.insert || {};
			const baseRemoved = io_basePropertyChanges.remove;
			const baseModified = io_basePropertyChanges.modify;
			let done = false;
		...
		if (in_appliedPropertyChanges.insert) {
			...
			for (let i = 0; i < insertedTypeids.length; i++) {
				const typeid = insertedTypeids[i];
				...
				for (let j = 0; j < insertedKeys.length; j++) {
					...
					if (
						...
						deeplyEqualCS =
							!!insertedEntries[key].insert === !!removalCS[key].remove;
						...
						const removedEntry = isObject(removalCS[key])
							...
							: removalCS[key];
						deeplyEqualCS =
							deeplyEqualCS && isEqual(insertedEntry, removedEntry);
					}
		...
		if (in_appliedPropertyChanges.modify) {
			...
			// If no typeids are included, we just use a placeholder for the iteration below
			const modifiedTypeids = isPrimitiveTypeid
				? [undefined]
				: Object.keys(modifiedEntries);
				...
				for (let j = 0; j < modifyKeys.length; j++) {
					...
					if (isPrimitiveTypeid) {
						...
						if (baseInserted[key] !== undefined) {
							...
							if (
								baseModified[key] &&
								Object.hasOwn(baseModified[key], "value")
							) {
						...
						if (
							baseInserted[typeid] &&
							baseInserted[typeid][key] !== undefined
						) {
							// If this entry was added by this ChangeSet, we modify the insert operation according to the
							// new ChangeSet
							if (isEntryPrimitiveType && typeid !== "String") {
								...
								if (
									...
									Object.hasOwn(baseInserted[typeid][key], "value")
								) {
							...
							if (isEntryPrimitiveType && typeid !== "String") {
								...
								if (typeid === "Int64" || typeid === "Uint64") {
									...
									if (appliedVal && Object.hasOwn(appliedVal, "value")) {
							...
							baseModified[typeid][key] = cloneDeep(
								modifiedEntries[typeid][key],
							);
						}
					}
				}
		...
		this._cleanIndexedCollectionChangeSet(
	...
	export const _rebaseIndexedCollectionChangeSetForProperty = function (
		...
		const addChanges = (
			...
			const keys = Array.isArray(in_collection)
				? in_collection
				: Object.keys(in_collection);
			...
			for (let j = 0; j < keys.length; j++) {
				...
				changesByKeys[key][in_changePrefix] = changesByKeys[key][
					in_changePrefix
				]
                ...
                */
				...
				if (modification.own === "modify" && modification.other === "modify") {
					if (
						...
						if (
							typeof ownValue === "object" &&
							Object.hasOwn(ownValue, "value")
						) {
						...
						if (
							...
							Object.hasOwn(rebaseValue, "value")
						) {
						...
						if (
							...
							if (
								ownValue[0] === rebaseValue[0] &&
								ownValue[1] === rebaseValue[1]
							) {
					...
					if (!isPrimitiveTypeid) {
						...
						// Delete the modification from the rebased ChangeSet
						delete io_rebasePropertyChangeSet.modify[modification.otherTypeid][
							key
						];
					}
				} else if (
					(modification.own === "modify" || modification.own === "remove") &&
					(modification.other === "remove" ||
					...
					modification.own === "insert" &&
					modification.other === "insert"
				) {
					if (
						isPrimitiveTypeid ||
						TypeIdHelper.isPrimitiveType(modification.ownTypeid)
					) {
						const insertMap = modification.otherTypeid
						...
						if (modification.otherTypeid) {
							...
							modifyMap =
								io_rebasePropertyChangeSet.modify[modification.otherTypeid];
							oldValue =
								in_ownPropertyChangeSet.insert[modification.ownTypeid][key];
						} else {
							io_rebasePropertyChangeSet.modify =
								io_rebasePropertyChangeSet.modify || {};
	 ...
	 * @private
	 */
	export const _cleanIndexedCollectionChangeSet = (
		...
		if (in_containsTypeids) {
			const typeidList = Object.keys(changes.insert || {});
		...
		// First remove unused typeid sections
		if (in_containsTypeids) {
			const typeidList = Object.keys(changes.remove || {});
			for (let j = 0; j < typeidList.length; j++) {
		...
		// First remove unused typeid sections
		if (in_containsTypeids) {
			const typeidList = Object.keys(changes.modify || {});
			for (let j = 0; j < typeidList.length; j++) {
