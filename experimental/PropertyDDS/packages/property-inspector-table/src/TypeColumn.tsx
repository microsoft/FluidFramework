...
export const TypeColumn: React.FunctionComponent<ITypeColumn> = ({ rowData }) => {
	...
	additionalType =
		additionalType && additionalType.replace(/>/g, "") !== "Enum"
			? additionalType.replace(/>/g, "")
			: undefined;
	return (
		<Tooltip
			enterDelay={500}
		...
		>
			<div className={classes.typesBox}>
				{rowData.isConstant ? (
					<CustomChip label={"const"} className={classes.constAndContextColor} />
				) : null}
				...
				{rowData.isReference && typeid !== "Reference" ? (
					<CustomChip label={"Reference"} className={classes.defaultColor} />
				) : null}
			</div>
		</Tooltip>
	);
