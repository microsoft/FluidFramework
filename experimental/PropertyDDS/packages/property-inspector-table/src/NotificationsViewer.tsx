...
export const notificationContext: INotificationContext = {
	...
	pushNotification: (notification) => {
		notificationList.push({ id: generateNotificationId(), ...notification });
	},
...
class NotificationViewer extends React.Component<
	WithStyles<typeof styles> & Omit<SnackbarProps, "classes" | "open">,
...
> {
	...
	public state = {
		notificationList: notificationContext.notificationList,
		open: false,
		messageInfo: "",
	};
	...
	private readonly pushNotification: INotificationContext["pushNotification"] = (
		notificationObjToBePushed,
	) => {
		let newArr;
		this.setState((prevState) => {
			...
			if (!prevState.open && prevState.notificationList.length === 0) {
				...
				newArr = prevState.notificationList.concat([
					{ id: generateNotificationId(), ...notificationObjToBePushed },
				]);
	...
	private readonly removeNotification: INotificationContext["removeNotification"] = (id) => {
		this.setState((prevState) => {
			const index = prevState.notificationList.findIndex(
				(notification: INotification) => notification.id === id,
			);
	...
	private readonly removeNotificationByIndex: INotificationContext["removeNotificationByIndex"] =
		(notificationIndex) => {
...
const StyledNotificationViewer = withStyles(styles, { name: "NotificationViewer" })(
	NotificationViewer,
);
