...
export const getIconFromTypeId = (inTypeid): React.ReactNode => {
	...
	return (
		typeidToIconMap[inTypeid] || (
			<SvgIcon svgId={"propertyset-24"} height={iconHeight} width={iconWidth} />
		)
	);
};
...
export const getDefaultInspectorTableIcons = (rowData: IInspectorRow): React.ReactNode => {
	...
	if (rowData.isReference) {
		...
		icon = (
			<SvgIcon svgId={"reference-24"} height={iconHeight} width={iconWidth} fill={color} />
		);
	} else {
