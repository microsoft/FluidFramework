...
export const iconAspectRatio: number =
	parseFloat(defaultIconSize.width) / parseFloat(defaultIconSize.height);

export const computeIconSize = (width: number): IIconSize =>
	width * 0.5 >= parseFloat(defaultIconSize.width)
		? defaultIconSize
		: {
...
const Empty: React.FunctionComponent<IEmptyProps & WithStyles<typeof styles>> = (props) => {
	...
	return (
		<div className={classNames(props.classes.root)}>
			...
			<div className={props.classes.centered}>{description}</div>
		</div>
	);
};
