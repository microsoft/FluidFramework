...
const nonNumericTypes = new Set(["Bool", "Uint64", "Int64", "String", "Reference"]);
...
export const Field: React.FunctionComponent<IEditableValueCellProps> = ({
	rowData,
	...restProps
}) => {
	...
	if (!isEditableTreeRow(rowData)) {
		...
		try {
			property = (parent as ContainerProperty).get(rowData.name);
		} catch {
			typeid = "Reference";
		}
	...
	return <ViewComponent onSubmit={onInlineEditEnd} rowData={rowData} {...restProps} />;
};
