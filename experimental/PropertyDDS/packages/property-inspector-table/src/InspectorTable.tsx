 ...
 */
const defaultSort = { key: "name", order: TableSortOrder.ASC } as {
	key: React.Key;
	order: SortOrder;
};
...
> extends React.Component<ITableProps & IInspectorTableProps<IRowData<T>>, IInspectorTableState> {
		...
		if (props.activeRepositoryGuid !== state.expandedRepoGuid) {
			return {
				...
				expandedRepoMap: state.expandedRepoGuid
					? { ...state.expandedRepoMap, [state.expandedRepoGuid]: { ...state.expanded } }
					: { ...state.expandedRepoMap },
			};
		}
		return newState;
	}
	...
	public constructor(props: Readonly<IInspectorTableProps<T>>) {
		...
		const { followReferences, dataCreationHandler, dataCreationOptionGenerationHandler } =
			props;
		...
		this.toTableRowOptions = {
			addDummy: true,
			ascending: defaultSort.order === TableSortOrder.ASC,
			depth: 0,
			followReferences,
		};
		...
		this.debouncedSearchChange = debounce((searchExpression: string) => {
			...
			if (searchExpression.length > 0) {
				...
				const searchControls = this.startSearch(
					searchExpression,
	...
	public componentDidMount() {
		...
		if (data) {
			const updatedTableRows = this.props.toTableRows!(
				{ data, id: "" },
				this.props,
				this.toTableRowOptions,
			);
	...
	public componentDidUpdate(prevProps: ITableProps, prevState: IInspectorTableState) {
		...
		if (
			(prevProps !== this.props || prevState.sortBy.order !== sortBy.order) &&
			!checkoutInProgress
		) {
			if (data) {
				const updatedTableRows = this.props.toTableRows!(
				...
				<Empty
					description={
						<div className={classes.expiredNotice}>
							<span>
								Expired repositories are destroyed after 30 days.
								<br />
	...
	private readonly generateColumns = (width: number) => {
		...
		for (const currentId of this.props.columns) {
			...
			if (
				this.props.columnsRenderers !== undefined &&
				this.props.columnsRenderers[currentId] !== undefined
			) {
				newColumn.cellRenderer = (args: any) => {
					const { foundMatches, currentResult, matchesMap } = this.state;
					...
					return this.props.columnsRenderers![currentId]({
						...
						searchResult,
						renderCreationRow: this.renderCreationRow,
					});
				};
			}
	...
	private readonly handleCreateData = async (
		rowData: T,
		name: string,
		type: string,
		context: string,
	...
	private readonly continueSearchOnDemand = () => {
		const newState: Pick<
			IInspectorTableState,
			"searchAbortHandler" | "searchInProgress" | "searchState"
		> = {
	...
	private readonly handleSearchExpressionChange = (
	...
	private readonly handleRowExpanded = ({
		expanded: newExpandedFlag,
		rowData,
	}: {
		expanded: boolean;
		rowData: T;
	...
	private readonly updateSearchState = (
		...
		if (
			(this.state.currentResult === -1 || this.state.currentResult === undefined) &&
			foundMatches.length > 0
		) {
