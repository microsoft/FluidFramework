...
export const showNextResult = (
	data: IInspectorRow[],
	currentlyExpanded: IExpandedMap,
	allMatches: IInspectorSearchMatch[],
	resultIndex: number,
	...
	return {
		expandedRows: currentlyExpanded,
		rowIdx: rowInfo.idx,
		columnIdx: desiredDataItem.indexOfColumn,
	};
};
 ...
 */
function findMatchingElementIndexInDataSet(
	data: IInspectorRow[],
	currentlyExpanded: IExpandedMap,
	matchingElement: IInspectorSearchMatch,
	rowCounter = 0,
	...
	for (const row of data) {
		...
		if (row.id in currentlyExpanded) {
			const res = findMatchingElementIndexInDataSet(
				row.children!,
				currentlyExpanded,
				matchingElement,
				rowCounter,
...
const updateHandler = (
	...
	searchState.updateScheduled = window.setTimeout(() => {
		callback(
			searchState.foundMatches,
			searchState.matchesMap,
			done,
			searchState.childToParentMap,
		);
	}, 0);
};
...
export const search = (
	...
	recursive = false,
	entryPoint = true,
	fillExpanded: any = defaultFillExpanded,
): IInspectorSearchControls => {
	...
	for (; levelState.index < rows.length; ++levelState.index) {
		...
		for (let columnIndex = 0; columnIndex < columns.length; columnIndex++) {
			...
			if (
				(cell !== undefined ? String(cell).toLowerCase() : "").includes(
					searchState.expression!,
				)
			) {
		...
		if (item.children && !item.isReference) {
			...
			if (item.children.length > 0) {
				...
				if (
					searchState.newMatchFound ||
					(searchState.chunkSize === 0 && searchState.scheduled !== undefined)
				) {
					if (entryPoint && searchState.newMatchFound) {
						updateHandler(handleUpdate, searchState, false);
					}
