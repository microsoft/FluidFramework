...
export const getReferenceValue = (rowData: IInspectorRow) => {
	...
	if (Utils.isReferenceCollectionTypeid(getCollectionTypeid(parentProp))) {
		...
		]) as unknown as ReferenceProperty;
...
const getShortId = (parentPath: string, childId: string | undefined = undefined): string => {
	...
	const absolutePath = childId !== undefined ? parentPath + idSeparator + childId : parentPath;
...
const isExpandable = (
	...
	return dataCreation
		? data && (context !== "single" || !isPrimitive(typeid))
		: data &&
				((context !== "single" && (data.size > 0 || data.length > 0)) ||
...
const addAdditionalRow = (subRows, id, parentProperty) => {
	...
	const canAddData =
		(isDefinedParent || parentProperty.isRoot()) &&
...
export const toTableRows = (
	...
	const dataCreation =
		props.readOnly !== true &&
		!parentIsConstant &&
		!!props.dataCreationHandler &&
		!!props.dataCreationOptionGenerationHandler;
	...
	switch (dataContext) {
		case "single": {
			sortedKeys.forEach((key) => {
				const newRow = singlePropertyTableRow(
					data,
					key,
					id,
					props,
					{ ...OPTION_DEFAULTS, ...options, dataCreation },
					pathPrefix,
				);
				subRows.push(newRow);
			});
		...
		default: {
			sortedKeys.forEach((key) => {
				const newRow = collectionChildTableRow(
					data,
					key,
					id,
					props,
	...
	if (dataCreation) {
		addAdditionalRow(subRows, id, parentProperty);
	}
	return subRows;
};
const createInvalidReference = (
	parentData: BaseProxifiedProperty,
	propertyId: string,
	parentRowId: string,
	props: IToTableRowsProps,
...
export const singlePropertyTableRow = (
	...
	const determinedValue = getPropertyValue(
		parentData,
		propertyId,
		currentContext,
		currentTypeid,
...
export const collectionChildTableRow = (
	collectionPropertyProxy: BaseProxifiedProperty,
	propertyId: string,
	parentRowId: string,
	props: IToTableRowsProps,
	options: IPropertyToTableRowOptions,
	...
	const propertyProxy = (
		prop && PropertyFactory.instanceOf(prop, "BaseProperty")
			? PropertyProxy.proxify(prop)
			: prop
	) as BaseProxifiedProperty;
	const { depth, addDummy, dataCreation, followReferences, ascending } = options;
	const parentIsConstant = !!options.parentIsConstant;
	...
	if (!isReferenceCollection || (isReferenceCollection && followReferences)) {
		if (
			...
			const { referencedPropertyParent, relativePathFromParent } =
				PropertyProxy.getParentOfReferencedProperty(
					collectionProperty as ReferenceArrayProperty,
					propertyId,
				);
			if (referencedPropertyParent && relativePathFromParent) {
				...
				const baseProperty = (referencedPropertyParent as ContainerProperty).get(
			...
			isEnumProperty(collectionProperty.get(propertyId)!) &&
			collectionProperty.getContext() === "map"
		) {
	...
	propertyId =
		collectionProperty.getContext() === "set" ? (propertyProxy as any).guid : propertyId;
	const determinedValue = getPropertyValue(
		collectionPropertyProxy,
		propertyId,
		currentContext,
...
export const fillExpanded = (
	...
	for (const row of innerRows) {
		if (row.id in expanded) {
			const newPathPrefix =
				row.parent && row.isReference
					? pathPrefix + (row.parent.getAbsolutePath() as string) + idSeparator + row.name
					: pathPrefix;
			...
			if (row.children && row.children[0].context === "d") {
				row.children = toTableRows(
					...
					{
						...toTableRowsOptions,
						parentIsConstant: row.isConstant || row.parentIsConstant,
					},
					newPathPrefix,
				);
			}
...
export const getPropertyValue = (
	parent: ContainerProperty | BaseProxifiedProperty,
	id: string,
	context: string,
	typeid: string,
	followReferences = true,
	...
	try {
		...
		if (
			Utils.isReferenceCollectionTypeid(parentProperty.getTypeid()) ||
	...
	let propertyProxy =
		parentProperty.getContext() === "map" ? (parentProxy as any).get(id) : parentProxy[id];
	...
	if (!followReferences && TypeIdHelper.isReferenceTypeId(typeid)) {
		determinedValue =
			parentContextIsSingle || isReferenceArrayProperty(parentProperty)
				? parentProxy[`${id}*`]
				: (parentProxy as any).get(`${id}*`);
...
export function nameCellRenderer({
	...
	const { checkoutInProgress, rowIconRenderer, width, dataGetter, readOnly, classes } =
		tableProps;
	if (checkoutInProgress) {
		return getCellSkeleton(width);
	}
	if (cellData && dataGetter && rowData.context) {
		// cell data comes from data getter
		return cellData;
	} else {
		const nameCell = (
			<NameCell
