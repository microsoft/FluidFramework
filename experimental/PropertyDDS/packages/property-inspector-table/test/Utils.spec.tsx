...
describe("InspectorTable", () => {
	...
	describe("toTableRows", () => {
		...
		it("should adhere to n depth", () => {
			...
			expect(coordDepth1.children!.length).toEqual(
				workspace.get("CoordinateSystem3D").getIds().length,
			);
		});
		...
		it("should stop at single references", () => {
			...
			expect(validReference.value).toEqual(
				workspace.get("ValidReference", {
			...
			expect(invalidReference.value).toEqual(
				workspace.get("InvalidReference", {
					referenceResolutionMode: BaseProperty.REFERENCE_RESOLUTION.NEVER,
				}).value,
			);
		});
		...
		it("depth should take precedence to addDummy flag", () => {
			...
			expect(complexProp.children!.length).toEqual(
				workspace.get("CoordinateSystem3D").getIds().length,
			);
		});
		...
		describe("collections", () => {
			it("should infer proper typeids for children", () => {
				...
				const nonPrimitiveCollectionMapRow = findRow(
					"map",
					nonPrimitiveCollectionRow.children!,
				);
				expect(nonPrimitiveCollectionMapRow.children!.length).toEqual(
					nonPrimitiveCollectionMap.getIds().length,
				);
		...
		it("should sort correctly", () => {
			...
			const testRows = toTableRows(
				{ data: sortingRootProxy, id: initId } as IInspectorRow,
				props,
				{ ascending: true },
			);
			...
			const testRowsDesc = toTableRows(
				{ data: sortingRootProxy, id: initId } as IInspectorRow,
				props,
				{ ascending: false },
			);
	...
	describe("fillExpanded", () => {
		...
		it("should work with set and other collections expanded", () => {
			...
			expect(set.children!.length).toEqual(
				workspace.get([testId, "arrayOfReferences"]).getIds().length,
			);
			expect(set.children![1].children!.length).toEqual(
				workspace.get([testId, "arrayOfReferences", "1"]).length,
			);
			expect(set.children![17].children!.length).toEqual(
				workspace.get([testId, "arrayOfReferences", "17"]).length,
			);
		});
	});
	...
	describe("showNextMatchingResult", () => {
		...
		it("should work when other items were expanded by user", () => {
			...
			return getAllMatchesFromRows(
				...
				const nextMatchInfo = showNextResult(
					rows,
					expanded,
					result.matches,
					1,
					result.childToParentMap,
				);
	...
	describe("expandAll", () => {
		...
		it("should expand collections of references", () => {
			...
			expect(expanded.includes(getHash("/ReferenceCollections/arrayOfReferences"))).toEqual(
				true,
			);
		...
		it("should not expand reference", () => {
			expect(expanded.includes(getHash("/ReferenceCollections/arrayOfReferences/1"))).toEqual(
				false,
			);
		});
