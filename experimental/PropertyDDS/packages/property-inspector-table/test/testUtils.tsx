...
export const addProperty = (wrapper, primitiveValue, contextValue, name?) => {
	...
	const newPropertyOption = allOptions.find(
		(primitiveProperty) => primitiveProperty.label === primitiveValue,
	);
	const newContextOption = contextSelection
		.props()
		.options.filter((context) => context.value === contextValue)[0];
...
export const findEditableCell = (wrapper, typeList: string[], name = "") => {
	...
	editCell =
		name || typeof name !== "string"
			? editCell.filterWhere((n) => n.props().rowData.name === name.toString())
			: editCell;
...
export const testDynamicDropdown = (wrapper) => {
	...
	if (contextSelector.props().options.length !== 3) {
		// only 3 context options for NodeProperty
		return false;
	}
	...
	if (contextSelector.props().options.length !== 4) {
		// 4 context options for NamedNodeProperty, incl. Set
		return false;
	}
	...
	if (contextSelector.props().value === "set") {
		// unset context on change of property type
		return false;
	}
