...
export const getPopulateFunctionWithSerializedBranchData =
	(serializedBranchData: any) => (workspace: MockWorkspace) => {
		if (!serializedBranchData) {
			return;
		}
...
export const populateWorkspace = (workspace: MockWorkspace) => {
	...
	workspace.root.insert(
		"BooleanFalse",
		PropertyFactory.create("Bool", "single", false) as BaseProperty,
	);
	workspace.root.insert(
		"BooleanTrue",
		PropertyFactory.create("Bool", "single", true) as BaseProperty,
	);
	workspace.root.insert(
		"String",
	...
	workspace.root.insert(
		"Point3D",
		PropertyFactory.create(point3DSchema.typeid, "single", {
			x: 1,
			y: 2,
			z: 3,
		}) as BaseProperty,
	...
	workspace.root.insert(
		"NonPrimitiveCollections",
		PropertyFactory.create(nonPrimitiveCollectionsSchema.typeid) as BaseProperty,
	);
	workspace
		.get<MapProperty>(["NonPrimitiveCollections", "map"])!
		.set("outlier", PropertyFactory.create(coordinateSystem3DSchema.typeid) as BaseProperty);
	workspace.root.insert(
	...
	workspace.root.insert(
		uniqueIdentifier,
		PropertyFactory.create("String", "single", uniqueIdentifier)!,
	);
	...
	for (const type of primitives) {
		for (const context of collections) {
			workspace.root.insert(
				type.toLowerCase() + context,
				PropertyFactory.create(type, context.toLowerCase()) as BaseProperty,
			);
		}
	}
	...
	workspace.root.insert(
		"ReferenceToElementOfArrayOfReferences",
		PropertyFactory.create(
			"Reference",
			"single",
			"/ReferenceCollections.arrayOfReferences[0]",
		) as BaseProperty,
	);
	workspace.root.insert(
		"ReferenceToInvalidElementOfArrayOfReferences",
		PropertyFactory.create(
			"Reference",
			"single",
			"/ReferenceCollections.arrayOfReferences[5]",
		) as BaseProperty,
	);
};
