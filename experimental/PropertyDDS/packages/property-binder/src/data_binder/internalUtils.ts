 ...
 */
const joinPaths = function(in_path1: string | undefined, in_path2: string | undefined): string | undefined {
...
const _generateIndices = function(in_arrayIterator: ArrayChangeSetIterator): number[] {
    return (in_arrayIterator.opDescription.operation![1] as Array<any>).map(function(_value: any, index: any) {
...
const _invokeNodePropertyCallbacks = function(in_invokeContext: InvokeContext, in_handlers: any) {
    ...
    if (nestedChangeSet.remove && in_handlers.collectionRemove) {
        const currentChangeSet = nestedChangeSet.remove.reduce(function(result: { [x: string]: any; }, key: string | number) {
            result[key] = key;
            return result;
        }, {});
        _callCorrespondingHandlers(
 ...
 * @hidden
 */
const _invokePropertyCallbacks = function(in_invokeContext: InvokeContext, in_operationHandlers: any, in_bindToReference: boolean) {
...
const recursivelyVisitHierarchy = function(
                ...
                if (ids) {
                    _.each(ids, function(id) {
                        ...
                        if (in_dataBindingTreeNode) {
                            in_dataBindingTreeNode = in_dataBindingTreeNode.getNodeForTokenizedPath([skippedId])! as any;
                        }
    ...
    _recursiveStep(in_rootPropertyElement, in_elementAbsolutePath, tokenizedPath, dataBindingTreeNode as any);
};
 ...
 *
 */
const forEachProperty = function(in_rootProperty: BaseProperty, in_callback: (property: BaseProperty) => boolean) {
    const _recursiveStep_forEachProperty = function(in_property: BaseProperty) {
        const recurse = in_callback(in_property);

        if (recurse) {
            if ((in_property as ContainerProperty).getIds && in_property.getTypeid() !== 'String' && !isPrimitiveCollection(in_property)) {
 ...
 * @hidden
 */
const visitTypeHierarchy = function(in_typeID: string, in_callback: (typeid: string) => boolean): boolean {
...
const installForEachPrototypeMember = function(in_dataBindingConstructor: typeof DataBinding) {
    if (!in_dataBindingConstructor.prototype.hasOwnProperty('_forEachPrototypeMember')) {
        in_dataBindingConstructor.prototype._forEachPrototypeMember = function(in_propertyName: string | number, in_callback: (arg0: any) => void) {
 ...
 */
const getOrCreateMemberOnPrototype = function <T = Object>(in_dataBindingConstructor: typeof DataBinding, in_propertyName: string, in_default: T): T {
...
const _unregisterPathHandle = function(in_handle: any, in_details: any) {
    ...
    const allPathHandles = getOrCreateMemberOnPrototype<any[]>(in_details.bindingConstructor, '_allPathHandles', []);
 ...
 */
const createHandle = function(in_constructor: typeof DataBinding, in_escapedPaths: (string | number)[],
    in_operations: string[], in_callback: any): DataBinderHandle {
    const details = {
    ...
    for (var i = 0; i < in_operations.length; ++i) {
        ...
        // are affected.
        const callback = in_escapedPaths.length === 1 ? in_callback : makeCallbackOncePerChangeSet(in_callback);
        for (var j = 0; j < in_escapedPaths.length; ++j) {
            const handlerList = _getHandlerList(in_constructor, in_escapedPaths[j] as string, '_registeredPaths', operation);
            handlerList.push({
 ...
 * @hidden
 */
const invokeWithProperty = function(this: any, in_callback: Function, in_requireProperty: boolean, in_modificationContext: ModificationContext) {
 ...
 * @hidden
 */
const invokeWithCollectionProperty = function(this: any, in_callback: Function, in_requireProperty: boolean, in_key: string | number, in_modificationContext: ModificationContext) {
    if (_hasValidPropertyFromContext(in_modificationContext)) {
        var property = in_modificationContext.getProperty();
 ...
 * @hidden
 */
const createRegistrationFunction = function(in_registrationType: string, in_callbackArgs: IArguments): Function {
