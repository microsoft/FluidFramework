...
describe("Decorated DataBinding", function () {
	...
	const personTemplate = {
		...
		properties: [{ id: "parent", typeid: "test:personInfo-1.0.0" }],
	...
	it("should call onPropertyChange callback", function () {
		...
		expect(dataBindingInstance.onPropertyRegisteredCallback.mock.calls[0][0]).toBeInstanceOf(
			BaseProperty,
		);
	});
	...
	it("should call onPathRegisteredCallback callback", function () {
		...
		expect(dataBindingInstance.onPathRegisteredCallback.mock.calls[0][0]).toBeInstanceOf(
			ModificationContext,
		);
	});
	...
	it("should call onValuesRegisteredCallback callback", function () {
		...
		expect(dataBindingInstance.onValuesRegisteredCallback.mock.calls[0][0]).toEqual(
			testProp.get(["nickname"]).value,
		);
	});
