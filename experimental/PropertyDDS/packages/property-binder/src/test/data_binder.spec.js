...
describe("DataBinder", function () {
	...
	describe("basic functionality", function () {
		...
		it("should be possible to pass a workspace to the constructor", function () {
			var dataBinder;
			expect(() => {
				dataBinder = new DataBinder(workspace);
			}).not.toThrow();
			expect(dataBinder.isAttached()).toEqual(true);
		...
		it("should be possible to modify a workspace that was passed to the constructor", function () {
			expect(() => {
				new DataBinder(workspace);
			}).not.toThrow();
			workspace.root.insert("children", PropertyFactory.create("Float32", "array"));
			workspace.root
				.get("children")
				.insert(0, PropertyFactory.create("Float32", "single", 1));
		});
		...
		it("should be possible to pass and modify a populated workspace to the constructor", function () {
			...
			workspace.root
				.get("children")
				.insert(1, PropertyFactory.create("Float32", "single", 2));
		});
		...
		it("should be able to activate all bindings of a bindingtype", function () {
			...
			workspace.root.insert(
				"inherited",
				PropertyFactory.create(InheritedChildTemplate.typeid, "single"),
			);
			...
			expect(dataBinder.resolve("inherited", "BINDING")).toBeInstanceOf(
				InheritedChildDataBinding,
			);
		...
		it("it should be possible to register DataBindings on demand", function () {
			...
			workspace.root
				.get("child1")
				.insert("myFloat2", PropertyFactory.create("Float64", "single"));

			var handle = dataBinder.register(bindingType, typeid, ParentDataBinding, {
		...
		it("should create/remove DataBindings when corresponding property set is added/removed", function () {
			...
			expect(childDataBinding.onPreModify).toHaveBeenCalledTimes(
				childDataBinding.onModify.mock.calls.length,
			);
		...
		it("should notify DataBindings of primitive changes", function () {
			...
			var primitiveChildrenPset = PropertyFactory.create(
				PrimitiveChildrenTemplate.typeid,
				"single",
				initialValues,
			);
			...
			expect(primitiveChildrenDataBinding.onPreModify).toHaveBeenCalledTimes(
				primitiveChildrenDataBinding.onModify.mock.calls.length,
			);
		...
		it("should notify DataBinding with special characters in the path", function () {
			...
			expect(textSpy).toHaveBeenCalledTimes(0);
			workspace.root
				.get(["parent", "nested.test", 'child "with" quotes', "text"])
				.setValue("test");
			expect(textSpy).toHaveBeenCalledTimes(1);
		...
		it("array of templates with array of primitives", function () {
			...
			// Register the DataBindings
			dataBinder.register("BINDING", PrimitiveChildrenTemplate.typeid, ChildDataBinding, {
				context: "all",
			});

			var primitiveChildren = {};

			primitiveChildren["array"] = PropertyFactory.create(
				PrimitiveChildrenTemplate.typeid,
				"single",
			);
			initializeProperties(primitiveChildren["array"]);

			primitiveChildren["set"] = PropertyFactory.create(
				PrimitiveChildrenTemplate.typeid,
				"single",
			);
			initializeProperties(primitiveChildren["set"]);

			primitiveChildren["map"] = PropertyFactory.create(
				PrimitiveChildrenTemplate.typeid,
				"single",
			);
			initializeProperties(primitiveChildren["map"]);

			var wrapperPset = PropertyFactory.create(
				PrimitiveChildWrapperTemplate.typeid,
				"single",
			);
			...
			wrapperPset._properties.primitiveChildrenMap.insert(
				"thisIsAKey",
				primitiveChildren["map"],
			);
	...
	describe("arrays", function () {
		...
		var setupDataBinder = function () {
			...
			expect(parentDataBinding.onPostCreate).toHaveBeenCalledTimes(1);
			expect(parentDataBinding.onPreModify).toHaveBeenCalledTimes(
				parentDataBinding.onModify.mock.calls.length,
			);
		...
		it("should notify parent when child DataBinding is added in array", function (done) {
			...
			var pathPrefixes = ["", "nested."]; // The paths to arrays to run this test for
			...
			for (var i = 0; i < pathPrefixes.length; i++) {
				...
				arrayPset
					.resolvePath(unrepresentedSubArrayPath)
		...
		it("should be able to mix insert and push for child DataBindings in an array", function (done) {
			...
			expect(dataBinder.resolve("myArrayPset.subArray[0]", "BINDING").getProperty()).toEqual(
				child2,
			);
			expect(dataBinder.resolve("myArrayPset.subArray[1]", "BINDING").getProperty()).toEqual(
				child1,
			);
			expect(dataBinder.resolve("myArrayPset.subArray[2]", "BINDING").getProperty()).toEqual(
				child3,
			);

			expect(
				dataBinder.resolve("myArrayPset.subArray[0]", "BINDING").onPostCreate,
			).toHaveBeenCalledTimes(1);
			expect(
				dataBinder.resolve("myArrayPset.subArray[1]", "BINDING").onPostCreate,
			).toHaveBeenCalledTimes(1);
			expect(
				dataBinder.resolve("myArrayPset.subArray[2]", "BINDING").onPostCreate,
			).toHaveBeenCalledTimes(1);
		...
		it("should notify appropriate DataBinding of modifications - subArray", function (done) {
			...
			for (var i = 0; i < pathPrefixes.length; i++) {
				var childPset = PropertyFactory.create(ChildTemplate.typeid, "single");
				var unrepresentedPset = PropertyFactory.create(
					UnrepresentedTemplate.typeid,
					"single",
				);
				var subArrayPath = pathPrefixes[i] + "subArray";
				var unrepresentedSubArrayPath = pathPrefixes[i] + "unrepresentedSubArray";
		...
		it("should notify parent when child DataBinding is removed from array", function (done) {
			...
			for (i = 0; i < pathPrefixes.length; i++) {
				...
				for (j = 0; j < 3; j++) {
					...
					unrepresentedPsets.push(
						PropertyFactory.create(UnrepresentedTemplate.typeid, "single"),
					);
					arrayPset.resolvePath(unrepresentedSubArrayPath).push(unrepresentedPsets[j]);

					expect(dataBinder._dataBindingCreatedCounter).toEqual(1);
					const childDataBinding = dataBinder.resolve(
						childPsets[j].getAbsolutePath(),
						"BINDING",
					);
					expect(childDataBinding).toBeInstanceOf(ChildDataBinding);
		...
		it("should handle multiple array operations", function (done) {
			...
			const createdDataBinding = dataBinder.resolve(
				arrayPset.resolvePath(subArrayPath).get(1),
				"BINDING",
			);
		...
		it("should remove all DataBindings when clearing an array", function (done) {
			...
			// Clear the array
			arrayPset
				.resolvePath(subArrayPath)
				.removeRange(0, arrayPset.resolvePath(subArrayPath).length);
		...
		it("should handle multiple removes within a scope", function (done) {
			...
			expect(dataBinder.resolve("myArrayPset." + subArrayPath + "[0]", "BINDING")).toEqual(
				childDataBindings[0],
			);
			expect(dataBinder.resolve("myArrayPset." + subArrayPath + "[1]", "BINDING")).toEqual(
				childDataBindings[2],
			);
			expect(dataBinder.resolve("myArrayPset." + subArrayPath + "[2]", "BINDING")).toEqual(
				childDataBindings[3],
			);
			tearDownDataBinder();
			done();
		...
		it("should handle multiple operations within a scope", function (done) {
			...
			expect(dataBinder._dataBindingCreatedCounter).toEqual(3);
			for (i = 0; i < 3; ++i) {
				childDataBindings.push(
					dataBinder.resolve(childPsets[i + 1].getAbsolutePath(), "BINDING"),
				);
			}
			dataBinder._resetDebugCounters();
			expect(
				dataBinder.resolve("myArrayPset." + subArrayPath + "[0]", "BINDING").getProperty(),
			).toEqual(childPsets[1]);
			expect(
				dataBinder.resolve("myArrayPset." + subArrayPath + "[1]", "BINDING").getProperty(),
			).toEqual(childPsets[2]);
			expect(
				dataBinder.resolve("myArrayPset." + subArrayPath + "[2]", "BINDING").getProperty(),
			).toEqual(childPsets[3]);

			expect(
				dataBinder.resolve("myArrayPset." + subArrayPath + "[0]", "BINDING").onPostCreate,
			).toHaveBeenCalledTimes(1);
			expect(
				dataBinder.resolve("myArrayPset." + subArrayPath + "[1]", "BINDING").onPostCreate,
			).toHaveBeenCalledTimes(1);
			expect(
				dataBinder.resolve("myArrayPset." + subArrayPath + "[2]", "BINDING").onPostCreate,
			).toHaveBeenCalledTimes(1);
			...
			actChildDataBinding = dataBinder.resolve(
				arrayPset.resolvePath(subArrayPath).get(0).getAbsolutePath(),
				"BINDING",
			);
			// the other one will be at index 2
			const secondCreatedDataBinding = dataBinder.resolve(
				arrayPset.resolvePath(subArrayPath).get(2).getAbsolutePath(),
				"BINDING",
			);
			dataBinder._resetDebugCounters();
			expect(arrayPset.resolvePath(subArrayPath).getLength()).toEqual(3);
			...
			expect(
				dataBinder.resolve("myArrayPset." + subArrayPath + "[0]", "BINDING").getProperty(),
			).toEqual(childPsets[0]);
			expect(
				dataBinder.resolve("myArrayPset." + subArrayPath + "[1]", "BINDING").getProperty(),
			).toEqual(childPsets[1]);
			expect(
				dataBinder.resolve("myArrayPset." + subArrayPath + "[2]", "BINDING").getProperty(),
			).toEqual(childPsets[10]);
			expect(dataBinder.resolve("myArrayPset." + subArrayPath + "[0]", "BINDING")).toEqual(
				actChildDataBinding,
			);
			expect(dataBinder.resolve("myArrayPset." + subArrayPath + "[1]", "BINDING")).toEqual(
		...
		it("should handle remove and push within a scope", function (done) {
			...
			dataBinder.register(
				"BINDING",
				InheritedChildrenTemplate.typeid,
				InheritedChildDataBinding,
				{ context: "all" },
			);
			dataBinder.register("BINDING", ChildTemplate.typeid, ChildDataBinding, {
			...
			expect(containerDataBinding.onPreModify).toHaveBeenCalledTimes(
				containerDataBinding.onModify.mock.calls.length,
			);

			expect(dataBinder._dataBindingCreatedCounter).toEqual(4);
			const childDataBinding2 = dataBinder.resolve(
				containerDataBinding.getProperty().get(["children", "0"]),
				"BINDING",
			);
	...
	describe("sets", function () {
		...
		it("should notify parent when child is added in set", function () {
			...
			var pathPrefixes = ["", "nested."]; // The paths to sets to run this test for
	...
	describe("maps", function () {
		...
		var setupDataBinder = function () {
			...
			parentDataBinding.onModify.mockClear();
			parentDataBinding.onPreModify.mockClear();
			expect(dataBinder.resolve("myMapPset", "BINDING").onPostCreate).toHaveBeenCalledTimes(
				1,
			);
			dataBinder._resetDebugCounters();
		};
		...
		it("should notify parent when child is added in set", function () {
			...
			for (var i = 0; i < pathPrefixes.length; i++) {
				...
				workspace.pushNotificationDelayScope();
				mapPset
					.resolvePath(unrepresentedSubMapPath)
					.insert(unrepresented1.getGuid(), unrepresented1);
				mapPset
					.resolvePath(unrepresentedSubMapPath)
					.insert(unrepresented2.getGuid(), unrepresented2);
				workspace.popNotificationDelayScope();
				expect(dataBinder._dataBindingCreatedCounter).toEqual(0);
		...
		it("should notify appropriate DataBinding of modifications - submap", function () {
			...
			for (var i = 0; i < pathPrefixes.length; i++) {
				...
				mapPset
					.resolvePath(unrepresentedSubMapPath)
					.insert(unrepresentedPset.getGuid(), unrepresentedPset);
				workspace.popNotificationDelayScope();
	...
	describe("nodeProperty", function () {
		...
		it("should be notified of dynamically added/removed child properties", function () {
			...
			appendPath.forEach(function (path) {
				...
				var unrepresentedChild = PropertyFactory.create(
					UnrepresentedTemplate.typeid,
					"single",
					{ text: "hello" },
				);
		...
		it("should be notified of removed child dataBindings. when an intermediate node is removed (from a chain) part 1", function () {
			setupDataBinder();
		...
		it("should be notified of removed child dataBindings. when an intermediate node is removed (from a chain) part 2", function () {
			setupDataBinder();
		...
		it("should create dataBindings that already exist", function () {
			...
			var unrepresented = PropertyFactory.create(UnrepresentedTemplate.typeid, "single", {
				text: "unrepresentedChild",
			});
			var nestedUnrepresented = PropertyFactory.create(
				UnrepresentedTemplate.typeid,
				"single",
			...
			const childDataBinding = dataBinder.resolve(child.getAbsolutePath(), "BINDING");
			const nestedChildDataBinding = dataBinder.resolve(
				nestedChild.getAbsolutePath(),
				"BINDING",
			);
			parentDataBinding = dataBinder.resolve(nodePset.getAbsolutePath(), "BINDING");
			...
			expect(nestedChildDataBinding.onPreModify).toHaveBeenCalledTimes(
				nestedChildDataBinding.onModify.mock.calls.length,
			);
	...
	describe("Inheritance", function () {
		...
		it("should create DataBinding when only the inherited template is represented", function () {
			...
			var inheritedChildPset = PropertyFactory.create(
				InheritedChildTemplate.typeid,
				"single",
			);
			...
			expect(dataBinder._dataBindingCreatedCounter).toEqual(1);
			const childDataBinding = dataBinder.resolve(
				inheritedChildPset.getAbsolutePath(),
				"BINDING",
			);
			expect(childDataBinding).toBeInstanceOf(ChildDataBinding);
		...
		it("should create DataBinding with correct template if the template and its parent are both registered", () => {
			...
			workspace.root.insert(inheritedChildPset);
			expect(dataBinder._dataBindingCreatedCounter).toEqual(2);
			const inheritedChildDataBinding = dataBinder.resolve(
				inheritedChildPset.getAbsolutePath(),
				"BINDING",
			);
		...
		it("should create dataBindings using registered grandparent templates", function () {
			...
			var multipleInheritedPset = PropertyFactory.create(
				MultipleInheritedTemplate.typeid,
				"single",
			);
			...
			expect(dataBinder._dataBindingCreatedCounter).toEqual(1);
			const multipleInheritedDataBinding = dataBinder.resolve(
				multipleInheritedPset.getAbsolutePath(),
				"BINDING",
			);
	...
	describe("options and callbacks", function () {
		...
		it("should be possible to register to a path, detach, and then reattach", function () {
			...
			dataBinder.registerOnPath("myPrimitiveChildTemplate.aString", ["remove"], callbackSpy);
			workspace.root.insert(
				"myPrimitiveChildTemplate",
				PropertyFactory.create(PrimitiveChildrenTemplate.typeid, "single"),
			);
		...
		it("should be possible to register to the same path twice", function () {
			...
			dataBinder.registerOnPath("myPrimitiveChildTemplate.aString", ["insert"], callbackSpy1);
			dataBinder.registerOnPath("myPrimitiveChildTemplate.aString", ["insert"], callbackSpy2);
		...
		it("should be possible to register retroactively on a path and use requestChangesetPostProcessing", function () {
			...
			workspace.root.insert(
				"mypath",
				PropertyFactory.create(PrimitiveChildrenTemplate.typeid),
			);
		...
		it("should be possible to unregister before attaching to a workspace", function () {
			...
			const handle = dataBinder.registerOnPath(
				"myPrimitiveChildTemplate.aString",
				["insert"],
				callbackSpyUnregistered,
			);
		...
		it("should execute deferred callbacks from DataBinding.registerOnPath() inside requestChangesetPostProcessing", function () {
			var callbackCount = 0;
			...
			workspace.root.insert(
				"primitiveChildrenPset",
				PropertyFactory.create(PrimitiveChildrenTemplate.typeid, "single", initialValues),
			);
			expect(dataBinder._dataBindingCreatedCounter).toEqual(1);
			dataBinder._resetDebugCounters();
		...
		it("should execute deferred callbacks from dataBinder.registerOnPath() inside requestChangesetPostProcessing", function () {
			var callbackCount = 0;
			var callbackError = false;
		...
		it("should not create an DataBinding when forbidden by exact path when registration is delayed", function () {
			...
			dataBinder.register("BINDING", ChildTemplate.typeid, ChildDataBinding, {
				exactPath: "myChildTemplate",
				includePrefix: "myChildTemplate",
			}); // includePrefix is ignored
		...
		it("should be able to undefine one binding type, multiple case", function () {
			...
			workspace.root.insert("child", childPset);
			const inheritedChildPset = PropertyFactory.create(
				InheritedChildTemplate.typeid,
				"single",
			);
		...
		it("should not be able to replace the same DataBinding multiple times with different options", function () {
			...
			expect(dataBinder._dataBindingCreatedCounter).toEqual(0);
			workspace.root.insert(
				"myOtherChildTemplate",
				PropertyFactory.create(ChildTemplate.typeid, "single"),
			);
			expect(dataBinder._dataBindingCreatedCounter).toEqual(0);
			workspace.root.insert(
				"yetAnotherChildTemplate",
				PropertyFactory.create(ChildTemplate.typeid, "single"),
			);
			expect(dataBinder._dataBindingCreatedCounter).toEqual(0);
			workspace.root.insert(
		...
		it("removalContext.getDataBinding()", function () {
			...
			class myDerivedDataBinding extends ParentDataBinding {
				constructor(params) {
					...
					this.onRemove = jest.fn(function (in_removalContext) {
						...
						expect(in_removalContext.getDataBinding("DataBindingTest1")).toEqual(
							parentDataBinding,
						);
					});
				}
			...
			workspace.root.insert(parentPset);
			expect(dataBinder._dataBindingCreatedCounter).toEqual(2);
			parentDataBinding = dataBinder.resolve(
				parentPset.getAbsolutePath(),
				"DataBindingTest1",
			);
			const derivedDataBinding = dataBinder.resolve(
				parentPset.getAbsolutePath(),
				"DataBindingTest2",
			);
			...
			class myDerivedDataBinding extends ParentDataBinding {
				constructor(params) {
					...
					this.onModify = jest.fn(function (in_modificationContext) {
						...
						expect(in_modificationContext.getDataBinding("DataBindingTest2")).toEqual(
							this,
						);
						expect(in_modificationContext.getDataBinding("DataBindingTest1")).toEqual(
							parentDataBinding,
						);
					});
				}
		...
		it("can tell if inserts/removes are simulated or real - attach/detach", function () {
			...
			dataBinder.register("DataBinding", PrimitiveChildrenTemplate.typeid, MyDataBinding);
		...
		it("can tell if inserts/removes are simulated or real - destroy handle", function () {
			...
			dataBinder.attachTo(workspace);
			const handle = dataBinder.register(
				"DataBinding",
				PrimitiveChildrenTemplate.typeid,
				MyDataBinding,
			);
		...
		it("should correctly bind to collections", function () {
			...
			var arrayInsertSpy = jest.fn(function (in_index, in_context) {
				...
				expect(in_context.getProperty().getAbsolutePath()).toEqual(
					in_context.getAbsolutePath(),
				);
			});
			...
			var primitiveArrayInsertSpy = jest.fn(function (in_index, in_context) {
				...
				expect(in_context.getAbsolutePath()).toEqual(
					"/root.parentPrimitiveArray[" + in_index + "]",
				);
			});
			...
			dataBinder.register(
				"BINDING",
				"array<" + ParentTemplate.typeid + ">",
				ParentDataBinding,
			);
			dataBinder.register(
				"BINDING",
				"map<" + ParentTemplate.typeid + ">",
				InheritedChildDataBinding,
			);
			...
			workspace.root
				.get("root")
				.insert("parentArray", PropertyFactory.create(ParentTemplate.typeid, "array"));
			expect(dataBinder._dataBindingCreatedCounter).toEqual(1);
			arrayProperty = workspace.root.get(["root", "parentArray"]);
			const parentArrayDataBinding = dataBinder.resolve(
				arrayProperty.getAbsolutePath(),
				"BINDING",
			);
			...
			expect(parentArrayDataBinding.onPreModify).toHaveBeenCalledTimes(
				parentArrayDataBinding.onModify.mock.calls.length,
			);
			...
			workspace.root
				.get("root")
				.insert("parentPrimitiveArray", PropertyFactory.create("Int32", "array"));
			expect(dataBinder._dataBindingCreatedCounter).toEqual(1);
			primitiveArrayProperty = workspace.root.get(["root", "parentPrimitiveArray"]);
			const primitiveArrayDataBinding = dataBinder.resolve(
				primitiveArrayProperty.getAbsolutePath(),
				"BINDING",
			);
			...
			expect(primitiveArrayDataBinding.onPreModify).toHaveBeenCalledTimes(
				primitiveArrayDataBinding.onModify.mock.calls.length,
			);
			primitiveArrayDataBinding.onModify.mockClear();
			primitiveArrayDataBinding.onPreModify.mockClear();
			dataBinder._resetDebugCounters();

			primitiveArrayProperty.insertRange(
				0,
				_.map([0, 1, 2, 3, 4, 5], function (i) {
					return i;
				}),
			...
			workspace.root
				.get("root")
				.insert("parentMap", PropertyFactory.create(ParentTemplate.typeid, "map"));
			expect(dataBinder._dataBindingCreatedCounter).toEqual(1);
			mapProperty = workspace.root.get(["root", "parentMap"]);
			const parentMapDataBinding = dataBinder.resolve(
				mapProperty.getAbsolutePath(),
				"BINDING",
			);
			...
			expect(parentMapDataBinding.onPreModify).toHaveBeenCalledTimes(
				parentMapDataBinding.onModify.mock.calls.length,
			);
		...
		it.skip("should correctly bind to array paths even if they are already created/not yet removed", function () {
			...
			ParentDataBinding.registerOnPath("", ["collectionInsert"], arrayInsertSpy);
			ParentDataBinding.registerOnPath("", ["collectionRemove"], arrayRemoveSpy);
			const arrayHandle = dataBinder.register(
				"BINDING",
				"array<" + ParentTemplate.typeid + ">",
				ParentDataBinding,
			);
		...
		it.skip("should correctly bind to prim array paths even if they are already created/not yet removed", function () {
			...
			ChildDataBinding.registerOnPath("", ["collectionInsert"], primitiveArrayInsertSpy);
			ChildDataBinding.registerOnPath("", ["collectionRemove"], primitiveArrayRemoveSpy);
			const primArrayHandle = dataBinder.register(
				"BINDING",
				"array<Int32>",
				ChildDataBinding,
			);
		...
		it.skip("should correctly bind to map paths even if they are already created/not yet removed", function () {
			...
			const mapHandle = dataBinder.register(
				"BINDING2",
				"map<" + ParentTemplate.typeid + ">",
				DerivedDataBinding,
			);
		...
		it("hasDataBinding", function () {
			...
			expect(
				dataBinder.hasDataBinding("BINDING", "autodesk.vegetables:potato-1.0.0"),
			).toEqual(false);
		});
		...
		it("should correctly pass different userData with different activations", function () {
			...
			dataBinder.activateDataBinding("BINDING", ChildTemplate.typeid, {
				exactPath: "/foo",
				userData: myUserDataFoo,
			});
			// ...and bar with myUserDataBar
			dataBinder.activateDataBinding("BINDING", ChildTemplate.typeid, {
				exactPath: "/bar",
				userData: myUserDataBar,
			});
		...
		it("should honor options arg when activating w/o typeid", function () {
			...
			workspace.root
				.get("foo")
				.insert("child", PropertyFactory.create(ChildTemplate.typeid, "single"));
			expect(dataBinder._dataBindingCreatedCounter).toEqual(1);
			...
			workspace.root
				.get("foo")
				.insert("parent", PropertyFactory.create(ParentTemplate.typeid, "single"));
			expect(dataBinder._dataBindingCreatedCounter).toEqual(2);
		...
		it("excludePrefix should take precedence over includePrefix", function () {
			...
			workspace.root
				.get("foo")
				.insert("baz", PropertyFactory.create(ParentTemplate.typeid, "single"));
			// bindings must be created at /foo, /foo.baz, but not at /foo.bar
			expect(dataBinder._dataBindingCreatedCounter).toEqual(2);
			workspace.root
				.get("foo")
				.insert("bar", PropertyFactory.create(ParentTemplate.typeid, "single"));
			expect(dataBinder._dataBindingCreatedCounter).toEqual(2);
		});
