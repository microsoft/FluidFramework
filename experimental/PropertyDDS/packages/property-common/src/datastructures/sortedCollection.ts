...
export class SortedCollection<T> extends Collection<T> {
    ...
    getNearestNextItem(in_key: string) {
        ...
        return closestNextIndex === this.getCount() ? undefined : this.item(this._sortedKeys[closestNextIndex]);
    }
    ...
    getNearestPreviousItem(in_key: string | number) {
        ...
        return closestPreviousIndex === 0 ? undefined : this.item(this._sortedKeys[closestPreviousIndex - 1]);
    }
}
