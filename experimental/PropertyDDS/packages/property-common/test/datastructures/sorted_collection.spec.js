...
(function() {
  ...
  const SortedCollection = require('../..').Datastructures.SortedCollection;
  ...
  describe('SortedCollection', function() {
    ...
    it('should test against adding values for keys that are of type number or string', function() {
      var collection2 = new SortedCollection();
    ...
    it('should get the nearest next item in the collection', function() {
      var nearestNextItem = collection.getNearestNextItem('0.0.1');
      expect(nearestNextItem).to.equal('1.1.0');

      var nearestNextItem = collection.getNearestNextItem('5.0.0');
      expect(nearestNextItem).to.equal('101.203.345');

      var nearestNextItem = collection.getNearestNextItem('201.203.345');
      expect(nearestNextItem).to.equal(undefined);

      var nearestNextItem = collection.getNearestNextItem('2.0.1');
      expect(nearestNextItem).to.equal('2.1.0');

      var nearestNextItem = collection.getNearestNextItem('2.1.1');
      expect(nearestNextItem).to.equal('3.0.0');

      var nearestNextItem = collection.getNearestNextItem('3.0.0');
      expect(nearestNextItem).to.equal('3.0.0');
    });

    it('should get the nearest previous item in the collection', function() {
      var nearestNextItem = collection.getNearestPreviousItem('0.0.1');
      expect(nearestNextItem).to.equal(undefined);

      var nearestNextItem = collection.getNearestPreviousItem('5.0.0');
      expect(nearestNextItem).to.equal('4.0.0');

      var nearestNextItem = collection.getNearestPreviousItem('201.203.345');
      expect(nearestNextItem).to.equal('101.203.345');

      var nearestNextItem = collection.getNearestPreviousItem('2.0.1');
      expect(nearestNextItem).to.equal('2.0.0');

      var nearestNextItem = collection.getNearestPreviousItem('2.1.1');
      expect(nearestNextItem).to.equal('2.1.0');
    ...
    it('should clone the sorted collection', function() {
      ...
      expect(clone instanceof SortedCollection).to.be.true;
      expect(clone._sortedKeys).to.eql(['1.1.0', '1.2.1', '1.10.5', '2.0.0', '2.1.0', '3.0.0', '4.0.0', '101.203.345']);
    });
  });
})();
