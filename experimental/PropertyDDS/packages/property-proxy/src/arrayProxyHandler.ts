...
export const arrayProxyHandler: ProxyHandler<ComponentArray> = {
	...
	get(target: ComponentArray, key: string, receiver) {
		if (typeof target[key] === "function") {
			...
			if (
				typeof key !== "symbol" &&
				Number(key) >= 0 &&
				Number(key) < target.getProperty().getLength()
			) {
				const property = target.getProperty();
				const isReferenceArray = PropertyFactory.instanceOf(property, "Reference", "array");
				if (
					isReferenceArray &&
					(asteriskFound || getTrapSpecialCases.includes(target.lastCalledMethod))
				) {
	...
	getOwnPropertyDescriptor(target: ComponentArray, key: string | typeof proxySymbol) {
		if (key !== "length") {
			...
			return {
				configurable: false,
				enumerable: false,
				value: target.getProperty().getLength(),
				writable: true,
			};
		}
	},
		...
		switch (typeof key) {
			case "string": {
				...
				return (
					...
					numeric >= 0 &&
					numeric < target.getProperty().getLength()
				);
			}
			default:
				return key === proxySymbol;
	...
	set(target: ComponentArray, key: string | symbol, value: any) {
		...
		if (isNaN(numeric_key)) {
			...
			if (
				numeric_key < 0 ||
				!Number.isInteger(numeric_key) ||
				String(numeric_key) !== processed_key
			) {
				throw new Error(PropertyProxyErrors.INVALID_PROPERTY);
			}
			...
			if (
				isReferenceArray &&
				forceType<ReferenceArrayProperty>(property) &&
				!specialCases &&
				!asteriskFound &&
				!insert
			) {
				...
				if (
					...
					property.get(numeric_key)!.getContext() === "single"
				) {
