...
export namespace Utilities {
	 ...
	 */
	export function wrapWithPushPopNotificationDelayScope(
	...
	export function prepareElementForInsertion(
		property: BaseProperty,
		element: BaseProperty | any,
		caller?: string,
	): any {
		// Check if element exists and is a proxied property
		if (
			element &&
			typeof element.getProperty === "function" &&
		...
		if (PropertyFactory.instanceOf(element, "BaseProperty")) {
			if (
				...
				if (
					element.getParent() &&
					property.getContext() === "array" &&
					forceType<ArrayProperty>(property)
				) {
			...
			if (
				...
				typeof element[Symbol.iterator] === "function"
			) {
		...
		if (context === "single") {
			...
			if (PropertyFactory.instanceOf(value, "BaseProperty")) {
				if (
					...
					forceType<ReferenceProperty>(property)
				) {
					property.set(value);
				} else {
	...
	export function setValueOfReferencedProperty(
		...
		const proxiedReferencedPropertyParent = PropertyProxy.proxify(
			referencedPropertyParent as NonPrimitiveTypes,
		);
	 ...
	 */
	export const containsAsterisk = (key: any) =>
	 ...
	 */
	export function proxifyInternal(
		property: ContainerProperty | ReferenceType,
		key: string | number,
		caretFound: boolean,
		isReferenceCollection: boolean = false,
	) {
		const context = property.getContext();
		// TODO(marcus): this cast is a workaround for resolving the type check
		...
		if (PropertyFactory.instanceOf(propertyAtKey, "BaseProperty")) {
			if (caretFound && propertyAtKey.isPrimitiveType()) {
				if (
					PropertyFactory.instanceOf(propertyAtKey, "Enum") &&
					forceType<EnumProperty>(propertyAtKey)
				) {
					return propertyAtKey.getEnumString();
				} else if (
					PropertyFactory.instanceOf(propertyAtKey, "Uint64") ||
			...
			if (caretFound) {
				...
				if (!contextIsSingle && isReferenceCollection) {
					...
					const data = PropertyProxy.getParentOfReferencedProperty(
						property as ReferenceMapProperty | ReferenceArrayProperty,
						key,
					);
				...
				if (typeid === "Uint64") {
					...
					forceType<EnumArrayProperty>(other_property) &&
					forceType<number>(key)
				) {
