...
describe("JS-Object-like property accessing ", function () {
	...
	function prepareRootNode() {
		...
		testProperty = PropertyFactory.create(
			"autodesk.appframework.tests:myGenericTemplate-1.0.0",
		);
		...
		rootNode
			.get("myGenericArray")
			.push(PropertyFactory.create("Int32", "array", [0, 1, 2, 3]));
		rootNode
			.get("myGenericArray")
			.push(PropertyFactory.create("Int32", "map", { a: 0, b: 1, c: 2 }));
		rootNode.get("myGenericArray").push(PropertyFactory.create("NamedProperty", "set"));
		rootNode
			.get("myGenericArray")
			.get(2)
			.set(PropertyFactory.create("NamedProperty", "single"));
		rootNode
			.get("myGenericArray")
			.get(2)
			.set(PropertyFactory.create("NamedProperty", "single"));
		...
		rootNode
			.get("myGenericMap")
			.insert("array", PropertyFactory.create("Int32", "array", [0, 1, 2, 3]));
		rootNode
			.get("myGenericMap")
			.insert("map", PropertyFactory.create("Int32", "map", { a: 0, b: 1, c: 2 }));
	...
	describe("Proxy properties directly", function () {
		it("should throw if something other than a property is supplied", function () {
			...
			expect(() => {
				PropertyProxy.proxify(someNonProperty as any);
			}).toThrow("PropertyProxy-000");
		});
		...
		it("should be able to proxy Array/Map/SetProperties directly", function () {
			const proxiedI32Array = PropertyProxy.proxify(
				rootNode.resolvePath("myTestProperty.myI32Array") as ArrayProperty,
			);
			expect(proxiedI32Array.length).toEqual(5);

			const proxiedI32Map = PropertyProxy.proxify(
				rootNode.resolvePath("myTestProperty.myMap") as MapProperty,
			);
			expect(proxiedI32Map.size).toEqual(3);

			const proxiedSet = PropertyProxy.proxify(
				rootNode.resolvePath("myTestProperty.myBookSet") as SetProperty,
			);
	...
	describe("The following work as JS object: ", function () {
		...
		it("should be able to obtain the proxied property and its direct children via getProperty()", function () {
			...
			expect(() => {
				state.getProperty(["myTestProperty", "myVector"]);
			}).toThrow("PropertyProxy-010");
		});
		...
		it("should be possible to use the `in` operator", function () {
			...
			expect(
				rootNode.resolvePath("myTestProperty.myI32Array").getLength() in
					state.myTestProperty.myI32Array,
			).toEqual(false);
		});

		describe("NodeProperty", function () {
			it("should be able to insert primitive and non-primitive properties", function () {
				state.myFirstPrimitivePropertyInsertedViaProxy = PropertyFactory.create(
					"Int32",
					"single",
					42,
				);
				expect(rootNode.get("myFirstPrimitivePropertyInsertedViaProxy").getValue()).toEqual(
					42,
				);
				...
				expect(
					rootNode.get("myFirstNonPrimitivePropertyInsertedViaProxy").get("x").getValue(),
				).toEqual(1);
				expect(
					rootNode.get("myFirstNonPrimitivePropertyInsertedViaProxy").get("y").getValue(),
				).toEqual(2);
				...
				expect(() => {
					state.shouldNotWork = 1;
				}).toThrow();
				expect(() => {
					state.shouldNotWork2 = { a: 1 };
				}).toThrow();
				expect(() => {
					state.shouldNotWork3 = undefined;
				}).toThrow();

				// Trying to set on non-dynamic property should not work
				expect(() => {
					state.myBook.year = PropertyFactory.create("Int32", "single", 1977);
				}).toThrow("PropertyProxy-001");
			});
			...
			it("should be able to delete primitive and non-primitive properties", function () {
				...
				expect(() => {
					delete state.myTestProperty.myVector;
				}).toThrow("PropertyProxy-006");
			});
		});
		...
		describe("ReferenceProperties", function () {
			...
			describe("single", function () {
				it("should access the referenced property", function () {
					...
					rootNode
						.resolvePath("myTestProperty.myReference*")
						.setValue("/myTestProperty.myI32Array[0]");
					expect(state.myTestProperty.myReference).toEqual(0);
					...
					rootNode
						.resolvePath("myTestProperty.myReference*")
						.setValue("/myTestProperty.myComplexArray[0]");
					expect(state.myTestProperty.myReference.getProperty()).toEqual(
						state.myTestProperty.myComplexArray[0].getProperty(),
					);
				});
				...
				it("should be able to change the referenced property", function () {
					...
					state.myTestProperty.myReference = { x: 7, y: 8 };
					expect(
						rootNode.resolvePath("myTestProperty.myComplexArray[0].x").getValue(),
					).toEqual(7);
					expect(
						rootNode.resolvePath("myTestProperty.myComplexArray[0].y").getValue(),
					).toEqual(8);
				...
				it("should be able to assign another path/property to reference another property", function () {
					...
					expect(state.myTestProperty.getProperty("myReference")).toEqual(
						state.myTestProperty.getProperty("myF32Number"),
					);

					// Complicated Relative Path
					state.myTestProperty["myReference*"] = "../myBook";
					expect(state.myTestProperty.getProperty("myReference")).toEqual(
						state.getProperty("myBook"),
					);
				...
				it("should throw if setting via * is used on a non reference property", function () {
					expect(() => {
						state.myTestProperty["myF32Number*"] = "something";
				...
				it("should throw if not in in the same tree and referenced via absolute path", function () {
					...
					prop.insert(
						"ref",
						PropertyFactory.create("Reference", "single", "/myTestProperty.myF32Number"),
					);
					const proxiedProp = PropertyProxy.proxify(prop);
					expect(() => {
						proxiedProp.ref = 5;
					}).toThrow("PropertyProxy-009");
			...
			describe.skip("RepositoryReference", function () {
				it("should return property if accessed via * syntax", function () {
					...
					expect(state.repoRefArray["0*"].getProperty()).toEqual(
						rootNode.resolvePath("repoRefArray[0]*"),
					);
					expect(state.repoRefMap.get("a*").getProperty()).toEqual(
						rootNode.resolvePath("repoRefMap[a]*"),
					);
				});
			...
			describe("array", function () {
				...
				beforeAll(function () {
					...
					for (let i = 0; i < refArray.getLength(); ++i) {
						...
						if (PropertyFactory.instanceOf(entry, "BaseProperty")) {
							refArraySum +=
								PropertyFactory.instanceOf(entry, "ContainerProperty") && entry.has("x")
				...
				it("should be able to access stored reference path strings via *", function () {
					for (let i = 0; i < refArray.getLength(); ++i) {
						expect(state.myTestProperty.myReferenceArray[`${i}*`]).toEqual(
							refArray.getValue(i),
						);
					}
				});
				...
				it("check .concat() functionality", function () {
					const concat = state.myTestProperty.myReferenceArray.concat(
						state.myTestProperty.myI32Array,
					);
					...
					const synthToString = `${refArrayEntriesToString.join(",")},${rootNode
						.resolvePath("myTestProperty.myI32Array")
						.getEntriesReadOnly()
						.toString()}`;
				...
				it("check .every() functionality", function () {
					expect(
						state.myTestProperty.myReferenceArray.every((element) => {
				...
				it("check .filter() functionality", function () {
					const filtered = state.myTestProperty.myReferenceArray.filter(
						(element) => element === 1111,
					);
				...
				it("check .find() functionality", function () {
					expect(
						state.myTestProperty.myReferenceArray.find((element) => element === 3),
					).toEqual(3);
				});

				it("check .findIndex() functionality", function () {
					expect(
						state.myTestProperty.myReferenceArray.findIndex((element) => element === 3),
					).toEqual(0);
				});
				...
				it("check .foreach() functionality", function () {
					...
					state.myTestProperty.myReferenceArray.forEach((el) => {
						...
						if (el.getProperty && el.getProperty().getContext() === "array") {
							el.forEach((anotherEl) => {
								referenceArraySum += anotherEl;
							});
						}
					});
				...
				it("check .includes() functionality", function () {
					...
					expect(
						state.myTestProperty.myReferenceArray.includes(
							rootNode.resolvePath("myTestProperty.myVector"),
						),
					).toEqual(true);
					expect(state.myTestProperty.myReferenceArray.includes({ x: 1, y: 2 })).toEqual(
						false,
					);
				});
				...
				it("check .reduce() functionality", function () {
					expect(
						state.myTestProperty.myReferenceArray.reduce((accumulator, currentValue) => {
				...
				it("check .reduceRight() functionality", function () {
					expect(
						state.myTestProperty.myReferenceArray.reduceRight(
				...
				it("check .some() functionality", function () {
					expect(
						state.myTestProperty.myReferenceArray.some((element) => element === 3),
					).toEqual(true);
					expect(
						state.myTestProperty.myReferenceArray.some((element) => element === 4),
					).toEqual(false);
				});
				...
				describe("Setting", function () {
					const reset = () => {
						...
						rootNode
							.resolvePath("myTestProperty.myComplexMap[firstEntry]")
							.setValues({ x: 10, y: 20 });
					...
					it("should be able to change the referenced properties", function () {
						...
						expect(
							rootNode.resolvePath("myTestProperty.myComplexMap[firstEntry].x").getValue(),
						).toEqual(3);
						expect(
							rootNode.resolvePath("myTestProperty.myComplexMap[firstEntry].y").getValue(),
						).toEqual(4);
					...
					it("should be able to assign another path/property to reference another property", function () {
						...
						// Property
						state.myTestProperty.myReferenceArray["0*"] = rootNode.resolvePath(
							"myTestProperty.myF32Number",
						);
						expect(rootNode.resolvePath("myTestProperty.myReferenceArray[0]")).toEqual(
					...
					it("check .pop() functionality", function () {
						...
						expect(popped.getProperty()).toEqual(
							rootNode.resolvePath("myTestProperty.myComplexMap[firstEntry]"),
						);
					});

					it("check .push() functionality", function () {
						rootNode.resolvePath("myTestProperty.myReferenceArray").clear();
						state.myTestProperty.myReferenceArray.push(
							"myVector",
							rootNode.resolvePath("myTestProperty.myF32Number"),
							"/myBook",
						);
					...
					it("check .reverse() functionality", function () {
						...
						expect(
							state.myTestProperty.myReferenceArray.getProperty().getValues().toString(),
						).toEqual(values.reverse().toString());
					});

					it("check .sort() functionality", function () {
						// Primitive
						rootNode
							.resolvePath("myTestProperty.myReferenceArray")
							.setValues([
								"myI32Array[0]",
								"myI32Array[1]",
								"myI32Array[2]",
						...
						for (let i = 0; i < state.myTestProperty.myReferenceArray.length; ++i) {
							expect(rootNode.resolvePath("myTestProperty.myReferenceArray").get(i)).toEqual(
								rootNode
									.resolvePath("myTestProperty.myI32Array")
									.get(state.myTestProperty.myReferenceArray.length - 1 - i),
							);
						}
						...
						rootNode
							.resolvePath("myTestProperty.myReferenceArray")
							.setValues(["myComplexArray[0]", "myComplexArray[1]"]);
						...
						for (let i = 0; i < state.myTestProperty.myReferenceArray.length; ++i) {
							expect(
								...
								rootNode
									.resolvePath("myTestProperty.myComplexArray")
									.get(state.myTestProperty.myReferenceArray.length - 1 - i)
									.get("x")
									.getValue(),
							);
						...
						rootNode
							.resolvePath("myTestProperty.myReferenceArray")
							.setValues(["myComplexArray[1]", "myMultiHopReference", "myI32Array[0]"]);
						...
						expect(
							rootNode.resolvePath("myTestProperty.myReferenceArray").getValue(0),
						).toEqual("myI32Array[0]");
						expect(
							rootNode.resolvePath("myTestProperty.myReferenceArray").getValue(1),
						).toEqual("myMultiHopReference");
						expect(
							rootNode.resolvePath("myTestProperty.myReferenceArray").getValue(2),
						).toEqual("myComplexArray[1]");
					});

					it("check .splice() functionality", function () {
						const removed = state.myTestProperty.myReferenceArray.splice(
							0,
							6,
							"myI32Array[0]",
							"myI32Array[1]",
						...
						expect(removed[0]).toEqual(
							rootNode.resolvePath("myTestProperty.myF32Number").getValue(),
						);
						expect(removed[1]).toEqual(
							rootNode.resolvePath("myTestProperty.myF32Number").getValue(),
						);
						expect(removed[2]).toEqual(
							rootNode.resolvePath("myTestProperty.myF32Number").getValue(),
						);

						expect(removed[3].getProperty()).toEqual(
							rootNode.resolvePath("myTestProperty.myVector"),
						);
						expect(removed[4].getProperty()).toEqual(
							rootNode.resolvePath("myTestProperty.myVector"),
						);
						expect(removed[5].getProperty()).toEqual(
							rootNode.resolvePath("myTestProperty.myVector"),
						);

						expect(
							rootNode.resolvePath("myTestProperty.myReferenceArray").getLength(),
						).toEqual(32);
					...
					it("check .unshift() functionality", function () {
						rootNode.resolvePath("myTestProperty.myReferenceArray").clear();
						state.myTestProperty.myReferenceArray.unshift(
							"myVector",
							rootNode.resolvePath("myTestProperty.myF32Number"),
							"/myBook",
							"../myTestProperty",
					...
					it(
						"should throw if trying to set a property referenced via an absolute path " +
							"if the ReferenceArray is not yet in the property tree",
						function () {
							// Property not in property tree
							const tempRefArray = PropertyProxy.proxify(
								PropertyFactory.create("Reference", "array", ["/myTestProperty.myF32Number"]),
							);
							expect(() => {
								tempRefArray[0] = 100;
							}).toThrow();
					...
					it("should throw if trying to set invalid references", function () {
						...
						rootNode
							.resolvePath("myTestProperty.myReferenceArray")
							.insert(0, "relativeInvalid");
						rootNode
							.resolvePath("myTestProperty.myReferenceArray")
							.insert(1, "/absoluteInvalid");
		...
		describe("Float32", function () {
			...
			it("Setting Float32 number", function () {
				...
				expect(() => {
					state.myTestProperty.myF32Number = [1, 2, 3];
		...
		describe("Enum/EnumArray", function () {
			...
			it("setting", function () {
				...
				expect(rootNode.resolvePath("myTestProperty.myEnumCases.myEnum").getValue()).toEqual(
					2,
				);
				expect(
					rootNode.resolvePath("myTestProperty.myEnumCases.myEnum").getEnumString(),
				).toEqual("dos");
				...
				expect(() => {
					state.myTestProperty.myEnumCases.myEnum = "notAValidEnumString";
				}).toThrow();
				expect(() => {
					state.myTestProperty.myEnumCases.myEnum = "100";
				}).toThrow();

				state.myTestProperty.myEnumCases.myEnumArray = ["dos", 1];
				expect(rootNode.resolvePath("myTestProperty.myEnumCases.myEnumArray").get(0)).toEqual(
					2,
				);
				expect(rootNode.resolvePath("myTestProperty.myEnumCases.myEnumArray").get(1)).toEqual(
					1,
				);
		...
		describe("(U)int64/(U)int64Array/(U)int64Map", function () {
			it("accessing", function () {
				const uint64Value = rootNode
					.resolvePath("myTestProperty.myUint64Int64Cases.myUint64")
					.getValue();
				const valueOfUint64ArrayAtZero = rootNode.resolvePath(
					"myTestProperty.myUint64Int64Cases.myUint64Array[0]",
				);
				const valueOfInt64MapAtA = rootNode.resolvePath(
					"myTestProperty.myUint64Int64Cases.myInt64Map[a]",
				);
				...
				expect(state.myTestProperty.myUint64Int64Cases.myUint64Array[0]).toEqual(
					valueOfUint64ArrayAtZero,
				);
				expect(state.myTestProperty.myUint64Int64Cases.myUint64Array["0^"]).toEqual(stringVal);
				expect(state.myTestProperty.myUint64Int64Cases.myInt64Map.get("a")).toEqual(
					valueOfInt64MapAtA,
				);
				expect(state.myTestProperty.myUint64Int64Cases.myInt64Map.get("a^")).toEqual(
					stringVal,
				...
				expect(state.myTestProperty.myUint64Int64Cases.refToUint64ArrayEntry).toEqual(
					valueOfUint64ArrayAtZero,
				);
				expect(state.myTestProperty.myUint64Int64Cases["refToUint64ArrayEntry^"]).toEqual(
					stringVal,
				);
				expect(state.myTestProperty.myUint64Int64Cases.refToInt64MapEntry).toEqual(
					valueOfInt64MapAtA,
				);
				expect(state.myTestProperty.myUint64Int64Cases["refToInt64MapEntry^"]).toEqual(
					stringVal,
				...
				// Ref Array
				// ref to Uint64
				expect(state.myTestProperty.myUint64Int64Cases.refArrayToUint64Int64[0]).toEqual(
					uint64Value,
				);
				expect(state.myTestProperty.myUint64Int64Cases.refArrayToUint64Int64["0^"]).toEqual(
					stringVal,
				);
				expect(state.myTestProperty.myUint64Int64Cases.refArrayToUint64Int64[1]).toEqual(
					uint64Value,
				);
				expect(state.myTestProperty.myUint64Int64Cases.refArrayToUint64Int64["1^"]).toEqual(
					stringVal,
				...
				expect(state.myTestProperty.myUint64Int64Cases.refArrayToUint64Int64[2]).toEqual(
					valueOfUint64ArrayAtZero,
				);
				expect(state.myTestProperty.myUint64Int64Cases.refArrayToUint64Int64["2^"]).toEqual(
					stringVal,
				);
				expect(state.myTestProperty.myUint64Int64Cases.refArrayToUint64Int64[3]).toEqual(
					valueOfUint64ArrayAtZero,
				);
				expect(state.myTestProperty.myUint64Int64Cases.refArrayToUint64Int64["3^"]).toEqual(
					stringVal,
				...
				expect(state.myTestProperty.myUint64Int64Cases.refArrayToUint64Int64[4]).toEqual(
					valueOfInt64MapAtA,
				);
				expect(state.myTestProperty.myUint64Int64Cases.refArrayToUint64Int64["4^"]).toEqual(
					stringVal,
				);
				expect(state.myTestProperty.myUint64Int64Cases.refArrayToUint64Int64[5]).toEqual(
					valueOfInt64MapAtA,
				);
				expect(state.myTestProperty.myUint64Int64Cases.refArrayToUint64Int64["5^"]).toEqual(
					stringVal,
				...
				expect(state.myTestProperty.myUint64Int64Cases.refArrayToUint64Int64[6]).toEqual(
					uint64Value,
				);
				expect(state.myTestProperty.myUint64Int64Cases.refArrayToUint64Int64["6^"]).toEqual(
					stringVal,
				);
				expect(state.myTestProperty.myUint64Int64Cases.refArrayToUint64Int64[7]).toEqual(
					uint64Value,
				);
				expect(state.myTestProperty.myUint64Int64Cases.refArrayToUint64Int64["7^"]).toEqual(
					stringVal,
				...
				expect(state.myTestProperty.myUint64Int64Cases.refArrayToUint64Int64[8]).toEqual(
					valueOfUint64ArrayAtZero,
				);
				expect(state.myTestProperty.myUint64Int64Cases.refArrayToUint64Int64["8^"]).toEqual(
					stringVal,
				);
				expect(state.myTestProperty.myUint64Int64Cases.refArrayToUint64Int64[9]).toEqual(
					valueOfUint64ArrayAtZero,
				);
				expect(state.myTestProperty.myUint64Int64Cases.refArrayToUint64Int64["9^"]).toEqual(
					stringVal,
				...
				expect(state.myTestProperty.myUint64Int64Cases.refArrayToUint64Int64[10]).toEqual(
					valueOfInt64MapAtA,
				);
				expect(state.myTestProperty.myUint64Int64Cases.refArrayToUint64Int64["10^"]).toEqual(
					stringVal,
				);
				expect(state.myTestProperty.myUint64Int64Cases.refArrayToUint64Int64[11]).toEqual(
					valueOfInt64MapAtA,
				);
				expect(state.myTestProperty.myUint64Int64Cases.refArrayToUint64Int64["11^"]).toEqual(
					stringVal,
				...
				// Ref Map
				// ref to Uint64
				expect(state.myTestProperty.myUint64Int64Cases.refMapToUint64Int64.get("a")).toEqual(
					uint64Value,
				);
				expect(state.myTestProperty.myUint64Int64Cases.refMapToUint64Int64.get("a^")).toEqual(
					stringVal,
				);
				expect(state.myTestProperty.myUint64Int64Cases.refMapToUint64Int64.get("b")).toEqual(
					uint64Value,
				);
				expect(state.myTestProperty.myUint64Int64Cases.refMapToUint64Int64.get("b^")).toEqual(
					stringVal,
				...
				// ref to entry of Uint64Array
				expect(state.myTestProperty.myUint64Int64Cases.refMapToUint64Int64.get("c")).toEqual(
					valueOfUint64ArrayAtZero,
				);
				expect(state.myTestProperty.myUint64Int64Cases.refMapToUint64Int64.get("c^")).toEqual(
					stringVal,
				);
				expect(state.myTestProperty.myUint64Int64Cases.refMapToUint64Int64.get("d")).toEqual(
					valueOfUint64ArrayAtZero,
				);
				expect(state.myTestProperty.myUint64Int64Cases.refMapToUint64Int64.get("d^")).toEqual(
					stringVal,
				...
				// ref to entry of Int64Map
				expect(state.myTestProperty.myUint64Int64Cases.refMapToUint64Int64.get("e")).toEqual(
					valueOfInt64MapAtA,
				);
				expect(state.myTestProperty.myUint64Int64Cases.refMapToUint64Int64.get("e^")).toEqual(
					stringVal,
				);
				expect(state.myTestProperty.myUint64Int64Cases.refMapToUint64Int64.get("f")).toEqual(
					valueOfInt64MapAtA,
				);
				expect(state.myTestProperty.myUint64Int64Cases.refMapToUint64Int64.get("f^")).toEqual(
					stringVal,
				...
				// ref to ref to Uint64
				expect(state.myTestProperty.myUint64Int64Cases.refMapToUint64Int64.get("g")).toEqual(
					uint64Value,
				);
				expect(state.myTestProperty.myUint64Int64Cases.refMapToUint64Int64.get("g^")).toEqual(
					stringVal,
				);
				expect(state.myTestProperty.myUint64Int64Cases.refMapToUint64Int64.get("h")).toEqual(
					uint64Value,
				);
				expect(state.myTestProperty.myUint64Int64Cases.refMapToUint64Int64.get("h^")).toEqual(
					stringVal,
				...
				// ref to ref to entry of Uint64Array
				expect(state.myTestProperty.myUint64Int64Cases.refMapToUint64Int64.get("i")).toEqual(
					valueOfUint64ArrayAtZero,
				);
				expect(state.myTestProperty.myUint64Int64Cases.refMapToUint64Int64.get("i^")).toEqual(
					stringVal,
				);
				expect(state.myTestProperty.myUint64Int64Cases.refMapToUint64Int64.get("j")).toEqual(
					valueOfUint64ArrayAtZero,
				);
				expect(state.myTestProperty.myUint64Int64Cases.refMapToUint64Int64.get("j^")).toEqual(
					stringVal,
				...
				// ref to ref to entry of Int64Map
				expect(state.myTestProperty.myUint64Int64Cases.refMapToUint64Int64.get("k")).toEqual(
					valueOfInt64MapAtA,
				);
				expect(state.myTestProperty.myUint64Int64Cases.refMapToUint64Int64.get("k^")).toEqual(
					stringVal,
				);
				expect(state.myTestProperty.myUint64Int64Cases.refMapToUint64Int64.get("l")).toEqual(
					valueOfInt64MapAtA,
				);
				expect(state.myTestProperty.myUint64Int64Cases.refMapToUint64Int64.get("l^")).toEqual(
					stringVal,
		...
		describe("Simple non primitive type (Vector)", function () {
			...
			it("Setting", function () {
				...
				// Property
				state.myTestProperty.myVector = PropertyFactory.create(
					vector2DTemplate.typeid,
					"single",
					{ x: 5, y: 6 },
				);
				...
				expect(() => {
					state.myTestProperty.myF32Number = [{ x: 1, y: 2 }];
				}).toThrow("PropertyProxy-007");
			});
		});
		...
		describe("Array (with primitive type entries)", function () {
			describe("Int32 array", function () {
				...
				it("check .concat() functionality", function () {
					const concat = state.myTestProperty.myI32Array.concat(
						["a", "b", "c"],
						state.myTestProperty.myI32Array,
					);
				...
				it("check .every() functionality", function () {
					expect(state.myTestProperty.myI32Array.every((element) => element < 50)).toEqual(
						true,
					);
					expect(state.myTestProperty.myI32Array.every((element) => element < 20)).toEqual(
						false,
					);
				});
				...
				it("check .find() functionality", function () {
					expect(
						state.myTestProperty.myI32Array.find((element) => element < 15 && element > 5),
					).toEqual(10);
				});

				it("check .findIndex() functionality", function () {
					expect(
						state.myTestProperty.myI32Array.findIndex(
							(element) => element < 15 && element > 5,
						),
					).toEqual(1);
				});
				...
				it("check .reduce() functionality", function () {
					expect(
						state.myTestProperty.myI32Array.reduce((accumulator, currentValue) => {
				...
				it("check .reduceRight() functionality", function () {
					expect(
						state.myTestProperty.myI32Array.reduceRight((previousValue, currentValue) => {
				...
				it("check .some() functionality", function () {
					expect(state.myTestProperty.myI32Array.some((element) => element > 10)).toEqual(
						true,
					);
					expect(state.myTestProperty.myI32Array.some((element) => element > 50)).toEqual(
						false,
					);
				});
			...
			describe("Setting", function () {
				...
				it("check .copyWithin() functionality", function () {
					...
					expect(
						rootNode.get("myTestProperty").get("myI32Array").getValues().toString(),
					).toEqual("30,10,20,30,40");
				});

				it("check .fill() functionality", function () {
					state.myTestProperty.myI32Array.fill(0);
					for (let i = 0; i < state.myTestProperty.myI32Array.length; i++) {
						expect(rootNode.get("myTestProperty").get("myI32Array").get(i.toString())).toEqual(
							0,
						);
					}
				...
				it("check .push() functionality", function () {
					...
					expect(() => state.myTestProperty.myI32Array.push([1, 2, 3])).toThrow(
						"PropertyProxy-002",
					);
				});
				...
				it("check .reverse() functionality", function () {
					...
					expect(
						rootNode.resolvePath("myTestProperty.myI32Array").getValues().toString(),
					).toEqual("40,30,20,10,0");
				});
				...
				it("check .shift() functionality", function () {
					...
					expect(rootNode.resolvePath("myTestProperty.myI32Array").getLength()).toEqual(
						oldLength - 1,
					);
				...
				it("check .splice() functionality", function () {
					...
					state.myTestProperty.myI32Array.splice(
						5,
						0,
					...
					expect(() => state.myTestProperty.myI32Array.splice(1, 0, [1, 2, 3])).toThrow(
						"PropertyProxy-002",
					);
				});
				...
				it("check .unshift() functionality", function () {
					...
					expect(
						rootNode.resolvePath("myTestProperty.myI32Array").getValues().toString(),
					).toEqual("-20,-30,-10,0,10,20,30,40");

					expect(() => state.myTestProperty.myI32Array.unshift([1, 2, 3])).toThrow(
						"PropertyProxy-002",
					);
				});
				...
				it("should update proxy from remote changes", function () {
					...
					expect(state.myTestProperty.myI32Array[0]).toEqual(
						rootNode.resolvePath("myTestProperty.myI32Array[0]"),
					);
		...
		describe("Array with complex type entries", function () {
			...
			it("check .every() functionality", function () {
				expect(
					state.myTestProperty.myComplexArray.every((element) => element.x < 100),
				).toEqual(true);
				expect(state.myTestProperty.myComplexArray.every((element) => element.y < 20)).toEqual(
					false,
				);
			});

			it("check .filter() functionality", function () {
				const filtered = state.myTestProperty.myComplexArray.filter(
					(element) => element.x < 10,
				);
			...
			it("check .find() functionality", function () {
				expect(
					state.myTestProperty.myComplexArray.find(
						(element) => element.x < 15 && element.x > 5,
					).x,
				).toEqual(10);
			});
			...
			it("check .findIndex() functionality", function () {
				expect(
					state.myTestProperty.myComplexArray.findIndex(
						(element) => element.x < 15 && element.x > 5,
					),
				).toEqual(1);
			});
			...
			it("check .includes() functionality", function () {
				expect(
					state.myTestProperty.myComplexArray.includes(state.myTestProperty.myComplexArray[0]),
				).toEqual(arrayWithJsOutfit.includes(arrayWithJsOutfit[0]));
				expect(
					state.myTestProperty.myComplexArray.includes(
						rootNode.resolvePath("myTestProperty.myComplexArray[0]"),
					),
				).toEqual(true);
				expect(state.myTestProperty.myComplexArray.includes({ x: 1, y: 2 })).toEqual(
					arrayWithJsOutfit.includes({ x: 1, y: 2 }),
				);
				expect(
					state.myTestProperty.myComplexArray.includes(
				...
				).toEqual(arrayWithJsOutfit.includes(arrayWithJsOutfit[0], 1));
				expect(
					state.myTestProperty.myComplexArray.includes(
						state.myTestProperty.myComplexArray[0],
						-1,
				...
				).toEqual(arrayWithJsOutfit.includes(arrayWithJsOutfit[1], 2));
				expect(
					state.myTestProperty.myComplexArray.includes(
						state.myTestProperty.myComplexArray[1],
						100,
					),
				).toEqual(arrayWithJsOutfit.includes(arrayWithJsOutfit[1], 100));
			});

			it("check .join() functionality", function () {
				expect(state.myTestProperty.myComplexArray.join(" ")).toEqual(
					arrayWithJsOutfit.join(" "),
				);
			});
			...
			it("check .map() functionality", function () {
				expect(
					state.myTestProperty.myComplexArray.map((element) => element.x * 2).toString(),
				).toEqual("2,20");
			});

			it("check .reduce() functionality", function () {
				expect(
					state.myTestProperty.myComplexArray.reduce((accumulator, currentValue) => {
			...
			it("check .reduceRight() functionality", function () {
				expect(
					state.myTestProperty.myComplexArray.reduceRight((previousValue, currentValue) => {
			...
			it("check .some() functionality", function () {
				expect(state.myTestProperty.myComplexArray.some((element) => element.x > 1)).toEqual(
					true,
				);
				expect(state.myTestProperty.myComplexArray.some((element) => element.x > 50)).toEqual(
					false,
				);
			});
			...
			describe("Setting", function () {
					...
					rootNode
						...
						.insertRange(0, [
							PropertyFactory.create(
								"autodesk.appframework.tests:myVector2D-1.0.0",
								"single",
								{ x: 1, y: 2 },
							),
							PropertyFactory.create(
				...
				it("should set via direct access", function () {
					...
					expect(
						rootNode.get("myTestProperty").get("myComplexArray").get("0").get("x").getValue(),
					).toEqual(30);
					expect(
						rootNode.get("myTestProperty").get("myComplexArray").get("0").get("y").getValue(),
					).toEqual(40);
					...
					expect(
						rootNode.get("myTestProperty").get("myComplexArray").get("0").get("z").getValue(),
					).toEqual(1);
				});
				...
				it("should set an element out of range", function () {
					...
					expect(
						rootNode.get("myTestProperty").get("myComplexArray").get(10).get("x").getValue(),
					).toEqual(100);
					expect(
						rootNode.get("myTestProperty").get("myComplexArray").get(10).get("y").getValue(),
					).toEqual(100);
				});

				it("check .concat() functionality", function () {
					const concat = state.myTestProperty.myComplexArray.concat(
						["a", "b", "c"],
				...
				it("check .copyWithin() functionality", function () {
					...
					rootNode.resolvePath("myTestProperty.myComplexArray").push(
						PropertyFactory.create(vector3DTemplate.typeid, "single", {
							x: 100,
							y: 100,
							z: 1,
						}),
					...
					expect(
						rootNode.get("myTestProperty").get("myComplexArray").get(0).getValues().x,
					).toEqual(100);
					expect(
						rootNode.get("myTestProperty").get("myComplexArray").get(0).getValues().y,
					).toEqual(100);
					expect(
						rootNode.get("myTestProperty").get("myComplexArray").get(0).getValues().z,
					).toEqual(1);

					expect(
						rootNode.get("myTestProperty").get("myComplexArray").get(2).getValues().x,
					).toEqual(100);
					expect(
						rootNode.get("myTestProperty").get("myComplexArray").get(2).getValues().y,
					).toEqual(100);
					expect(
						rootNode.get("myTestProperty").get("myComplexArray").get(2).getValues().z,
					).toEqual(1);

					expect(rootNode.get("myTestProperty").get("myComplexArray").get(2)).toEqual(entry);

					expect(() => {
						state.myTestProperty.myComplexArray[0] = rootNode.resolvePath(
							"myTestProperty.myVector",
						);
					}).toThrow();
				});
				...
				it("check .fill() functionality", function () {
					...
					state.myTestProperty.myComplexArray.fill(
						PropertyProxy.proxify(
							PropertyFactory.create(vector2DTemplate.typeid, "single", {
								x: 5,
								y: 6,
							}),
						),
				...
				it("check .push() functionality", function () {
					...
					expect(
						rootNode.get("myTestProperty").get("myComplexArray").get("2").get("x").getValue(),
					).toEqual(3);
					expect(
						rootNode.get("myTestProperty").get("myComplexArray").get("2").get("y").getValue(),
					).toEqual(4);

					// multiple elements
					expect(
						state.myTestProperty.myComplexArray.push({ x: 30, y: 40 }, { x: 5, y: 6 }),
					).toEqual(5);
					expect(rootNode.get("myTestProperty").get("myComplexArray").getLength()).toEqual(5);
					expect(
						rootNode.get("myTestProperty").get("myComplexArray").get("3").get("x").getValue(),
					).toEqual(30);
					expect(
						rootNode.get("myTestProperty").get("myComplexArray").get("3").get("y").getValue(),
					).toEqual(40);
					expect(
						rootNode.get("myTestProperty").get("myComplexArray").get("4").get("x").getValue(),
					).toEqual(5);
					expect(
						rootNode.get("myTestProperty").get("myComplexArray").get("4").get("y").getValue(),
					).toEqual(6);
					...
					expect(
						rootNode.get("myTestProperty").get("myComplexArray").get("5").get("x").getValue(),
					).toEqual(50);
					expect(
						rootNode.get("myTestProperty").get("myComplexArray").get("5").get("y").getValue(),
					).toEqual(60);
					expect(
						rootNode.get("myTestProperty").get("myComplexArray").get("6").get("x").getValue(),
					).toEqual(7);
					expect(
						rootNode.get("myTestProperty").get("myComplexArray").get("6").get("y").getValue(),
					).toEqual(8);
					...
					expect(
						rootNode.get("myTestProperty").get("myComplexArray").get("7").get("x").getValue(),
					).toEqual(70);
					expect(
						rootNode.get("myTestProperty").get("myComplexArray").get("7").get("y").getValue(),
					).toEqual(80);
					expect(
						rootNode.get("myTestProperty").get("myComplexArray").get("7").get("z").getValue(),
					).toEqual(1);
				});
				...
				it("check .reverse() functionality", function () {
					...
					expect(
						rootNode.resolvePath("myTestProperty.myComplexArray[1].x").getValue(),
					).toEqual(1);
				...
				it("check .shift() functionality", function () {
					...
					expect(rootNode.resolvePath("myTestProperty.myComplexArray").getLength()).toEqual(
						oldLength - 1,
					);
				...
				it("check .sort() functionality", function () {
					...
					expect(
						rootNode.resolvePath("myTestProperty.myComplexArray[2].x").getValue(),
					).toEqual(1);

					expect(rootNode.resolvePath("myTestProperty.myComplexArray[0]").getTypeid()).toEqual(
						vector3DTemplate.typeid,
					);
					expect(rootNode.resolvePath("myTestProperty.myComplexArray[1]").getTypeid()).toEqual(
						vector2DTemplate.typeid,
					);
					expect(rootNode.resolvePath("myTestProperty.myComplexArray[2]").getTypeid()).toEqual(
						vector2DTemplate.typeid,
					);
				...
				it("check .swap() functionality", function () {
					...
					expect(
						rootNode.resolvePath("myTestProperty.myComplexArray[1].y").getValue(),
					).toEqual(2);
					expect(
						rootNode.resolvePath("myTestProperty.myComplexArray[0].x").getValue(),
					).toEqual(10);
				...
				it("check .splice() functionality", function () {
					...
					expect(
						rootNode.resolvePath("myTestProperty.myComplexArray").getValues().toString(),
					).toEqual(newArrayWithJsOutfit.toString());
					...
					expect(
						rootNode.resolvePath("myTestProperty.myComplexArray").getValues().toString(),
					).toEqual(arrayWithJsOutfit.toString());
				});

				it("check .unshift() functionality", function () {
					const oldLength = state.myTestProperty.myComplexArray.length;
					expect(state.myTestProperty.myComplexArray.unshift({ x: -1, y: -2 })).toEqual(
						oldLength + 1,
					);
					expect(state.myTestProperty.myComplexArray.length).toEqual(oldLength + 1);
					expect(rootNode.resolvePath("myTestProperty.myComplexArray").getLength()).toEqual(
				...
				it("should be possible to assign a new iterable", function () {
					...
					expect(() => {
						state.myTestProperty.myComplexArray = [
							rootNode.get("myTestProperty").get("myVector"),
						];
					}).toThrow();
		...
		describe("Array of collections", function () {
			...
			it("should be able to set entries of nested collections", function () {
				...
				rootNode
					...
					.unshift(PropertyFactory.create(vector2DTemplate.typeid, "array"));
				...
				rootNode
					.get("myGenericArray")
					.insert(1, PropertyFactory.create(vector2DTemplate.typeid, "map"));
		...
		describe("Map", function () {
			describe("Int32 Maps", function () {
				...
				describe("Setting", function () {
					...
					it("check .delete() functionality", function () {
						...
						expect(rootNode.get("myTestProperty").get("myMap").has("firstNumber")).toEqual(
							true,
						);
					...
					it("check .set() functionality", function () {
						...
						state.myTestProperty.myMap.set(
							"fifthNumber",
			...
			describe("Maps with complex entries", function () {
				...
				it("should reflect remote changes", function () {
					...
					rootNode.resolvePath("myTestProperty.myComplexMap").insert(
						"fourthEntry",
						PropertyFactory.create(vector2DTemplate.typeid, "single", {
							x: 70,
							y: 80,
				...
				describe("Setting", function () {
					...
					it("check .clear() functionality", function () {
						expect(
							rootNode.resolvePath("myTestProperty.myComplexMap").getIds().length,
						).toEqual(3);
					...
					it("check .delete() functionality", function () {
						...
						expect(
							rootNode.get("myTestProperty").get("myComplexMap").has("firstEntry"),
						).toEqual(true);
						expect(state.myTestProperty.myComplexMap.delete("firstEntry")).toEqual(true);
						expect(state.myTestProperty.myComplexMap.delete("nonExistingEntry")).toEqual(
							false,
						);
					...
					it("check .set() functionality", function () {
						// replace entry
						state.myTestProperty.myComplexMap.set(
							"firstEntry",
							PropertyFactory.create(
								"autodesk.appframework.tests:myVector2D-1.0.0",
								"single",
						...
						expect(
							rootNode.resolvePath("myTestProperty.myComplexMap[firstEntry].z").getValue(),
						).toEqual(1);

						// insert entry
						state.myTestProperty.myComplexMap.set("fourthEntry", { x: 70, y: 80 });
						expect(
							rootNode.resolvePath("myTestProperty.myComplexMap[fourthEntry].x").getValue(),
						).toEqual(70);
						expect(
							rootNode.resolvePath("myTestProperty.myComplexMap[fourthEntry].y").getValue(),
						).toEqual(80);
						...
						expect(
							rootNode.resolvePath("myTestProperty.myComplexMap[fifthEntry].x").getValue(),
						).toEqual(90);
						expect(
							rootNode.resolvePath("myTestProperty.myComplexMap[fifthEntry].y").getValue(),
						).toEqual(100);
						...
						expect(
							rootNode.resolvePath("myTestProperty.myComplexMap[sixthEntry].x").getValue(),
						).toEqual(110);
						expect(
							rootNode.resolvePath("myTestProperty.myComplexMap[sixthEntry].y").getValue(),
						).toEqual(120);
						expect(
							rootNode.resolvePath("myTestProperty.myComplexMap[sixthEntry].z").getValue(),
						).toEqual(1);
					});
					...
					it("should be possible to assign a new iterable", function () {
						const checkAssignment = () => {
							expect(
								rootNode.resolvePath("myTestProperty.myComplexMap[a].x").getValue(),
							).toEqual(1);
							expect(
								rootNode.resolvePath("myTestProperty.myComplexMap[a].y").getValue(),
							).toEqual(2);
							expect(
								rootNode.resolvePath("myTestProperty.myComplexMap[b].x").getValue(),
							).toEqual(3);
							expect(
								rootNode.resolvePath("myTestProperty.myComplexMap[b].y").getValue(),
							).toEqual(4);
							rootNode.resolvePath("myTestProperty.myComplexMap").clear();
						};
						...
						const entriesAsProperties = (): [string, BaseProperty][] => [
							[
								entries[0][0],
								PropertyFactory.create(vector2DTemplate.typeid, "single", entries[0][1]),
							],
							[
								entries[1][0],
								PropertyFactory.create(vector2DTemplate.typeid, "single", entries[1][1]),
							],
						];
			...
			describe("Map of collections", function () {
				...
				it("should be able to set entries of nested collections", function () {
					...
					// Assign primitive array property
					state.myGenericMap.set(
						"array",
						state.constantCollections.primitiveArray.getProperty().clone(),
					);
					...
					state.myGenericMap.set(
						"array",
						state.constantCollections.nonPrimitiveArray.getProperty().clone(),
					);
					...
					state.myGenericMap.set(
						"map",
						state.constantCollections.primitiveMap.getProperty().clone(),
					);
					...
					state.myGenericMap.set(
						"map",
						state.constantCollections.nonPrimitiveMap.getProperty().clone(),
					);
					expect(rootNode.resolvePath("myGenericMap[map]").getIds().length).toEqual(2);
					...
					state.myGenericMap.set(
						"set",
						state.constantCollections.bookSet.getProperty().clone(),
					);
		...
		describe("Set", function () {
			...
			it("check .add() functionality", function () {
				...
				// Add property
				state.myTestProperty.myBookSet.add(
					PropertyFactory.create(bookDataTemplate.typeid, "single", {
						author: "Goethe",
						book: "Faust",
					}),
			...
			it("check .delete() functionality", function () {
				...
				expect(
					rootNode.get("myTestProperty").get("myBookSet").has(myProperty.getId()),
				).toEqual(true);
			...
			it("should be possible to assign a new iterable", function () {
				const checkAssignment = () => {
					...
					rootNode
						.resolvePath("myTestProperty.myBookSet")
						.getAsArray()
				...
				// Assigning a non-iterable should throw
				expect(() => {
					state.myTestProperty.myBookSet = booksAsProperties()[0];
				}).toThrow("PropertyProxy-003");
			});
		});
	});
