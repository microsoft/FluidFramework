 ...
 * @hidden
 */
const createArrayIterator = (target: ComponentArray) =>
	function* () {
...
const prepareElementsForInsertion = (
	...
	elements: (BaseProperty | Record<string, unknown>)[],
...
class ComponentArray extends Array {
	...
	includes(searchElement: any, fromIndex) {
		...
		for (let i = startSearchIdx; i < this.property.length; ++i) {
			...
			if (
				PropertyFactory.instanceOf(prop, "BaseProperty") &&
	...
	shift() {
		...
		return PropertyFactory.instanceOf(first, "BaseProperty")
			? PropertyProxy.proxify(first)
			: first;
	}
		...
		if (PropertyFactory.instanceOf(this.property, "Reference", "array")) {
			...
			for (let i = 0; i < this.property.getLength(); ++i) {
				...
				if (PropertyFactory.instanceOf(referenced, "BaseProperty")) {
					referencedAndReference.push([
						PropertyProxy.proxify(referenced),
						this.property.getValue(i),
					]);
				} else {
	...
	splice(start, deleteCount, ...items) {
		...
		const deleteUntil =
			Number(deleteCount) === deleteCount && startValue + deleteCount < arrayLength
				? deleteCount
		...
		if (itemsToAdd.length > 0) {
			...
			Utilities.wrapWithPushPopNotificationDelayScope(this.property, () =>
				this.property.insertRange(startValue, preparedElements),
			);
		}
	...
	unshift(...elements) {
		...
		Utilities.wrapWithPushPopNotificationDelayScope(this.property, () =>
			this.property.insertRange(0, preparedElements),
		);
		return this.property.getLength();
	}
}
