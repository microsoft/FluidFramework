...
export function randomSquaresBoardGenerator(propertyNode: any, numberOfSquares: number) {
	const squares: any = {};
	...
	propertyNode.insert(
		PropertyFactory.create("autofluid:squaresBoard-1.0.0", undefined, {
			squares,
		}),
	);
}
...
export class SquaresApp {
	...
	init() {
		...
		this.dataBinder.defineRepresentation(
			"view",
			"autofluid:coloredSquare-1.0.0",
			(property) => {
				assert(
				...
				return new Square(
					...
					(pos: IPoint2D) => {
						...
						this.dataBinder.requestChangesetPostProcessing(
							_.debounce(() => {
