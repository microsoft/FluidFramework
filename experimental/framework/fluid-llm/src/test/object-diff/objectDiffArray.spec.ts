...
describe("objectDiff - arrays", () => {
	...
	it("nested array", () => {
		assert.deepStrictEqual(objectDiff(["test", ["test"]], ["test", ["test", "test2"]]), [
			{
				type: "CREATE",
				path: [1, 1],
				value: "test2",
	...
	it("object in array in object", () => {
		assert.deepStrictEqual(
			objectDiff({ test: ["test", { test: true }] }, { test: ["test", { test: false }] }),
			[
				{
					type: "CHANGE",
					path: ["test", 1, "test"],
	...
	it("array to object", () => {
		assert.deepStrictEqual(objectDiff({ data: [] }, { data: { val: "test" } }), [
