...
describe("sharedTreeObjectDiff - basics", () => {
	...
	it("change raw value", () => {
		const treeNode = new SimpleObjectTreeNode({ test: true });
		assert.deepStrictEqual(sharedTreeObjectDiff(treeNode as unknown as Record<string, unknown>, { test: false }), [
			{
				type: "CHANGE",
				path: ["test"],
				value: false,
	...
	it("remove raw value", () => {
		...
		const diffs = sharedTreeObjectDiff(treeMapNode as unknown as Record<string, unknown>, { test: true });
	...
	it("replace object with null", () => {
		class SimpleObjectTreeNode2 extends schemaFactory.map("SimpleMapTreeNode2", [SimpleObjectTreeNode, schemaFactory.null]) {};
		const innerTreeNode = new SimpleObjectTreeNode({ test: true });
		const treeNode = new SimpleObjectTreeNode2({ object: innerTreeNode });
		const diffs = sharedTreeObjectDiff(treeNode as unknown as Record<string, unknown>, { object: null });
		assert.deepStrictEqual(diffs, [
			{
				type: "CHANGE",
				path: ["object"],
	...
	it("replace object with other value", () => {
		class SimpleObjectTreeNode2 extends schemaFactory.map("SimpleMapTreeNode2", [SimpleObjectTreeNode, schemaFactory.string]) {};
		const innerTreeNode = new SimpleObjectTreeNode({ test: true });
		const treeNode = new SimpleObjectTreeNode2({ object: innerTreeNode });
		const diffs = sharedTreeObjectDiff(treeNode as unknown as Record<string, unknown>, { object: "string" });
		assert.deepStrictEqual(diffs,
			[
				{
					type: "CHANGE",
