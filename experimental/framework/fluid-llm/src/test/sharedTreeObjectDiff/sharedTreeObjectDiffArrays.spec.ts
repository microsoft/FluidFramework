...
describe("sharedTreeObjectDiff - arrays", () => {
	...
	class StringArrayNode extends schemaFactory.array("StringTreeArrayNode", schemaFactory.string) {};
	...
	it("nested array", () => {
		class NestedStringArrayNode extends schemaFactory.array("NestedStringTreeArrayNode", [schemaFactory.string, StringArrayNode]) {};
	...
	it("array in object change", () => {
		...
		const treeNode = new ObjectTreeNode({state: ["test", new SimpleObjectTreeNode({ test: true })]});
	...
	it("array to object", () => {
		assert.deepStrictEqual(
			sharedTreeObjectDiff({ data: [] }, { data: { val: "test" } }), [
