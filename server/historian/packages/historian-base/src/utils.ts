...
export function getRequestErrorTranslator(
    ...
    const requestErrorTranslator = (error: any): never => {
        if (typeof error === "object" && error instanceof Error && error.name === "NetworkError") {
            ...
            winston.error(`${standardLogErrorMessage}: [statusCode: ${networkError.code}], [message: ${networkError.message ?? "undefined"}]`);
            // eslint-disable-next-line max-len
            Lumberjack.error(`${standardLogErrorMessage}: [statusCode: ${networkError.code}], [message: ${networkError.message ?? "undefined"}]`, lumberProperties);
            throw error;
