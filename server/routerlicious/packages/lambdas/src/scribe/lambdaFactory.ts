...
export class ScribeLambdaFactory extends EventEmitter implements IPartitionLambdaFactory {
    ...
    public async create(config: Provider, context: IContext): Promise<IPartitionLambda> {
        ...
        if (!document) {
            context.log.info(`Creating NoOpLambda due to missing ${tenantId}/${documentId}`, metaData);
        ...
        if (document.scribe === undefined || document.scribe === null) {
            context.log.info(`New document. Setting empty scribe checkpoint for ${tenantId}/${documentId}`, metaData);
            ...
            context.log.info(`Loading scribe state from service summary for ${tenantId}/${documentId}`, metaData);
            document.scribe = latestSummary.scribe;
            opMessages = latestSummary.messages;
        } else {
            context.log.info(`Loading scribe state from cache for ${tenantId}/${documentId}`, metaData);
        }
