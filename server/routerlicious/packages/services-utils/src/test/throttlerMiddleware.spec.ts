    ...
    describe("throttle", () => {
        ...
        it("sends 429 with message and retryAfter when limit exceeded", async () => {
            ...
            const response = await supertest
                .get(endpoint)
                .expect(429);
            assert.strictEqual(response.body.retryAfterMs, 1000);
        ...
        it("separately throttles multiple throttle id prefixes with same suffix", async () => {
            ...
            const response1 = await supertest
                .get(`${endpoint}/1`)
                .expect(429);
            assert.strictEqual(response1.body.retryAfterMs, 1000);
            assert.strictEqual(response1.body.message, "throttled");

            const response2 = await supertest
                .get(`${endpoint}/2`)
                .expect(429);
            assert.strictEqual(response2.body.retryAfterMs, 1000);
