...
export const bindTelemetryContext = (): RequestHandler => {
	return (req, res, next) => {
		...
		Lumberjack.info("[DHRUV DEBUG] correlationId check");
		if (!telemetryContextProperties.correlationId) {
			Lumberjack.info("[DHRUV DEBUG] correlationId generated by server");
			telemetryContextProperties.correlationId = uuid();
			telemetryContextProperties.correlationIdSource = "server";
		} else {
			Lumberjack.info("[DHRUV DEBUG] correlationId provided by client");
