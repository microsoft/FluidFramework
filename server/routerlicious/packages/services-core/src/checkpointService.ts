 ...
 export class CheckpointService implements ICheckpointService {
    ...
    async writeCheckpointToCollection(documentId: string, tenantId: string, service: string, checkpoint: IScribe | IDeliState, isLocal: boolean = false) {
        ...
        if (!this.localCheckpointEnabled || ! this.checkpointRepository) {
            ...
            .catch((error) => {
                Lumberjack.error(`Error writing checkpoint to global collection.`, lumberProperties, error);
            });
            return;
        }

        await (isLocal ? this.checkpointRepository.writeCheckpoint(documentId, tenantId, checkpoint)
            .catch((error) => {
                Lumberjack.error(
                ...
                this.checkpointRepository.deleteCheckpoint(documentId, tenantId)
                .catch((error) => {
                    Lumberjack.error(
    ...
    async removeCheckpointFromCollection(documentId: string, tenantId: string, service: string, isLocal: boolean = false) {
        const lumberProperties = getLumberBaseProperties(documentId, tenantId);
        const checkpointFilter = {
            documentId,
            tenantId
        ...
        await (isLocal && this.localCheckpointEnabled ? this.checkpointRepository.removeServiceCheckpoint(documentId, tenantId)
            .catch((error) => {
                Lumberjack.error(`Error removing checkpoint from local databse.`, lumberProperties, error);
            }) : this.documentRepository.updateOne(
                checkpointFilter, {
                    [service]: ""
                }
            ));
    ...
    async restoreFromCheckpoint(documentId: string, tenantId: string, service: string, document: IDocument) {
        let checkpoint;
        ...
        try {
            if (!this.localCheckpointEnabled || !this.checkpointRepository){
                ...
                checkpoint = await this.checkpointRepository.getCheckpoint(documentId, tenantId)
                .catch((error) => {
            ...
            Lumberjack.error(`Error finding the last checkpoint.`, getLumberBaseProperties(documentId, tenantId), error)
        }
