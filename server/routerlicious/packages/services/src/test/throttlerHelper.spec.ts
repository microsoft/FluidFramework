...
describe("ThrottlerHelper", () => {
	...
	it("throttles on many individual operations", async () => {
		...
		assert.strictEqual(
			response.throttleStatus,
			true,
			`operation ${operationsPerCooldown + 1} should be throttled`,
		);
	});
	...
	it("throttles on a few operation batches", async () => {
		...
		assert.strictEqual(
			response.throttleStatus,
			true,
			`operation ${operationsPerCooldown / weight + 1} should be throttled`,
		);
	});
	...
	it("throttles on one operation batch", async () => {
		...
		assert.strictEqual(
			response.throttleStatus,
			true,
			`operation with ${weight - operationsPerCooldown} excess weight should be throttled`,
		);
	});
	...
	it("un-throttles after cooldown", async () => {
		...
		assert.strictEqual(
			response.throttleStatus,
			false,
			`operation after ${cooldownIntervalInMs + 1}ms should not be throttled`,
		);
	});
	...
	it("un-throttles after double cooldown", async () => {
		...
		assert.strictEqual(
			response.throttleStatus,
			true,
			`operation after ${cooldownIntervalInMs - 1}ms should still be throttled`,
		);
		...
		assert.strictEqual(
			response.throttleStatus,
			false,
			`operation after ${cooldownIntervalInMs * 2 + 1}ms should not be throttled`,
		);
	});
	...
	it("throttles based on stored throttlingMetrics", async () => {
		...
		assert.strictEqual(
			response.throttleStatus,
			true,
			"operation after 0ms should be throttled",
		);
	...
	it("gives accurate retryAfterInMs duration when only one cooldownIntervalInMs required", async () => {
		...
		assert.strictEqual(
			response.throttleStatus,
			false,
			`operation after ${retryAfter}ms should not be throttled`,
		);
	});
	...
	it("gives minimum retryAfterInMs duration when only one cooldownIntervalInMs required", async () => {
		...
		assert.strictEqual(
			response.throttleStatus,
			true,
			`operation after ${retryAfter}ms should still be throttled`,
		);
	});
	...
	it("does not allow operation burst to exceed operation burst limit", async () => {
		...
		assert.strictEqual(
			...
			true,
			`operation with ${
				opsBurstLimit - operationsPerCooldown
			} excess weight should be throttled`,
		);
	});
});
