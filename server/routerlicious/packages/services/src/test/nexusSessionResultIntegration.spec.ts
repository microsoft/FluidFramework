	...
	describe("Services", () => {
		describe("NexusSessionResult Integration", () => {
			...
			it("should include session-level op and signal counts in NexusSessionResult telemetry", async () => {
				...
				assert.ok(
					lumberjackStub.called,
					"Lumberjack.newLumberMetric should have been called",
				);

				const metricCall = lumberjackStub.getCall(0);
				assert.equal(
					metricCall.args[0],
					LumberEventName.NexusSessionResult,
					"Should create NexusSessionResult metric",
				);

				// Verify the telemetry properties include session-level counts
				const telemetryProps = metricCall.args[1];
				assert.equal(
					telemetryProps[CommonProperties.sessionOpCount],
					50,
					"Should include total session op count (42 + 8)",
				);
				assert.equal(
					telemetryProps[CommonProperties.sessionSignalCount],
					17,
					"Should include total session signal count (15 + 2)",
				);
