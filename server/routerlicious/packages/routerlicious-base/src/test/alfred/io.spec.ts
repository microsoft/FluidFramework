...
Submitted Messages: ${JSON.stringify(messages, undefined, 2)}`);
    ...
    describe("storage", () => {
        ...
        beforeEach(async() => {
            ...
            const globalDbEnabled = false;
            const testDocumentRepository = new TestDocumentRepository();
            const stub = Sinon.stub(testDocumentRepository, "findAndCreateDocument");
        ...
        it("create document with summary", async () => {
            ...
            const docDetails = await testStorage.createDocument(testTenantId, testId, summaryTree, 10, 1, defaultHash, url, url, url, [["code", proposal]]);
