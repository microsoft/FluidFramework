            ...
            MemberExpression: function checkMemberExpression(node) {
                ...
                if (hasIndexSignature(type) || isOptionalProperty(type, property) || isIndexAccess) {
                    if (node.parent.type === "MemberExpression" && node.parent.object === node) {
                        ...
                        const isInForInLoop = node.object.type === "Identifier" && forInLoopVariables.has(node.object.name);
                        const isComputedPropertyAccess = node.computed && node.property.type === "Identifier" && forInLoopVariables.has(node.property.name);
                        if (
                            !node.optional &&
                            !isOptionalChain &&
