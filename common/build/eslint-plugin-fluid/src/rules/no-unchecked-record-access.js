...
function propertyHasBeenChecked(node, context) {
    ...
    while (current) {
        if (current.optional || current.type === "ChainExpression" || current.type === "TSNonNullExpression") {
            return true;
        }
        ...
        if (parent.type === "IfStatement" || parent.type === "ConditionalExpression") {
            if (parent.test) {
                ...
                if (parent.test.type === "BinaryExpression") {
                    ...
                    if ((parent.test.operator === "!==" || parent.test.operator === "!=") &&
                        ((nodesAreEquivalent(parent.test.left, node) && isUndefinedNode(parent.test.right)) ||
        ...
        if (containingBlock) {
            for (const stmt of containingBlock.body) {
                ...
                if (stmt.type === "IfStatement" && stmt.test?.type === "BinaryExpression" && stmt.test.operator === "in") {
