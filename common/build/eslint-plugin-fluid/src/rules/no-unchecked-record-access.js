...
module.exports = {
  ...
  create(context) {
    ...
    // Main function to check property access
...
function isTypeAllowedToBeUndefined(tsNode, typeChecker) {
  ...
  if (symbol && symbol.valueDeclaration) {
    ...
    if (signatureDeclaration.type && signatureDeclaration.type.kind === ts.SyntaxKind.TypeReference) {
      const typeNode = signatureDeclaration.type;
      if (typeNode.typeName.text === 'Promise') {
        return typeNode.typeArguments && typeNode.typeArguments.some(arg =>
          arg.kind === ts.SyntaxKind.UnionType &&
          arg.types.some(t => t.kind === ts.SyntaxKind.UndefinedKeyword)
        );
      }
    }
    // Check for direct union with undefined
    return signatureDeclaration.type &&
           signatureDeclaration.type.kind === ts.SyntaxKind.UnionType &&
           signatureDeclaration.type.types.some(t => t.kind === ts.SyntaxKind.UndefinedKeyword);
  }
  return false;
}
