...
describe("ESLint Rule Tests", function () {
	...
	it("Should report an error for unchecked record access", async function () {
		...
		assert.strictEqual(result.messages[8].line, 87);
		assert.strictEqual(
			result.messages[9].message,
			"'indexedRecordOfStrings.a' is possibly 'undefined'",
		);
		assert.strictEqual(result.messages[9].line, 106);
		assert.strictEqual(
		...
		assert.strictEqual(result.messages[10].line, 108);
		assert.strictEqual(
			result.messages[11].message,
			"'indexedRecordOfStrings.a' is possibly 'undefined'",
		);
		assert.strictEqual(result.messages[11].line, 113);
		assert.strictEqual(
		...
		assert.strictEqual(result.messages[12].line, 116);
		assert.strictEqual(
			result.messages[13].message,
			"'nestedObj.nested.a' is possibly 'undefined'",
		);
		assert.strictEqual(result.messages[13].line, 147);
	});
