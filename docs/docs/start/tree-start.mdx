...
:::warning
It's easy to get into trouble by initializing the tree view *after* connecting to the [container](../build/containers.mdx).
The more detailed recommended init flow is described [here](../data-structures/tree/recommended-init-flow.mdx).
:::
