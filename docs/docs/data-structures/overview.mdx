...
[SharedTree](./tree) is a hierarchical data structure that looks and feels like simple JavaScript objects.
It is a tree of data with complex nodes, including maps that work similarly to [SharedMap](#sharedmap), and several kinds of leaf nodes, including boolean, string, number, null, and [Fluid handles](../concepts/handles).
...
[SharedTree](./tree) is an incredibly flexible DDS that allows developers to represent a variety of data models (primitives, objects, lists, maps in any nested orientation).
