...
SharedTree provides safe, backward-compatible ways to update your [schemas](../schema-definition.mdx) without breaking existing data or older application versions (see [understanding compatibility](./index.mdx#understanding-compatibility)).
...
- **Stored Schema**: The schema persisted with each document, it describes the tree it is stored alongside
...
The stored schema completely describes all data in the tree.
...
This constraint enables virtualized documents where only portions are loaded at a time.
...
Documents persist indefinitely and applications must handle any document version.
While schema growth sounds concerning, it's negligible since schema size is constant per document regardless of data volume.

## Monitoring Schema Compatibility
...
This separation allows multiple application versions with different schemas to collaborate on the same document safely.

### Understanding Compatibility

**Backward compatibility** means old clients can still read documents created with new schemas (not breaking older app versions).
**Forward compatibility** means new clients can read documents created with old schemas (not breaking existing data).
SharedTree supports both to ensure seamless collaboration between different application versions as you evolve your schema.
...
When you make backward-compatible changes to your schema, you can upgrade existing documents to use the new schema using [`view.upgradeSchema()`](../../../api/fluid-framework/treeview-interface#upgradeschema-method):
...
```typescript
// Example: Controlled upgrade rollout
const UPGRADE_ENABLED = featureFlags.enableSchemaUpgrade;

function handleDocumentOpen(tree: ITree) {
    const view = tree.viewWith(configuration);

    if (!view.compatibility.isEquivalent && UPGRADE_ENABLED) {
        if (view.compatibility.canUpgrade) {
            // Log upgrade for monitoring
            telemetry.track("schemaUpgrade", {
                fromVersion: view.compatibility.stored.version,
                toVersion: view.compatibility.view.version
            });
...
- [Compatibility Policies](./compatibility-policies) - Sample policies for handling schema differences
