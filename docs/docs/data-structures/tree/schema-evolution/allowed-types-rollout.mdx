...
To add support for reading strings, we import the alpha APIs and use [`staged`](../../../api/fluid-framework/schemastaticsalpha-interface#staged-propertysignature) to mark the new allowed type:
...
For this schema change, the schema does not need to be [upgraded](./index.mdx#schema-upgrade-process) because [`staged`](../../../api/fluid-framework/schemastaticsalpha-interface#staged-propertysignature) allowed types are removed from stored schemas.
...
Once client saturation has been reached, remove the [`staged()`](../../../api/fluid-framework/schemastaticsalpha-interface#staged-propertysignature) wrapper from the new allowed type and call `upgradeSchema` to make a change to the stored schema:
...
- [`SchemaStaticsAlpha.staged()` API](../../../api/fluid-framework/schemastaticsalpha-interface#staged-propertysignature) - API documentation
