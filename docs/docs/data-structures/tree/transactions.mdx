...
This means that, on the client where the transaction is created, the transaction may be applied then reverted on and off multiple times as the client learns of concurrent changes and re-evaluates the transaction's constraints.
...
This is currently only the case for `noChange` constraints in reverts, but we generalize this behavior to all usages of the `noChange` constraint in the future.
