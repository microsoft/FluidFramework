...
The `nodeInDocument` constraint is also available for use in the old transaction API via `Tree.runTransaction`. This is the only constraint which can be applied via `Tree.runTransaction`.
