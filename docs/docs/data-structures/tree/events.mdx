---
title: Events
sidebar_position: 5
---

SharedTree's events are subscribed to via "on" methods which return an unsubscribe function.
The returned unsubscribe function should be called when the event subscription is no longer needed.

```typescript
const unsubscribe = Tree.on(myTreeNode, "nodeChanged", () => {...});

// Later at some point when the event subscription is not needed anymore
unsubscribe();
```

## Whole-Tree Events

[`TreeView`](https://fluidframework.com/docs/api/fluid-framework/treeview-interface) exposes [events for the whole tree](https://fluidframework.com/docs/api/fluid-framework/treeviewevents-interface).

## Node-Level Events

The [`Tree`](https://fluidframework.com/docs/api/fluid-framework/#tree-variable) singleton provides [APIs for working with nodes](https://fluidframework.com/docs/api/fluid-framework/treenodeapi-interface)
including [events](https://fluidframework.com/docs/api/fluid-framework/treechangeevents-interface).

`Tree` supports two node-level events, subscribed to via `Tree.on`:

-   `nodeChanged` fires when one or more properties of `node` change.
-   `treeChanged` fires when one or more properties of `node` or any node in its subtree changes.

See the API docs for each event for further details.
