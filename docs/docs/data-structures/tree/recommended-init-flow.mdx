...
When working with SharedTree, the most critical rule is: **never initialize after attaching the container**.

If you attach (connect) a new container before initializing it, you create a dangerous race condition.
...
The solution: initialize **before** attaching when creating a new container. Use `compatibility.canInitialize` to validate your assumptions and catch bugs early.
...
4. Initialize the tree with your initial data
...
export class CollaborationFluidClient {
	...
	private async loadFluidContainer(containerId: string): Promise<IFluidContainer<typeof FluidContainerSchema>> {
		const client = this.initClient();
		const { container } = await client.getContainer(containerId, FluidContainerSchema, "2");
		return container;
	}
...
By initializing **before** calling `attach()`, you ensure:
...
- Use `compatibility.canInitialize` to validate your assumptions about container state
- For **new containers**: create → view → **verify `canInitialize` is `true`** → initialize → attach
- For **existing containers**: load → view → **verify `canInitialize` is `false`** → use existing tree
