...
1. `/docs`: `build`
    1. Run [markdown-magic](https://github.com/DavidWells/markdown-magic/) to update some shared content in the source `Markdown` files (both in the `docs` directory and throughout the rest of the repo).
    1. Run API documentation generation to transform `JSON`-formatted API reports generated by `API-Extractor` into `Markdown`-formatted documentation.
