...
- Document important assumptions and invariants that the code depends on.
    ...
    > Use an assertion to validate an input requirement only when callers of supported, non-internal APIs cannot violate that requirement.
    > Otherwise, validate the input at the supported API boundary with an appropriate error.
    > Also document the requirement on the corresponding API.
