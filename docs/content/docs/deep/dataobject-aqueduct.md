...
<!-- IMPORTANT: Don't manually edit the auto-generated content below. Instead, edit the source file (look at the path
     argument) and then run "npm run readme:update" from the root of the repo. That will update all autogenerated
     content, so you should commit the resulting changes. -->

<!-- AUTO-GENERATED-CONTENT:START (INCLUDE:path=packages/framework/aqueduct/README.md&start=3&end=-6) -->
