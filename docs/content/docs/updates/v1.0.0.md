...
- If an AzureClient is used to open multiple documents/containers, the internal in-memory cache was not handling that scenario correctly, so the "latest" snapshot would be polluted by the first document loaded. This has been fixed.
