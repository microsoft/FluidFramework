...
*Property* in the *Workspace*. This is implemented via various callback functions: these callbacks are called when a
*Property* with a *TypeId* that either corresponds to or inherits from the *DataBinding’s* registered *TypeId*
is inserted into the *Workspace*, when the *Property* is modified and when the *Property* is removed from
the *Workspace*.
...
they are in a different subtree in the *Property* hierarchy. For accessing alternative views of the *Workspace* data,
it is recommended to use :ref:`DataBinder Runtime Representations` instead.

Modifying Properties
^^^^^^^^^^^^^^^^^^^^

One important limitation of any *DataBinding* callback is that **modifying** *Properties* (including insert/remove
operations) in the *Workspace* is forbidden. If any callbacks wish to modify *Properties* in the *Workspace*, they
...
to |defineDataBinding()| as mentioned in :ref:`DataBinding Definitions <DataBinding Definition and Activation>`). A *DataBinding* constructor is expected
...
the binding (as well as all other *Properties* in the *Workspace*) will already have been updated by *HFDM* to the
...
*Workspace*. The ``onPreRemove()`` callback will be called preorder, that is before it’s called for any *DataBindings*
...
.. literalinclude:: /categories/data/forge-appfw-databinder/test/data_binder/data_binding.spec.js
   :language: javascript
   :start-after: SnippetStart{DataBinder.Object3DBinding}
   :end-before: SnippetEnd{DataBinder.Object3DBinding}
...
(If using Decorators, such static initialization functions are no longer required. See `Decorators (advanced)`_ for
more details.) Relative-path callbacks may be registered by one of the following functions (all provided by the
``DataBinding`` class):

* |registerOnPath()| gives you a |ModificationContext| containing a nested HFDM *ChangeSet*
  for the relative path we have bound to. We can also access the modified property from here.
* |registerOnProperty()| gives you the changed *Property* (unless you register to the ``remove`` event, where
  the *Property* has already been removed by *HFDM* and *DataBinder* doesn't have access to it when processing the
  *ChangeSet* which contains the relevant ``remove`` event).
* |registerOnValues()| gives you the result of calling ``getValue()`` when we register for a primitive (i.e. builtin)
...
forbidden in relative path callbacks. If any relative-path callbacks wish to modify *Properties* in the *Workspace*,
...
An example for calling the |registerOnPath()| function with all possible options may look like this:
...
the processing of the current *ChangeSet* is finished and may freely modify *Properties* in the *Workspace*. Of
...
of the |ModificationContext| given by |registerOnPath()|.

An example for calling the |registerOnProperty()| function with all possible options may look like this:

.. code-block:: javascript

  this.registerOnProperty('path', ['insert', 'modify'], this.callback, {isDeferred: true, requireProperty: true});

The first three arguments have the same meaning as for |registerOnPath()|, except that the relative path supplied
...
|registerOnPath()|, it also supports the ``requireProperty`` option which is also a binary value that defaults to ``false``.
...
Similarly to |registerOnPath()|, the callbacks registered using this function either receive a single argument when
...
Similarly to |registerOnProperty()|, this function is a convenience function which gives callbacks the value of the
*Property* associated with the relative path instead of the |ModificationContext| given by |registerOnPath()|.

.. code-block:: javascript

  this.registerOnProperty('path', ['insert', 'modify'], this.callback, {isDeferred: true, requireProperty: true});

Here all arguments have the same meaning as for |registerOnProperty()|.
...
2. Recursive creation of *DataBindings* below the currently processed *Property* in the *Workspace*.
...
2. Recursive traversal below the currently processed *Property* in the *Workspace*. Any necessary callbacks to
...
1. ``onPreRemove`` callback. If there are more than one *DataBindings* associated with the current *Property*,
   the order in which these are called is not defined.
2. Recursive traversal below the currently processed *Property* in the *Workspace*. Any necessary callback to
...
If any *Property* in the *Workspace* has more than one child, the order in which they are traversed is not defined.
...
certain *TypeId* in a *Workspace*. However, an application may wish to hear about changes to one specific *Property*,
...
The arguments expected by this function have the same meaning as for |registerOnPath()|, except the first one. The
first argument must be a string, and it represents an *absolute path* inside the *Workspace*: this is the exact path
that we want to listen to. It may or may not start with ``/``, it is always assumed to be an absolute path starting at
the root of the *Workspace*. The callback will be fired for all events specified in the second argument that happen
...
wish to modify *Properties* in the *Workspace*, they can signal that they wish to be executed after the processing of
...
*DataBinder* processes the changes to the *Workspace* the *Properties* are already updated (removed) and the
...
.. |defineDataBinding()|              replace:: :forge-appfw-databinder-api:`defineDataBinding() <classes/databinder/#definedatabinding>`
...
.. |registerOnPath()|                 replace:: :forge-appfw-databinder-api:`registerOnPath() <classes/databinding/#registeronpath>`
.. |registerOnProperty()|             replace:: :forge-appfw-databinder-api:`registerOnProperty() <classes/databinding/#registeronproperty>`
.. |registerOnValues()|               replace:: :forge-appfw-databinder-api:`registerOnValues() <classes/databinding/#registeronvalues>`
