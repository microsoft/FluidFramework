...
2. For packages with "./legacy" APIs, "api-extractor/api-extractor-model.current.json"'s "mainEntryPointFilePath" value should be set to the least stable .d.ts types path in use. E.g. this preference order: `"<projectFolder>/lib/alpha.d.ts"`, `"<projectFolder>/lib/beta.d.ts"`, and finally `"<projectFolder>/lib/public.d.ts"`. Eventually, this configuration could be automated via policy checker. Packages without "./legacy" use a "api-extractor/api-extractor-model.json" configuration for model generation.
