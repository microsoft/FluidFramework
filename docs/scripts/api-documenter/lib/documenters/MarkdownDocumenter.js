...
class MarkdownDocumenter {
    ...
    _writeFrontMatter(stringBuilder, item) {
        this._frontMatter.kind = item.kind;
        this._frontMatter.title = item.displayName.replace(/"/g, '').replace(/!/g, '');
        let apiMembers = item.members;
        switch (item.kind) {
            case "Class" /* Class */:
                const classItem = item;
                if (classItem.tsdocComment) {
                    const tmpStrBuilder = new tsdoc_1.StringBuilder();
                    const summary = classItem.tsdocComment.summarySection;
            ...
            case "Package" /* Package */:
                ...
                apiMembers =
                    item.kind === "Package" /* Package */
                        ? item.entryPoints[0].members
                        : item.members;
                break;
            default:
                break;
        }
        this._frontMatter.members = new Map();
