        ...
        switch (item.kind) {
            case ApiItemKind.Class:
                ...
                if (classItem.tsdocComment) {
                    ...
                    this._markdownEmitter.emit(tmpStrBuilder, classItem.tsdocComment!.summarySection, {
                        contextApiItem: item,
                        onGetFilenameForApiItem: (apiItemForFilename: ApiItem) => {
                            return this._getLinkFilenameForApiItem(apiItemForFilename);
                        }
                    });
                    this._frontMatter.metadata = tmpStrBuilder.toString().replace(/"/g, "'").trim();
                    if (this._frontMatter.metadata.length > 120) {
                        this._frontMatter.metadata = this._frontMatter.metadata.substring(0, 120) + "...";
                    }
                }
